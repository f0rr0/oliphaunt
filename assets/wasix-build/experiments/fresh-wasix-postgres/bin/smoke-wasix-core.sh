...
if [ "$wasmer_status" -ne 0 ]; then
  {
    ...
    printf 'Run `%s/bin/install-wasmer.sh`, or set `WASMER_BIN` to a compatible Wasmer binary.\n' "$FRESH_ROOT"
  } >>"$report"
  echo "blocked: Wasmer CLI not found; see $report" >&2
  exit 2
fi
...
if [ "$initdb_status" -ne 0 ]; then
  ...
  if grep -q '/bin/sh' "$initdb_log" || grep -q 'could not execute command ".*postgres.*-V"' "$initdb_log"; then
    blocker="WASIX popen/system currently routes through /bin/sh, but no WASIX shell is mounted; initdb still needs a real process-spawn path for PostgreSQL subcommands"
...

{
  ...
  printf 'Next gate: start `%s/bin/postgres` under Wasmer and connect with `%s/bin/psql` over a real WASIX socket.\n' \
