...
CONTAINER_PGVECTOR="${CONTAINER_PGVECTOR:-/work/assets/checkouts/pgvector}"
...
"$DOCKER" run --rm \
  ...
  bash -lc '
    ...
    make -s -j"$JOBS" -C "$PGVECTOR" \
      PG_CONFIG=/work/assets/wasix-build/pg_config_wasix.sh \
      PG_CPPFLAGS="-I$BUILD_DIR/src/include -I$PGSRC/src/include" \
      OPTFLAGS="" \
      all
  '
