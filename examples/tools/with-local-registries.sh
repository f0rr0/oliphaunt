...
mkdir -p "$cargo_home"
export CARGO_REGISTRIES_OLIPHAUNT_LOCAL_INDEX="$({
  tools/dev/bun.sh tools/release/example-cargo-registry.mjs configure \
    --cargo-home "$cargo_home" \
    --index "$cargo_index" \
    --patch-candidates
})"
export OLIPHAUNT_LOCAL_CARGO_INDEX="$cargo_index"
