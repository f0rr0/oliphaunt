...
impl BuildOutputs {
    ...
    fn discover_product(source_lane: &str, product: AssetProduct) -> Result<Self> {
    ...
    fn discover_product_for_aot(source_lane: &str, product: AssetProduct) -> Result<Self> {
...
fn validate_sealed_runtime_exports(runtime: &WasmLinkMetadataOut) -> Result<()> {
    let policy_path =
        repo_relative_path("runtimes/liboliphaunt-wasix/assets/generated/wasix-dl.exports");
...
pub(crate) fn generate_wasix_export_list(write: bool, source_lane: &str) -> Result<()> {
    ...
    if write {
        let path = Path::new("runtimes/liboliphaunt-wasix/assets/generated/wasix-dl.exports");
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).with_context(|| format!("create {}", parent.display()))?;
        }
...
fn package_aot_artifacts(
    ...
    if !source_dir.exists() {
        ...
        bail!(
            "AOT source directory {} is missing; run `bash runtimes/liboliphaunt-wasix/tools/serialize-aot.sh --target-triple {target}{source_lane_arg}` before packaging",
            source_dir.display()
        );
    }
...
pub(crate) fn package_extension_aot_artifacts(
    ...
    if !source_dir.exists() {
        ...
        bail!(
            "AOT source directory {} is missing; run `bash runtimes/liboliphaunt-wasix/tools/serialize-aot.sh --target-triple {target}{source_lane_arg}` before packaging extension AOT artifacts",
            source_dir.display()
        );
    }
...
fn crate_aot_artifact_dir(target: &str) -> PathBuf {
    Path::new("runtimes/liboliphaunt-wasix/crates/aot")
        .join(target)
        .join("artifacts")
}
...
pub(crate) fn effective_source_pins(
    ...
    pins.push(SourcePin {
        ...
        kind: SourceKind::Git,
        url: "runtimes/liboliphaunt-wasix/postgres/series".to_owned(),
        mirror_url: None,
