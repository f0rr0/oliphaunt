...
const POSTGRES_SHARED_SOURCE_MANIFEST_PATH: &str = "third-party/postgres/source.toml";
const POSTGRES_PATCH_SERIES_PATH: &str = "runtimes/liboliphaunt-wasix/postgres/series";
const POSTGRES_PREPARE_SCRIPT: &str =
    "runtimes/liboliphaunt-wasix/assets/build/prepare_postgres_source.sh";
...
fn assets(args: Vec<String>) -> Result<()> {
    match args.first().map(String::as_str) {
        ...
        None => {
            bail!(
                "usage: cargo run -p xtask -- assets <check|stage-runtime|import-download|unpack|install-local|package|package-aot|check-aot>"
            )
        }
    }
}
...
fn print_usage() {
    ...
    eprintln!("  bash runtimes/liboliphaunt-wasix/tools/serialize-aot.sh --target-triple <triple>");
    eprintln!(
