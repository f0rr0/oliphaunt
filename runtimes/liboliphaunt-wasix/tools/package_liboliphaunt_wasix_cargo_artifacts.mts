...
function packageSpecs(assetDir, extractRoot, version) {
  ...
  specs.push({
    ...
    target: 'portable',
    kind: 'wasix-runtime',
    templateDir: path.join(ROOT, 'runtimes/liboliphaunt-wasix/crates/assets'),
  ...
  for (const [targetId, packageName] of Object.entries(AOT_PACKAGES).sort(([left], [right]) =>
    ...
    specs.push({
      ...
      target: triple,
      kind: 'wasix-aot',
      templateDir: path.join(ROOT, 'runtimes/liboliphaunt-wasix/crates/aot', triple),
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === '--help' || value === '-h') {
      console.log(
        'usage: runtimes/liboliphaunt-wasix/tools/package_liboliphaunt_wasix_cargo_artifacts.mts [--asset-dir DIR] [--extensions-only] [--extension-part-bytes BYTES] [--output-dir DIR] [--work-dir DIR] [--version VERSION] [--extension-artifact-root DIR...]',
      );
      process.exit(0);
