...
JOBS="${JOBS:-4}"
CONTAINER_ROOT="${CONTAINER_ROOT:-/work/runtimes/liboliphaunt-wasix/assets/build}"
...
"$DOCKER" run --rm \
  ...
  bash -lc '
    set -euo pipefail
    . ./runtimes/liboliphaunt-wasix/assets/build/docker_wasix_env.sh
    . ./runtimes/liboliphaunt-wasix/assets/build/profile_flags.sh
    oliphaunt_wasix_apply_wasix_profile configure
    profile_signature="$(oliphaunt_wasix_wasix_profile_signature)"
    configure_script=./runtimes/liboliphaunt-wasix/assets/build/configure_wasix_dl.sh
    icu_prefix="$(./runtimes/liboliphaunt-wasix/assets/build/build_wasix_icu.sh)"
    ...
    if [ "$needs_configure" = "1" ]; then
      ...
      sha256sum ./runtimes/liboliphaunt-wasix/assets/build/wasix_shim/oliphaunt_wasix_bridge.c \
    ...
    ./runtimes/liboliphaunt-wasix/assets/build/link_wasix_runtime.sh \
      "$BUILD_DIR" \
      "$ICU_PREFIX" \
      "$CONTAINER_GENERATED_ROOT/build/wasix-oliphaunt/oliphaunt_wasix_bridge.o" \
      ./runtimes/liboliphaunt-wasix/assets/generated/wasix-dl.exports \
