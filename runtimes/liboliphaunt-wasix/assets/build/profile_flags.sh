...
oliphaunt_wasix_wasix_profile_signature() {
  ...
  if [ -f ./runtimes/liboliphaunt-wasix/assets/build/configure_wasix_dl.sh ]; then
    printf 'configure_postgres_wasix_dl_sha256=%s\n' "$(sha256sum ./runtimes/liboliphaunt-wasix/assets/build/configure_wasix_dl.sh | awk '{print $1}')"
  fi
  if [ -f ./runtimes/liboliphaunt-wasix/assets/build/build_wasix_icu.sh ]; then
    printf 'build_wasix_icu_sha256=%s\n' "$(sha256sum ./runtimes/liboliphaunt-wasix/assets/build/build_wasix_icu.sh | awk '{print $1}')"
  fi
}
