...
oliphaunt_wasix_extension_wasix_target_values() {
  ...
  local extension="$2"
  local key="$3"
  local target="$repo_root/extensions/external/$extension/targets/wasix.toml"
  "$repo_root/tools/dev/bun.sh" \
    "$repo_root/runtimes/liboliphaunt-wasix/assets/build/wasix-toml-value.mts" \
    string-list \
    "$target" \
    "$key"
}
...
oliphaunt_wasix_extension_recipe_value() {
  ...
  local extension="$2"
  local key="$3"
  local recipe="$repo_root/extensions/external/$extension/recipe.toml"
  "$repo_root/tools/dev/bun.sh" \
    "$repo_root/runtimes/liboliphaunt-wasix/assets/build/wasix-toml-value.mts" \
    string \
    "$recipe" \
