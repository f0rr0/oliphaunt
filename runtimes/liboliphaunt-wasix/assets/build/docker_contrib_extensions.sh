...
CONTAINER_PLAN="${CONTAINER_PLAN:-/work/extensions/generated/contrib-build.tsv}"
...
"$DOCKER" run --rm \
  ...
  bash -lc '
    ...
    . ./runtimes/liboliphaunt-wasix/assets/build/source_lane.sh
    ...
    build_portable_uuid() {
      ...
      local source_dir="/work/runtimes/liboliphaunt-native/portable-uuid"
    ...
    while IFS=$'\''\t'\'' read -r id sql_name contrib_dir module_file archive stable; do
      ...
      while IFS= read -r component; do
        ...
        /work/tools/dev/bun.sh \
          /work/extensions/tools/native-component-contract.mts \
