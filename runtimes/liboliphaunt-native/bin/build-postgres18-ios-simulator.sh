...
source_manifest="$repo_root/third-party/postgres/source.toml"
patch_dir="$repo_root"
patch_series_file="$repo_root/runtimes/liboliphaunt-native/postgres/series"
...
stamp="$build_dir/.liboliphaunt-ios-simulator-build.sha256"
...
report_failure() {
  ...
  trap - EXIT
  printf 'PostgreSQL iOS simulator build failed during %s (line %s, status %s)\n' \
...
liboliphaunt_sources=(
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_native.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_error.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_runtime.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_protocol.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_config.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_process.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_trace.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_fs.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_backup_state.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_archive.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_archive_tar.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_static_extensions.c"
  "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_builtin_extensions.c"
)
...
if [ "$(uname -s)" != "Darwin" ]; then
  echo "PostgreSQL iOS simulator build requires Darwin" >&2
  exit 2
fi
...
libtool_path="$(xcrun --find --sdk iphonesimulator libtool 2>/dev/null || true)"
...
cxx_string="${cxx[*]}"
snowball_nm=(nm)
native_cflags="$(oliphaunt_native_release_cflags -fPIC -DOLIPHAUNT_EMBEDDED -DOLIPHAUNT_EMBEDDED_MOBILE_SHMEM)"
...
desired_hash() {
  {
    ...
    shasum -a 256 \
      "$repo_root/extensions/external/postgis/tools/reproducible-time.sh" \
      "$repo_root/extensions/external/postgis/tools/reproducible-bin/date"
...
apply_patch_series() {
  bash "$repo_root/third-party/postgres/apply-series.sh" "$build_dir" "$patch_series_file"
}
...
artifact_ready() {
  ...
  oliphaunt_text_matches_ere "$build_metadata" '(^|[[:space:]])platform[[:space:]]+IOSSIMULATOR([[:space:]]|$)' || return 1
  local symbols
...
build_generated_headers() {
  (
    ...
    if ! generated_headers_ready; then
      explain_generated_header_state
      echo "PostgreSQL iOS simulator generated headers are incomplete" >&2
      exit 1
    fi
  )
}
...
build_backend_objects() {
  (
    ...
    if ! generated_headers_ready; then
      explain_generated_header_state
      echo "PostgreSQL iOS simulator backend objects require generated headers" >&2
      exit 1
    fi
    if backend_objects_ready; then
      echo "reusing PostgreSQL iOS simulator backend objects" >&2
      return
    fi
    ...
    if ! backend_objects_ready; then
      ...
      echo "PostgreSQL iOS simulator backend objects are incomplete" >&2
      exit 1
    fi
  )
}
...
build_support_libraries() {
  (
    ...
    if ! generated_headers_ready; then
      explain_generated_header_state
      echo "PostgreSQL iOS simulator support libraries require generated headers" >&2
      exit 1
    fi
    ...
    if ! support_libraries_ready; then
      echo "PostgreSQL iOS simulator support libraries do not provide the embedded signal boundary" >&2
      exit 1
    fi
  )
}
...
build_plpgsql_objects() {
  (
    ...
    if plpgsql_objects_ready; then
      echo "reusing PostgreSQL iOS simulator PL/pgSQL objects" >&2
      return
    fi
    ...
    if ! plpgsql_objects_ready; then
      oliphaunt_tail_log_excerpt "$make_log" >&2 || true
      echo "PostgreSQL iOS simulator PL/pgSQL objects are incomplete" >&2
      exit 1
    fi
  )
}
...
build_jit_objects() {
  (
    ...
    if jit_objects_ready; then
      echo "reusing PostgreSQL iOS simulator JIT stub objects" >&2
      return
    fi
    ...
    if ! jit_objects_ready; then
      oliphaunt_tail_log_excerpt "$make_log" >&2 || true
      echo "PostgreSQL iOS simulator JIT stub objects are incomplete" >&2
      exit 1
    fi
  )
}
...
build_liboliphaunt_objects() {
  ...
  for source in "${liboliphaunt_sources[@]}"; do
    ...
    "${cc[@]}" $liboliphaunt_cflags \
      -I"$repo_root/runtimes/liboliphaunt-native/include" \
      -I"$repo_root/runtimes/liboliphaunt-native/src" \
      -c "$source" \
      -o "$object"
  done
}
...
build_openssl_dependency() {
  ...
  local build_root="$work_root/openssl-ios-simulator"
  local install_root="$work_root/openssl-ios-simulator-install"
  ...
  (
    cd "$build_root"
    CFLAGS="-O2 -fPIC -mios-simulator-version-min=${min_ios}" \
      ./Configure iossimulator-arm64-xcrun \
  ...
  if [ -z "$installed_archive" ]; then
    echo "OpenSSL iOS simulator build did not produce libcrypto.a" >&2
    exit 1
  fi
...
build_uuid_dependency() {
  ...
  local source_dir="$repo_root/runtimes/liboliphaunt-native/portable-uuid"
  ...
  [ -s "$archive" ] || {
    echo "portable UUID iOS simulator build did not produce $archive" >&2
    exit 1
  }
  mobile_static_dependency_archives+=("$archive")
}
...
usage() {
  cat >&2 <<'MSG'
usage: runtimes/liboliphaunt-native/bin/build-postgres18-ios-simulator.sh [--check-current]
MSG
}
...
case "$script_mode" in
  ...
  --check-current)
    if artifact_ready && (cd "$build_dir" && generated_headers_ready && backend_objects_ready && support_libraries_ready && plpgsql_objects_ready && oliphaunt_mobile_builtin_snowball_objects_ready && jit_objects_ready) && [ -f "$stamp" ] && [ "$(cat "$stamp")" = "$(desired_hash)" ]; then
      echo "iOS simulator liboliphaunt dylib is current"
      exit 0
    fi
    echo "iOS simulator liboliphaunt dylib is missing or stale" >&2
    exit 1
    ;;
  *)
    usage
