...
macos_module_nm_audit="$repo_root/runtimes/liboliphaunt-native/tools/audit-macos-module-nm.awk"
macos_provider_collision_audit="$repo_root/runtimes/liboliphaunt-native/tools/audit-macos-provider-collisions.awk"
...
filter_native_extension_selection() {
  ...
  local contrib_plan="$repo_root/extensions/generated/contrib-build.tsv"
  local pgxs_plan="$repo_root/extensions/generated/pgxs-build.tsv"
...
liboliphaunt_build_fingerprint() {
  {
    ...
    shasum -a 256 "$repo_root/runtimes/liboliphaunt-native/include/oliphaunt.h"
    shasum -a 256 "$repo_root/runtimes/liboliphaunt-native/src/liboliphaunt_internal.h"
    local source
...
native_extension_component_field() {
  ...
  "$repo_root/tools/dev/bun.sh" \
    "$repo_root/extensions/tools/native-component-contract.mts" \
...
native_postgis_dependency_fingerprint() {
  {
    ...
    shasum -a 256 "$repo_root/extensions/catalog/native-components.toml"
...
extension_build_fingerprint() {
...

  {
    ...
    shasum -a 256 "$repo_root/extensions/generated/contrib-build.tsv"
    shasum -a 256 "$repo_root/extensions/generated/pgxs-build.tsv"
    ...
    if native_extensions_include_postgis; then
      local dependency
      local postgis_time_helper="$repo_root/extensions/external/postgis/tools/reproducible-time.sh"
    ...
    hash_extension_source_tree "$repo_root/runtimes/liboliphaunt-native/portable-uuid"
  } | shasum -a 256 | awk '{print $1}'
}
...
desired_build_hash="$(
  {
    ...
    printf 'fetch_script=%s\n' "$(shasum -a 256 "$repo_root/third-party/postgres/fetch-source.sh" | awk '{print $1}')"
...
portable_uuid_dir="$repo_root/runtimes/liboliphaunt-native/portable-uuid"
...
if [ "$script_mode" != "build" ] && [ "$script_mode" != "--runtime-only" ]; then
  cat >&2 <<'MSG'
usage: runtimes/liboliphaunt-native/bin/build-postgres18-macos.sh [--runtime-only|--print-required-extension-artifacts|--check-oliphaunt-current|--check-extension-artifacts-current]
MSG
  exit 2
fi
...
configure_postgis_parser_tools() {
  postgis_bison_tool="$(native_postgis_find_modern_bison || true)"
  if [ -z "$postgis_bison_tool" ]; then
    native_postgis_fail "PostGIS requires GNU bison 3 or newer; install Homebrew bison or set OLIPHAUNT_BISON"
  fi
  postgis_configure_env+=(
    "BISON=$postgis_bison_tool"
    "YACC=$postgis_bison_tool -y"
  )
  postgis_make_args+=(
    "BISON=$postgis_bison_tool"
    "YACC=$postgis_bison_tool -y"
