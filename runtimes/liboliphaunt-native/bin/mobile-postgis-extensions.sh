...
oliphaunt_postgis_reproducible_epoch() {
  ...
  local time_helper="$repo_root/extensions/external/postgis/tools/reproducible-time.sh"
...
build_postgis_libiconv_dependency() {
  ...
  if [ ! -f "$source_dir/configure" ] || [ ! -f "$source_dir/.oliphaunt-source-pin" ]; then
    oliphaunt_postgis_fail \
      "pinned libiconv source checkout is missing; run bash third-party/tools/fetch-sources.sh native-runtime --force"
  fi
  bash \
    "$repo_root/third-party/tools/fetch-sources.sh" extensions --verify-only >/dev/null
  rm -rf "$build_root" "$dependency_dir"
