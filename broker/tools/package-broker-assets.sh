...
if [[ "$target_id" == linux-*-gnu ]]; then
  ...
  broker_bin="$cargo_target_dir/release/$broker_stage_name"
  broker/tools/build-linux-broker-baseline.sh "$cargo_target_dir"
else
...
tools/dev/bun.sh broker/tools/broker-dependency-license-contract.mts stage \
  "$stage" \
  --target "$target_id"

tools/packaging/archive-directory.mts "$stage" "$out_dir/$asset"
tools/dev/bun.sh broker/tools/broker-dependency-license-contract.mts check-archive \
  "$out_dir/$asset" \
  --target "$target_id"
...
bun broker/tools/check-release-assets.mts "${check_args[@]}"
