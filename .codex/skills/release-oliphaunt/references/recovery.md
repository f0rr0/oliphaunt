...
5. Resume in dependency order from the newest validated checkpoint. Skip only immutable identities whose registry checksum/SRI/payload/file manifest matches the frozen lock.
6. Seal bootstrap only after every expected identity has a receipt. Promote draft GitHub releases only after the final all-registry receipt proof and clean-install probes pass. npm's normal tag is attached by its immutable version publish because OIDC does not authorize a later dist-tag mutation.
