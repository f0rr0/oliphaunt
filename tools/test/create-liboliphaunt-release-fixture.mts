...
import { releaseNoticeRows } from '../../src/shared/artifact-packaging/release-notices.mts';
import {
  logicalTreeSha256,
  nativeClusterSeedCompatibilityKey,
} from '../../src/shared/cluster-seed-contract/native-manifest.mts';
import { NATIVE_MOBILE_ABI_TARGETS_BY_DOMAIN } from '../../src/shared/artifact-packaging/native-mobile-abi-contract.mts';
...
import { nativeRuntimeResourceManifestFixture } from './native-runtime-fixture.mts';
...
function nativeToolsEntries(target) {
  ...
  const entries = Object.fromEntries(
    NATIVE_TOOLS_TOOL_STEMS.map((tool) => [
      `runtime/bin/${tool}${suffix}`,
      windows ? windowsPeFixture({ imports: WINDOWS_VC_RUNTIME_DLLS }) : nativeBinary(target),
    ]),
  );
  if (windows) {
    for (const [name, bytes] of Object.entries(windowsVcRuntimeEntries())) {
      if (name.startsWith('runtime/bin/')) entries[name] = bytes;
    }
  }
  return entries;
}
