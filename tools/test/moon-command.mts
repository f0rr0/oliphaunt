...
export function moonCommand(environment = process.env) {
  ...
  if (result.error) {
    throw new Error(
      `Moon ${PINNED_VERSION} is required, but ${command} failed to start: ${result.error.message}`,
    );
  }
  const actual = result.stdout.trim().match(/^moon\s+([^\s]+)$/u)?.[1];
  if (result.status !== 0 || actual !== PINNED_VERSION) {
    throw new Error(
      `Moon ${PINNED_VERSION} is required, but ${command} reported ${actual ?? 'an invalid version'}`,
    );
  }
  VERIFIED.add(key);
  return command;
