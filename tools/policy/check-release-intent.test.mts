...
const alternateObjects = command('git', [
  'rev-parse',
...
function commitTree(subject, timestamp, parent = 'HEAD') {
  ...
  return command('git', ['commit-tree', tree, '-p', parent], {
    input: `${subject}\n`,
    env: {
      ...gitEnvironment,
...
function releaseIntent({
  ...
  const result = spawnSync('bash', [SCRIPT, subject, base, head, branch, eventName, fullRef], {
    cwd: ROOT,
    encoding: 'utf8',
    env: gitEnvironment,
  });
