...
import { CI_JOB_TARGETS } from '../graph/ci_plan.mts';
...
const graphResult = spawnSync(moonCommand(), ['task-graph', '--json'], {
  cwd: ROOT,
  env: moonEnvironment(),
  encoding: 'utf8',
  ...
  for (const file of ['release.yml', 'mobile-e2e.yml']) {
    ...
    for (const job of Object.values(consumer.jobs)) {
      for (const step of job.steps ?? []) {
        ...
        if (!/download-build-artifacts[.]mts|require-workflow-success[.]sh/u.test(run)) continue;
  ...
  for (const [workflowJob, job] of Object.entries(workflow.jobs)) {
    ...
    for (const [index, step] of steps.entries()) {
      ...
      for (const transfer of transfers) {
        assert.ok(
          direct.has(transfer),
          `${workflowJob} transfers non-direct dependency ${transfer}`,
        );
      }
...
test('mobile artifact downloads materialize under the ABI finalizer inputs', () => {
  ...
  try {
    for (const entry of cases) {
      const step = workflow.jobs[entry.job].steps.find(
        ({ with: options }) => options?.name === entry.artifact,
      );
      ...
      assert.ok(
        inputs.some((input) =>
          Object.values(input).some((value) =>
            String(value).includes(entry.path.replace(/\/[^/]+$/u, '/')),
          ),
        ),
      );
