...
if grep -Eiq 'pglite[-_]oxide|github\.com/f0rr0/pglite-oxide|PostgreSQL 17\.5' README.md; then
  fail "the repository landing page must describe Oliphaunt, not the retired pglite-oxide identity"
fi

if grep -Eq '(^|[ `])pnpm (doctor|fmt:check|check|test|release-check)([ `]|$)' README.md CONTRIBUTING.md; then
  fail "root contributor docs must use Moon tasks; package.json intentionally has no task-alias scripts"
fi
