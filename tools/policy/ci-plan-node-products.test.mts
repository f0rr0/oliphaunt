...
import { planJobsForAffected } from '../graph/ci_plan.mts';
...
test('product prose selects packaging and the cold action graph includes required compilation', () => {
  ...
  const actions = actionTargets('oliphaunt-js:release-package');
...
test('shared implementation documentation has no product consumers', () => {
  ...
  assert.equal(
    result.tasks.some((target) => target.endsWith(':compile')),
    false,
  );
});
...
test('Node Direct source does not rebuild the independently versioned JavaScript SDK', () => {
  ...
  assert.deepEqual(result.jobs, ['affected', 'node-direct', 'node-direct-release-assets']);
...
test('combined JavaScript SDK and WASIX N-API changes release only changed products', () => {
  const result = effects(['src/runtimes/wasix-napi/src/lib.rs', 'src/sdks/js/src/client.ts']);
...
test('WASIX extension staging follows its own code and produced runtime artifact', () => {
  const packager = effects('src/extensions/artifacts/wasix/tools/package-release-assets.mts');
...
test('executable packagers and Rust test configuration select their real owners', () => {
  const nativeExtensions = effects(
...
test('WASIX N-API production helpers keep the release builder affected', () => {
  for (const relativePath of [
    'src/bindings/wasix-ts/tools/pgwire-client.mts',
    'src/runtimes/wasix-napi/tools/package-platform.sh',
    'tools/dev/deno.sh',
...
test('CI planner changes select the focused graph proof', () => {
  const result = effects('tools/graph/ci_plan.mts');
...
test('release mutation tests follow release helpers, not policy or workflow files', () => {
  for (const relativePath of ['.moon/tasks/javascript-quality.yml', '.github/workflows/ci.yml']) {
    const result = effects(relativePath);
    assert.equal(result.tasks.includes('release-tools:unit'), false);
    if (relativePath === '.moon/tasks/javascript-quality.yml') {
      assert.equal(result.tasks.includes('policy-tools:unit'), false);
    }
  }
  const result = effects('.github/scripts/release-candidate-lib.mts');
...
test('release helper changes invalidate only their product artifacts', () => {
  const kotlin = effects('src/sdks/kotlin/tools/stage-release-artifacts.mts');
  ...
  const nodeDirect = effects('src/runtimes/node-direct/tools/check-release-assets.mts');
