...
require_text docs/maintainers/repo-structure.md "only its Android" \
  "repo structure docs must distinguish Kotlin development targets from the published Android SDK"
require_text docs/maintainers/repo-structure.md "AAR/plugin/ABI surfaces are published" \
  "repo structure docs must state the exact public Kotlin/Android distribution boundary"
...
require_text src/sdks/react-native/OliphauntReactNative.podspec 's.dependency "Oliphaunt", native_sdk_version' \
  "selection-neutral React Native podspec must delegate base SDK resolution to CocoaPods"
reject_text src/sdks/react-native/OliphauntReactNative.podspec "vendored_frameworks" \
  "selection-neutral React Native podspec must not embed app-specific carrier selection"
require_text src/sdks/react-native/tools/stage-ios-app.mjs 's.vendored_frameworks = "frameworks/base/**/*.{framework,xcframework}", "frameworks/extensions/**/*.xcframework"' \
  "app-owned React Native iOS staging must link selected base and extension frameworks"
...
require_text docs/maintainers/sdk-parity-policy.md '| Kotlin | Android apps | `oliphaunt` | Android direct |' \
  "SDK parity docs must identify Android direct as Kotlin's published runtime"
require_text docs/maintainers/sdk-parity-policy.md 'Host-native compilations are development/parity evidence and are not published' \
  "SDK parity docs must distinguish Kotlin host evidence from published platforms"
require_text docs/maintainers/sdk-parity-policy.md 'Android broker/server must be separate platform adapters, not direct-mode aliases' \
  "SDK parity docs must reject false Kotlin mode aliases"
...
require_text src/sdks/react-native/OliphauntReactNative.podspec 's.source_files = "ios/*.{h,m,mm,swift}"' \
  "selection-neutral React Native podspec must compile only package-owned bridge sources"
reject_text src/sdks/react-native/OliphauntReactNative.podspec 'ios/generated/static-registry' \
  "selection-neutral React Native podspec must not compile an app-specific extension registry"
require_text src/sdks/react-native/tools/stage-ios-app.mjs 's.source_files = "generated/static-registry/*.c"' \
  "app-owned React Native payload podspec must compile the selected static-extension registry"
...
reject_text src/sdks/react-native/OliphauntReactNative.podspec 's.resources' \
  "selection-neutral React Native podspec must not copy app-specific runtime resources"
require_text src/sdks/react-native/tools/stage-ios-app.mjs 's.resources = "resources/OliphauntReactNativeResources.bundle"' \
  "app-owned React Native payload podspec must copy the selected runtime resources"
require_text src/sdks/react-native/tools/stage-ios-app.mjs '"frameworks/extensions/**/*.xcframework"' \
  "app-owned React Native payload podspec must link selected extension XCFrameworks"
