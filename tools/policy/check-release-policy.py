...
def check_release_metadata(graph: dict) -> None:
    ...
    for product, config in products.items():
        ...
        if config.get("kind") == "exact-extension-artifact":
            ...
            for dependency in ("extension-runtime-contract", "liboliphaunt-native", "liboliphaunt-wasix"):
                if scopes.get(dependency) != "production":
                    fail(f"{project_id} must declare a production Moon dependency on {dependency}")

    extension_model = projects.get("extension-model")
    if extension_model is None:
        fail("extension-model project is missing")
    if "extensions" in project_dependency_scopes(extension_model):
...
def check_release_planning(graph: dict) -> None:
    ...
    contrib_extension_products = expected_contrib_extension_products_from_manifest()
    contains_cases = {
        ...
        | contrib_extension_products,
        "src/extensions/contrib/postgres18.toml": contrib_extension_products,
...
def check_ci_policy() -> None:
    ...
    if required_needs != {"affected", "release-intent", "checks", "tests", "builds", "e2e"}:
        fail(
            "required.needs must be the CI phase gates only: "
            f"['affected', 'release-intent', 'checks', 'tests', 'builds', 'e2e']; got {sorted(required_needs)}"
        )
...
def check_release_workflow_policy() -> None:
    ...
    for forbidden in (
        ...
        if forbidden in release_script:
            fail(
                "release CLI must validate staged liboliphaunt-wasix release archives, "
                f"not raw WASIX build inputs or private crate payloads: found {forbidden!r}"
            )
    for snippet in (
        ...
        if snippet not in release_script:
            fail(f"release-staged WASIX assets must validate staged GitHub release assets: missing {snippet!r}")
    for forbidden in (
        ...
        if forbidden in release_script:
            fail(f"liboliphaunt-wasix must not publish private WASIX runtime crates to crates.io: found {forbidden!r}")
    for snippet in (
