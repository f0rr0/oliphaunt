...
changed="$(
  git diff --name-only "${base_ref}...HEAD" -- \
    Cargo.toml \
    Cargo.lock \
    crates/pglite-oxide/Cargo.toml \
    crates/assets/Cargo.toml \
    crates/aot \
    examples/tauri-sqlx-vanilla/src-tauri/Cargo.lock \
    tools/scripts/check-example-lockfiles.sh \
    tools/scripts/sync-example-lockfiles.py
)"
...
tools/scripts/sync-example-lockfiles.py --check
