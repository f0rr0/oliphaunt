...
product_manifest_path = root / "crates/pglite-oxide/Cargo.toml"
...
for table_name, deps in dependency_tables(product_manifest):
    for dep_key, spec in deps.items():
        ...
        if name in product_deps:
            errors.append(f"{name} is declared more than once in pglite-oxide dependencies")
...
for manifest_path in internal_manifest_paths:
    ...
    if version != product_version:
        errors.append(
            f"{manifest_path}: {name} version {version} does not match pglite-oxide version {product_version}"
        )

    dep = product_deps.get(name)
    if dep is None:
        errors.append(f"crates/pglite-oxide/Cargo.toml does not depend on internal crate {name}")
        continue
    table_name, spec = dep
    version_req = dependency_version(spec)
    if version_req != expected_req:
        errors.append(
            f"crates/pglite-oxide/Cargo.toml {table_name}.{name} uses version {version_req!r}; expected {expected_req!r}"
        )
    path = dependency_path(spec)
    if path is None:
        errors.append(f"crates/pglite-oxide/Cargo.toml {table_name}.{name} must keep a path dependency")
        continue
    expected_path = manifest_path.parent.resolve()
    actual_path = (product_manifest_path.parent / path).resolve()
    if actual_path != expected_path:
        errors.append(
            f"crates/pglite-oxide/Cargo.toml {table_name}.{name} points at {path!r}; expected {manifest_path.parent}"
        )
...
for name in extra_deps:
    errors.append(f"crates/pglite-oxide/Cargo.toml depends on unknown internal crate {name}")
