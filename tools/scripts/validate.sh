...
require() {
  if ! command -v "$1" >/dev/null 2>&1; then
    ...
    echo "run tools/scripts/bootstrap-tools.sh to install the pinned local toolchain" >&2
    exit 1
  fi
}
...
run_root_publish_dry_run() {
  ...
  if grep -Eq 'no matching package named `pglite-oxide-(assets|aot-[^`]+)` found|failed to select a version for the requirement `pglite-oxide-(assets|aot-[^`]+) = "=' "$tmp"; then
    cat >&2 <<'MSG'
warning: pglite-oxide publish dry-run could not resolve exact internal crate
...
validate_lint() {
  require cargo
  run tools/scripts/check-dependency-invariants.sh
...
validate_examples() {
  require cargo
  require npm
  run tools/scripts/sync-example-lockfiles.py --check
...
validate_package() {
  ...
  run tools/scripts/check-crate-size.sh --enforce
}
...
validate_semver() {
  ...
  run cargo semver-checks check-release --package pglite-oxide --manifest-path crates/pglite-oxide/Cargo.toml
}
...
validate_release() {
  ...
  if [ "${PGLITE_OXIDE_RELEASE_STAGED:-0}" != "1" ]; then
    ...
    (
      ...
      PGLITE_OXIDE_RELEASE_STAGED=1 tools/scripts/validate.sh release --allow-dirty
    )
    return 0
  fi
  require_release_portable_assets
...
case "$mode" in
  ...
  *)
    cat >&2 <<'MSG'
usage: tools/scripts/validate.sh <mode> [--allow-dirty]
