...
import { compareText, exactExtensionProducts } from "../release/release-artifact-targets.mjs";
...
const MOBILE_SMOKE_EXTENSION_PRODUCTS = new Set(["oliphaunt-extension-vector"]);
...
function moon(args) {
  return commandJson(moonBin(), args);
}

function affectedProjectsAndTasks() {
  const summary = commandJson("tools/dev/bun.sh", ["tools/graph/affected.mjs", "summary"]);
  return {
...
export function selectedExtensionProductsForPlan(directProjects, tasks, jobs) {
  ...
  if (intersects(jobs, new Set(Object.keys(MOBILE_JOB_SURFACES)))) {
    for (const product of MOBILE_SMOKE_EXTENSION_PRODUCTS) {
...
export function planForFullRun({
  ...
  if (nativeTarget !== "all") {
    ...
    if (nativeTarget.startsWith("android-") || nativeTarget === "ios-xcframework") {
      ...
      focusedJobs = setUnion(BASE_JOBS, new Set(["liboliphaunt-native-desktop", "broker-runtime", "node-direct"]));
  ...
  const jobs = setUnion(BASE_JOBS, BUILDER_JOBS, WASM_RUNTIME_JOBS);
...
function renderPlan({ jobs, projects, tasks, reason, selectedTargets }) {
  const selectedExtensionProducts = selectedExtensionProductsForPlan(new Set(), tasks, jobs);
  return renderPlanWithSelection({ jobs, projects, tasks, reason, selectedTargets, selectedExtensionProducts });
}

function renderPlanWithSelection({ jobs, projects, tasks, reason, selectedTargets, selectedExtensionProducts }) {
  return {
    jobs: sorted(jobs),
    ...
    tasks: sorted(tasks),
    liboliphaunt_native_desktop_runtime_matrix: jobs.has("liboliphaunt-native-desktop")
    ...
    liboliphaunt_native_android_runtime_matrix: jobs.has("liboliphaunt-native-android")
      ? liboliphauntNativeAndroidRuntimeMatrix(process.env.NATIVE_TARGET || "all", selectedTargets ?? undefined)
      : emptyMatrix(),
    liboliphaunt_native_ios_runtime_matrix: jobs.has("liboliphaunt-native-ios")
      ? liboliphauntNativeIosRuntimeMatrix(process.env.NATIVE_TARGET || "all", selectedTargets ?? undefined)
      : emptyMatrix(),
    extension_artifacts_native_matrix: jobs.has("extension-artifacts-native")
      ? extensionArtifactsNativeMatrix(
          process.env.NATIVE_TARGET || "all",
    ...
    extension_artifacts_wasix_matrix: jobs.has("extension-artifacts-wasix")
      ? extensionArtifactsWasixMatrix("all", selectedExtensionProducts ?? undefined)
      : emptyMatrix(),
    liboliphaunt_wasix_aot_runtime_matrix: jobs.has("liboliphaunt-wasix-aot")
      ? liboliphauntWasixAotRuntimeMatrix(process.env.WASM_TARGET || "all")
      : emptyMatrix(),
    extension_package_products: sorted(selectedExtensionProducts ?? new Set()),
    extension_package_products_csv: sorted(selectedExtensionProducts ?? new Set()).join(","),
    ...
    react_native_android_mobile_app_matrix: jobs.has("mobile-build-android")
      ? reactNativeAndroidMobileAppMatrix(process.env.NATIVE_TARGET || "all", selectedTargets ?? undefined)
      : emptyMatrix(),
    broker_runtime_matrix: jobs.has("broker-runtime")
      ? brokerRuntimeMatrix(process.env.NATIVE_TARGET || "all")
      : emptyMatrix(),
    node_direct_runtime_matrix: jobs.has("node-direct")
      ? nodeDirectRuntimeMatrix(process.env.NATIVE_TARGET || "all")
      : emptyMatrix(),
    reason,
  };
}
