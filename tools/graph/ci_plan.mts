...
const TASKS_BY_TARGET = (() => {
  if (!process.env.OLIPHAUNT_MOON_TASK_GRAPH_FILE)
    fail('Moon task snapshot is required; use bash tools/graph/ci-plan.sh');
...
export function planJobsForAffected(tasks) {
  ...
  const directlySelectedJobs = jobsForTargets(downstreamTaskClosure(tasks), {
...
function usage() {
  return `usage: bash tools/graph/ci-plan.sh [command]
