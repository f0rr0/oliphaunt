...
if [ "${1:-}" = image ] && [ "${2:-}" = inspect ]; then
  ...
  case "$*" in
    ...
    *'cargo build --locked --offline --manifest-path /workspace/src/wasix/node-addon/Cargo.toml'*)
...
broker=src/native/broker/tools/build-linux-broker-baseline.sh
wasix=src/wasix/node-addon/tools/build-linux-wasix-napi-baseline.sh
