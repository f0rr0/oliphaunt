...
function pe({ machine = 0x8664, imports = [], delayImports = [] } = {}) {
  ...
  if (imports.length > 0) {
    ...
    imports.forEach((name, index) => {
      buffer.writeUInt32LE(writeName(name), 0x200 + index * 20 + 12);
    });
  }
...
function fixture(root = mkdtempSync(path.join(tmpdir(), 'oliphaunt-vc-runtime-'))) {
...
if (['prepare', 'verify'].includes(process.argv[2])) {
  ...
  if (process.argv[2] === 'prepare') {
    ...
    for (const [name, file] of [
      ...
      writeFileSync(path.join(root, `${name}.before`), readFileSync(file));
    }
  } else {
    assert.notDeepEqual(readFileSync(producer), readFileSync(path.join(root, 'producer.before')));
    assert.deepEqual(readFileSync(runtime), readFileSync(path.join(root, 'runtime.before')));
    assert.deepEqual(readFileSync(receipt), readFileSync(path.join(root, 'receipt.before')));
    assert.match(
      readFileSync(path.join(root, 'strip.log'), 'utf8'),
      /preservedAppLocalVcRuntime=.*vcruntime140\.dll/u,
    );
...
describe('Windows VC runtime dependency closure', () => {
  test('parses normal and delay-load imports without an external Windows tool', () => {
    const parsed = inspectPortableExecutable(
      pe({
  ...
  test('the provider profile carries the exact supported-extension union', () => {
    ...
    try {
      ...
      assert.deepEqual(
        readFileSync(path.join(bin, WINDOWS_VC_RUNTIME_RECEIPT), 'utf8')
          .trim()
          .split('\n')
      ...
      assert.deepEqual(
        verifyWindowsVcRuntimeClosure({ root: carrier, searchRoots: [bin], profile: 'provider' })
          .required,
        WINDOWS_VC_RUNTIME_PROFILES.provider,
      );
    } finally {
  ...
  test('resolves exactly one initialized x64 CRT and atomically stages the audited union', () => {
    ...
    try {
      ...
      assert.deepEqual(
        readFileSync(path.join(first, 'vcruntime140.dll')),
        readFileSync(path.join(source, 'vcruntime140.dll')),
      );
      assert.deepEqual(
        readFileSync(path.join(second, 'vcruntime140.dll')),
        readFileSync(path.join(source, 'vcruntime140.dll')),
      );
      assert.match(
      ...
      assert.equal(
        WINDOWS_VC_RUNTIME_DLLS.filter((name) => name !== 'vcruntime140.dll').some((name) =>
          readFileSync(path.join(first, WINDOWS_VC_RUNTIME_RECEIPT), 'utf8').includes(name),
        ),
        false,
      );
    } finally {
      ...
      assert.throws(
        () =>
          stageWindowsVcRuntime({
            root: carrier,
            redistRoot: redist,
            destinations: [path.join(carrier, 'bin')],
          }),
  ...
  test('atomically replaces a stale regular destination from the initialized toolchain', () => {
    ...
    try {
      ...
      writeFileSync(
        path.join(destination, 'app.exe'),
        pe({ imports: [WINDOWS_VC_RUNTIME_DLLS[0]] }),
      );
  ...
  test('rejects a missing app-local DLL and an undeclared future VC runtime import', () => {
    ...
    try {
      ...
      assert.equal(
        verifyWindowsVcRuntimeClosure({ root: payload, searchRoots: [bin] }).inventory.length,
        2,
      );
