...
function uniqueXmlText(block, tag, context, { required = true } = {}) {
  const matches = [
    ...block.matchAll(new RegExp(`<${tag}(?:\\s[^>]*)?>([\\s\\S]*?)</${tag}>`, 'gu')),
  ];
...
function xmlBlocks(block, tag) {
  return [...block.matchAll(new RegExp(`<${tag}(?:\\s[^>]*)?>([\\s\\S]*?)</${tag}>`, 'gu'))].map(
    (match) => match[1],
  );
}
...
function normalizedFiles(files, context) {
  ...
  for (const entry of files) {
    const name =
      typeof entry === 'string'
        ? path.basename(entry)
        : path.basename(entry?.name ?? entry?.path ?? '');
...
function requireMetadata(project, context) {
  ...
  const validLicense = xmlBlocks(licenses, 'license').some(
    (license) =>
  ...
  const validDeveloper = xmlBlocks(developers, 'developer').some(
    (developer) =>
      (xmlText(developer, 'name')?.length ?? 0) > 0 &&
      ((xmlText(developer, 'email')?.length ?? 0) > 0 ||
        (xmlText(developer, 'url')?.length ?? 0) > 0),
  );
  if (!validDeveloper) {
    throw error(
      `${context} must define at least one developer with a nonempty name and email or url`,
    );
  }
...
export function validateMavenCentralPublication({ pomText, files, context = 'Maven publication' }) {
  ...
  if (projectMatches.length !== 1) {
    throw error(
      `${context} POM must contain exactly one <project> document, found ${projectMatches.length}`,
    );
  }
  ...
  const groupId = requireSafeCoordinate(
    uniqueXmlText(coordinates, 'groupId', context),
    `${context} groupId`,
  );
  const artifactId = requireSafeCoordinate(
    uniqueXmlText(coordinates, 'artifactId', context),
    `${context} artifactId`,
  );
  const version = requireSafeCoordinate(
    uniqueXmlText(coordinates, 'version', context),
    `${context} version`,
  );
  const packaging = requireSafeCoordinate(
    uniqueXmlText(coordinates, 'packaging', context, { required: false }) ?? 'jar',
    `${context} packaging`,
  );
  ...
  for (const name of required) {
    if (!names.has(name)) {
      throw error(
        `${context} (${groupId}:${artifactId}:${version}, packaging ${packaging}) is missing required file ${name}`,
      );
    }
  }
