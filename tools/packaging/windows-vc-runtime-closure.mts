...
const VC_RUNTIME_IMPORT =
  /^(?:atl|concrt|mfc|mfcm|msvcp|ucrtbase|vcamp|vcomp|vcruntime)[a-z0-9_]*\.dll$/iu;
...
function parsePortableExecutable(input, label = 'portable executable') {
  ...
  const directory = (index) => {
    if (
      ...
      dataDirectoryOffset + (index + 1) * 8 > optional + optionalSize
    ) {
      return { rva: 0, size: 0 };
    }
    return {
  ...
  if (delayed.rva !== 0) {
    ...
    const end =
      delayed.size > 0 ? Math.min(buffer.length, descriptor + delayed.size) : buffer.length;
    let terminated = false;
...
export function windowsVcRuntimeImports(input, label) {
  return inspectPortableExecutable(input, label).imports.filter((name) =>
    VC_RUNTIME_IMPORT.test(name),
  );
}
...
export function resolveInitializedVcRuntimeDirectory(redistRoot = process.env.VCToolsRedistDir) {
  if (typeof redistRoot !== 'string' || !redistRoot.trim()) {
    throw failure(
      'VCToolsRedistDir is not set; initialize the exact x64 MSVC developer environment first',
    );
  }
  ...
  const candidates = readdirSync(x64, { withFileTypes: true })
    .filter(
      (entry) => entry.isDirectory() && !entry.isSymbolicLink() && entry.name === CRT_DIRECTORY,
    )
...
function checkedRuntimeImport(name, importer) {
  ...
  if (!WINDOWS_VC_RUNTIME_DLLS.includes(normalized)) {
    throw failure(
      `${importer} imports undeclared or debug VC runtime ${name}; update the audited production closure from actual binary evidence`,
    );
  }
  return normalized;
}
...
function parseReceipt(directory) {
  ...
  if (!isRegularFile(receipt)) {
    throw failure(
      `${directory} is missing regular VC runtime digest receipt ${WINDOWS_VC_RUNTIME_RECEIPT}`,
    );
  }
...
function removeStaleRuntimeFiles(destination, requiredNames) {
  ...
  for (const allowed of WINDOWS_VC_RUNTIME_DLLS) {
    ...
    if (!isRegularFile(stale))
      throw failure(`refusing to remove non-regular stale VC runtime ${stale}`);
    rmSync(stale);
  }
}

export function stageWindowsVcRuntime({
  root,
  redistRoot,
  sourceDirectory,
  destinations,
...
export function verifyWindowsVcRuntimeClosure({ root, searchRoots, profile }) {
  ...
  for (const searchRoot of resolvedSearchRoots) {
    ...
    if (missing.length > 0 || extra.length > 0) {
      throw failure(
        `${searchRoot} VC runtime closure mismatch; missing [${missing.sort().join(', ')}], extra [${extra.sort().join(', ')}]`,
      );
    }
    if (expected.size === 0) {
      if (names.has(WINDOWS_VC_RUNTIME_RECEIPT.toLowerCase())) {
        throw failure(
          `${searchRoot} has a VC runtime receipt but its carrier imports no VC runtime`,
        );
      }
      continue;
    }
    ...
    if ([...receipt.keys()].sort().join('\0') !== [...expected].sort().join('\0')) {
      throw failure(
        `${searchRoot} digest receipt does not exactly describe its import-derived VC runtime closure`,
      );
    }
  }
...
export function main(argv = process.argv.slice(2)) {
  ...
  try {
    ...
    if (args.command === 'verify') {
      ...
      const result = verifyWindowsVcRuntimeClosure({
        root: args.root,
