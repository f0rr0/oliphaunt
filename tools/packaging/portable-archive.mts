...
function validateAndroidAlignmentExtra(value, file, label, { dataOffset, method, name }) {
  ...
  const expectedPadding =
    (expectedAlignment - (unpaddedDataOffset % expectedAlignment)) % expectedAlignment;
  if (
...
function validateZipExtra(
  ...
  file,
  label,
  { source = false, androidApkLocal = false, dataOffset = 0, method = -1, name = '' } = {},
) {
  const seen = new Set();
  for (let offset = 0; offset < extra.length; ) {
    if (androidApkLocal && extra.subarray(offset).every((byte) => byte === 0)) {
      ...
      if (method !== 0 || paddingLength !== expectedPadding || dataOffset % alignment !== 0) {
    ...
    seen.add(id);
    if (
      !ZIP_ALLOWED_EXTRA_FIELDS.has(id) &&
      !(source && id === 0x000a) &&
      !(androidApkLocal && id === ANDROID_ALIGNMENT_EXTRA_FIELD_ID)
    ) {
      throw archiveError(
        file,
    ...
    if (id === 0x000a) {
      if (
        ...
        value.readUInt16LE(4) !== 1 ||
        value.readUInt16LE(6) !== 24
      ) {
        throw archiveError(file, 'has malformed NTFS timestamp ZIP metadata');
      }
    } else if (id === ANDROID_ALIGNMENT_EXTRA_FIELD_ID) {
...
function apkZipDescriptorLength(buffer, entry, available, file) {
  ...
  if (candidates.length !== 1) {
    throw archiveError(
      file,
      `has an invalid or ambiguous ZIP descriptor before APK metadata for ${entry.name}`,
    );
  }
  return candidates[0];
}
...
function validateApkSigningBlock(buffer, recordsEnd, centralOffset, file) {
  ...
  if (gap < 32) {
    throw archiveError(
      file,
      `has an unrecognized ${gap}-byte gap before its APK central directory`,
    );
  }
...
function zipEntryType(versionMadeBy, externalAttributes, rawName, file) {
  ...
  return { mode: host === 0 ? (type === 'directory' ? 0o755 : 0o644) : unixMode, type };
}
...
// Actions artifact ZIPs can exceed 4 GiB. Inspect only central headers; unzip
// handles names, decompression and CRCs without buffering the archive in JS.
export function validateZipEntryTypes(file) {
  const fd = openSync(file, 'r');
  try {
    const length = fstatSync(fd).size;
    const read = (offset, size) => {
      if (!Number.isSafeInteger(offset) || offset < 0 || offset > length - size)
        throw archiveError(file, 'has a truncated ZIP record');
      const buffer = Buffer.alloc(size);
      let done = 0;
      while (done < size) {
        const count = readSync(fd, buffer, done, size - done, offset + done);
        if (!count) throw archiveError(file, 'has a truncated ZIP record');
        done += count;
      }
      return buffer;
    };
    const tailOffset = Math.max(0, length - 65_557);
    const tail = read(tailOffset, length - tailOffset);
    const endOffset = findZipEnd(tail, file);
    const end = tail.subarray(endOffset);
    let centralEnd = tailOffset + endOffset;
    let count = end.readUInt16LE(10);
    let size = end.readUInt32LE(12);
    let offset = end.readUInt32LE(16);
    if (end.readUInt16LE(4) || end.readUInt16LE(6) || end.readUInt16LE(8) !== count)
      throw archiveError(file, 'uses multi-disk ZIP metadata');
    const locator = centralEnd >= 20 ? read(centralEnd - 20, 20) : Buffer.alloc(20);
    if (locator.readUInt32LE(0) === 0x07064b50) {
      if (locator.readUInt32LE(4) !== 0 || locator.readUInt32LE(16) !== 1)
        throw archiveError(file, 'uses multi-disk ZIP64 metadata');
      const zip64Offset = safeZipUInt64(locator, 8, file, 'ZIP64 end offset');
      const zip64 = read(zip64Offset, 56);
      if (
        zip64.readUInt32LE(0) !== 0x06064b50 ||
        safeZipUInt64(zip64, 4, file, 'ZIP64 end size') !== centralEnd - 20 - zip64Offset - 12 ||
        zip64.readUInt32LE(16) ||
        zip64.readUInt32LE(20) ||
        zip64.readBigUInt64LE(24) !== zip64.readBigUInt64LE(32)
      )
        throw archiveError(file, 'has invalid ZIP64 end metadata');
      centralEnd = zip64Offset;
      count = safeZipUInt64(zip64, 32, file, 'ZIP64 entry count');
      size = safeZipUInt64(zip64, 40, file, 'ZIP64 central size');
      offset = safeZipUInt64(zip64, 48, file, 'ZIP64 central offset');
    }
    if (!count || count > size / 46 || offset + size !== centralEnd)
      throw archiveError(file, 'has an invalid ZIP central-directory extent');
    for (let index = 0; index < count; index++) {
      if (offset > centralEnd - 46) throw archiveError(file, 'has a truncated ZIP central header');
      const header = read(offset, 46);
      if (header.readUInt32LE(0) !== 0x02014b50)
        throw archiveError(file, 'has an invalid ZIP central header');
      const kind = (header.readUInt32LE(38) >>> 16) & 0o170000;
      if (header.readUInt16LE(8) & 1 || ![0, 0o100000, 0o040000].includes(kind))
        throw archiveError(file, 'contains an encrypted, symbolic-link, or special ZIP member');
      offset += 46 + header.readUInt16LE(28) + header.readUInt16LE(30) + header.readUInt16LE(32);
    }
    if (offset !== centralEnd)
      throw archiveError(file, 'has an invalid ZIP central-directory size');
  } finally {
    closeSync(fd);
  }
}
...
function inflateZipEntry(buffer, entry, file, maxEntryBytes) {
  ...
  if (entry.method === 0) {
    ...
    try {
      ...
      throw archiveError(
        file,
        `has invalid or oversized deflate data for ${entry.name}: ${cause.message}`,
      );
    }
...
function readZipEntries(file, archiveLimits, { source = false, androidApk = false } = {}) {
  ...
  for (let index = 0; index < entryCount; index += 1) {
    ...
    validateZipExtra(
      ...
      { source, androidApkLocal: androidApk, dataOffset, method, name },
    );
...
function gzipPortableText(compressed, start, end, file, label) {
  ...
  if (bytes.some((byte) => byte < 0x20 || byte > 0x7e) || bytes.length === 0) {
...
export function strictGunzip(compressed, file, maxOutputLength) {
  if (
  ...
  if ((flags & 0x04) !== 0) {
    ...
    offset += 2;
    if (extraLength > trailerOffset - offset)
      throw archiveError(file, 'has truncated gzip extra data');
    offset += extraLength;
  }
  if ((flags & 0x08) !== 0) {
  ...
  if ((flags & 0x02) !== 0) {
    ...
    if (expectedHeaderCrc !== actualHeaderCrc)
      throw archiveError(file, 'has an invalid gzip header CRC');
    offset += 2;
  }
  ...
  if (inflated.buffer.length !== expectedSize)
    throw archiveError(file, 'has an invalid gzip payload size');
...
export function decompressSingleZstdFrame(
  ...
  // Zstd decoders may consume concatenated frames. Read block extents first;
  // decompression still owns payload validity, dictionaries and checksums.
  // https://github.com/facebook/zstd/blob/dev/doc/zstd_compression_format.md
  const descriptor = boundedSlice(compressed, 4, 1, label, 'Zstandard header')[0];
  const singleSegment = (descriptor & 0x20) !== 0;
  let offset =
    5 +
    (singleSegment ? 0 : 1) +
    [0, 1, 2, 4][descriptor & 3] +
    [singleSegment ? 1 : 0, 2, 4, 8][descriptor >>> 6];
  while (true) {
    const block = boundedSlice(compressed, offset, 3, label, 'Zstandard block header').readUIntLE(
      0,
      3,
    );
    const type = (block >>> 1) & 3;
    if (type === 3) throw archiveError(label, 'has a reserved Zstandard block type');
    const size = type === 1 ? 1 : block >>> 3;
    boundedSlice(compressed, offset + 3, size, label, 'Zstandard block');
    offset += 3 + size;
    if (block & 1) break;
  }
  if (descriptor & 4) {
    boundedSlice(compressed, offset, 4, label, 'Zstandard checksum');
    offset += 4;
  }
  if (offset !== compressed.length)
    throw archiveError(label, 'contains trailing data or multiple Zstandard frames');
  try {
    return zlib.zstdDecompressSync(compressed, { maxOutputLength: checkedMaxOutputBytes });
  } catch (cause) {
...
function parseTarHeader(header, file, archiveLimits, { source = false, posixOnly = false } = {}) {
  ...
  const gnuUstar =
    ...
    header[263] === 0x20 &&
    header[264] === 0;
  const v7 = source && header.subarray(257).every((byte) => byte === 0);
  if (!posixUstar && !gnuUstar && !v7) throw archiveError(file, 'contains a non-ustar header');
  ...
  const prefix = v7 ? '' : tarString(header, 345, 155, file, 'prefix', { allowEmpty: true });
  ...
  const type =
    typeFlag === 0 || typeFlag === 0x30
      ...
      : typeFlag === 0x35
        ? 'directory'
        : source && typeFlag === 0x31
          ? 'hardlink'
          : source && typeFlag === 0x32
            ? 'symlink'
            : null;
  ...
  const linkTarget = tarString(header, 157, 100, file, `link name for ${raw}`, {
    allowEmpty: true,
  });
  if (!['symlink', 'hardlink'].includes(type) && linkTarget !== '') {
  ...
  const deviceMajor = tarOctal(header, 329, 8, file, `device major for ${raw}`, {
    allowEmpty: true,
  });
  const deviceMinor = tarOctal(header, 337, 8, file, `device minor for ${raw}`, {
    allowEmpty: true,
  });
  ...
  // POSIX typeflag 5 establishes a directory without requiring a trailing slash.
  const name = portableMemberName(
    type === 'directory' && !raw.endsWith('/') ? raw + '/' : raw,
    type,
    file,
    { allowRoot: true },
  );
...
/** Source pins additionally permit V7 tar and links, validated before extraction. */
export function readSourceArchiveEntries(file, options = {}) {
  const archiveLimits = limits(options);
  requireRegularArchive(file, archiveLimits.maxArchiveBytes);
  if (file.endsWith('.zip')) return readZipEntries(file, archiveLimits, { source: true });
  return parseTarEntries(
    decompressedTarBuffer(readFileSync(file), file, archiveLimits, 'gzip'),
    file,
    archiveLimits,
    { source: true },
  );
...
export function readPortableTarZstdBufferEntries(input, options = {}) {
  ...
  if (!Buffer.isBuffer(input) && !(input instanceof Uint8Array)) {
    throw archiveError(
      options.label ?? 'nested.tar.zst',
      'must be provided as a Buffer or Uint8Array',
    );
  }
...
export async function readPortableTarStream(stream, file, options = {}, extraction = undefined) {
  ...
  try {
    for await (const chunk of stream) {
      ...
      if (streamed > archiveLimits.maxExpandedBytes + archiveLimits.maxEntries * 1024 + 10240)
        throw archiveError(file, 'exceeds its total stream-size limit');
      let data = Buffer.concat([pending, chunk]);
      pending = Buffer.alloc(0);
      while (data.length) {
        if (payload || padding) {
          const size = Math.min(payload || padding, data.length);
          if (payload) {
            if (descriptor !== undefined) {
              let offset = 0;
              while (offset < size) {
                const written = writeSync(descriptor, data, offset, size - offset);
                if (written === 0) throw archiveError(file, 'could not write extracted member');
                offset += written;
              }
            }
            payload -= size;
            if (payload === 0 && descriptor !== undefined) {
              closeSync(descriptor);
              descriptor = undefined;
            }
          } else {
            if (data.subarray(0, size).some((byte) => byte !== 0))
              throw archiveError(file, 'has non-zero ustar padding');
            padding -= size;
          }
        ...
        if (entry.name === null) {
          ...
          if (extraction && (!extraction.member || extraction.member === entry.name)) {
            ...
            if (entry.isDirectory) mkdirSync(output, { recursive: true });
              ...
              descriptor = openSync(output, 'wx', entry.mode);
              if (payload === 0) {
                closeSync(descriptor);
                descriptor = undefined;
              }
    ...
    if (pending.length || payload || padding || zeroBlocks < 2)
      throw archiveError(file, 'has a truncated ustar stream');
    return checkedEntries(entries, file, archiveLimits);
  } finally {
...
export async function readPortableTarGzipInventory(file, options = {}, extraction = undefined) {
  requireRegularArchive(file, limits(options).maxArchiveBytes);
  const input = createReadStream(file);
  const gunzip = zlib.createGunzip();
  input.on('error', (error) => gunzip.destroy(error));
  try {
    return await readPortableTarStream(input.pipe(gunzip), file, options, extraction);
  } finally {
    input.destroy();
    gunzip.destroy();
  }
...
/** Extract only into a private tree, promoting it after complete stream validation. */
export async function extractPortableTarGzipTree(
  archive,
  destination,
  options = {},
  member = undefined,
  ...
  const temp = mkdtempSync(path.join(parent, '.archive-extract-'));
  try {
    const entries = await readPortableTarGzipInventory(archive, options, { root: temp, member });
    if (member && !entries.get(member)?.isFile)
      throw archiveError(archive, 'is missing regular member ' + member);
    for (const entry of [...entries.values()].reverse()) {
      if (entry.isDirectory && !member) chmodSync(path.join(temp, entry.name), entry.mode);
    }
...
export function extractPortableArchiveTree(archive, destination, sourcePrefix = '', options = {}) {
  const root = sourcePrefix.replace(/\/+$/u, '');
  const prefix = root ? root + '/' : '';
  const entries = [...readPortableArchiveEntries(archive, options).values()].filter(
    (entry) => (entry.name === root && entry.isDirectory) || entry.name.startsWith(prefix),
  );
  if (entries.length === 0) throw archiveError(archive, `is missing directory ${sourcePrefix}`);
  const parent = path.dirname(path.resolve(destination));
  mkdirSync(parent, { recursive: true });
  const temp = mkdtempSync(path.join(parent, '.archive-extract-'));
  try {
    for (const entry of entries) {
      const output = path.join(temp, entry.name.slice(prefix.length));
      if (entry.isDirectory) {
        mkdirSync(output, { recursive: true });
      } else {
        mkdirSync(path.dirname(output), { recursive: true });
        writeFileSync(output, entry.data(), { flag: 'wx', mode: entry.mode });
        chmodSync(output, entry.mode);
      }
    }
    for (const entry of entries.filter((entry) => entry.isDirectory).reverse()) {
      chmodSync(path.join(temp, entry.name.slice(prefix.length)), entry.mode);
    }
    rmSync(destination, { recursive: true, force: true });
    renameSync(temp, destination);
    chmodSync(
      destination,
      entries.find((entry) => entry.name === root && entry.isDirectory)?.mode ?? 0o755,
    );
  } finally {
    rmSync(temp, { recursive: true, force: true });
  }
...
export function archiveLogicalTreeRows(entries, file, prefix) {
  const rows = [];
  for (const [name, entry] of entries) {
    if (!name.startsWith(prefix) || entry.isDirectory) continue;
    if (!entry.isFile)
      throw new Error(`${file} member ${name} under ${prefix} must be a regular file`);
    rows.push({
      path: name.slice(prefix.length),
      bytes: typeof entry.data === 'function' ? entry.data() : entry.data,
    });
  }
  if (rows.length === 0) throw new Error(`${file} contains no files under ${prefix}`);
  return rows;
}
