...
import { tarArchive, tarOctal } from './testdata/tar-fixture.mts';
...
test('reads a strict ZIP and validates payload bytes', async (t) => {
...
test('accepts an unambiguous FAT-origin ZIP member', async (t) => {
...
test('accepts Android legacy and 0xd935 alignment only in the APK profile', async (t) => {
  ...
  const sharedLibraryPadding =
    (16 * 1024 - (unpaddedSharedLibraryOffset % (16 * 1024))) % (16 * 1024);
...
test('rejects malformed or misplaced Android alignment metadata', async (t) => {
  const cases = [
    [
...
test('accepts a framed APK Signing Block and unknown extension pairs', async (t) => {
  ...
  for (const descriptor of [undefined, 'unsigned', 'signed']) {
    const file = fixtureFile(
      ...
      zipArchive([{ name: 'AndroidManifest.xml', data: 'manifest', descriptor }], {
        beforeCentral: block,
      }),
    ).file;
  ...
  const manifestRecordBytes =
    30 + Buffer.byteLength('AndroidManifest.xml') + Buffer.byteLength('manifest');
  const canonicalPadding = 4096 - manifestRecordBytes;
  const padded = fixtureFile(
    t,
    'signed-canonical-padding.apk',
    zipArchive([{ name: 'AndroidManifest.xml', data: 'manifest' }], {
...
test('rejects malformed APK Signing Block gaps and descriptors', async (t) => {
  ...
  const invalidDescriptor = fixtureFile(
    ...
    zipArchive([{ name: 'AndroidManifest.xml', descriptor: 'signed', descriptorCrc: 0x12345678 }], {
      beforeCentral: valid,
    }),
  ).file;
  assert.throws(
  ...
  const internalGap = fixtureFile(
    t,
    'internal-gap.apk',
    zipArchive([{ name: 'first', afterData: Buffer.alloc(4) }, { name: 'second' }]),
  ).file;
  assert.throws(() => readAndroidApkEntries(internalGap), /ambiguous 4-byte gap/u);
});

test('retains entry safety and integrity checks in the Android APK profile', async (t) => {
  const cases = [
  ...
  assert.deepEqual([...readAndroidApkEntries(caseSensitive).keys()], ['res/2F.xml', 'res/2f.xml']);
  assert.throws(() => readPortableArchiveEntries(caseSensitive), /case\/NFC-colliding/u);
});

test('requires ZIP directory type flags and trailing path markers to agree', async (t) => {
  ...
  for (const [name, row] of [
    [
      'missing-marker.zip',
      { name: 'root', data: '', externalAttributes: ((0o040755 << 16) | 0x10) >>> 0 },
    ],
...
test('rejects truncated ZIPs, duplicates, case collisions, and file-parent collisions', async (t) => {
...
test('rejects ZIP links, special entries, unsafe paths, and ambiguous creator types', async (t) => {
  const cases = [
    ...
    [
      'unsafe.zip',
      { name: '../escape', externalAttributes: 0o100644 << 16 },
      /unsafe archive member/u,
    ],
...
test('rejects ZIP local-central mismatch, unsupported flags/extras, size bombs, and CRC errors', async (t) => {
  ...
  const cases = [
    ...
    [
      'bomb.zip',
      { name: 'file', data: '0123456789', method: 8 },
      /entry-size limit/u,
      { maxEntryBytes: 5 },
    ],
...
test('reads strict ustar and rejects links, bad checksums, padding, and end markers', async (t) => {
  ...
  assert.throws(
    () => readPortableArchiveEntries(numericJunk),
    /non-padding bytes after the ustar checksum terminator/u,
  );
  ...
  assert.throws(
    () => readPortableArchiveEntries(deviceField),
    /sets device numbers on non-device/u,
  );
...
test('accepts POSIX directory type flags and rejects file entries with directory markers', async (t) => {
  ...
  const file = fixtureFile(
    t,
    'file-with-marker.tar.gz',
    tarArchive([{ name: 'root/', type: '0', mode: 0o644 }]),
  ).file;
  assert.throws(() => readPortableArchiveEntries(file), /type\/path-marker mismatch/u);
});

test('rejects trailing bytes, concatenated gzip members, and corrupt gzip trailers', async (t) => {
  ...
  assert.throws(
    ...
    /trailing data or multiple gzip members/u,
  );
...
test('reads one Zstandard frame and rejects trailing bytes or concatenated frames', async (t) => {
  ...
  assert.throws(
    () => decompressSingleZstdFrame(Buffer.concat([valid, zstdCompressSync(Buffer.alloc(0))])),
    /trailing data or multiple Zstandard frames/,
  );
  const concatenated = fixtureFile(t, 'concatenated.tar.zst', Buffer.concat([valid, valid])).file;
  assert.throws(
    () => readPortableArchiveEntries(concatenated),
    /trailing data or multiple Zstandard frames/u,
  );
...
test('strictly parses an in-memory tar.zst with the same bounded portable contract', () => {
  ...
  assert.deepEqual([...entries.keys()], ['oliphaunt', 'oliphaunt/bin', 'oliphaunt/bin/postgres']);
  assert.equal(entries.get('oliphaunt/bin/postgres').data().toString(), 'runtime');
  assert.equal(decompressSingleZstdFrame(compressed, { label: 'nested frame' }).equals(tar), true);
  ...
  for (const [label, rows, pattern] of [
    ...
    assert.throws(() => readPortableTarZstdBufferEntries(candidate, { label }), pattern, label);
  }
...
test('rejects symlink archive inputs before parsing', async (t) => {
...
test('accepts ZIPs emitted by the canonical archive-directory producer', async (t) => {
  ...
  await archiveDirectory(source, output, { keepParent: true });
...
test('streaming portable TAR validates split headers, padding, trailers and limits', async () => {
  const tar = gunzipSync(tarArchive([{ name: 'file', data: 'payload' }]));
  async function* chunks(bytes, size = 37) {
    for (let i = 0; i < bytes.length; i += size) yield bytes.subarray(i, i + size);
  }
  const entries = await readPortableTarStream(chunks(tar), 'stream.tar');
  assert.equal(entries.get('file').size, 7);
  for (const bytes of [
    tar.subarray(0, 513),
    tar.subarray(0, -1),
    Buffer.concat([tar, Buffer.alloc(512, 1)]),
  ])
    await assert.rejects(readPortableTarStream(chunks(bytes), 'stream.tar'));
  const padding = Buffer.from(tar);
  padding[519] = 1;
  await assert.rejects(readPortableTarStream(chunks(padding), 'stream.tar'), /padding/);
  await assert.rejects(
    readPortableTarStream(chunks(tar), 'stream.tar', { maxExpandedBytes: 6 }),
    /expanded/,
  );
  await assert.rejects(
    readPortableTarStream(chunks(Buffer.alloc(12800)), 'stream.tar', {
      maxExpandedBytes: 1,
      maxEntries: 1,
    }),
    /stream-size/,
...
test('artifact ZIP type validation supports sparse ZIP64 and rejects unsafe or truncated metadata', async (t) => {
  const root = mkdtempSync(path.join(tmpdir(), 'oliphaunt-zip64-'));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  const file = path.join(root, 'artifact.zip');
  for (const row of [
    { name: 'payload' },
    { name: 'directory/', externalAttributes: 0o40755 << 16, data: '' },
    { name: 'fat', versionMadeBy: 20, externalAttributes: 0x20 },
    { name: 'untyped', externalAttributes: 0o644 << 16 },
  ]) {
    writeFileSync(file, zipArchive([row]));
    validateZipEntryTypes(file);
  }
  for (const row of [
    { name: 'encrypted', flags: 1 },
    { name: 'symlink', externalAttributes: 0o120777 << 16 },
    { name: 'device', externalAttributes: 0o20644 << 16 },
  ]) {
    writeFileSync(file, zipArchive([row]));
    assert.throws(() => validateZipEntryTypes(file), /encrypted, symbolic-link, or special/);
  }
  const ordinary = zipArchive([{ name: 'payload' }]);
  const end = Buffer.from(ordinary.subarray(-22));
  const central = ordinary.subarray(end.readUInt32LE(16), ordinary.length - 22);
  const centralOffset = 2 ** 32 + 4096;
  ...
  try {
    writeSync(
      fd,
      Buffer.concat([central, zip64, locator, end]),
      0,
      central.length + 98,
      centralOffset,
    );
  ...
  validateZipEntryTypes(file);
  const badCount = Buffer.from(ordinary);
  badCount.writeUInt16LE(2, badCount.length - 14);
  badCount.writeUInt16LE(2, badCount.length - 12);
  for (const broken of [ordinary.subarray(0, -1), badCount]) {
    writeFileSync(file, broken);
    assert.throws(() => validateZipEntryTypes(file), /ZIP/);
  }
});
