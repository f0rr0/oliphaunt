...
import { matchingAssets, writeChecksumManifest } from './write-checksum-manifest.mts';

const TOOL = path.join(import.meta.dir, 'write-checksum-manifest.mts');
...
test('preserves recursive and root-relative glob semantics with deterministic output order', () => {
  ...
  try {
    ...
    expect(relativeFiles(root, matchingAssets(root, ['*.tar.gz']))).toEqual(['root.tar.gz']);
    expect(relativeFiles(root, matchingAssets(root, ['nested/*.tar.gz']))).toEqual([
      'nested/b.tar.gz',
    ]);
    expect(relativeFiles(root, matchingAssets(root, ['./*.zip']))).toEqual(['z.zip']);
...
test('does not follow file or directory symlinks while walking assets', () => {
  ...
  try {
    ...
    expect(relativeFiles(root, matchingAssets(root, ['**/*.tar.gz']))).toEqual(['real.tar.gz']);
  } finally {
    rmSync(parent, { recursive: true, force: true });
  }
});

test('writes the caller-facing checksum manifest deterministically', async () => {
