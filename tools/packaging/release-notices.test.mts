...
test('defines stable canonical member names in deterministic order', () => {
  assert.deepEqual(
    ...
    [
      ...
      ['THIRD_PARTY_NOTICES.liboliphaunt-native.md', 'src/native/runtime/THIRD_PARTY_NOTICES.md'],
      ['THIRD_PARTY_NOTICES.oliphaunt-wasix.md', 'src/wasix/sdks/rust/THIRD_PARTY_NOTICES.md'],
    ],
  );
  assert.throws(
    () => releaseNoticeRows({ products: ['unknown'] }),
    /unsupported release notice product/u,
  );
  assert.deepEqual(
  ...
  assert.throws(
    () => releaseNoticeRows({ components: ['unknown'] }),
    /unsupported release license component/u,
  );
...
test('stages exact bytes and removes stale product notices', async (t) => {
  ...
  assert.deepEqual(assertReleaseNoticesInDirectory(stage, { products: ['native'] }), [
    'LICENSE',
    'THIRD_PARTY_NOTICES.md',
    'THIRD_PARTY_NOTICES.liboliphaunt-native.md',
  ]);
...
test('rejects byte drift and POSIX directory mode drift', async (t) => {
  const { stage } = fixture(t);
...
test('rejects unsafe prefixes and unsafe staging destinations', async (t) => {
  const { root, stage } = fixture(t);
  ...
  assert.throws(() => stageReleaseNotices(linkedStage), /symlink or non-directory ancestor/u);
});

test('rejects caller-created directory aliases and non-directory ancestors', async (t) => {
...
test('validates exact archive members and canonical bytes', async (t) => {
  const { root, stage } = fixture(t);
  stageReleaseNotices(stage, { products: ['native'] });
  writeFileSync(path.join(stage, 'payload.txt'), 'payload\n');
  const archive = path.join(root, 'carrier.tar.gz');
  await archiveDirectory(stage, archive, { keepParent: true });
  assert.deepEqual(
...
test('exact archive validation rejects unknown legal namespace members', async (t) => {
  const { root, stage } = fixture(t);
  ...
  writeFileSync(
    archive,
    zstdCompressSync(
      createDeterministicTar(stage, prefix, {
        fail(message) {
...
test('validates exact notices in a real zstd-compressed ustar carrier', async (t) => {
  const { root, stage } = fixture(t);
...
test('rejects archive byte drift and accepts readable executable notices', async (t) => {
  ...
  await archiveDirectory(stage, byteArchive, { keepParent: true });
  assert.throws(
    () =>
