...
function makeFixture() {
  ...
  for (const directory of ['docs', 'man'])
    mkdirSync(path.join(actionSource, directory), { recursive: true });
  const actionMembers = [
    'LICENSE.txt',
    'README.md',
    'actionlint',
...
function assertNoInstallerDebris(fixture) {
  assert.deepEqual(
    readdirSync(fixture.bin).filter((entry) =>
      /^\.(?:cargo-binstall|actionlint)\.(?:download|install)\./u.test(entry),
    ),
    [],
  );
}
...
test('Cargo fallback is exact, locked, isolated, atomic, and never reuses a partial download', () => {
  ...
  const marker = readFileSync(
    path.join(cargoHome, 'bin', '.cargo-binstall.oliphaunt-source'),
    'utf8',
  );
