...
async function runMeasuredBenchmark(planSource, options) {
  ...
  try {
    ...
    const sequence = [];
    const runs = { candidate: [], comparison: [], 'comparison-main-thread': [] };
    const pids = new Set();
    ...
    for (let repeat = 0; repeat < planSource.plan.measurement.pairedRepeats; repeat += 1) {
      ...
      for (const engine of order) {
        ...
        sequence.push({ phase: 'gated', repeat, engine, pid: result.process.pid });
        runs[engine].push(result);
      }
    }

    // Keep comparator-only diagnostics after the complete qualifying phase so their
    // cache and thermal history cannot perturb a later gated process.
    for (let repeat = 0; repeat < planSource.plan.measurement.pairedRepeats; repeat += 1) {
      const engine = 'comparison-main-thread';
      const result = await runEngine({
        engine,
        repeat,
        planSource,
      ...
      pids.add(result.process.pid);
      sequence.push({ phase: 'diagnostic', repeat, engine, pid: result.process.pid });
      runs[engine].push(result);
    }
    ...
    console.log(
      `wasix-node benchmark: ${summary.gate.passed ? 'PASS' : 'FAIL'} ` +
        `geomean=${summary.geomeanRatio.toFixed(4)} gate<=${summary.gate.maxGeomeanRatio.toFixed(2)} ` +
...
async function runEngine({ engine, repeat, planSource, candidateRoot, scratch }) {
  ...
  if (engine === 'candidate') childArgs.push('--candidate-root', candidateRoot);
  try {
...
function validateEngineReport(report, engine, repeat, planSource) {
  ...
  const expected =
    engine === 'candidate' ? planSource.plan.engines.candidate : planSource.plan.engines.comparison;
  const diagnostic = engine === 'comparison-main-thread';
  const executionBoundary = diagnostic
    ? expected.diagnosticExecutionBoundary
    : expected.executionBoundary;
  const isolationImplementation = diagnostic
    ? 'none-main-thread-diagnostic'
    : expected.isolationImplementation;
  const timingBoundary = diagnostic ? 'caller-around-public-api' : expected.timingBoundary;
  if (
  ...
  if (engine !== 'candidate' && report.engine.version !== expected.version) {
    throw new Error(`${engine} repeat ${repeat} used version ${report.engine.version}`);
  }
}

function summarizeRuns(plan, runs) {
  const candidateRuns = orderedRuns(runs.candidate, 'candidate', plan.measurement.pairedRepeats);
  const comparisonRuns = orderedRuns(runs.comparison, 'comparison', plan.measurement.pairedRepeats);
  ...
  const mainThreadDiagnostic = summarizeDiagnostic(plan, runs['comparison-main-thread']);
  return {
    correctness,
    metrics,
    ...
    geomeanRatio,
    gate,
    mainThreadDiagnostic,
  };
}

function summarizeDiagnostic(plan, runs) {
  const ordered = orderedRuns(runs, 'comparison-main-thread', plan.measurement.pairedRepeats);
  return {
    gated: false,
    executionBoundary: plan.engines.comparison.diagnosticExecutionBoundary,
    correctness: summarizeCorrectness({ candidate: ordered, comparison: [] }, plan),
    metrics: metricIds(plan).map((id) => {
      const samplesMs = ordered.map((run) => metricValue(run, id));
      return { id, samplesMs, medianMs: median(samplesMs) };
    }),
    startupComponents: summarizeSingleStartupComponents(ordered),
  };
}
...
function summarizeSingleStartupComponents(runs) {
  return startupComponentIds().map((id) => {
    const samplesMs = runs.map((run) => startupComponentValue(run, id));
    return { id, samplesMs, medianMs: median(samplesMs) };
  });
}

function summarizeCorrectness(runs, plan) {
  const all = [...runs.candidate, ...runs.comparison];
...
function markdownReport(report) {
  const rows = report.summary.metrics
    .map(
      (metric) =>
  ...
  const diagnosticById = new Map(
    report.summary.mainThreadDiagnostic.metrics.map((metric) => [metric.id, metric]),
  );
  const startupById = new Map(
    report.summary.startupComponents.components.map((component) => [component.id, component]),
  );
  const mainStartupById = new Map(
    report.summary.mainThreadDiagnostic.startupComponents.map((component) => [
      component.id,
      component,
    ]),
  );
  const startupRows = ['public-open', 'immediate-first-query']
    .map((id) => {
      const component = startupById.get(id);
      const main = mainStartupById.get(id);
      return (
        `| \`${id}\` | ${component.candidateMedianMs.toFixed(3)} | ` +
        `${component.comparisonMedianMs.toFixed(3)} | ` +
        `${main.medianMs.toFixed(3)} |`
      );
    })
    .join('\n');
  const diagnosticRows = report.summary.metrics
    .map((metric) => {
      const diagnostic = diagnosticById.get(metric.id);
      return (
        `| \`${metric.id}\` | ${metric.comparisonMedianMs.toFixed(3)} | ` +
        `${diagnostic.medianMs.toFixed(3)} |`
      );
    })
    .join('\n');
...
- Plan: \`${report.plan.id}\`
- Candidate: \`${report.plan.engines.candidate.package}\` (${report.plan.engines.candidate.executionBoundary})
...
- Comparison: \`${report.plan.engines.comparison.package}@${report.plan.engines.comparison.version}\` (${report.plan.engines.comparison.executionBoundary})
...
- Comfortable-win gate: **${report.summary.gate.passed ? 'PASS' : 'FAIL'}** (geomean ${report.summary.geomeanRatio.toFixed(4)}, required <= ${report.summary.gate.maxGeomeanRatio.toFixed(2)})

| Metric (lower is better) | Oliphaunt median ms | PGlite median ms | Median paired ratio |
| --- | ---: | ---: | ---: |
${rows}

The gated engines ran in ${report.plan.measurement.pairedRepeats} same-repeat pairs against fresh in-memory databases. Their launch order alternated evenly, and each paired candidate/PGlite ratio was computed before taking the per-metric median. Every gated operation is timed end-to-end by the host around one public worker RPC. The cold-to-first-result metric combines public open and the immediate first user query so moving lazy work between those phases cannot change their weight. Its components are reported below but are not separately gated.

| Startup component (not separately gated) | Oliphaunt median ms | PGlite worker end-to-end median ms | PGlite main-thread median ms (ungated) |
| --- | ---: | ---: | ---: |
${startupRows}

Bulk operations use each package's public \`execProtocolRaw\` with identical PostgreSQL Simple Query bytes and return raw response bytes across the worker boundary. Outside the timed call, the harness decodes that exact response and requires its command tags and result rows before validating database state. The gate is eligible only after canonical response streams and all recorded PostgreSQL settings agree.

PGlite's official worker wrapper targets browser Worker and Web Locks APIs, so the gated control uses the small harness-owned \`worker_threads\` RPC adapter recorded in provenance. PGlite's official browser benchmark places its timer inside that browser worker; this Node harness records the methodology as reference only and does not collect or serialize comparator-only timing telemetry during gated calls. Direct main-thread measurements are explicitly ungated and run only after the complete gated phase.

| Metric | PGlite worker end-to-end median ms (gated) | PGlite main-thread median ms (ungated) |
| --- | ---: | ---: |
${diagnosticRows}
`;
}
