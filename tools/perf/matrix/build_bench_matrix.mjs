...
async function main() {
  ...
  const legacyRttSqlx = collectRun(node, 'rtt', 'legacy_wasix_sqlx')
  const legacySpeedSqlx = collectRun(node, 'speed', 'legacy_wasix_sqlx')
  ...
  const headlineModes = [
    ...
    {
      label: 'legacy WASIX SQLx',
      rttRun: legacyRttSqlx,
      speedRun: legacySpeedSqlx,
      openMicros: legacyRttSqlx.openMicros,
      connectMicros: legacyRttSqlx.connectMicros,
      setupMicros: legacyRttSqlx.setupMicros,
    },
  ]
  ...
  const speedMaps = {
    ...
    legacySqlx: indexTestsById(legacySpeedSqlx),
  }
  ...
  lines.push(
    `- legacy control packages: \`${nodeServer.package}@${nodeServer.version}\`, \`${nodeServer.socketPackage}@${nodeServer.socketVersion}\``,
  )
  ...
  lines.push(`- Speed source: exact upstream SQL from \`benchmarks/native/sql\``)
  lines.push('')
  lines.push('## Headline')
  lines.push('')
  lines.push('| Metric | native pg + SQLx | oliphaunt-wasix + SQLx | legacy WASIX SQLx |')
  lines.push('|---|---:|---:|---:|')
  ...
  lines.push('')
  lines.push(`- oliphaunt-wasix + SQLx RTT vs legacy WASIX SQLx: ${formatRatio(rttAverageMicros(oxideRttSqlx), rttAverageMicros(legacyRttSqlx))}`)
  lines.push(`- oliphaunt-wasix + SQLx RTT vs native pg + SQLx: ${formatRatio(rttAverageMicros(oxideRttSqlx), rttAverageMicros(nativeRttSqlx))}`)
  lines.push(`- oliphaunt-wasix + SQLx speed total vs legacy WASIX SQLx: ${formatRatio(speedTotalMicros(oxideSpeedSqlx), speedTotalMicros(legacySpeedSqlx))}`)
  lines.push(`- oliphaunt-wasix + SQLx speed total vs native pg + SQLx: ${formatRatio(speedTotalMicros(oxideSpeedSqlx), speedTotalMicros(nativeSpeedSqlx))}`)
  lines.push('')
  lines.push('## Speed Suite')
  lines.push('')
  lines.push('| ID | Test | native pg + SQLx | oliphaunt-wasix + SQLx | legacy WASIX SQLx |')
  lines.push('|---|---|---:|---:|---:|')
  ...
  for (const test of oxideSpeedSqlx.tests) {
    ...
    const legacySqlx = speedMaps.legacySqlx.get(test.id).elapsedMicros
    lines.push(
      `| ${test.id} | ${test.label} | ${formatMillis(nativeSqlx / 1000)} | ${formatMillis(oxideSqlx / 1000)} | ${formatMillis(legacySqlx / 1000)} |`,
    )
  }
  ...
  lines.push('## Notes')
  lines.push('')
  lines.push('- This matrix is meant for local reproducibility, not universal absolute claims. Different CPUs, filesystems, runtime versions, and native Postgres builds will move the numbers.')
  ...
  lines.push('- The SQLx-to-SQLx comparison to focus on in product docs is `native pg + SQLx` vs `oliphaunt-wasix + SQLx` vs `legacy WASIX SQLx`.')
  lines.push('')
