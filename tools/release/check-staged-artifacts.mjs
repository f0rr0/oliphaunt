...
function packagePath(product) {
  return releaseMetadataRelativePath(nonEmptyString(productConfig(product).path, `${product}.path`), `${product}.path`);
}
...
function extensionMetadata(product) {
  ...
  const topLevelSqlName = nonEmptyString(config.extension_sql_name, `${product}.extension_sql_name`);
...
function rejectSdkRuntimePayload(product, artifact, names) {
  for (const name of names) {
    ...
    for (const pattern of SDK_RUNTIME_PAYLOAD_PATTERNS) {
      if (pattern.test(name)) {
        fail(`${product} SDK artifact ${rel(artifact)} must not include runtime/extension payload ${name}`);
      }
    }
  }
}
...
async function checkSdkProduct(product, { require }) {
  ...
  if (["oliphaunt-js", "oliphaunt-react-native"].includes(product)) {
    ...
    for (const archive of archives) {
      rejectSdkRuntimePayload(product, archive, archiveZipNames(archive));
      checked = true;
    }
...
function extensionArtifactKindAllowed(family, target, kind) {
  ...
  if (target === "ios-xcframework") {
    return new Set(["runtime", "ios-xcframework"]).has(kind);
  }
  if (target.startsWith("android-")) {
    return new Set(["runtime", "android-static-archive"]).has(kind);
  }
  return kind === "runtime";
}
...
async function checkExtensionProduct(product, { require, requireFullTargets }) {
  ...
  for (const asset of assets) {
    ...
    const { family, target, kind, name, path: pathValue, sha256, bytes } = asset;
  ...
  if (!deepEqual(releaseData.sourceIdentity, extensionSourceIdentity(product))) {
  ...
  for (const product of exactExtensionProducts(PREFIX)) {
    ...
    if (target === "ios-xcframework") {
      ...
      if (frameworkMatches.length !== 1) {
        fail(`${sqlName} exact-extension package must contain one iOS XCFramework asset`);
      }
    }
    return;
  }
