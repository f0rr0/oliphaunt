...
def check_sdk_product(product: str, *, require: bool) -> bool:
    ...
    if product in {"oliphaunt-js", "oliphaunt-react-native"}:
        ...
        if not listing.is_file():
            if require:
                fail(f"{product} must stage a Cargo package file list under {rel(root)}")
            return False
        entries = {
            line.strip()
        ...
        for required_entry in {
            ...
            if required_entry not in entries:
                fail(f"{product} package file list is missing {required_entry}")
        for entry in entries:
            if entry.startswith(("target/", "src/runtimes/", "src/extensions/generated/")):
                fail(
                    f"{product} package file list contains generated or external payload entry {entry}"
                )
        checked = True
    else:
