...
export const PUBLICATION_CANDIDATE_MANIFEST_PATH =
  'target/release/publication-candidate-manifest.json';
...
function atomicTar(output, entries) {
  ...
  const temporaryRoot = mkdtempSync(
    path.join(path.dirname(destination), '.publication-candidate-pack-'),
  );
...
function capsuleEntries(
  ...
  for (const entry of entries) {
    ...
    if (prior !== undefined)
      throw error(`capsule paths collide by case: ${prior} and ${entry.path}`);
...
function tarOctal(header, offset, length, context) {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 0)
    throw error(`${context} is outside the safe integer range`);
  return parsed;
}
...
function extractCanonicalTar(transport, stage) {
  ...
  try {
    while (position < archiveStat.size) {
      ...
      if (observed.length >= MAX_ARCHIVE_ENTRIES)
        throw error(`publication candidate exceeds ${MAX_ARCHIVE_ENTRIES} entries`);
      ...
      if (prior !== undefined)
        throw error(`publication candidate paths collide by case: ${prior} and ${entry.path}`);
      ...
      if (remainder !== 0) {
        ...
        if (padding.some((byte) => byte !== 0))
          throw error(`${entry.path} has nonzero ustar padding`);
        position += padding.length;
      }
      observed.push({ ...entry, sha256: hash.digest('hex') });
    }
    if (zeroBlocks !== 2)
      throw error('publication candidate is missing its two-block ustar end marker');
    if (position !== archiveStat.size)
      throw error('publication candidate contains bytes after its ustar end marker');
    if (
      observed.map(({ path: member }) => member).join('\n') !==
      observed
...
function verifyObservedEntries(observed, expectedEntries) {
  const actual = observed.map(({ path: member, size, sha256: digest }) => ({
    path: member,
    size,
    sha256: digest,
  }));
  const expected = expectedEntries.map(({ path: member, size, sha256: digest }) => ({
    path: member,
    size,
    sha256: digest,
  }));
  if (stableJson(actual) !== stableJson(expected)) {
    throw error(
      `capsule file set or bytes differ from the approved lock: expected=${JSON.stringify(expected)}, actual=${JSON.stringify(actual)}`,
    );
  }
}
...
export function verifyExtractBootstrapCapsule({
  ...
  const approvedLockBytes = readMetadataFile(
    path.resolve(approvedLock),
    'separately downloaded approved publication lock',
  );
  ...
  try {
    ...
    const embeddedLockFile = workspaceFile(
      stage,
      PUBLICATION_CANDIDATE_LOCK_PATH,
      'embedded publication lock path',
    );
    ...
    if (!embeddedLockBytes.equals(approvedLockBytes)) {
      throw error(
        'embedded publication lock is not byte-identical to the separately downloaded approved lock',
      );
    }
    const lock = loadPublicationLock(embeddedLockFile);
    assertPublicationLockSource(lock, headRef);
    const manifestFile = workspaceFile(
      stage,
      PUBLICATION_CANDIDATE_MANIFEST_PATH,
    ...
    if (
      !parsed.bytes.equals(expectedManifestBytes) ||
      stableJson(parsed.manifest) !== stableJson(expected.manifest)
    ) {
      throw error(
        'publication candidate manifest does not exactly describe its approval, lock, and files',
      );
    }
...
function parseArgs(argv) {
  ...
  if (!new Set(['pack', 'verify-extract']).has(command)) {
    throw error(
      'usage: bootstrap-publication-capsule.mts <pack|verify-extract> --products-json JSON --head-ref SHA ' +
  ...
  if (unknown.length > 0)
    throw error(
      `unsupported ${command} arguments: ${unknown.map((name) => `--${name}`).join(' ')}`,
    );
  ...
  if (missing.length > 0)
    throw error(`${command} requires ${missing.map((name) => `--${name}`).join(' ')}`);
