...
import { ROOT } from './release-cli-utils.mts';
...
export function normalPublicationPlan(lock, products) {
  ...
  for (const carrier of carriers) {
    const omitted = carrier.dependencies.filter(
      (dependency) => allById.has(dependency) && !selectedIds.has(dependency),
    );
    if (omitted.length > 0) {
  ...
  while (remaining.size > 0) {
    const ready = [...remaining]
      ...
      .sort(
        (left, right) =>
          left.firstPublishOrder - right.firstPublishOrder || compareText(left.id, right.id),
      );
    if (ready.length === 0) {
      throw error(
        `publication unit dependency cycle: ${[...remaining].sort(compareText).join(', ')}`,
      );
    }
...
function parseArgs(argv) {
  ...
  if (!lockFile || !productsJson) {
    throw error('usage: normal-publication-plan.mts --lock FILE --products-json JSON');
  }
  let products;
  try {
...
if (import.meta.main) {
  try {
    ...
    console.log(
      JSON.stringify(
        normalPublicationPlan(loadPublicationLock(args.lockFile), args.products),
        null,
        2,
      ),
    );
