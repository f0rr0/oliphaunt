...
async function validateAppleXcframeworkAsset(file) {
  ...
  for (const library of libraries) {
    ...
    platforms.add(`${platform}\0${typeof variant === "string" ? variant : ""}`);
  ...
  const missing = required.filter(([platform, variant]) => !platforms.has(`${platform}\0${variant}`));
  if (missing.length > 0) {
    const rendered = missing
      .map(([platform, variant]) => `${platform}${variant ? `-${variant}` : ""}`)
      .sort()
      .join(", ");
    fail(`SwiftPM Apple XCFramework asset ${file} is missing required slice(s): ${rendered}`);
  }
}
