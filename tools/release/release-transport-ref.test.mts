...
test('transport names normalize one exact full commit into an unambiguous lightweight-tag ref', () => {
  ...
  for (const invalid of ['', 'abc', `${SHA}0`, 'z'.repeat(40)]) {
    assert.throws(
      () => normalizeReleaseTransportCommit(invalid),
      /full lowercase-compatible 40-character SHA/u,
    );
  }
});
...
test('an exact existing transport bypasses moving main only for genuine root reruns', async () => {
  for (const [contentWriteAdmission, operation] of [
    ...
    assert.match(
      calls[0].url,
      new RegExp(`/git/ref/tags/${RELEASE_TRANSPORT_TAG_PREFIX}${SHA}$`, 'u'),
    );
  }
...
test('ensure creates one exact lightweight tag after proving it absent', async () => {
  ...
  assert.ok(
    calls.every(({ init }) => init.redirect === 'error' && init.signal instanceof AbortSignal),
  );
...
test('verify accepts only an existing exact direct commit ref', async () => {
  assert.equal(
    (
      await verifyReleaseTransportRef({
        commit: SHA,
  ...
  for (const [contentWriteAdmission, operation, wrongOperation] of [
    ...
    for (const overrides of [
      ...
      await assert.rejects(
        ...
        new RegExp(
          `${contentWriteAdmission} admission requires the exact root ${operation} GitHub run`,
          'u',
        ),
      );
...
test('current-main proof checks the authenticated exact branch ref and fails closed', async () => {
  const observed = [];
  const invoke = (value, status = 200, env = rootEnvironment('publish')) =>
    proveCurrentMain({
      commit: SHA,
      environment: env,
      fetchImpl: async (url, init) => {
  ...
  const ref = { ref: 'refs/heads/main', object: { type: 'commit', sha: SHA } };
  await invoke(ref);
  assert.equal(observed[0].url, `https://api.github.com/repos/${REPO}/git/ref/heads/main`);
  assert.equal(observed[0].init.redirect, 'error');
  assert.equal(new Headers(observed[0].init.headers).get('authorization'), `Bearer ${TOKEN}`);
  for (const value of [
    null,
    { ...ref, ref: 'refs/heads/other' },
    { ...ref, object: { type: 'tag', sha: SHA } },
    { ...ref, object: { type: 'commit', sha: OTHER_SHA } },
  ]) {
    await assert.rejects(() => invoke(value), /main moved|invalid commit ref/u);
  }
  await assert.rejects(() => invoke({}, 403), /HTTP 403/u);
  const before = observed.length;
  await assert.rejects(
    () => invoke(ref, 200, environment({ GITHUB_REF: 'refs/heads/other' })),
