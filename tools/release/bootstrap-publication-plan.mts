import { lockedCarriers } from './publication-lock.mts';
...
export function bootstrapPublicationPlan(lock, products) {
  ...
  const carriers = allCarriers
    .filter(
      (carrier) => selected.has(carrier.product) && BOOTSTRAP_ECOSYSTEMS.has(carrier.ecosystem),
    )
    .slice();
  ...
  return carriers.map(
    ({
      ...
      dependencies: dependencies.filter((dependency) =>
        BOOTSTRAP_ECOSYSTEMS.has(allById.get(dependency).ecosystem),
      ),
...
// The admitted subset must retain every dependency not already proven public.
export function bootstrapPublicationSchedule(plan, satisfiedCarrierIds) {
  const satisfied = new Set(satisfiedCarrierIds);
  const positions = new Map(plan.map((carrier, index) => [carrier.id, index]));
  if (positions.size !== plan.length || plan.some((carrier) => satisfied.has(carrier.id)))
    throw error('admitted bootstrap carriers must be unique and not already satisfied');
  return plan.map((carrier, index) =>
    carrier.dependencies.flatMap((id) => {
      const dependency = positions.get(id);
      if (dependency === undefined) {
        if (!satisfied.has(id))
          throw error(carrier.id + ' refers to unknown unsatisfied dependency ' + id);
        return [];
      }
      if (dependency >= index) throw error(carrier.id + ' is not ordered after dependency ' + id);
      return [dependency];
    }),
  );
}
