...
const REQUEST_ATTEMPTS = Math.max(1, Number.parseInt(process.env.OLIPHAUNT_REGISTRY_QUERY_ATTEMPTS || "3", 10) || 3);
...
async function requestJson(url, label) {
  ...
  for (let attempt = 0; attempt < REQUEST_ATTEMPTS; attempt += 1) {
    try {
      ...
      lastError = error;
      if (error instanceof RegistryHttpError && !retryableStatus(error.status)) {
        throw error;
      }
    }
    if (attempt + 1 < REQUEST_ATTEMPTS) {
      await sleep(REQUEST_RETRY_DELAY_SECONDS);
    }
  }
