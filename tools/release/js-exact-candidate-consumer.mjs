...
function inspectCandidateInputs(roots, iosExtensionArtifactRoot, contract) {
  ...
  const immutableRoots = [...roots, iosExtensionArtifactRoot];
  return {
    files: [...files, ...iosExtensionFiles].map((file) => ({
...
function main(argv) {
  ...
  try {
    ...
    const inputEnvelopeSha256 = sha256Bytes(JSON.stringify(inputEvidence.files));
    status.inputEnvelopeSha256 = inputEnvelopeSha256;
    status.inputArtifactCount = inputEvidence.files.length;
    status.inputArtifactBytes = inputEvidence.files.reduce((total, artifact) => total + artifact.bytes, 0);
    ...
    writeFileSync(path.join(evidenceRoot, "exact-candidate.json"), `${JSON.stringify(evidence, null, 2)}\n`, "utf8");
    copyFileSync(path.join(consumerRoot, "package-lock.json"), path.join(evidenceRoot, "package-lock.json"));
    copyFileSync(path.join(registryRoot, "report.json"), path.join(evidenceRoot, "local-registry-report.json"));
    status.state = "passed";
  ...
  let finalCause = aggregateExactCandidateErrors(
    "exact-candidate consumer and local-registry cleanup failed",
