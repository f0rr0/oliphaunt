...
import { requestGithubGraphql, requestGithubJsonWithRetry } from './github-read.mts';
...
const BOOTSTRAP_SECRETS = ['CRATES_IO_BOOTSTRAP_TOKEN', 'NPM_BOOTSTRAP_TOKEN'];
...
export function auditGitHubReleaseControls(
  ...
  const hasProtection =
    snapshot?.branchProtection != null && Object.keys(snapshot.branchProtection).length > 0;
  ...
  findings.push(
    repository.full_name === CANONICAL_REPOSITORY
      ...
      : finding(
          'FAIL',
          'repository.identity',
          `expected ${CANONICAL_REPOSITORY}, got ${repository.full_name ?? 'missing'}`,
        ),
  );
  findings.push(
  ...
  findings.push(
    repository.allow_merge_commit === false
      ...
      : finding(
          ...
          'disable merge commits to keep squash-only history',
        ),
  );
  findings.push(
    repository.allow_rebase_merge === false
      ...
      : finding(
          'WARN',
          'repository.rebase-merge',
          'disable rebase merging to keep squash-only history',
        ),
  );
  findings.push(
  ...
  const graphIdentityIsExact =
    graphProtection?.nameWithOwner === CANONICAL_REPOSITORY &&
  ...
  const forcePushBypassInventoryIsComplete =
  ...
  const forcePushBypassesBlocked = forcePushBypassInventoryIsComplete && forcePushBypassCount === 0;
  findings.push(
    graphIdentityIsExact && graphForcePushesBlocked && forcePushBypassesBlocked
      ? finding(
          'PASS',
          'branch.force-push-bypass',
          `${DEFAULT_BRANCH} has no actor-specific force-push bypass`,
        )
      : finding(
          'FAIL',
          'branch.force-push-bypass',
  ...
  findings.push(
    extraChecks.length === 0
      ...
      : finding(
          'WARN',
          'branch.aggregate-only',
          `remove redundant required checks: ${list(extraChecks)}`,
        ),
  );
  ...
  if (governance === 'team') {
    findings.push(
      reviews && Number.isSafeInteger(approvalCount) && approvalCount >= 1
        ? finding('PASS', 'branch.pr-review', 'pull requests require independent approval')
        : finding(
            'FAIL',
            'branch.pr-review',
            'team-governed pull requests must require at least one approval',
          ),
    );
  } else {
    findings.push(
      reviews && approvalCount === 0
        ? finding(
            'PASS',
            'branch.pr-review',
            'solo pull requests do not require an unavailable self-approval',
          )
        : finding(
            'FAIL',
            'branch.pr-review',
  ...
  findings.push(
    bypassCount === 0
      ...
      : finding(
          ...
          `${bypassCount} actor(s) can bypass pull-request review`,
        ),
  );
  if (governance === 'team') {
    findings.push(
      reviews?.require_last_push_approval === true
        ? finding(
            'PASS',
            'branch.last-push-review',
            'a different maintainer must approve the last push',
          )
        : finding(
            'WARN',
            'branch.last-push-review',
            'with a second maintainer, require approval of the last push by someone else',
          ),
    );
  } else {
    findings.push(
      reviews?.require_last_push_approval !== true
        ? finding(
            'PASS',
            'branch.last-push-review',
            'solo governance does not require an unavailable second approver',
          )
        : finding(
            'WARN',
            'branch.last-push-review',
            'last-push approval can make a solo-maintained repository unmergeable',
          ),
    );
  }
  ...
  for (const environmentName of Object.keys(expected).sort()) {
    ...
    if (!entry) {
      findings.push(
        finding('FAIL', `environment.${environmentName}.exists`, `${environmentName} must exist`),
      );
      continue;
    }

    findings.push(
      finding('PASS', `environment.${environmentName}.exists`, `${environmentName} exists`),
    );
    findings.push(
      exactDeploymentPolicy(entry)
        ? finding(
            'PASS',
    ...
    const missingSecrets = readyBootstrap
      ? [
          ...
          ...(actualSecrets.some((secret) => BOOTSTRAP_SECRETS.includes(secret))
            ? []
            : BOOTSTRAP_SECRETS),
        ]
    ...
    findings.push(
      unexpectedSecrets.length === 0
        ? finding(
            'PASS',
            `environment.${environmentName}.secrets-isolated`,
            `${environmentName} has no unexpected secret names`,
          )
        : finding(
            'FAIL',
            `environment.${environmentName}.secrets-isolated`,
            `${environmentName} has unexpected secret names: ${list(unexpectedSecrets)}`,
          ),
    );
    ...
    if (governance === 'team') {
      findings.push(
        reviewerCount(rule) > 0
          ? finding(
              'PASS',
              `environment.${environmentName}.reviewer`,
              `${environmentName} requires an environment reviewer`,
            )
          : finding(
              'WARN',
              `environment.${environmentName}.reviewer`,
              `configure an independent reviewer for ${environmentName} when a second maintainer is available`,
            ),
      );
      findings.push(
        rule?.prevent_self_review === true
          ? finding(
              'PASS',
              `environment.${environmentName}.self-review`,
              `${environmentName} prevents self-review`,
            )
          : finding(
              'WARN',
              `environment.${environmentName}.self-review`,
              `enable prevent-self-review for ${environmentName} with team governance`,
            ),
      );
    } else {
      findings.push(
        rule?.prevent_self_review !== true
          ? finding(
              'PASS',
              `environment.${environmentName}.self-review`,
              `${environmentName} remains operable by a solo maintainer`,
            )
          : finding(
              'WARN',
              `environment.${environmentName}.self-review`,
              `disable prevent-self-review for ${environmentName} while the repository has one maintainer`,
            ),
      );
    }
  }
...
export function githubApiUrl(endpoint) {
  const url = new URL(endpoint, 'https://api.github.com/');
  const root = `/repos/${CANONICAL_REPOSITORY}`;
  if (
    /[\u0000-\u0020\u007f]/u.test(endpoint) ||
...
function githubApiGet(endpoint) {
  return requestGithubJsonWithRetry(githubApiUrl(endpoint));
}

async function collectBranchProtectionGraphql(graphqlRead) {
  const [owner, name] = CANONICAL_REPOSITORY.split('/');
  const response = await graphqlRead(
...
async function paged(apiGet, endpoint, collectionKey) {
  const values = [];
  let page = 1;
  while (true) {
    const separator = endpoint.includes('?') ? '&' : '?';
    const response = await apiGet(`${endpoint}${separator}per_page=100&page=${page}`);
...
export async function collectGitHubReleaseControls(
  apiGet = githubApiGet,
  graphqlRead = requestGithubGraphql,
) {
  const repository = await apiGet(`repos/${CANONICAL_REPOSITORY}`);
  const branchProtection = await apiGet(
    `repos/${CANONICAL_REPOSITORY}/branches/${DEFAULT_BRANCH}/protection`,
  );
  const branchProtectionGraphql = await collectBranchProtectionGraphql(graphqlRead);
  const actionsWorkflowPermissions = await apiGet(
    `repos/${CANONICAL_REPOSITORY}/actions/permissions/workflow`,
  );
  const environmentList = await paged(
    apiGet,
    `repos/${CANONICAL_REPOSITORY}/environments`,
    'environments',
  );
  const environmentByName = new Map(
    environmentList.map((environment) => [environment.name, environment]),
  );
  const environments = {};
  ...
  for (const environmentName of Object.keys(expectedEnvironments('idle')).sort()) {
    ...
    const environment = await apiGet(root);
    const branchPolicies =
      environment.deployment_branch_policy?.custom_branch_policies === true
        ? await paged(apiGet, `${root}/deployment-branch-policies`, 'branch_policies')
        : [];
    const secrets = await paged(apiGet, `${root}/secrets`, 'secrets');
...
function usage() {
  return `usage: tools/dev/bun.sh tools/release/audit-github-release-controls.mts [options]
...
async function main(argv) {
  ...
  try {
    ...
    const snapshot = options.fixture
      ...
      : await collectGitHubReleaseControls();
    ...
    console.error(`audit-github-release-controls.mts: ${error.message}`);
    process.exitCode = 2;
  }
}
