...
def validate_product_local_targets() -> None:
    ...
    if central_targets:
        fail(
            "artifact targets must live under product-local targets directories, "
            f"not central release metadata: {central_targets}"
        )

    expected_runtime_files = {
        "liboliphaunt-native": (
            "src/runtimes/liboliphaunt/native/targets",
            {
                "android-arm64-v8a.toml",
                "android-x86_64.toml",
                "apple-spm-xcframework.toml",
                "checksums.toml",
                "ios-xcframework.toml",
                "linux-arm64-gnu.toml",
                "linux-x64-gnu.toml",
                "macos-arm64.toml",
                "macos-x64.toml",
                "package-size.toml",
                "runtime-resources.toml",
                "windows-x64-msvc.toml",
            },
        ),
        "liboliphaunt-wasix": (
            "src/runtimes/liboliphaunt/wasix/targets",
            {
                "checksums.toml",
                "linux-arm64-gnu.toml",
                "linux-x64-gnu.toml",
        ...
        "oliphaunt-node-direct": (
            "src/runtimes/node-direct/targets",
            {
                "checksums.toml",
                "linux-arm64-gnu.toml",
                "linux-x64-gnu.toml",
    ...
    for product, (directory, expected_files) in expected_runtime_files.items():
        actual_files = {path.name for path in (ROOT / directory).glob("*.toml")}
        if actual_files != expected_files:
            fail(
                f"{product} target metadata files must be exact and explicit: "
                f"{sorted(actual_files)} vs {sorted(expected_files)}"
            )
...
def validate_extension_artifact_targets() -> None:
    ...
    for product in extension_products:
        path = extension_artifact_targets.artifact_target_file(product)
        if not path.is_file():
            fail(f"{product} must declare exact-extension artifact targets at {path.relative_to(ROOT)}")
...
def validate_ci_release_artifacts() -> None:
    ...
    if "swift-sdk-package:\n    name: build-swift-sdk\n    needs:\n      - affected\n      - tests\n      - liboliphaunt-native-ios" not in ci:
        fail("Swift SDK package artifacts must depend on tests and the iOS native target builder that produces the Apple release asset")
    require_text(
        "tools/graph/ci_plan.py",
    ...
    require_text(
        ".github/workflows/release.yml",
        "tools/release/release.py publish --product liboliphaunt-wasix --step github-release-assets",
...
def validate_typescript_runtime_targets() -> None:
    for target in artifact_targets.artifact_targets(
        ...
        template = ts_template(target.asset)
        if target.published:
            require_text(path, template, f"TypeScript native resolver must advertise {target.id}")
            require_text(path, target.target, f"TypeScript native resolver must expose target id {target.target}")
        else:
            reject_text(path, template, f"TypeScript native resolver must not advertise unpublished target {target.id}")
    ...
    for target in artifact_targets.artifact_targets(
        ...
        template = ts_template(target.asset)
        if target.published:
            require_text(path, template, f"TypeScript broker resolver must advertise {target.id}")
            require_text(path, target.target, f"TypeScript broker resolver must expose target id {target.target}")
        else:
            reject_text(path, template, f"TypeScript broker resolver must not advertise unpublished target {target.id}")
    ...
    for target in artifact_targets.artifact_targets(
        ...
        template = ts_template(target.asset)
        if target.published:
            require_text(path, template, f"TypeScript Node direct resolver must advertise {target.id}")
            require_text(path, target.target, f"TypeScript Node direct resolver must expose target id {target.target}")
        else:
            reject_text(path, template, f"TypeScript Node direct resolver must not advertise unpublished target {target.id}")
