...
const AGGREGATOR_BUILD_RS = String.raw`use sha2::{Digest, Sha256};
...
function writeAggregatorCrate(
  ...
  for (let index = 0; index < partCount; index += 1) {
    ...
    dependencyLines.push(`${partName} = { version = "=${version}" }`);
...
function writeToolsFacadeCrate(sourceRoot, { version, toolsTargets }) {
  ...
  for (const target of [...toolsTargets].sort((left, right) => compareText(left.target, right.target))) {
    const packageName = cargoPackageName(target.target, { packageBase: TOOLS_PRODUCT });
    dependencyBlocks.push(
      [
        "",
        `[target.'cfg(${rustArtifactCargoTargetCfg(target)})'.dependencies]`,
...
function writePackagesManifest(packages, outputDir) {
  const data = {
    ...
    packages: packages.map((item) => ({
      ...
      cratePath: item.cratePath === null ? null : rel(item.cratePath),
    })),
  };
...
function usage() {
  fail(
    "usage: tools/release/package-liboliphaunt-cargo-artifacts.mjs [--asset-dir DIR] [--output-dir DIR] [--version VERSION] [--target TARGET]... [--part-bytes BYTES]",
  );
}
...
async function main(argv) {
  ...
  const sourceRoot = path.join(ROOT, "target/liboliphaunt/cargo-package-sources");
  const cargoTargetDir = path.join(ROOT, "target/liboliphaunt/cargo-package-target");
  ...
  packages.push(writeToolsFacadeCrate(sourceRoot, {
    version: args.version,
  ...
  for (const item of packages) {
    console.log(`${item.name} ${item.role} ${item.cratePath === null ? "<source-only>" : rel(item.cratePath)}`);
  }
}
