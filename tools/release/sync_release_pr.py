...
def sync_pnpm_node_direct_optional_specifiers(changes: list[Change], *, write: bool) -> None:
    expected_version = f"workspace:{product_metadata.read_current_version(NODE_DIRECT_PRODUCT)}"
    lines = PNPM_LOCKFILE.read_text(encoding="utf-8").splitlines(keepends=True)
    expected_packages = set(NODE_DIRECT_OPTIONAL_PACKAGES)
    seen: set[str] = set()
    file_changes: list[str] = []

    for index, line in enumerate(lines):
        body, _ = strip_newline(line)
        package_match = PNPM_NODE_DIRECT_KEY_RE.match(body)
    ...
    if missing:
        fail(f"{rel(PNPM_LOCKFILE)} is missing Node direct optional package specifiers: {', '.join(sorted(missing))}")
    if file_changes:
