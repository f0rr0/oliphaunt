...
import {
  ...
  githubApiUrl,
  summarizeFindings,
} from './audit-github-release-controls.mts';
...
async function expectGraphqlCollectionFailure(payload, expected) {
  ...
  await expect(collectGitHubReleaseControls(apiGet, () => payload)).rejects.toThrow(expected);
}
...
describe('GitHub release controls', () => {
  ...
  test('classifies the known unsafe configuration as hard failures and hygiene warnings', () => {
    ...
    expect(findings.find(({ id }) => id === 'environment.release-bootstrap.exists')?.status).toBe(
      'FAIL',
    );
    expect(
      findings.find(({ id }) => id === 'environment.release-dry-run.secrets-isolated')?.status,
    ).toBe('PASS');
  ...
  test('all release environments accept only main', () => {
    for (const environmentName of [
      ...
      expect(
        findings.find(({ id }) => id === `environment.${environmentName}.branch-policy`)?.status,
      ).toBe('FAIL');
    }
  });
  ...
  test('accepts only the lock-required revocable registry credentials while bootstrap is ready', () => {
    for (const token of ['CRATES_IO_BOOTSTRAP_TOKEN', 'NPM_BOOTSTRAP_TOKEN']) {
      ...
      const finding = findings.find(
        ({ id }) => id === 'environment.release-bootstrap.secrets-present',
      );
  ...
  test('idle bootstrap mode rejects prematurely installed bootstrap tokens', () => {
    ...
    const finding = findings.find(
      ({ id }) => id === 'environment.release-bootstrap.secrets-isolated',
    );
    expect(finding?.status).toBe('FAIL');
    expect(finding?.message).toContain('CRATES_IO_BOOTSTRAP_TOKEN');
    expect(finding?.message).toContain('NPM_BOOTSTRAP_TOKEN');
  ...
  test('rejects unverifiable crates.io capacity assertions as stale release secrets', () => {
    ...
    expect(
      findings.find(({ id }) => id === 'environment.release-publish.secrets-present')?.status,
    ).toBe('PASS');
    const isolation = findings.find(
      ({ id }) => id === 'environment.release-publish.secrets-isolated',
    );
  ...
  test('rejects an unknown bootstrap lifecycle state', () => {
    expect(() =>
      auditGitHubReleaseControls(fixture('desired-solo'), {
...
describe('GitHub API snapshot collection', () => {
  test('accepts only canonical repository read URLs', async () => {
    expect(githubApiUrl('repos/f0rr0/oliphaunt')).toBe(
      'https://api.github.com/repos/f0rr0/oliphaunt',
    );
    expect(() => githubApiUrl('repos/another-owner/another-repo')).toThrow(
      /refusing non-canonical/u,
    );
    expect(() => githubApiUrl('repos/f0rr0/oliphaunt-typo')).toThrow(/refusing non-canonical/u);
  });

  test('uses only the fixed canonical read endpoints', async () => {
    const source = fixture('desired-solo');
    const endpoints = [];
    const graphReads = [];
    const apiGet = (endpoint) => {
      ...
      if (endpoint === 'repos/f0rr0/oliphaunt/branches/main/protection')
        return source.branchProtection;
      if (endpoint === 'repos/f0rr0/oliphaunt/actions/permissions/workflow')
        return source.actionsWorkflowPermissions;
      ...
      const environmentName = Object.keys(source.environments).find((name) =>
        endpoint.includes(encodeURIComponent(name)),
      );
      if (!environmentName) throw new Error(`unexpected endpoint ${endpoint}`);
      if (
        endpoint === `repos/f0rr0/oliphaunt/environments/${encodeURIComponent(environmentName)}`
      ) {
    ...
    const graphqlRead = (document, variables) => {
      ...
      return graphqlPayload(source);
    };

    expect(await collectGitHubReleaseControls(apiGet, graphqlRead)).toEqual(source);
    ...
    expect(
      endpoints.every(
        (endpoint) =>
          endpoint === 'repos/f0rr0/oliphaunt' ||
          endpoint.startsWith('repos/f0rr0/oliphaunt/') ||
          endpoint.startsWith('repos/f0rr0/oliphaunt?'),
      ),
    ).toBe(true);
  });

  test('fails closed when GraphQL omits the response data', async () => {
    await expectGraphqlCollectionFailure({}, /exact canonical main ref/u);
  });

  test('fails closed when GraphQL omits the main branch-protection rule', async () => {
    const source = fixture('desired-solo');
    const payload = graphqlPayload(source);
    payload.data.repository.ref.branchProtectionRule = null;
    await expectGraphqlCollectionFailure(payload, /complete main branch-protection rule/u);
  });

  test('fails closed when the force-push bypass collection requires pagination', async () => {
    const source = fixture('desired-solo');
    const payload = graphqlPayload(source);
    payload.data.repository.ref.branchProtectionRule.bypassForcePushAllowances.pageInfo.hasNextPage = true;
    await expectGraphqlCollectionFailure(payload, /bypass inventory is incomplete or malformed/u);
  });

  test('fails closed when the force-push bypass total disagrees with its nodes', async () => {
    const source = fixture('desired-solo');
    const payload = graphqlPayload(source);
    payload.data.repository.ref.branchProtectionRule.bypassForcePushAllowances.totalCount = 1;
    await expectGraphqlCollectionFailure(payload, /bypass inventory is incomplete or malformed/u);
  });

  test('fails closed when GraphQL returns the wrong repository identity', async () => {
    const source = fixture('desired-solo');
    const payload = graphqlPayload(source);
  ...
  test('fails closed when GraphQL returns the wrong ref', async () => {
    const source = fixture('desired-solo');
    const payload = graphqlPayload(source);
