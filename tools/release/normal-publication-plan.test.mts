...
import { buildPlan, loadGraph } from '../../src/shared/product-metadata/release-graph.mts';
import { normalPublicationPlan } from './normal-publication-plan.mts';

function carrier({ id, product, publishOrder, dependencies = [] }) {
...
function realSelection(changedFile) {
  ...
  const catalog = loadPublicationCatalog('normal-publication-plan.test', {
...
describe('normal publication plan', () => {
  test('executes exact frozen carriers in lock-derived dependency order', () => {
    const value = lock([
      ...
      carrier({
        ...
        product: 'sdk',
        publishOrder: 3,
        dependencies: ['npm:@example/runtime'],
      }),
    ]);
  ...
  test('preserves dependencies into and out of the Maven atomic unit', () => {
    const value = lock([
      ...
      carrier({
        ...
        product: 'sdk',
        publishOrder: 1,
        dependencies: ['npm:@example/input'],
      }),
      carrier({
        id: 'npm:@example/output',
        product: 'output',
        publishOrder: 2,
        dependencies: ['maven:dev.example:sdk'],
      }),
    ]);
  ...
  test('fails closed when a selected carrier omits a locked dependency', () => {
    ...
    expect(() => normalPublicationPlan(value, ['sdk'])).toThrow(
      /selection omits locked dependencies: cargo:runtime/u,
    );
  });
  ...
  test('fails closed on cycles instead of inventing an execution order', () => {
    ...
    expect(() => normalPublicationPlan(value, ['a', 'b'])).toThrow(/dependency cycle/u);
  });
  ...
  test('real release selections include exact embedded consumers without unrelated products', () => {
    ...
    expect(
      extensionSqlNames('oliphaunt-extension-contrib-pg18', 'normal-publication-plan.test'),
    ).toContain('amcheck');
    const contribCarriers = runtime.catalog.carriers.filter(({ name }) =>
      name.includes('extension-contrib-pg18'),
    );
    expect(contribCarriers.length).toBeGreaterThan(0);
