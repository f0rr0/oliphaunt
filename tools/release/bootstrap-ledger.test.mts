...
test('immutable checkpoints resume 417 Cargo plus 214 npm identities and reject registry-byte conflicts', () => {
  ...
  try {
    ...
    assert.equal(
      loadBootstrapLedger(chain, lock, ['alpha']).checkpointDigest,
      genesis.checkpointDigest,
    );
    ...
    assert.equal(
      loadBootstrapLedger(chain, lock, ['alpha'], { requireComplete: true }).checkpointDigest,
      complete.checkpointDigest,
    );
    assert.equal(validateBootstrapLedger(complete, lock, ['alpha']), complete);

    const conflicting = structuredClone(receipts[0]);
    conflicting.registryProof.digest =
      conflicting.ecosystem === 'cargo' ? fixedHash('9') : Buffer.alloc(64).toString('base64');
    assert.throws(
    ...
    assert.throws(
      () => validateBootstrapLedger(tamperedChain, lock, ['alpha']),
      /digest mismatch/u,
    );
  } finally {
