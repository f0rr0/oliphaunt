...
describe('ephemeral example Cargo policy', () => {
  test('owns every registry example version and native runtime binding', () => {
    ...
    expect(new Set(bindings.map(({ file }) => file))).toEqual(
      new Set(EXAMPLE_CARGO_POLICIES.map(({ crateDir }) => `${crateDir}/Cargo.toml`)),
    );
    for (const policy of EXAMPLE_CARGO_POLICIES) {
      expect(
        bindings.filter(({ policyId, kind }) => policyId === policy.id && kind === 'dependency'),
  ...
  test('rejects stale runtime metadata, duplicate dependencies, and dependency scope drift', () => {
    ...
    expect(
      validateExampleManifestPolicy(policy, manifest, bindings).some((failure) =>
        failure.includes('direct Oliphaunt dependencies'),
      ),
    ).toBe(true);
  });
  ...
  test('rejects prerelease Wasmer drift', () => {
    ...
    expect(
      validateResolvedPackagePolicy('fixture.lock', packages, {
        wasixToolchain: true,
        toolchainVersions,
      }),
  ...
  test('rejects duplicate or drifted WebC identities from fresh consumer locks', () => {
    const duplicate = [...wasmerPackages, { name: 'webc', version: '11.0.0', source: cratesIo }];
    expect(
      validateResolvedPackagePolicy('fixture.lock', duplicate, {
        wasixToolchain: true,
        toolchainVersions,
      }),
    ...
    expect(
      validateResolvedPackagePolicy('fixture.lock', drifted, {
        wasixToolchain: true,
        toolchainVersions,
      }),
  ...
  test('rejects advisory versions below their floors', () => {
    expect(
      validateResolvedPackagePolicy('fixture.lock', [
  ...
  test('rejects unknown Wasmer and virtual packages instead of accepting stable drift', () => {
    const failures = validateResolvedPackagePolicy(
      'fixture.lock',
      [
        ...wasmerPackages,
