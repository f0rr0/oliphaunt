...
function packageRelative(packagePath, relativePath, context, prefix) {
  ...
  const file = path.posix.normalize(
    path.posix.join(packageRoot, relativePath.replaceAll('\\', '/')),
  );
...
function packageDescriptors(product, graphProduct, packagePath, config, prefix) {
  ...
  if (typeof versionFile === 'string' && versionFile.length > 0) {
    canonical = {
      path: packageRelative(packagePath, versionFile, `${product}.version-file`, prefix),
      type: 'raw',
    };
...
function setObjectPath(value, parts, before, after, context, prefix) {
  ...
  if (cursor[key] !== before) {
    throw error(
      prefix,
      `${context}.${parts.join('.')} contains ${JSON.stringify(cursor[key])}, expected ${before}`,
    );
  }
  cursor[key] = after;
}
...
function replaceGeneric(text, before, after, context, prefix) {
  ...
  if (markedLines.length === 1) {
    ...
    if (versions.length !== 1 || versions[0] !== before) {
      throw error(
        prefix,
        `${context} version marker must own exactly the current version ${before}`,
      );
    }
    return text.replace(markedLines[0], markedLines[0].replace(before, after));
  }
  const blockMatch = /x-release-please-start-version(?<body>[\s\S]*?)x-release-please-end/u.exec(
    text,
  );
  ...
  if (
    ...
    throw error(
      prefix,
      `${context} must have one Release Please marker or marker block owning ${before}`,
    );
  }
...
function mergeExistingChangelog(text, candidate, context, prefix) {
  ...
  if (matches.length !== 1) {
    throw error(
      prefix,
      `${context} must contain exactly one release heading for ${candidate.before}`,
    );
  }
  if (
  ...
  const releaseEnd =
    releaseHeadings.find(({ index }) => index > releaseStart)?.index ?? lines.length;
  ...
  if (sectionMatches.length > 1) {
    throw error(
      prefix,
      `${context} release ${candidate.before} contains duplicate ${sectionHeading} sections`,
    );
  }
  if (sectionMatches.length === 1) {
...
function packagesByProduct(releasePleaseConfig, prefix) {
  ...
  for (const [packagePath, packageConfig] of Object.entries(packages).sort(([left], [right]) =>
    ...
    if (typeof product !== 'string' || product.length === 0 || byProduct.has(product)) {
      throw error(
        prefix,
        `release-please package ${packagePath} has a missing or duplicate component`,
      );
    }
...
export function synchronizeReleaseCandidates({
  ...
  for (const candidate of candidates) {
    ...
    if (candidate.before === candidate.after && !mergeExisting) {
      throw error(
        prefix,
        `${candidate.product} release candidate does not advance from ${candidate.before}`,
      );
    }
    ...
    const detail =
      `${candidate.product} ${mergeExisting ? 'shared-source' : 'dependency-only'} changelog ` +
      `for ${candidate.after}`;
    if (mergeExisting) {
  ...
  if (duplicatePaths.length > 0) {
    throw error(
      prefix,
      `release candidate outputs overlap: ${[...new Set(duplicatePaths)].join(', ')}`,
    );
  }
  if (write) {
