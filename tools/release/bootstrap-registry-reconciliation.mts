...
function inventoryStates(ecosystem, expectedCarriers, inventory) {
  if (
    ...
    throw error(
      `${ecosystem} inventory must contain selected, published, pending-version, and missing-name lists`,
    );
  }
  const expectedKeys = uniqueSet(
    expectedCarriers.map(({ name, version }, index) =>
      requiredIdentity({ name, version }, `${ecosystem} plan entry ${index}`),
    ),
    `${ecosystem} plan`,
  );
  const selectedKeys = uniqueSet(
    inventory.selectedIdentities.map((identity, index) =>
      requiredIdentity(identity, `${ecosystem} selected identity ${index}`),
    ),
    `${ecosystem} selected identities`,
  );
  if (!sameSet(expectedKeys, selectedKeys)) {
    throw error(
      `${ecosystem} inventory selection does not exactly match the frozen bootstrap plan`,
    );
  }

  const published = uniqueSet(
    inventory.publishedIdentities.map((identity, index) =>
      requiredIdentity(identity, `${ecosystem} published identity ${index}`),
    ),
    `${ecosystem} published identities`,
  );
  const pendingVersions = uniqueSet(
    inventory.pendingVersions.map((identity, index) =>
      requiredIdentity(identity, `${ecosystem} pending version ${index}`),
    ),
  ...
  const missingNames = uniqueSet(
    inventory.missingNames.map((name, index) => {
      if (typeof name !== 'string' || name.length === 0) {
        throw error(`${ecosystem} missing name ${index} must be a non-empty string`);
      }
      if (!byName.has(name))
        throw error(`${ecosystem} inventory contains unknown missing name ${name}`);
      return name;
    }),
  ...
  for (const carrier of expectedCarriers) {
    ...
    const matches =
      Number(published.has(key)) +
      Number(pendingVersions.has(key)) +
      Number(missingNames.has(carrier.name));
    if (matches !== 1) {
      throw error(
        `${carrier.id} must have exactly one registry inventory state, observed ${matches}`,
      );
    }
  ...
  for (const key of [...published, ...pendingVersions]) {
    if (!expectedKeys.has(key))
      throw error(`${ecosystem} inventory contains an identity outside the frozen bootstrap plan`);
  }
  return states;
}
...
export function reconcileBootstrapRegistryState({
  ...
  const publicCarrierIds = plan
    .filter(({ id }) => states.get(id) === 'published')
    .map(({ id }) => id);
  ...
  if (checkpoint !== null) {
    ...
    for (const receipt of checkpoint.receipts) {
      if (!planIds.has(receipt?.id)) {
        throw error(
          `bootstrap checkpoint contains receipt outside the active plan: ${String(receipt?.id)}`,
        );
      }
      if (states.get(receipt.id) !== 'published') {
        throw error(
          `${receipt.id} has an immutable receipt but its exact registry version is not public`,
        );
      }
    }
  }
...
export function resolveBootstrapScope(plan, reconciliation, checkpoint = null) {
  ...
  if (scopeIds.length === 0) {
    throw error(
      'the approved candidate has no absent Cargo/npm package names to bootstrap; use normal publish',
    );
  }
