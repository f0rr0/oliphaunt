...
import { PLATFORM_COMPATIBILITY_POLICY } from './platform-compatibility-policy.mts';
import { allArtifactTargets, extensionArtifactTargets } from './release-artifact-targets.mts';
...
const artifacts = allArtifactTargets().filter((target) => BINARY_ARTIFACT_KINDS.has(target.kind));
...
function validateCoverage(policy, artifacts, extensions) {
  ...
  const unused = Object.keys(policy)
    .filter((target) => !uses.has(target))
    .sort();
  if (unused.length > 0) {
    throw new Error(
      `platform compatibility contract(s) are not used by a carrier: ${unused.join(', ')}`,
    );
  }
  return uses;
}
...
describe('platform compatibility policy', () => {
  test('is an exact bidirectional map of native, broker, Node, and extension carriers', () => {
    const uses = validateCoverage(PLATFORM_COMPATIBILITY_POLICY, artifacts, extensions);
