...
function rowsFromAssetCounts(assetCounts) {
  ...
  for (const [product, assetCount] of assetCounts) {
    if (
      ...
      throw error(
        'assetCounts must contain non-empty product names and non-negative safe integer counts',
      );
    }
  ...
  return rows.sort(
    (left, right) => right.assetCount - left.assetCount || compareText(left.product, right.product),
  );
}
 ...
 */
export function githubReleaseAssetUploadWaveWindowMs(
  rows,
  {
  ...
  for (const row of rows) {
    if (
      ...
      throw error(
        'upload wave rows must contain unique products and non-negative safe integer counts',
      );
    }
...
export function concurrentGithubReleaseAssetUploadPlan(
  assetCounts,
  {
