...
} from './platform-compatibility-policy.mts';
import {
...
export const DESKTOP_TARGETS = {
  'linux-arm64-gnu': {
    ...
    wasixLlvmUrl:
      'https://github.com/wasmerio/llvm-custom-builds/releases/download/22.x/llvm-linux-aarch64.tar.xz',
  ...
  'windows-x64-msvc': {
    ...
    wasixLlvmUrl:
      'https://github.com/wasmerio/llvm-custom-builds/releases/download/22.x/llvm-windows-amd64.tar.xz',
...
const PRODUCT_PRESETS = {
  'postgres-tools-native': 'postgres-tools-native',
  'postgres-tools-wasix': 'postgres-tools-wasix',
...
function productConfigs(prefix) {
  if (!productsCache.has(prefix)) productsCache.set(prefix, loadProducts(prefix));
  return productsCache.get(prefix);
}

export function contribCarrierDescriptor(prefix = 'release-artifact-targets.mts') {
  ...
  for (const [family, owner] of [
    ['native', descriptor.nativeOwner],
    ['wasix', descriptor.wasixOwner],
  ]) {
    if (productConfigs(prefix)[owner] === undefined) {
...
export function extensionReleaseProduct(product, family, prefix = 'release-artifact-targets.mts') {
...
export function extensionReleaseVersion(product, family, prefix = 'release-artifact-targets.mts') {
...
export function extensionArtifactProductRoot(
  ...
  prefix = 'release-artifact-targets.mts',
) {
...
export function extensionArtifactProductsForReleaseProducts(
  releaseProducts,
  { family = null, prefix = 'release-artifact-targets.mts' } = {},
) {
  if (
    !Array.isArray(releaseProducts) ||
    releaseProducts.some((product) => typeof product !== 'string' || !product)
  ) {
...
function artifactTargetConfig(product, expectedPreset, prefix) {
  ...
  if (config.preset !== expectedPreset) {
    fail(
      prefix,
      `Moon release metadata for ${product} artifactTargets.preset must be ${expectedPreset}`,
    );
  }
  return config;
}
...
function productTargets(product, expectedPreset, knownTargets, prefix) {
  ...
  const duplicates = [
    ...new Set(targets.filter((target, index) => targets.indexOf(target) !== index)),
  ];
  if (duplicates.length > 0) {
    fail(
      prefix,
      `Moon release metadata for ${product} artifactTargets.targets contains duplicates`,
    );
  }
  ...
  if (unknown.length > 0) {
    fail(
      prefix,
      `Moon release metadata for ${product} declares unknown artifact target(s): ${unknown.join(', ')}`,
    );
  }
  return targets;
}
...
function nativeLibraryRelativePath(target) {
  ...
  fail('release-artifact-targets.mts', `unsupported liboliphaunt native target ${target}`);
}
...
export function liboliphauntNativeBuildRoot(target) {
  if (!(target in NATIVE_RUNTIME_TARGETS)) {
    fail('release-artifact-targets.mts', `unknown liboliphaunt-native target ${target}`);
  }
  const roots = {
...
export function liboliphauntAndroidAbi(target) {
  ...
  if (!abi) {
    fail(
      'release-artifact-targets.mts',
      `unsupported React Native Android runtime target ${target}`,
    );
  }
  return abi;
}
...
function postgresToolsNativeRows(prefix) {
  ...
  const targets = new Set(
    productTargets(
      product,
      PRODUCT_PRESETS[product],
      new Set(Object.keys(DESKTOP_TARGETS)),
      prefix,
    ),
  );
  const rows = [];
  for (const target of [...targets].filter((item) => item in DESKTOP_TARGETS).sort(compareText)) {
    ...
    rows.push({
      ...
      binary_compatibility: requiredBinaryCompatibility(target, `${product} native tools`, prefix),
      _source_file: 'Moon release metadata',
    });
  }
  return rows;
...
function postgresToolsWasixRows(prefix) {
  ...
  return targets.map((target) => {
    ...
    const platform = DESKTOP_TARGETS[target];
    return {
      id: `${product}.${portable ? 'portable' : `aot-${target}`}`,
      product,
      kind: portable ? 'wasix-tools' : 'wasix-tools-aot',
      target,
      ...(portable
        ? {}
        : {
            triple: platform.triple,
            runner: platform.runner,
            npm_package: `@oliphaunt/liboliphaunt-wasix-tools-${target === 'macos-arm64' ? 'darwin-arm64' : target === 'windows-x64-msvc' ? 'win32-x64-msvc' : target}`,
            npm_os: platform.npmOs,
            npm_cpu: platform.npmCpu,
            npm_libc: platform.npmLibc,
          }),
      asset: portable
        ? 'postgres-tools-wasix-{version}-portable.tar.gz'
        : `postgres-tools-wasix-{version}-aot-${target}.tar.gz`,
      surfaces: portable ? ['github-release'] : ['github-release', 'npm-optional'],
      _source_file: 'Moon release metadata',
    };
  });
}
...
function liboliphauntWasixPostmasterRows(prefix) {
  ...
  for (const target of productTargets(
    ...
    rows.push({
      ...
      surfaces: ['github-release'],
      binary_compatibility: requiredBinaryCompatibility(target, `${product} runtime`, prefix),
  ...
  for (const target of productTargets(
    ...
    rows.push({
      ...
      executable_relative_path:
        target === 'windows-x64-msvc' ? 'bin/oliphaunt-broker.exe' : 'bin/oliphaunt-broker',
...
function databaseResourceRows() {
  const rows = [
    {
      id: 'database-resources.swift-source',
    ...
    },
    {
      ...
      kind: 'icu-data',
      target: 'portable',
      asset: 'database-resources-{version}-icu-data.tar.gz',
      npm_package: '@oliphaunt/icu',
      surfaces: ['github-release', 'maven', 'swiftpm', 'react-native-ios', 'react-native-android'],
      _source_file: 'Moon release metadata',
    },
  ];
  for (const { family, target, profile, suffix } of seedCarrierIdentities()) {
    for (const extension of ['tar.zst', 'json']) {
      rows.push({
        id: `database-resources.${family}.${target}.${profile}.${extension}`,
        product: 'database-resources',
        kind: `${family}-seeds`,
        target,
...
export function rawArtifactTargetRows(prefix = 'release-artifact-targets.mts') {
  return [
    {
      id: 'oliphaunt-swift.bindings',
    ...
    ...postgresToolsNativeRows(prefix),
    ...postgresToolsWasixRows(prefix),
...
export function allArtifactTargets(
  { product = undefined, kind = undefined, surface = undefined } = {},
  prefix = 'release-artifact-targets.mts',
) {
  const products = productConfigs(prefix);
  const seen = new Set();
...
export function typescriptOptionalRuntimePackageProducts(prefix = 'release-artifact-targets.mts') {
...
export function nativeToolsOptionalPackageProducts(prefix = 'release-artifact-targets.mts') {
  const selected = allArtifactTargets(
    { product: 'postgres-tools-native', kind: 'native-tools' },
    prefix,
  );
  if (selected.length === 0) {
...
export function ciReleaseAssetArtifactRows(product, kind, prefix = 'release-artifact-targets.mts') {
  return ciArtifactRows(
    {
      ...
      name: (target) =>
        product === 'database-resources'
          ? `${product}-${kind}-${target.target}`
          : `${product}-release-assets-${target.target}`,
    },
    prefix,
  );
}

export function ciNpmPackageArtifactRows(product, kind, prefix = 'release-artifact-targets.mts') {
  return ciArtifactRows(
    {
      product,
      kind,
...
export function expectedAssetRows(
  { product, version, surface = 'github-release', kinds = undefined } = {},
  prefix = 'release-artifact-targets.mts',
) {
  ...
  if (
    kindSet !== undefined &&
    (kindSet.size === 0 ||
      [...kindSet].some((kind) => typeof kind !== 'string' || kind.length === 0))
  ) {
...
export function registryPackageRows(
  { product, packageKind = undefined } = {},
  prefix = 'release-artifact-targets.mts',
) {
  if (typeof product !== 'string' || product.length === 0) {
    fail(prefix, 'registry package rows require a product');
  }
  if (packageKind !== undefined && (typeof packageKind !== 'string' || packageKind.length === 0)) {
  ...
  if (
    ...
    declaredEntries.some((entry) => typeof entry !== 'string')
  ) {
...
export function releaseMetadata(product, prefix) {
  ...
  if (releaseMetadataCache.has(key)) return releaseMetadataCache.get(key);
  const config = productConfig(product, prefix);
  let directory = path.join(ROOT, config.path);
  let release;
  // Only literal product metadata is needed here. Dependency resolution stays in Moon.
  while (directory !== ROOT) {
    const file = path.join(directory, 'moon.yml');
    if (existsSync(file)) {
      const project = Bun.YAML.parse(readFileSync(file, 'utf8'));
      if (project.id !== product)
        fail(prefix, `Moon project at ${rel(file)} does not own ${product}`);
      release = project.project?.release;
      break;
    }
    directory = path.dirname(directory);
    if (!directory.startsWith(ROOT + path.sep)) break;
  }
  if (!release) {
  ...
  if (release.packagePath !== config.path) {
    fail(prefix, `Moon release metadata for ${product} must match package path ${config.path}`);
  }
  ...
  if (expectedPreset !== undefined) {
    ...
    if (
      ...
      fail(
        prefix,
        `Moon release metadata for ${product} must use artifactTargets preset ${expectedPreset}`,
      );
    }
  }
  releaseMetadataCache.set(key, release);
  return release;
}

const versionCache = new Map();

export function currentProductVersionSync(product, prefix = 'release-artifact-targets.mts') {
  const key = `${prefix}\0${product}`;
  if (!versionCache.has(key)) {
    const versionFile = versionFiles(product, prefix)[0];
...
export async function currentProductVersion(product, prefix = 'release-artifact-targets.mts') {
  return currentProductVersionSync(product, prefix);
}

export function expectedAssets(product, kind, version, prefix) {
  const assets = expectedAssetRows({ product, version, kinds: [kind] }, prefix).map(
    (row) => row.assetName,
  );
...
function productConfig(product, prefix) {
  const config = productConfigs(prefix)[product];
  if (!config) {
...
export function exactExtensionProducts(prefix = 'release-artifact-targets.mts') {
  const products = Object.entries(productConfigs(prefix))
...
export function exactExtensionReleaseProducts(prefix = 'release-artifact-targets.mts') {
  return Object.entries(productConfigs(prefix))
    .filter(([, config]) => EXTENSION_PRODUCT_KINDS.has(config.kind))
    .map(([product]) => product)
    .sort(compareText);
}

export function sdkPackageProducts(prefix = 'release-artifact-targets.mts') {
  const rows = Object.entries(productConfigs(prefix))
    .filter(
      ([, config]) =>
        config.kind === 'sdk' ||
        config.release_artifacts?.includes('cargo-crate') ||
        config.release_artifacts?.includes('npm-package'),
    )
    .map(([product]) => ({
      product,
      artifactName:
        product === 'oliphaunt-wasix-rust' || product === 'postgres-tools-wasix'
...
export function extensionSqlName(product, prefix = 'release-artifact-targets.mts') {
  const names = extensionSqlNames(product, prefix);
  if (names.length !== 1) {
    fail(
      prefix,
      `${product} owns ${names.length} exact extension members; use extensionSqlNames(product)`,
    );
  }
  return names[0];
}
...
function contribMemberRows(product, prefix) {
  ...
  for (const [index, row] of descriptor.members.entries()) {
    ...
    const sqlName = nonEmptyString(
      row['sql-name'],
      `${manifestPath}.extensions[${index}].sql-name`,
      prefix,
    );
    if (seenIds.has(id) || seenSqlNames.has(sqlName)) {
      fail(
        prefix,
        `${manifestPath} contains duplicate contrib member id or SQL name: ${id}/${sqlName}`,
      );
    }
    seenIds.add(id);
...
export function extensionSqlNamesForProducts(products, prefix = 'release-artifact-targets.mts') {
  const rows = [...products].flatMap((product) =>
    extensionSqlNames(product, prefix).map((sqlName) => ({ product, sqlName })),
  );
...
export function extensionSqlNames(product, prefix = 'release-artifact-targets.mts') {
  const contrib = contribCarrierDescriptor(prefix);
  if (product === contrib.artifactProduct) {
    return contribMemberRows(product, prefix)
      .map((row) => row.sqlName)
      .sort(compareText);
  }
  const config = productConfig(product, prefix);
  ...
  const manifestNames = contribMemberRows(product, prefix)
    .map((row) => row.sqlName)
    .sort(compareText);
  if (JSON.stringify(sorted) !== JSON.stringify(manifestNames)) {
    fail(
      prefix,
      `${product}.extension_sql_names must exactly match ${config.extension.member_manifest}`,
    );
  }
  return sorted;
}
...
function extensionCatalogRows(prefix) {
  ...
  if (catalog?.['format-version'] !== 1 || !Array.isArray(catalog.extensions)) {
    fail(
      prefix,
      `${rel(EXTENSION_CATALOG_PATH)} must use format-version 1 and define extension rows`,
    );
  }
  ...
  for (const [index, row] of catalog.extensions.entries()) {
    ...
    if (moduleFile !== undefined && (typeof moduleFile !== 'string' || !moduleFile)) {
      fail(
        prefix,
        `${rel(EXTENSION_CATALOG_PATH)} ${sqlName}.native-module-file must be a non-empty string when present`,
      );
    }
...
export function extensionPublicDependencySqlNames(
  ...
  if (
    ...
    fail(
      prefix,
      `${rel(EXTENSION_CATALOG_PATH)} ${sqlName}.dependencies must be an array of SQL names`,
    );
  }
  return [...new Set(dependencies.filter((dependency) => rows.has(dependency)))].sort(compareText);
}

export function extensionWasixAotMemberSqlNames(product, prefix = 'release-artifact-targets.mts') {
  const rows = extensionCatalogRows(prefix);
...
export function extensionProductForSqlName(sqlName, prefix = 'release-artifact-targets.mts') {
  nonEmptyString(sqlName, 'extension SQL name', prefix);
  const owners = exactExtensionProducts(prefix).filter((product) =>
    extensionSqlNames(product, prefix).includes(sqlName),
  );
  if (owners.length !== 1) {
    fail(
      prefix,
      `extension SQL name ${JSON.stringify(sqlName)} must have exactly one release product owner, found ${owners.join(', ') || 'none'}`,
    );
  }
  return owners[0];
}
...
export function extensionMemberPath(product, sqlName, prefix = 'release-artifact-targets.mts') {
  ...
  const contrib = contribCarrierDescriptor(prefix);
  if (product === contrib.artifactProduct) {
    const row = contribMemberRows(product, prefix).find(
...
export function extensionMetadata(product, prefix = 'release-artifact-targets.mts') {
  const contrib = contribCarrierDescriptor(prefix);
  if (product === contrib.artifactProduct) {
    ...
    if (!/^[1-9][0-9]*$/u.test(postgresMajor)) {
      fail(
        prefix,
        `${contrib.sourcePath}.postgresql.version must begin with a stable major version`,
      );
    }
    return {
      sqlName: undefined,
  ...
  if (config.kind === 'exact-extension-artifact') {
    ...
    if (sqlName !== sqlNames[0]) {
      fail(
        prefix,
        `${product}.extension.sql_name ${JSON.stringify(sqlName)} must match extension_sql_name ${JSON.stringify(sqlNames[0])}`,
      );
    }
  ...
  if (!(extensionClass in EXTENSION_VERSIONING_BY_CLASS)) {
    fail(
      prefix,
      `${product}.extension.class must be one of ${Object.keys(EXTENSION_VERSIONING_BY_CLASS).sort(compareText).join(', ')}`,
    );
  }
  ...
  if (versioning !== expectedVersioning) {
    fail(
      prefix,
      `${product}.extension.versioning must be ${JSON.stringify(expectedVersioning)} for class ${JSON.stringify(extensionClass)}, got ${JSON.stringify(versioning)}`,
    );
  }
  ...
  if (extensionClass === 'contrib' && sourcePath !== contrib.sourcePath) {
    fail(
      prefix,
      `${product}.extension.source.path must match the shared contrib source ${JSON.stringify(contrib.sourcePath)}`,
    );
  }
  if (extensionClass === 'external' && sourcePath !== `${packageRoot}/source.toml`) {
    fail(
      prefix,
      `${product}.extension.source.path must be ${packageRoot}/source.toml for external extensions`,
    );
  }
  if (
    extensionClass === 'first-party' &&
    !(sourcePath === packageRoot || sourcePath.startsWith(`${packageRoot}/`))
  ) {
    fail(
      prefix,
      `${product}.extension.source.path must stay inside ${packageRoot}/ for first-party extensions`,
    );
  }
  ...
  const postgresMajor = nonEmptyString(
    compatibility.postgres_major,
    `${product}.extension.compatibility.postgres_major`,
    prefix,
  );
  if (postgresMajor !== '18') {
    fail(
      prefix,
      `${product}.extension.compatibility.postgres_major must be '18', got ${JSON.stringify(postgresMajor)}`,
    );
  }
  ...
  if (contractPath !== contrib.runtimeContract) {
    fail(
      prefix,
      `${product}.extension.compatibility.extension_runtime_contract must match ${JSON.stringify(contrib.runtimeContract)}`,
    );
  }
  const nativeProduct = nonEmptyString(
    compatibility.native_runtime_product,
    `${product}.extension.compatibility.native_runtime_product`,
    prefix,
  );
  const wasixProduct = nonEmptyString(
    compatibility.wasix_runtime_product,
    `${product}.extension.compatibility.wasix_runtime_product`,
    prefix,
  );
  if (nativeProduct !== 'liboliphaunt-native') {
    fail(
      prefix,
      `${product}.extension.compatibility.native_runtime_product must be 'liboliphaunt-native'`,
    );
  }
  if (wasixProduct !== 'liboliphaunt-wasix') {
    fail(
      prefix,
      `${product}.extension.compatibility.wasix_runtime_product must be 'liboliphaunt-wasix'`,
    );
  }
  const nativeVersion = nonEmptyString(
    compatibility.native_runtime_version,
    `${product}.extension.compatibility.native_runtime_version`,
    prefix,
  );
  const wasixVersion = nonEmptyString(
    compatibility.wasix_runtime_version,
    `${product}.extension.compatibility.wasix_runtime_version`,
    prefix,
  );
  return {
    sqlName,
...
export function extensionSourceIdentity(product, prefix = 'release-artifact-targets.mts') {
  ...
  if (metadata.class === 'contrib') {
    ...
    if (postgresql === null || Array.isArray(postgresql) || typeof postgresql !== 'object') {
      fail(
        prefix,
        `${metadata.sourcePath} must declare [postgresql] for contrib extension products`,
      );
    }
    return {
      ...
      version: nonEmptyString(
        postgresql.version,
        `${metadata.sourcePath}.postgresql.version`,
        prefix,
      ),
  ...
  fail(
    prefix,
    `${product}.extension.class has unsupported source identity class ${JSON.stringify(metadata.class)}`,
  );
}
...
function runtimeExtensionTargetRows(prefix) {
  ...
  for (const target of allArtifactTargets(
    ...
    rows.push({
      ...
      family: 'native',
      kind:
        target.target === 'ios-xcframework' || target.target.startsWith('android-')
...
function readExtensionTargetRows(prefix) {
  ...
  const allowed = new Set(
    runtimeExtensionTargetRows(prefix).map((row) => `${row.target}\0${row.family}\0${row.kind}`),
  );
  ...
  for (const row of rows) {
    if (!allowed.has(`${row.target}\0${row.family}\0${row.kind}`)) {
      fail(
        prefix,
        `${relative} target row ${row.target}/${row.family}/${row.kind} is not backed by runtime artifact metadata`,
      );
    }
  }
  return rows;
...
export function extensionArtifactTargets(
  { product = undefined, family = undefined } = {},
  prefix = 'release-artifact-targets.mts',
) {
  ...
  for (const productId of products) {
    ...
    for (const sqlName of extensionSqlNames(productId, prefix)) {
      ...
      for (const [index, row] of readExtensionTargetRows(prefix).entries()) {
        ...
        const targetFamily = nonEmptyString(
          row.family,
          `${source} targets[${index}].family`,
          prefix,
        );
...
export function extensionTargetIds({ family }, prefix = 'release-artifact-targets.mts') {
  return [
...
export function extensionRegistryPackageTargetSets(
  product,
  prefix = 'release-artifact-targets.mts',
) {
  ...
  const mismatched = memberSignatures
    .filter(({ rows }) => JSON.stringify(rows) !== baseline)
    .map(({ sqlName }) => sqlName);
  if (mismatched.length > 0) {
    fail(
      prefix,
      `${product} bundle members must publish an identical target carrier set; mismatched members: ${mismatched.join(', ')}`,
    );
  }
  const nativeDynamicTargets = extensionPublishedTargets(
    product,
    'native',
    'native-dynamic',
    prefix,
  ...
  const androidTargets = extensionPublishedTargets(
    product,
    'native',
    'native-static-registry',
    prefix,
