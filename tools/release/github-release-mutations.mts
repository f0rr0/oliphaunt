...
export const GITHUB_RELEASE_SNAPSHOT_VISIBILITY_DELAYS_MS = Object.freeze([
...
export async function githubPaginatedArrayRead(repo, resource, options = {}) {
  ...
  return await requestGithubPages(`${prefix}/${resource}`, {
    ...options,
    itemsField: null,
...
export async function githubOptionalJsonRead(args, options = {}) {
  try {
    return await requestGithubRepositoryJson(args, options);
  } catch (error) {
...
export async function readTagRef(repo, tag, options = {}) {
  const label = `GitHub tag ref ${tag}`;
  const value = await githubOptionalJsonRead(
    `${repositoryPath(repo)}/git/ref/tags/${endpointSegment(tag, 'tag')}`,
    { ...options, label },
  );
...
export async function readReleaseByTag(repo, tag, options = {}) {
  const label = `GitHub release for tag ${tag}`;
  const value = await githubOptionalJsonRead(
    `${repositoryPath(repo)}/releases/tags/${endpointSegment(tag, 'tag')}`,
    { ...options, label },
  );
  return value === null ? null : assertReleaseShape(value, label);
}

export async function readReleaseById(repo, releaseId, options = {}) {
  if (!Number.isSafeInteger(releaseId) || releaseId <= 0) {
    throw mutationError('GitHub release id must be a positive integer');
  }
  const label = `GitHub release ${releaseId}`;
  const value = await githubOptionalJsonRead(`${repositoryPath(repo)}/releases/${releaseId}`, {
    ...options,
    label,
  });
  return value === null ? null : assertReleaseShape(value, label);
...
export async function readReleaseMap(repo, options = {}) {
  const label = 'GitHub release list';
  const releases = await githubPaginatedArrayRead(repo, 'releases', { ...options, label });
  const byTag = new Map();
  const byId = new Map();
...
export async function readReleaseAssets(repo, releaseId, options = {}) {
  if (!Number.isSafeInteger(releaseId) || releaseId <= 0) {
    throw mutationError('GitHub release id must be a positive integer');
  }
  const label = `GitHub release ${releaseId} asset list`;
  const assets = await githubPaginatedArrayRead(repo, `releases/${releaseId}/assets`, {
    ...options,
    label,
  });
...
export async function reconcileGitHubMutation({ inspect, label, mutate, options = {} }) {
  ...
  const inspectState = async (phase) => {
    ...
    return assertReconciliationState(
      await inspect({ attempt: mutationAttempts, phase, remainingMs }),
      label,
    );
  };

  while (mutationAttempts < settings.maxAttempts) {
    const before = await inspectState('pre-mutation reconciliation');
    ...
    if (remainingMs < settings.attemptTimeoutMs) {
      throw mutationError(
        `${label}: GitHub mutation deadline reached before attempt ${mutationAttempts + 1}`,
      );
    }
    mutationAttempts += 1;
    ...
    const after = await inspectState('post-mutation reconciliation');
  ...
  throw mutationError(`${label}: exhausted ${mutationAttempts} mutation attempt(s); ${detail}`, {
    cause: lastMutationError ?? undefined,
  });
}
...
function assertJsonMutation(endpoint, input) {
  if (typeof input !== 'string' || Buffer.byteLength(input, 'utf8') > 1024 * 1024) {
    throw mutationError('GitHub JSON mutation must use one bounded JSON payload');
  }
  let payload;
  try {
  ...
  if (/\/git\/refs$/u.test(endpoint)) {
    ...
    if (
      !/^refs\/tags\/[^\u0000-\u001f\u007f]+$/u.test(payload.ref) ||
      !FULL_SHA.test(payload.sha)
    ) {
      throw mutationError(
        'GitHub tag creation payload must bind one tag to a full lowercase commit SHA',
      );
    }
    return;
  }
...
function assertAssetMutation(endpoint, { method, file, input }) {
  if (input !== undefined || method !== 'POST') {
  ...
  repositoryPath(url.pathname.split('/').slice(2, 4).join('/'));
...
function assertMutationRequest(endpoint, { method, file, input }) {
  if (typeof endpoint !== 'string') throw mutationError('GitHub mutation endpoint is required');
  if (endpoint.startsWith('https://uploads.github.com/')) {
    assertAssetMutation(endpoint, { method, file, input });
    return;
  }
  repositoryPath(endpoint.split('/').slice(1, 3).join('/'));
  ...
  const releasePleaseLabelAdd = /^repos\/[^/\s]+\/[^/\s]+\/issues\/[1-9][0-9]*\/labels$/u.test(
    endpoint,
  );
  ...
  if (releasePleasePendingLabelDelete) {
    if (method !== 'DELETE' || input !== undefined || file !== undefined) {
      throw mutationError(
  ...
  if (file !== undefined || method !== (releasePromote ? 'PATCH' : 'POST'))
    throw mutationError(
      'GitHub mutation method or payload is outside the frozen release allowlist',
    );
  assertJsonMutation(endpoint, input);
}

export async function requestGithubMutation(endpoint, options = {}) {
  assertMutationRequest(endpoint, options);
  if (
  ...
  await reserveGitHubContentWrite({
    ...
    label:
      options.pacerLabel ??
      (endpoint.startsWith('https://uploads.github.com/')
        ? 'GitHub release asset upload'
        : `GitHub release ${options.method}`),
  ...
  if (upload) {
    ...
    if (!stat.isFile() || stat.isSymbolicLink())
      throw mutationError('GitHub upload requires a regular file');
  ...
  try {
    // Never replay a write here. Callers reconcile the exact remote state first.
