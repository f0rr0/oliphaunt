...
function crateArchiveLayout(cratePath) {
  ...
  if (roots.length !== 1) {
    throw error(
      `${cratePath} must contain exactly one top-level crate root, found ${roots.length}`,
    );
  }
  const root = roots[0];
  const manifest = entries.get(`${root}/Cargo.toml`);
  if (!manifest?.isFile) {
    throw error(`${cratePath} must contain exactly one top-level crate Cargo.toml`);
  }
  ...
  return Object.entries(table).map(([alias, value]) => {
    ...
    const artifact =
      artifactValue === undefined
        ? undefined
        : stringList(
            Array.isArray(artifactValue) ? artifactValue : [artifactValue],
            `dependency ${alias}.artifact`,
          );
...
function stringMapOfLists(value, context) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([name, members]) => [
      name,
...
function badges(value) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([name, fields]) => {
...
export function cargoPublishMetadataFromCrate(cratePath) {
  ...
  if (layout.root !== expectedRoot) {
    throw error(`${cratePath} top-level crate root must be ${expectedRoot}, found ${layout.root}`);
  }
  ...
  if (readmeFile !== null) {
    ...
    if (!layout.entries.get(readmeMember)?.isFile) {
      throw error(`${cratePath} does not contain declared README ${readmeFile}`);
    }
    readme = new TextDecoder('utf-8', { fatal: true }).decode(
      layout.entries.get(readmeMember).data(),
    );
  }
  return {
    name: pkg.name,
...
export async function publishFrozenCargoCrate({
  ...
  for (let rateLimitAttempt = 0; ; rateLimitAttempt += 1) {
    ...
    if (response.status !== 429) {
      throw error(
        `registry upload for ${identity} returned HTTP ${response.status}${detail ? `: ${detail}` : ''}`,
      );
    }
