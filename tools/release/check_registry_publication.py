...
def derived_exact_extension_maven_packages(product: str, version: str) -> list[RegistryPackage]:
    ...
    return [
        RegistryPackage(
            kind="maven",
            name=f"dev.oliphaunt.extensions:{product}-{target.target}",
            version=version,
        )
...
def product_registry_packages(
    ...
    *,
    ...
    allowed_graph_kinds: set[str] = set()
    if "crates-io" in publish_targets:
        allowed_graph_kinds.add("crates")
    expected_kinds = {
        "npm": "npm",
        "jsr": "jsr",
        "maven-central": "maven",
    }
    allowed_graph_kinds.update(kind for target, kind in expected_kinds.items() if target in publish_targets)
    stale_packages = sorted(
        f"{package.kind}:{package.name}"
        for package in graph_packages
        if package.kind not in allowed_graph_kinds
    )
    if stale_packages:
        fail(
            f"{product}.registry_packages contains entries without a matching registry publish target: "
            + ", ".join(stale_packages)
        )
