...
function validateSwiftReleasePleaseBootstrap(packagePath, packageConfig, manifestVersion) {
  ...
  if (packageConfig['initial-version'] !== '0.6.0') {
    fail(`${packagePath}.initial-version must be 0.6.0 to clear legacy unscoped SwiftPM tags`);
  }
  const current = parseStableVersion(manifestVersion, `${packagePath} manifest version`);
  const zero = parseStableVersion('0.0.0', 'unreleased SwiftPM seed');
  const firstPublicVersion = parseStableVersion('0.6.0', 'SwiftPM first public version');
  if (
    compareStableVersion(current, zero) > 0 &&
...
async function validateBootstrapChangelog(packagePath, changelogPath, manifestVersion) {
  const file = path.join(root, packagePath, changelogPath);
  const text = await fs.readFile(file, 'utf8');
  if (manifestVersion === '0.0.0' && text.trim() !== '') {
    const detail = text.trim() === '# Changelog'
      ? 'the preseed-only heading would make release-please append a duplicate Changelog section'
      : 'unreleased 0.0.0 products must start from an empty changelog owned by release-please';
    fail(`${rel(file)} must be empty before the first release: ${detail}`);
  }
}
...
const bootstrapLifecycleError = releasePleaseBootstrapLifecycleError(config, manifest);
if (bootstrapLifecycleError !== undefined) {
  fail(bootstrapLifecycleError);
}
...
const expectedChangelogSections = [
...
if (JSON.stringify(config['changelog-sections']) !== JSON.stringify(expectedChangelogSections)) {
  fail('release-please changelog-sections must expose exactly feat, fix, perf, refactor, and revert so accepted release intent cannot drift from generated releases');
}
...
for (const [packagePath, packageConfig] of Object.entries(packages)) {
  ...
  await validateBootstrapChangelog(packagePath, changelogPath, manifestVersion);
