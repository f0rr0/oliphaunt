...
} from './registry-http-retry.mts';
export { CRATES_IO_READ_START_INTERVAL_MILLISECONDS, createCratesIoReadGate };
export {
...
export const REGISTRY_BOOTSTRAP_CARGO_SECONDS_VARIABLE =
  'REGISTRY_BOOTSTRAP_CARGO_SECONDS_PER_CARRIER';
export const REGISTRY_BOOTSTRAP_NPM_SECONDS_VARIABLE = 'REGISTRY_BOOTSTRAP_NPM_SECONDS_PER_CARRIER';
export const REGISTRY_BOOTSTRAP_RECONCILIATION_SECONDS_VARIABLE =
  'REGISTRY_BOOTSTRAP_RECONCILIATION_SECONDS_PER_CARRIER';
...
async function crateResourceExists(
  resourceSegments,
  label,
  { apiBase, fetchImpl, nowImpl, deadlineEpochSeconds, readGate },
) {
  ...
  for (let attempt = 0; attempt < REQUEST_ATTEMPTS; attempt += 1) {
    try {
      ...
      const requestRemainingMilliseconds =
      ...
      const response = await fetchImpl(url, {
        ...
        signal: AbortSignal.timeout(
          Math.max(1, Math.min(REQUEST_TIMEOUT_MS, requestRemainingMilliseconds)),
        ),
      });
  ...
  throw error(
    `cannot determine whether Cargo identity ${label} exists on crates.io: ${lastFailure ?? 'unknown response'}`,
  );
}
...
export async function inspectCratesIoVersionState({
  ...
  return {
    ...
    publishedIdentities: withState
      .filter(({ state }) => state === 'published')
      .map(({ name, version }) => ({ name, version })),
    pendingVersions: withState
      .filter(({ state }) => state === 'pending-version')
      .map(({ name, version }) => ({ name, version })),
...
function validateBootstrapPublicationPlan(bootstrapPlan) {
  if (!Array.isArray(bootstrapPlan))
    throw error('bootstrap publication plan must be a carrier list');
  ...
  for (const [index, carrier] of bootstrapPlan.entries()) {
    for (const dependency of carrier.dependencies) {
      ...
      if (position === undefined)
        throw error(`${carrier.id} refers to unknown bootstrap dependency ${dependency}`);
      if (position >= index)
        throw error(`${carrier.id} is not ordered after bootstrap dependency ${dependency}`);
    }
  }
}

function validatedTokenBucketInputs({
  publicationCount,
  burst,
  refillSeconds,
  workSeconds,
...
export function bootstrapPublicationCriticalPathSeconds(bootstrapPlan, carrierSeconds) {
  ...
  for (const carrier of bootstrapPlan) {
    ...
    if (!Number.isSafeInteger(finishSeconds))
      throw error('bootstrap publication critical path exceeds the safe integer range');
...
function bootstrapPublicationSchedule({
  ...
  const stateByEcosystem = new Map([
    ['cargo', cargoState],
    ['npm', npmState],
  ]);
  for (const ecosystem of ['cargo', 'npm']) {
    const planned = new Set(
      bootstrapPlan.filter((carrier) => carrier.ecosystem === ecosystem).map(({ name }) => name),
    );
    const states = stateByEcosystem.get(ecosystem);
    if (
      [...planned].some((name) => !states.has(name)) ||
    ...
    for (const carrier of bootstrapPlan.filter((row) => row.ecosystem === ecosystem)) {
      if (states.get(carrier.name).version !== carrier.version) {
        throw error(
          `${ecosystem} bootstrap plan version disagrees with the exact version inventory`,
        );
      }
    }
  }
  ...
  for (const carrier of bootstrapPlan) {
    ...
    for (const dependency of carrier.dependencies) {
      ...
      if (finish === undefined) {
        throw error(
          `selected bootstrap carrier ${carrier.id} omits unsatisfied dependency ${dependency}`,
        );
      }
  ...
  if (unknown.length > 0) {
    throw error(
      `selected bootstrap carrier IDs are absent or dependency-ineligible: ${unknown.join(', ')}`,
    );
  }
  return {
  ...
  for (const carrier of bootstrapPlan) {
    ...
    const dependencyClosed = carrier.dependencies.every((dependency) => {
      ...
      const dependencyState = states
        .get(dependencyCarrier.ecosystem)
        .get(dependencyCarrier.name).state;
...
export function assessCratesIoBootstrapCapacity({
  ...
  if (
    ...
    throw error(
      'npm inventory must contain selected, published, pending-version, and missing-name lists',
    );
  }
  const missingCount = inventory.missingNames.length;
  const selectedCount = versionInventory
    ? inventory.selectedIdentities.length
    : inventory.selectedNames.length;
  ...
  if (
    versionInventory &&
    publishedCargoCount + cargoConflictCount + pendingCargoCount !== selectedCount
  ) {
  ...
  const parsedReconciliationSeconds = parseRegistryBootstrapReconciliationSeconds(
    reconciliationSecondsPerCarrier,
  );
  const parsedReserveSeconds = parseRegistryBootstrapReserveSeconds(reserveSeconds);
  const plannedPublicationSeconds =
    pendingCargoCount * parsedCargoSeconds + pendingNpmCount * parsedNpmSeconds;
  ...
  if (bootstrapPlan !== undefined) {
    if (!versionInventory)
      throw error('exact-version Cargo inventory is required with a bootstrap publication plan');
    const batch = selectBootstrapPublicationBatch({
      ...
      initialCargoTokens,
      availableSeconds:
        remainingSeconds < MINIMUM_MUTATION_WINDOW_SECONDS ? 0 : availablePublicationSeconds,
    });
    ...
    missingCount + missingNpmCount > 0 &&
    remainingSeconds >= MINIMUM_MUTATION_WINDOW_SECONDS
  ) {
    ...
    admittedNpmCount =
      pendingNpmCount * parsedNpmSeconds <= availablePublicationSeconds ? pendingNpmCount : 0;
  ...
  const admittedCount =
    bootstrapPlan === undefined ? admittedCargoCount + admittedNpmCount : admittedCarrierIds.length;
  ...
  return {
    ...
    notBeforeEpochSeconds:
      decision === 'defer' ? nowEpochSeconds + CRATES_IO_NEW_CRATE_REFILL_SECONDS : null,
    decision,
    timeSatisfied,
  ...
  function recordIdentities(identities, state) {
    ...
    for (const [index, identity] of identities.entries()) {
      ...
      if (expected === undefined || expected.version !== identity.version) {
        throw error(
          `${ecosystem} ${state} identity ${identity.name}@${identity.version} is absent from the exact selection`,
        );
      }
      if (observed.has(identity.name) || expected.state !== null) {
        throw error(
          `${ecosystem} identity ${identity.name} is duplicated across version-inventory states`,
        );
      }
