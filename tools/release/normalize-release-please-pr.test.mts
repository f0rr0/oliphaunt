...
const SCRIPT = path.resolve('.github/scripts/normalize-release-please-pr.sh');
...
function fixture(t) {
  ...
  writeFileSync(
    path.join(seed, 'release-please-config.json'),
    `${JSON.stringify(
      {
...
function identity(f, overrides = {}) {
  ...
  return [
    ...
    values.headSha,
    '--head-repository',
    values.headRepository,
    '--cross-repository',
    values.crossRepository,
    '--state',
    values.state,
    '--title',
    values.title,
...
function invoke(f, command, overrides = {}) {
  return run('bash', [SCRIPT, command, ...identity(f, overrides)], {
    cwd: f.work,
    check: false,
  });
}
...
test('normalizes the historical 100+35 file Release Please shape to one tree-identical exact-parent commit and pushes it', (t) => {
  const f = fixture(t);
  assert.equal(
    gitText(f.seed, ['diff-tree', '--no-commit-id', '--name-only', '-r', f.firstChunk]).split('\n')
      .length,
    100,
  );
  assert.equal(
    gitText(f.seed, ['diff-tree', '--no-commit-id', '--name-only', '-r', f.secondChunk]).split('\n')
      .length,
    35,
  );
  ...
  const bot = JSON.parse(readFileSync('tools/release/release-bot.json', 'utf8'));
  assert.equal(
    gitText(f.work, ['show', '-s', '--format=%cn <%ce>', 'HEAD']),
    `${bot.name} <${bot.email}>`,
  );

  const pushed = invoke(f, 'push');
  assert.equal(pushed.status, 0, pushed.stderr);
  const remoteHead = gitText(f.work, [
    'ls-remote',
    '--heads',
    'origin',
    `refs/heads/${BRANCH}`,
...
test('rejects a canonical-looking release branch with unrelated history', (t) => {
  ...
  writeFileSync(
    path.join(f.seed, 'release-please-config.json'),
    `${JSON.stringify(
      {
        'group-pull-request-title-pattern': 'chore(release): prepare ${branch} releases',
      },
