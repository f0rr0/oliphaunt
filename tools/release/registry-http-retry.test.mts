...
describe('registry HTTP backoff', () => {
  test('honors Retry-After seconds and dates', () => {
    ...
    expect(
      registryRetryDelaySeconds({ headers: new Headers({ 'Retry-After': '999' }), attempt: 0 }),
    ).toBe(300);
  });
  ...
  test('retries only transient registry statuses', () => {
    for (const status of [408, 425, 429, 500, 502, 503, 504])
      expect(registryStatusRetryable(status)).toBe(true);
    for (const status of [400, 401, 403, 404, 409, 501])
      expect(registryStatusRetryable(status)).toBe(false);
  });
});
