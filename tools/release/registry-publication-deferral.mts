...
export function requirePreMutationRegistryWindow({
  ...
  const availableMilliseconds =
    deadlineEpochSeconds * 1000 - nowEpochMilliseconds - reserveMilliseconds;
  if (availableMilliseconds < minimumMilliseconds) {
    throw new RegistryPublicationDeferredError({
      ...
      context:
        `${context} requires ${Math.ceil(minimumMilliseconds / 1000)}s before its first remote mutation; ` +
...
export function encodeRegistryPublicationDeferral(cause) {
  if (!isRegistryPublicationDeferredError(cause)) {
    throw error(
      'only a RegistryPublicationDeferredError can cross the safe child-process boundary',
    );
  }
  const record = {
