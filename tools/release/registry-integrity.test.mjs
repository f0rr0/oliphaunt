...
import { verifyLockedCarrierIntegrity, verifyLockedRegistryIntegrity } from "./registry-integrity.mjs";
...
test("proves Cargo checksum and npm SRI against frozen archive bytes and rejects same-version byte conflicts", async () => {
  ...
  try {
    ...
    const goodFetch = async (url) => Response.json(
      url.includes("crates")
        ? { version: { checksum: sha(crateBytes, "sha256") } }
        : { versions: { "1.0.0": { dist: { integrity: `sha512-${sha(npmBytes, "sha512", "base64")}` } } } },
    );
    ...
    const badFetch = async (url) => Response.json(
      url.includes("crates")
        ...
        : { versions: { "1.0.0": { dist: { integrity: `sha512-${Buffer.alloc(64).toString("base64")}` } } } },
    );
