...
describe("crates.io release capacity gates", () => {
  ...
  test("starts mutation clocks only after qualification, dry-runs, and lock matching", () => {
    ...
    const releaseTransport = workflow.indexOf("id: ensure_release_transport_ref");
    ...
    const bootstrapTransport = workflow.indexOf("id: ensure_bootstrap_transport_ref");
    ...
    expect(earlyCapacity).toBeLessThan(releaseTransport);
    expect(releaseTransport).toBeLessThan(normalDeadline);
    expect(finalCapacity).toBeLessThan(normalDeadline);
    expect(normalDeadline).toBeLessThan(registryMutation);
    expect(registryMutation).toBeLessThan(bootstrapTransport);
    expect(bootstrapTransport).toBeLessThan(bootstrapDeadline);
