...
function archiveMemberText(file, suffix, { exact = false } = {}) {
  const entries = [...readPortableArchiveEntries(file).values()].filter(
    (entry) => entry.isFile && (exact ? entry.name === suffix : entry.name.endsWith(suffix)),
  );
  if (entries.length !== 1) {
    throw error(`${rel(file)} must contain exactly one ${suffix}, found ${entries.length}`);
  }
...
function dependencyRows(ecosystem, tables) {
  ...
  return rows.sort((left, right) =>
    compareText(
      `${left.ecosystem}:${left.name}:${left.scope}`,
      `${right.ecosystem}:${right.name}:${right.scope}`,
    ),
  );
}
...
function cargoDependencyTables(manifest) {
  const tables = [
    ...
    ['development', manifest['dev-dependencies']],
  ];
...
function mavenArtifact(file) {
  ...
  const publication = validateMavenCentralPublication({
    pomText: text,
    files: artifactFiles.map((artifact) => ({
...
function mavenManifestArtifacts(file) {
  if (!rel(file).includes('/maven-artifacts/')) return [];
  return parseMavenArtifactManifest(file).map(({ groupId, artifactId, version, artifact }) => ({
    ecosystem: 'maven',
    name: groupId + ':' + artifactId,
    version,
    dependencies: [],
    artifacts: [
      { path: rel(artifact), sha256: sha256File(artifact), size: statSync(artifact).size },
    ],
  }));
}
...
function mergeArtifactRecord(records, record) {
  ...
  if (record.ecosystem !== 'maven') {
    const variants = new Map(
      [...existing.artifacts, ...record.artifacts].map((artifact) => [
        `${artifact.sha256}:${artifact.size}`,
        artifact,
      ]),
    );
    if (variants.size !== 1) {
      const candidates = [...existing.artifacts, ...record.artifacts]
        .map((artifact) => `${artifact.path} (${artifact.sha256}, ${artifact.size} bytes)`)
        .sort(compareText);
      throw error(
        `duplicate artifact identity ${id} has conflicting candidate bytes: ${candidates.join(', ')}`,
      );
    }
  ...
  existing.artifacts = [...byHash.values()].sort((left, right) =>
...
function discoverPublicationArtifactsMatching(roots, includeRecord) {
  ...
  const files = [...new Set(roots.flatMap((root) => walkFiles(path.resolve(ROOT, root))))].sort(
    compareText,
  );
  ...
  for (const file of files) {
    if (file.endsWith('.tgz')) {
      ...
      file.endsWith('.pom') &&
      !file.endsWith('-sources.pom') &&
      !file.endsWith('-javadoc.pom')
    ) {
  ...
  return [...records.values()].sort((left, right) =>
    compareText(`${left.ecosystem}:${left.name}`, `${right.ecosystem}:${right.name}`),
  );
}
...
function productDirectoryArtifact({
  product,
  id,
  role,
  kind,
...
function exactDirectFileSet(directory, expectedNames, context) {
  ...
  if (stableJson(actual) !== stableJson(expected)) {
    ...
    throw error(
      `${context} public release asset set mismatch: missing=${JSON.stringify(missing)}, extra=${JSON.stringify(extra)}`,
    );
  }
}
...
function validateChecksumManifest(file, payloadFiles, context) {
  ...
  const expected = new Map(
    payloadFiles.map((payload) => [path.basename(payload), sha256File(payload)]),
  );
  ...
  if (stableJson(declaredNames) !== stableJson(expectedNames)) {
    throw error(
      `${context} checksum entries do not exactly cover public payloads: expected=${JSON.stringify(expectedNames)}, actual=${JSON.stringify(declaredNames)}`,
    );
  }
...
function fixedGithubReleaseArtifacts(files, product) {
  ...
  for (const row of expected) {
    ...
    if (found.length !== 1) {
      throw error(
        `${product.id} requires exactly one public GitHub asset ${row.name}, found ${found.length}`,
      );
    }
  ...
  if (path.basename(directory) !== 'release-assets') {
    throw error(
      `${product.id} public GitHub assets must be staged directly in a release-assets directory, got ${rel(directory)}`,
    );
  }
  exactDirectFileSet(
    directory,
  ...
  if (checksumRows.length !== 1) {
    throw error(
      `${product.id} must declare exactly one canonical GitHub checksum asset, found ${checksumRows.length}`,
    );
  }
  ...
  return expected.map(({ target, name }) =>
    productArtifact({
      product: product.id,
...
function extensionAssetKindAllowed(family, target, kind) {
  ...
  if (target === 'ios-xcframework') {
    return (
      kind === 'runtime' || kind === 'ios-xcframework' || kind === 'ios-dependency-xcframework'
    );
  }
...
function exactExtensionIosContract(product, sqlName) {
  ...
  const nativeModuleStem =
    typeof row['native-module-stem'] === 'string' && row['native-module-stem'].length > 0
  ...
  if (staticRow === undefined || staticRow['native-module-stem'] !== nativeModuleStem) {
    throw error(
      `${product} native module ${nativeModuleStem} is absent from generated mobile static metadata`,
    );
  }
...
export function extensionRequiredAssetKeys(product) {
  if (extensionSqlNames(product, 'publication-lock').length > 1) {
    return extensionBundleCarrierRows(product).map(
      ({ family, target }) => `bundle:${family}:${target}`,
    );
  }
  return extensionRequiredArtifactRows(product).map(
    (row) =>
...
function validateExtensionManifestRow(product, manifestPath, member) {
  ...
  if (
    ...
    stableJson(sortedStrings(member.dependencies)) !==
      stableJson(
        [...(iosContract.metadata['selected-extension-dependencies'] ?? [])].sort(compareText),
      ) ||
    stableJson(sortedStrings(member.dataFiles)) !==
      stableJson([...(iosContract.metadata['runtime-share-data-files'] ?? [])].sort(compareText)) ||
    stableJson(sortedStrings(member.extensionSqlFileNames)) !==
      stableJson([...(iosContract.metadata['extension-sql-file-names'] ?? [])].sort(compareText)) ||
    stableJson(sortedStrings(member.extensionSqlFilePrefixes)) !==
      stableJson(
        [...(iosContract.metadata['extension-sql-file-prefixes'] ?? [])].sort(compareText),
      ) ||
    stableJson(sortedStrings(member.sharedPreloadLibraries)) !==
      stableJson([...(iosContract.metadata['shared-preload-libraries'] ?? [])].sort(compareText)) ||
    stableJson(sortedStrings(member.iosNativeDependencies)) !== stableJson(iosDependencies)
  ) {
    throw error(
      `${rel(manifestPath)} ${member.sqlName} semantic extension metadata is not canonical generated metadata`,
    );
  }
  if (iosContract.nativeModuleStem === null) {
    if (member.iosRegistration !== null) {
      throw error(
        `${rel(manifestPath)} SQL-only extension ${member.sqlName} must not carry iOS registration`,
      );
    }
  } else if (
    stagesIos &&
    ...
    throw error(
      `${rel(manifestPath)} native extension ${member.sqlName} lacks matching build-derived iOS registration`,
    );
  } else if (!stagesIos && member.iosRegistration !== null) {
    throw error(
      `${rel(manifestPath)} native extension ${member.sqlName} claims iOS registration without an iOS carrier`,
    );
  }
  return iosContract;
}
...
function extensionBundleGithubReleaseArtifacts({
  directory,
  manifest,
  manifestPath,
  members,
  ...
  if (
    !Array.isArray(manifest.carrierAssets) ||
    manifest.carrierAssets.length !== expectedGroups.length
  ) {
    throw error(
      `${rel(manifestPath)} must declare exactly ${expectedGroups.length} aggregate carrier assets`,
    );
  }
  ...
  for (const [index, row] of manifest.carrierAssets.entries()) {
    ...
    if (
      ...
      throw error(
        `${rel(manifestPath)} aggregate carrier ${row.name} is non-canonical, duplicated, missing, or byte-skewed`,
      );
    }
  ...
  if (
    stableJson([...carriersByGroup.keys()].sort(compareText)) !==
    stableJson(expectedGroups.map(({ family, target }) => `${family}\0${target}`).sort(compareText))
  ) {
    throw error(
      `${product.id} aggregate carriers do not exactly cover every published family/target`,
    );
  }
  ...
  for (const member of members) {
    ...
    if (!Array.isArray(member.assets) || member.assets.length === 0) {
      throw error(
        `${rel(manifestPath)} ${member.sqlName} must declare at least one logical artifact`,
      );
    }
    for (const row of member.assets) {
      if (
        ...
        throw error(
          `${rel(manifestPath)} ${member.sqlName} contains an invalid bundle member asset row`,
        );
      }
      ...
      if (row.kind === 'ios-xcframework' && row.identity !== iosContract.nativeModuleStem) {
        throw error(
          `${rel(manifestPath)} primary iOS XCFramework identity must be ${iosContract.nativeModuleStem}`,
        );
      }
      if (
        row.kind !== 'ios-dependency-xcframework' &&
        row.kind !== 'ios-xcframework' &&
        row.identity !== null
      ) {
        throw error(
          `${rel(manifestPath)} asset ${row.name} must not carry identity for ${row.kind}`,
        );
      }
      if (!extensionAssetKindAllowed(row.family, row.target, row.kind)) {
        throw error(
          `${rel(manifestPath)} contains invalid logical asset role ${member.sqlName}/${row.family}/${row.target}/${row.kind}`,
        );
      }
      const canonicalName = canonicalExtensionAssetName(product, row);
      if (row.name !== canonicalName) {
        throw error(
          `${rel(manifestPath)} logical asset ${row.name} is not canonical ${canonicalName}`,
        );
      }
      const logicalFile = path.resolve(ROOT, row.path);
      const expectedLogicalFile = path.join(
      ...
      if (
        ...
        throw error(
          `${rel(manifestPath)} logical asset metadata does not match ${rel(expectedLogicalFile)}`,
        );
      }
      const key = `${member.sqlName}:${row.family}:${row.target}:${row.kind}${row.identity === null ? '' : `:${row.identity}`}`;
      if (logicalRows.has(key))
        throw error(`${rel(manifestPath)} contains duplicate logical asset role ${key}`);
      const carrier = carriersByName.get(row.carrierAsset);
      if (
        carrier === undefined ||
        carrier.row.family !== row.family ||
        carrier.row.target !== row.target
      ) {
        throw error(
          `${rel(manifestPath)} ${key} references a missing or wrong-family aggregate carrier`,
        );
      }
  ...
  for (const { row, file, members: carrierMembers } of carriersByName.values()) {
    carrierMembers.sort((left, right) =>
      compareText(
    ...
    const memberNames = [...new Set(manifestMembers.map((member) => member.sqlName))].sort(
      compareText,
    );
    ...
    for (const archiveFile of expectedArchiveFiles)
      safeArchiveMember(archiveFile, `${rel(file)} member`);
    ...
    if (stableJson(bundleManifest) !== stableJson(expectedBundleManifest)) {
      throw error(
        `${rel(file)} bundle-manifest.json does not exactly freeze its nested member and legal locators`,
      );
    }
    if (!actualBundleManifestBytes.equals(expectedBundleManifestBytes)) {
      throw error(
        `${rel(file)} bundle-manifest.json must use the exact canonical bytes for its nested member and legal locators`,
      );
    }
    for (const { logicalFile, manifestMember, row: logicalRow } of carrierMembers) {
      ...
      if (
        ...
        throw error(
          `${rel(file)} nested payload ${archivePath} does not match its staged logical bytes`,
        );
      }
    }
    for (const legalFile of legalFiles) {
      ...
      if (
        ...
        throw error(
          `${rel(file)} legal member ${archivePath} does not match its canonical bytes and mode`,
        );
      }
    }
  }
  ...
  if (stableJson(publicManifest) !== stableJson(expectedPublicManifest)) {
    throw error(
      `${rel(publicManifestFile)} does not exactly expose the frozen aggregate member/carrier inventory`,
    );
  }
  ...
  if (swiftCarrierName !== null) {
    ...
    const expectedSwiftCarrier = buildSwiftExtensionCarrierManifest({
      ...
      nativeRuntimeVersion: extensionMetadata(product.id, 'publication-lock').compatibility
        .nativeRuntimeVersion,
      verifyMembers: false,
    });
    if (stableJson(actualSwiftCarrier) !== stableJson(expectedSwiftCarrier)) {
      throw error(
        `${rel(swiftCarrierFile)} does not exactly describe ${product.id} and its compatible native base`,
      );
    }
  }
  const controlFiles = [
  ...
  const expectedNames = [...[...carriersByName.keys()], ...controlFiles.map(([, name]) => name)];
  ...
  return [
    ...[...carriersByName.values()].map(({ row, file }) =>
      productArtifact({
...
function extensionGithubReleaseArtifacts(files, product) {
  ...
  if (manifests.length !== 1) {
    throw error(
      `${product.id} requires exactly one extension-artifacts.json in the staged roots, found ${manifests.length}`,
    );
  }
  ...
  if (members.length > 1) {
    return extensionBundleGithubReleaseArtifacts({
      directory,
      manifest,
      manifestPath,
      members,
  ...
  for (const member of members) {
    ...
    for (const row of member.assets) {
      if (
        ...
        Array.isArray(row) ||
        typeof row !== 'object' ||
        ![row.family, row.target, row.kind, row.name, row.path, row.sha256].every(
        ...
        throw error(
          `${rel(manifestPath)} ${member.sqlName} contains an invalid public extension asset row`,
        );
      }
      if (row.kind === 'ios-dependency-xcframework' && row.identity === null) {
        throw error(`${rel(manifestPath)} iOS dependency XCFramework ${row.name} lacks identity`);
      ...
      if (row.name !== canonicalName) {
        throw error(
          `${rel(manifestPath)} asset ${row.name} is not the canonical name ${canonicalName}`,
        );
      }
      const file = path.resolve(ROOT, row.path);
      if (file !== path.join(directory, row.name) || !isFile(file)) {
        throw error(
          `${rel(manifestPath)} asset ${row.name} must exist directly under ${rel(directory)}`,
        );
      }
  ...
  if (stableJson(expectedKeys) !== stableJson(actualKeys)) {
    throw error(
      `${product.id} extension asset roles do not exactly cover declared targets: expected=${JSON.stringify(expectedKeys)}, actual=${JSON.stringify(actualKeys)}`,
    );
  }
  ...
  if (stableJson(publicManifest) !== stableJson(expectedPublicManifest)) {
    throw error(
      `${rel(publicManifestFile)} does not exactly expose the canonical extension identity and frozen asset inventory`,
    );
  }
  const includeSwiftCarrier = family === null || family === 'native';
  const swiftCarrierName = includeSwiftCarrier
  ...
  if (swiftCarrierName !== null) {
    ...
    if (!isFile(swiftCarrierFile)) {
      throw error(
        `${product.id} requires independently consumable Swift iOS carrier ${swiftCarrierName}`,
      );
    }
    let actualSwiftCarrier;
    try {
...
function swiftReleaseInputs(files, product, { requireExtensionFixture }) {
  ...
  const artifacts = expectedFiles.map(([name, kind]) => {
    ...
    if (matches.length !== 1) {
      throw error(
        `${product.id} requires exactly one ${name} in the staged artifact roots, found ${matches.length}`,
      );
    }
  ...
  if (
    ...
    !readFileSync(path.resolve(ROOT, ownerCatalogArtifact.path)).equals(
      readFileSync(canonicalOwnerCatalog),
    )
  ) {
    throw error(
  ...
  const resourceInventoryArtifact = artifacts.find(
    ({ kind }) => kind === 'swiftpm-extension-resource-inventory',
  );
  ...
  if (
    ...
    !readFileSync(path.resolve(ROOT, resourceInventoryArtifact.path)).equals(
      releaseJavaScript(canonicalResourceInventory),
    )
  ) {
    throw error(
  ...
  if (carrierMatches.length !== 1) {
    throw error(
      `${product.id} requires exactly one source-tag carrier ${carrierName}, found ${carrierMatches.length}`,
    );
  }
  ...
  const fixtureManifests = files.filter(
    (file) =>
  ...
  if (fixtureManifests.length !== expectedFixtureCount) {
    ...
    throw error(
      `${product.id} ${selection} frozen Swift consumer fixture, found ${fixtureManifests.length}`,
    );
  }
...
function reactNativeReleaseInputs(files, product) {
  ...
  if (matches.length !== 1) {
    throw error(
      `${product.id} requires exactly one canonical aggregate iOS carrier manifest, found ${matches.length}`,
    );
  }
  return [
    productArtifact({
...
function runtimeOwnedExtensionGithubReleaseArtifacts(files, product) {
  ...
  return families.flatMap((family) =>
    extensionGithubReleaseArtifacts(files, {
...
function discoverProductArtifactsForSelection(roots, products, selectedProducts) {
  ...
  const artifacts = [];
  const contrib = contribCarrierDescriptor('publication-lock');
  const hasSelectedExtensionProducts = selectedProducts.some(
    (product) =>
  ...
  for (const product of products) {
    ...
    if (product.id === 'oliphaunt-swift') {
      artifacts.push(
        ...swiftReleaseInputs(files, product, {
  ...
  return artifacts.sort((left, right) =>
    compareText(`${left.product}:${left.id}`, `${right.product}:${right.id}`),
  );
}
...
function sourceIdentity(headRef) {
  ...
  if (
    ![snapshot.commit, snapshot.tree, snapshot.checkout].every((value) =>
      /^[0-9a-f]{40}$/u.test(value ?? ''),
    ) ||
    !(
      headRef === snapshot.ref ||
      headRef === snapshot.commit ||
      (headRef === 'HEAD' && snapshot.checkout === snapshot.commit)
    )
  ) {
    throw error(
      'source identity for ' +
        headRef +
        ' requires a matching Git snapshot from bash tools/release/with-source.sh',
    );
  }
  return { commit: snapshot.commit, tree: snapshot.tree };
}
...
export function validateCargoPayloadPartSets(carriers) {
  ...
  for (const carrier of carriers) {
    ...
    if (
      ...
      typeof carrier.parentCarrier !== 'string'
    ) {
  ...
  for (const parentId of [...parents].sort(compareText)) {
    ...
    const parts = [...(partsByParent.get(parentId) ?? [])].sort(
    ...
    if (stableJson(actualNumbers) !== stableJson(expectedNumbers)) {
      throw error(
        `${parentId} Cargo payload parts must be contiguous from part-001; found ${actualNumbers.map((part) => String(part).padStart(3, '0')).join(', ')}`,
      );
    }
    ...
    const dependencyIds = (parent.packageDependencies ?? [])
      .filter(
        (dependency) =>
          dependency.ecosystem === 'cargo' && dependency.name.startsWith(`${parent.name}-part-`),
      )
...
function assignPublishOrder(carriers, products) {
  ...
  const fallbackCompare = (leftId, rightId) => {
    ...
    return (
      (productOrder.get(left.product) ?? 9999) - (productOrder.get(right.product) ?? 9999) ||
...
export function buildPublicationCandidate({
  ...
  for (const artifact of artifacts) {
    ...
    if (artifact.version !== resolved.version) {
      throw error(
        `${artifact.ecosystem}:${artifact.name} artifact version ${artifact.version} does not match ${resolved.product} version ${resolved.version}`,
      );
    }
  ...
  if (!allowMissing && missing.length > 0) {
    throw error(
      `artifact set is missing ${missing.length} declared carrier(s): ${missing.join(', ')}`,
    );
  }
  for (const carrier of carriers) {
...
function validateCandidateCatalog(candidate) {
  ...
  const catalog = loadPublicationCatalog('publication-lock validation', {
    products: requestedProducts,
  });
  const expectedDigest = publicationCatalogDigest(catalog);
  if (candidate.catalogDigest !== expectedDigest) {
    throw error(
      `candidate catalogDigest mismatch: expected ${expectedDigest}, got ${candidate.catalogDigest}`,
    );
  }
  ...
  for (const carrier of candidate.carriers) {
    const expected = resolveActualCarrier(
      catalog,
      carrier.ecosystem,
      carrier.name,
      'publication-lock validation',
    );
  ...
  if (stableJson(candidate.missing) !== stableJson(expectedMissing)) {
    throw error(
      `candidate missing carrier set mismatch: expected=${JSON.stringify(expectedMissing)}, actual=${JSON.stringify(candidate.missing)}`,
    );
  }
}
...
function validateExtensionProductArtifactInventory(product, artifacts) {
  ...
  const expectedAssets = new Map(
    expectedExtensionGithubReleaseAssetRows(product).map((row) => [row.name, row]),
  );
  ...
  if (stableJson(expectedNames) !== stableJson(actualNames)) {
    throw error(
      `${product.id} frozen release assets do not cover every declared target role exactly`,
    );
  }
  for (const artifact of publicAssets) {
    ...
    if (
      ...
      throw error(
        `${product.id} frozen release asset ${artifact.name} has incorrect target, kind, or identity metadata`,
      );
    }
  }
...
function validateProductArtifactInventory(product, artifacts, { hasSelectedExtensionProducts }) {
  ...
  const expected = targets.map(
    (target) => `github-release:${target.asset.replaceAll('{version}', product.version)}`,
  );
  if (product.id === 'oliphaunt-swift') {
    expected.push(
  ...
  if (stableJson(expected) !== stableJson(actual)) {
    throw error(
      `${product.id} frozen product artifact inventory mismatch: expected=${JSON.stringify(expected)}, actual=${JSON.stringify(actual)}`,
    );
  }
}
...
export function validatePublicationCandidate(candidate) {
  ...
  if (
    !/^[0-9a-f]{40}$/u.test(candidate.source.commit) ||
    !/^[0-9a-f]{40}$/u.test(candidate.source.tree)
  ) {
    throw error('candidate source must contain full commit and tree SHAs');
  }
  if (
    !Array.isArray(candidate.products) ||
    !Array.isArray(candidate.carriers) ||
    !Array.isArray(candidate.productArtifacts) ||
  ...
  for (const carrier of candidate.carriers) {
    ...
    if (carrier.id !== `${carrier.ecosystem}:${carrier.name}`) {
      throw error(
        `${carrier.id} is not the canonical ${carrier.ecosystem}:${carrier.name} identity`,
      );
    }
    if (
      !(
        carrier.target === null ||
        (typeof carrier.target === 'string' && carrier.target.length > 0)
      )
    ) {
    ...
    if (
      ...
      !Array.isArray(carrier.packageDependencies) ||
      !Array.isArray(carrier.artifacts) ||
    ...
    for (const [index, dependency] of carrier.packageDependencies.entries()) {
      ...
      assertNonEmptyString(
        ...
        `${carrier.id}.packageDependencies[${index}].ecosystem`,
      );
      assertNonEmptyString(dependency.name, `${carrier.id}.packageDependencies[${index}].name`);
      assertNonEmptyString(
        dependency.requirement,
        `${carrier.id}.packageDependencies[${index}].requirement`,
      );
      ...
      if (!ECOSYSTEM_ORDER.has(dependency.ecosystem)) {
        throw error(
          `${carrier.id}.packageDependencies[${index}] has unsupported ecosystem ${dependency.ecosystem}`,
        );
      }
    }
  ...
  if (stableJson(publishOrders) !== stableJson(expectedPublishOrders)) {
    throw error(
      'candidate carriers must be stored in one contiguous, unique publishOrder sequence',
    );
  }
  ...
  for (const [index, carrier] of candidate.carriers.entries()) {
    const expectedDependencies = projectInternalDependencyIds(
      candidate.carriers,
      carrier.packageDependencies,
    );
    if (stableJson(carrier.dependencies) !== stableJson(expectedDependencies)) {
      throw error(
        `${carrier.id}.dependencies do not match its internal package dependency identities`,
      );
    }
  ...
  for (const artifact of candidate.productArtifacts) {
    ...
    if (
      ...
      !(
        ...
        (typeof artifact.identity === 'string' && artifact.identity.length > 0)
      ) ||
  ...
  if (candidate.packageEnvelopeDigest !== expectedEnvelope) {
    throw error(
      `candidate packageEnvelopeDigest mismatch: expected ${expectedEnvelope}, got ${candidate.packageEnvelopeDigest}`,
    );
  }
  return candidate;
}
...
export function assertPublicationLockSource(lock, headRef = 'HEAD') {
  ...
  if (lock.source.commit !== source.commit || lock.source.tree !== source.tree) {
    throw error(
      `publication lock source ${lock.source.commit}/${lock.source.tree} does not match ${headRef} ${source.commit}/${source.tree}`,
    );
  }
  return source;
}

export function lockedCarriers(
  lock,
  { product = undefined, products = undefined, ecosystem = undefined } = {},
) {
  const productSet = products === undefined ? undefined : new Set(products);
  return lock.carriers.filter(
    (carrier) =>
...
export function lockedCarrierFiles(lock, ecosystem, name) {
  ...
  if (matches.length !== 1) {
    throw error(
      `expected exactly one frozen ${ecosystem}:${name} carrier, found ${matches.length}`,
    );
  }
...
export function lockedCarrierFile(lock, ecosystem, name, suppliedPath = undefined) {
  ...
  if (carrier.artifacts.length !== 1) {
    throw error(
      `${carrier.id} must have exactly one publishable file, found ${carrier.artifacts.length}`,
    );
  }
...
export function lockedCarrierDirectory(lock, ecosystem, name, suppliedPath = undefined) {
  ...
  if (carrier.artifacts.length !== 1) {
    throw error(
      `${carrier.id} must have exactly one publishable directory, found ${carrier.artifacts.length}`,
    );
  }
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory()) {
    throw error(
      `${carrier.id} frozen artifact must be a regular non-symlink directory: ${artifact.path}`,
    );
  }
  const observed = directoryEnvelope(directory);
  if (observed.sha256 !== artifact.sha256 || observed.size !== artifact.size) {
    throw error(
      `${carrier.id} frozen directory bytes do not match the publication lock: ${artifact.path}`,
    );
  }
  if (suppliedPath !== undefined && path.resolve(ROOT, suppliedPath) !== directory) {
    throw error(
      `${carrier.id} publisher attempted to substitute ${rel(path.resolve(ROOT, suppliedPath))} for frozen ${artifact.path}`,
    );
  }
...
export function assertLockedIdentitySet(lock, actual, { product, products, ecosystem } = {}) {
  const expected = lockedCarriers(lock, { product, products, ecosystem })
    .map((carrier) => `${carrier.ecosystem}:${carrier.name}@${carrier.version}`)
    .sort(compareText);
  const observed = actual
    .map((item) => `${item.ecosystem ?? ecosystem}:${item.name}@${item.version}`)
    .sort(compareText);
  if (stableJson(expected) !== stableJson(observed)) {
    throw error(
      `frozen carrier set mismatch for ${product ?? products?.join(',') ?? 'all products'}/${ecosystem ?? 'all ecosystems'}: expected=${JSON.stringify(expected)}, actual=${JSON.stringify(observed)}`,
    );
  }
}
...
export function assertLockedArtifactSet(lock, actual, { product, products, ecosystem } = {}) {
  ...
  const expected = new Map(
    lockedCarriers(lock, { product, products, ecosystem }).map((carrier) => [carrier.id, carrier]),
  );
  for (const record of actual) {
    ...
    if (frozen === undefined || !Array.isArray(record.artifacts)) {
      throw error(
        `actual artifact record ${id} is absent from the frozen lock or has no byte envelope`,
      );
    }
    const frozenBytes = frozen.artifacts
      .map(({ sha256, size }) => `${sha256}:${size}`)
      .sort(compareText);
    const actualBytes = record.artifacts
      .map(({ sha256, size }) => `${sha256}:${size}`)
      .sort(compareText);
    if (stableJson(frozenBytes) !== stableJson(actualBytes)) {
      throw error(
        `frozen artifact bytes mismatch for ${id}: expected=${JSON.stringify(frozenBytes)}, actual=${JSON.stringify(actualBytes)}`,
      );
    }
  }
}
...
export function assertLockedProductArtifacts(lock, product, roots) {
  ...
  const envelope = (artifacts) =>
    artifacts
      .map(
        ({ product: owner, id, role, kind, target, identity, name, sha256, size }) =>
          `${owner}:${id}:${role}:${kind}:${target ?? ''}:${identity ?? ''}:${name}:${sha256}:${size}`,
      )
      .sort(compareText);
  if (stableJson(envelope(expected)) !== stableJson(envelope(actual))) {
    throw error(
      `frozen product artifact bytes mismatch for ${product}: expected=${JSON.stringify(envelope(expected))}, actual=${JSON.stringify(envelope(actual))}`,
    );
  }
}

export function lockedProductArtifactPaths(
  lock,
  product,
  { role = undefined, kind = undefined } = {},
) {
  const selected = lock.productArtifacts.filter(
    (artifact) =>
  ...
  return selected.map((artifact) => {
    ...
    if (relative === '' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
      throw error(
        `${product}:${artifact.id} path must remain inside the repository: ${artifact.path}`,
      );
    }
    let stat;
    try {
    ...
    if (
      ...
      throw error(
        `${product}:${artifact.id} bytes do not match the publication lock: ${artifact.path}`,
      );
    }
...
function main(argv) {
    ...
    const output = path.resolve(
      ...
      values.get('output') ??
        (command === 'create'
          ...
          : 'target/release/publication-candidate.json'),
    );
    const value = command === 'create' ? freezePublicationCandidate(candidate) : candidate;
    writeJson(output, value);
    console.log(
      `${rel(output)}\t${value.packageEnvelopeDigest}\t${value.lockDigest ?? 'candidate'}`,
    );
    return;
  }
  if (command === 'freeze') {
    const input = path.resolve(
      ROOT,
      values.get('candidate') ?? 'target/release/publication-candidate.json',
    );
  ...
  if (command === 'verify') {
    ...
    console.log(
      `${rel(file)} publication lock verified (${lock.carriers.length} carriers, envelope ${lock.packageEnvelopeDigest})`,
    );
    return;
  }
  console.log(
    'usage: tools/release/publication-lock.mts <candidate|create|freeze|verify> [--products-json JSON] [--artifact-root PATH ...] [--head-ref REF] [--output PATH] [--allow-missing]',
  );
