...
const PREFIX = 'verify_github_release_attestations.mts';
...
const GITHUB_RELEASE_ARTIFACT_ROLES = new Set(['github-release-asset', 'github-release-metadata']);
...
const PUBLIC_EXTENSION_RELEASE_LEGACY_KEYS = new Set(
  [...PUBLIC_EXTENSION_RELEASE_MANIFEST_KEYS].filter(
    (key) => !EXTENSION_OWNERSHIP_KEYS.includes(key),
  ),
);
...
const PUBLIC_EXTENSION_BUNDLE_LEGACY_KEYS = new Set(
  [...PUBLIC_EXTENSION_BUNDLE_MANIFEST_KEYS].filter(
    (key) => !EXTENSION_OWNERSHIP_KEYS.includes(key),
  ),
);
...
async function productTagHasContrib(product, version) {
  const tag = await productTag(product, version);
  const snapshot = process.env.RELEASE_TAG_CONTRIB;
  if (!snapshot) fail('release tag inspection requires with-release-tags.sh');
  const rows = readFileSync(snapshot, 'utf8').split('\0');
  if (rows.pop() !== '' || rows.length % 2 !== 0) fail('invalid release tag snapshot');
  const tags = new Map();
  for (let i = 0; i < rows.length; i += 2) {
    if (tags.has(rows[i]) || !['true', 'false'].includes(rows[i + 1]))
      fail('invalid release tag snapshot');
    tags.set(rows[i], rows[i + 1] === 'true');
  }
  if (!tags.has(tag)) fail(`cannot inspect exact product tag tree ${tag}`);
  return tags.get(tag);
}
...
async function expectedExtensionAssets(product, version, family = 'combined') {
  ...
  const releaseProduct =
    family === 'combined' ? product : extensionReleaseProduct(product, family, PREFIX);
...
async function expectedAssets(product, version) {
  ...
  if (product === 'liboliphaunt-wasix-postmaster') {
    return [
      ...productTargets(product, 'liboliphaunt-wasix-postmaster').map(
        (target) => `${product}-${version}-${target}.tar.zst`,
      ),
      `${product}-${version}-release-assets.sha256`,
    ].sort(compareText);
  }
...
async function exactResponseProof(response, expectedSize, context) {
  ...
  if (declared !== null && declared !== expectedSize) {
    ...
    throw new Error(
      `${context} Content-Length ${declared} does not match expected size ${expectedSize}`,
    );
  }
...
export async function requestReleaseControlBytes(
  url,
  name,
  expectedSize,
  { fetchImpl = fetch, timeoutMs = RELEASE_ASSET_TIMEOUT_MS } = {},
) {
  const size = expectedAssetSize(expectedSize, name, MAX_CONTROL_ASSET_BYTES);
  await reserveGitHubCoreRequest({ label: `download GitHub release control asset ${name}` });
  ...
  if (bytes.byteLength !== size) {
    throw new Error(
      `GitHub release asset ${name} size ${bytes.byteLength} does not match expected size ${size}`,
    );
  }
  return bytes;
}

export async function requestReleaseAssetProof(
  url,
  name,
  expectedSize,
  { fetchImpl = fetch, timeoutMs = RELEASE_ASSET_TIMEOUT_MS } = {},
  ...
  await reserveGitHubCoreRequest({ label: `prove GitHub release asset ${name}` });
  const response = await fetchImpl(releaseAssetApiUrl(url, name), {
    headers: authHeaders('application/octet-stream'),
    redirect: 'follow',
    signal: AbortSignal.timeout(timeoutMs),
...
function validateKeySet(object, expected, context) {
  ...
  if (missing.length > 0 || unexpected.length > 0) {
    fail(
      `${context} keys must be ${JSON.stringify([...expected].sort())}, got ${JSON.stringify([...actual].sort())}`,
    );
  }
}
...
function validateBundleCarrierAssets(assets, context, expectedMemberCount) {
  ...
  for (const [index, asset] of assets.entries()) {
    ...
    if (byName.has(asset.name) || groups.has(group))
      fail(`${context} repeats an aggregate carrier name or family/target`);
...
function validateBundleMemberAssets(assets, context, carriers, seenLocators) {
  ...
  for (const [index, asset] of assets.entries()) {
    ...
    if (asset.carrierRoot !== expectedRoot)
      fail(`${assetContext}.carrierRoot does not match ${asset.carrierAsset}`);
    if (
    ...
    if (seenLocators.has(locator))
      fail(`${context} repeats aggregate member locator ${asset.memberPath}`);
...
function canonicalIosDependencies() {
  ...
  canonicalIosDependenciesCache = new Map(
    lines.map((line) => {
      const fields = line.split('\t');
      const row = Object.fromEntries(header.map((key, index) => [key, fields[index] ?? '']));
      return [
        row['sql-name'],
        (row['ios-static-dependencies'] ?? '').split(',').filter(Boolean).sort(compareText),
      ];
    }),
  );
...
function canonicalMemberSemantics(product, sqlName) {
  ...
  return {
    ...
    dependencies: canonicalSortedUniqueStrings(
      row['selected-extension-dependencies'],
      `${product}/${sqlName}.selected-extension-dependencies`,
    ),
    dataFiles: canonicalSortedUniqueStrings(
      row['runtime-share-data-files'],
      `${product}/${sqlName}.runtime-share-data-files`,
    ),
    extensionSqlFileNames: canonicalSortedUniqueStrings(
      row['extension-sql-file-names'],
      `${product}/${sqlName}.extension-sql-file-names`,
    ),
    extensionSqlFilePrefixes: canonicalSortedUniqueStrings(
      row['extension-sql-file-prefixes'],
      `${product}/${sqlName}.extension-sql-file-prefixes`,
    ),
    nativeModuleStem,
    iosNativeDependencies:
...
function assertCanonicalIosRegistration(member, expected, context) {
  ...
  if (
    stableStringify(Object.keys(registration).sort(compareText)) !== stableStringify(expectedKeys)
  ) {
  ...
  if (
    ...
    throw new Error(
      `${context}.iosRegistration does not match its canonical native module identity`,
    );
  }
  const normalizedSymbols = registration.symbols.map((row, index) => {
    if (
      ...
      Array.isArray(row) ||
      typeof row !== 'object' ||
      stableStringify(Object.keys(row).sort(compareText)) !==
        stableStringify(['address', 'name']) ||
      ...
      throw new Error(
        `${context}.iosRegistration.symbols[${index}] is not a canonical C symbol mapping`,
      );
    }
...
export function assertCanonicalExtensionReleaseIdentity(
  ...
  {
    family = manifest?.family ?? 'combined',
    releaseProduct = manifest?.releaseProduct ?? product,
  } = {},
) {
  if (manifest === null || Array.isArray(manifest) || typeof manifest !== 'object') {
    throw new Error(`${context} must be an object`);
  }
  const ownershipFieldCount = EXTENSION_OWNERSHIP_KEYS.filter(
  ...
  throw new Error(
    `${context} has unsupported extension release manifest schema ${JSON.stringify(manifest.schema)}`,
  );
}
...
function canonicalSigstoreBundleForGhComparison(bundle) {
  ...
  const timestampVerificationData = canonical?.verificationMaterial?.timestampVerificationData;
  if (
...
function assertKeySet(value, keys, context) {
  ...
  if (stableStringify(actual) !== stableStringify(expected)) {
    throw new Error(
      `${context} keys must be ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`,
    );
  }
}
...
function normalizedRepository(value) {
  if (typeof value !== 'string' || !/^[^/\s\0]+\/[^/\s\0]+$/u.test(value) || value.includes('..')) {
...
export function frozenGithubReleaseAssets(lock) {
  ...
  for (const artifact of lock.productArtifacts) {
    ...
    if (!products.has(artifact.product)) {
      throw new Error(
        `frozen GitHub asset ${artifact?.name ?? '<unknown>'} belongs to unselected product ${artifact?.product ?? '<unknown>'}`,
      );
    }
    if (
    ...
    if (!Number.isSafeInteger(artifact.size) || artifact.size < 0) {
      throw new Error(
        `${artifact.product}/${artifact.name} frozen size must be a non-negative safe integer`,
      );
    }
    ...
    if (identities.has(identity)) {
      throw new Error(
        `publication lock contains duplicate GitHub asset ${artifact.product}/${artifact.name}`,
      );
    }
  ...
  return assets.sort(
    (left, right) => compareText(left.product, right.product) || compareText(left.name, right.name),
  );
}
...
async function mapConcurrent(values, concurrency, worker) {
  ...
  await Promise.all(
    Array.from({ length: Math.min(values.length, concurrency) }, () => runWorker()),
  );
...
async function requestGithubArrayPages({
  ...
  for (let page = 1; page <= GITHUB_RELEASE_LIST_MAX_PAGES; page += 1) {
    ...
    if (data.length > GITHUB_RELEASE_LIST_PAGE_SIZE) {
      throw new Error(`${context} page ${page} exceeds ${GITHUB_RELEASE_LIST_PAGE_SIZE} rows`);
    }
    rows.push(...data);
...
function normalizeRemoteAsset(asset, expected, product, tag) {
  ...
  if (asset.name !== expected.name) {
    throw new Error(
      `${product} GitHub release ${tag} asset name changed while it was being validated`,
    );
  }
  ...
  return {
    assetId: normalizeGithubId(
      asset.id,
      `${product} GitHub release ${tag} asset ${expected.name} id`,
    ),
    name: expected.name,
...
function exactProductSet(expected, actual, context) {
  ...
  if (missing.length > 0 || extra.length > 0 || actual.length !== actualSet.size) {
    throw new Error(
      `${context} mismatch: missing=${JSON.stringify(missing)}, extra=${JSON.stringify(extra)}, duplicate=${actual.length !== actualSet.size}`,
    );
  }
}
...
export function normalizeGithubReleaseSnapshot(lock, releases) {
  ...
  for (const product of products) {
    ...
    for (const expected of expectedAssets) {
      ...
      assertKeySet(
        asset,
        ['assetId', 'name', 'sha256', 'size'],
        `${context} asset ${expected.name}`,
      );
      if (asset.sha256 !== expected.sha256 || asset.size !== expected.size) {
        throw new Error(
          `${context} asset ${expected.name} differs from the frozen publication lock`,
        );
      }
...
async function queryLockedGithubReleasesOnce(
  lock,
  {
    authToken,
    deadlineMs,
  ...
  for (const [index, release] of listedReleases.entries()) {
    ...
    if (releasesByTag.has(release.tag_name)) {
      throw new Error(
        `${canonicalRepo} returned duplicate releases for selected tag ${release.tag_name}`,
      );
    }
    releasesByTag.set(release.tag_name, release);
  }
  const missingTags = [...selectedTags].filter((tag) => !releasesByTag.has(tag)).sort(compareText);
  ...
  for (const product of products) {
    ...
    if (
      ...
      throw new Error(
        `${product.id} GitHub release ${tag} metadata does not match the frozen publication lock`,
      );
    }
  }

  const exactAssetRows = new Map(
    await mapConcurrent(products, GITHUB_RELEASE_FALLBACK_QUERY_CONCURRENCY, async (product) => {
      const tag = frozenProductTag(product);
...
export async function queryLockedGithubReleases(lock, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const delay = Math.min(GITHUB_RELEASE_SNAPSHOT_MAX_RETRY_DELAY_MS, 1_000 * 2 ** (attempt - 1));
    const now = nowImpl();
    if (now + delay >= deadlineMs) {
      throw new Error(
        `GitHub release snapshot retry would exceed its deadline: ${lastError.message}`,
      );
    }
...
function normalizeAttestationSubjects(subjects, context) {
  ...
  return normalized.sort(
    (left, right) => compareText(left.name, right.name) || compareText(left.sha256, right.sha256),
  );
}
...
export function assertAttestationSubjectCoverage(assets, attestations) {
  ...
  if (assets.length === 0 && attestations.length > 0) {
    throw new Error(
      'attestation bundles contaminate a release selection with no frozen GitHub assets',
    );
  }
  ...
  for (const asset of assets) {
    requireSha256(
      asset?.sha256,
      `${asset?.product ?? '<unknown>'}/${asset?.name ?? '<unknown>'} sha256`,
    );
  ...
  for (const [index, attestation] of attestations.entries()) {
    ...
    const bundleSha256 = requireSha256(
      ...
      `attestation[${index}].bundleSha256`,
    );
    ...
    const subjects = normalizedReceiptSubjects(
      attestation.subjects,
      `attestation bundle ${bundleSha256}`,
    );
    for (const subject of subjects) {
      ...
      if (!expectedByKey.has(key)) {
        if (expectedByName.has(subject.name)) {
          throw new Error(
            `attestation bundle ${bundleSha256} subject ${subject.name} digest differs from the frozen GitHub asset`,
          );
        }
        throw new Error(
          `attestation bundle ${bundleSha256} contains non-frozen subject ${subject.name}`,
        );
      }
      if (covered.has(key)) {
  ...
  const missing = [...expectedByKey]
    ...
    .flatMap(([, matchingAssets]) =>
      matchingAssets.map((asset) => `${asset.product}/${asset.name}`),
    )
    .sort(compareText);
  if (missing.length > 0) {
...
export function validateGithubAttestationReceipt(receipt, lock, { repo = repository() } = {}) {
  ...
  if (
    ...
    throw new Error(
      'GitHub attestation receipt identity does not match the repository, source, or publication lock',
    );
  }
  ...
  if (receipt.receiptDigest !== expectedDigest) {
    throw new Error(
      `GitHub attestation receipt digest mismatch: expected ${expectedDigest}, got ${receipt.receiptDigest}`,
    );
  }
...
export function assertGithubReleaseSnapshotMatchesReceipt(receipt, releases) {
  if (stableStringify(releases) !== stableStringify(receipt?.releases)) {
    throw new Error(
      'GitHub release or asset IDs, names, sizes, or digests changed after the pre-mutation receipt was created',
    );
  }
}
...
async function verifyExtensionReleaseAssets(
  ...
  if (includeSwiftCarrier) {
    ...
    const swiftCarrierBytes = await requestBytes(
      swiftCarrierAsset.url,
      swiftCarrierName,
      swiftCarrierSize,
    );
    proofs.set(swiftCarrierName, {
      bytes: swiftCarrierBytes.byteLength,
  ...
  for (const name of [...checksumCoveredNames].sort(compareText)) {
    if (!actualAssets.has(name)) {
      fail(
        `${product} GitHub release ${await productTag(releaseProduct, version)} is missing checksum-covered asset ${name}`,
      );
    }
    ...
    if (manifestAsset !== undefined && remoteSize !== manifestAsset.bytes) {
      fail(
        `${product} GitHub release ${await productTag(releaseProduct, version)} asset ${name} size metadata mismatch`,
      );
    }
    ...
    if (proof.sha256 !== checksums.get(name)) {
      fail(
        `${product} GitHub release ${await productTag(releaseProduct, version)} asset ${name} checksum mismatch`,
      );
    }
    if (remoteSize !== proof.bytes) {
      fail(
        `${product} GitHub release ${await productTag(releaseProduct, version)} asset ${name} size mismatch`,
      );
    }
  }
  ...
  for (const asset of extensionAssets) {
    ...
    if (proof.bytes !== asset.bytes || proof.sha256 !== asset.sha256) {
      fail(
        `${product} GitHub release ${await productTag(releaseProduct, version)} asset ${asset.name} public manifest mismatch`,
      );
    }
  }
}
...
async function lockedLocalSubject(asset) {
  ...
  try {
    ...
    throw new Error(
      `${asset.product}/${asset.name} frozen local subject is missing: ${error.message}`,
    );
  }
  if (stat.isSymbolicLink() || !stat.isFile()) {
    throw new Error(
      `${asset.product}/${asset.name} frozen local subject must be a regular non-symlink file`,
    );
  }
  if (stat.size !== asset.size || path.basename(file) !== asset.name) {
    throw new Error(
      `${asset.product}/${asset.name} frozen local subject path or size differs from the publication lock`,
    );
  }
  return file;
}

function bundleVerificationFile({ bundlePath, file, head, repo }, directory) {
  if (!/^[0-9a-f]{40}$/u.test(head ?? ''))
    throw new Error('attestation source head must be a full lowercase commit SHA');
  const fields = [file, bundlePath, head, normalizedRepository(repo)];
  if (
    !directory ||
    fields.some((value) => typeof value !== 'string' || !value || value.includes('\0'))
  )
    throw new Error('attestation verification requires verify-github-release-attestations.sh');
  return { fields, stem: path.join(directory, sha256Bytes(Buffer.from(JSON.stringify(fields)))) };
}

export async function prepareBundleVerification(options, directory) {
  const { fields, stem } = bundleVerificationFile(options, directory);
  await fs.writeFile(stem + '.subject', fields.join('\0') + '\0', { flag: 'wx', mode: 0o600 });
  return statementSubjects(
    decodeBundleStatement(options.bundle, options.bundlePath),
    options.bundlePath,
  );
}

async function readGhBundleVerification(options) {
  const { bundle, bundlePath } = options;
  const { stem } = bundleVerificationFile(
    options,
    process.env.OLIPHAUNT_ATTESTATION_VERIFICATION_DIR,
  );
  const output = parseJsonBytes(
    await readBoundedRegularFile(
      stem + '.json',
      GH_ATTESTATION_VERIFY_MAX_OUTPUT_BYTES,
  ...
  if (!Array.isArray(output) || output.length !== 1) {
    throw new Error(
      `gh verification output for ${bundlePath} must contain exactly one verified attestation`,
    );
  }
...
export async function verifyAttestationBundles(
  lock,
  bundlePaths,
  {
    repo = repository(),
    verifyBundleImpl = readGhBundleVerification,
    publisherSha = lock.source.commit,
  } = {},
) {
  const canonicalRepo = normalizedRepository(repo);
  ...
  for (const bundlePath of bundlePaths) {
    ...
    if (stableStringify(verifiedSubjects) !== stableStringify(untrustedSubjects)) {
      throw new Error(
        `cryptographically verified subjects for ${bundlePath} differ from its DSSE statement`,
      );
    }
...
export async function writeImmutableReceipt(
  file,
  receipt,
  ...
  if (Buffer.byteLength(body) > MAX_ATTESTATION_RECEIPT_BYTES) {
    throw new Error(
      `GitHub attestation receipt ${file} exceeds ${MAX_ATTESTATION_RECEIPT_BYTES} bytes`,
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (existing.toString('utf8') !== body) {
        throw new Error(
          `refusing to replace existing non-identical GitHub attestation receipt ${file}`,
        );
      }
    }
  } catch (error) {
...
function receiptUsage() {
  console.log('usage:');
  console.log(
    '  bash tools/release/verify-github-release-attestations.sh pre-mutation --publication-lock FILE --head-ref REF --output FILE [--products-json JSON] [--attestation-bundle FILE ...]',
  );
  console.log(
    '  bash tools/release/verify-github-release-attestations.sh finalize --publication-lock FILE --head-ref REF --receipt FILE [--products-json JSON]',
  );
}

async function receiptMain(command, argv, prepareDirectory) {
  ...
  if (command === 'pre-mutation') {
    ...
    const attestations = await verifyAttestationBundles(lock, args.attestationBundles, {
      repo,
      publisherSha,
      ...(prepareDirectory
        ? { verifyBundleImpl: (options) => prepareBundleVerification(options, prepareDirectory) }
        : {}),
    });
...
function parseLegacyArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === '--product') {
      ...
      console.log(
        'usage: bash tools/release/verify-github-release-attestations.sh [--product ID...] [--products-json JSON] [--head-ref REF]',
      );
      receiptUsage();
      process.exit(0);
    } else {
...
async function legacyMain(argv, destination) {
  ...
  if (unknown.length > 0) {
    fail(
      `attestation verification is only defined for asset-backed products: ${unknown.join(', ')}`,
    );
  }
  ...
  for (const product of products) {
    ...
    for (const asset of assets) {
      const file = path.join(productDir, asset);
      await fs.writeFile(file + '.public', [repo, tag, asset, file].join('\0') + '\0');
    }
  }
}
...
async function main(argv) {
  ...
  if (command === '--prepare-bundles') return receiptMain('pre-mutation', argv.slice(2), argv[1]);
  if (command === '--prepare-public') return legacyMain(argv.slice(2), argv[1]);
  ...
  if (argv.includes('--help') || argv.includes('-h')) return parseLegacyArgs(argv);
  throw new Error('use verify-github-release-attestations.sh');
}
