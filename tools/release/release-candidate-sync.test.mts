...
test('shared-source release candidates update only their declared release files', (t) => {
  ...
  assert.deepEqual(changes.map(({ path: file }) => path.relative(state.root, file)).sort(), [
  ...
  assert.equal(
    JSON.parse(readFileSync(path.join(state.root, '.release-please-manifest.json'), 'utf8'))[
      PACKAGE_PATH
    ],
    '1.0.1',
  );
