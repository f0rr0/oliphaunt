...
function bumpVersion(version, intent, context, prefix) {
  ...
  if (version === '0.0.0') {
    throw transitionError(
      prefix,
      `${context} has no released baseline; let Release Please create its first release`,
    );
  }
...
function conventionalIntent(subject, body, version, commit, prefix) {
  ...
  const intent = breaking
    ? major === 0
      ? 'minor'
      : 'major'
    : match[1] === 'feat' && major > 0
      ? 'minor'
      : 'patch';
...
function carrierDescriptor(root, prefix) {
  ...
  if (
    ...
    throw transitionError(
      prefix,
      `${CONTRIB_CARRIERS_PATH} must name distinct native_owner and wasix_owner products`,
    );
  }
  return {
...
export function releasePleaseState(root, headRef, { shared = false } = {}) {
  ...
  const ref = shared ? readFileSync(path.join(state, 'shared-head'), 'utf8').trim() : capturedRef;
  if (end !== '' || realpathSync(root) !== capturedRoot || ref !== headRef)
    throw new Error('release history snapshot does not match the requested checkout/ref');
  return state;
}

function sharedCommits(state, owner) {
  const fields = readFileSync(path.join(state, owner + '.commits'), 'utf8').split('\0');
  if (fields.pop() !== '' || fields.length % 3 !== 0)
    throw new Error('invalid shared source commit snapshot');
  const commits = [];
  for (let index = 0; index < fields.length; index += 3) {
    const [commit, subject, body] = fields.slice(index, index + 3).map((value) => value.trim());
    if (!/^[0-9a-f]{40}$/u.test(commit)) throw new Error('invalid shared source commit SHA');
    commits.push({ commit, subject, body });
  }
  return commits;
}
...
export function sharedContribReleaseCandidates(
  ...
  const state = releasePleaseState(root, headRef, { shared: true });
  if (readFileSync(path.join(state, 'inputs'), 'utf8') !== inputs.join('\0') + '\0')
    throw transitionError(prefix, 'shared source inputs changed after history capture');
  ...
  for (const owner of owners) {
    ...
    if (product === undefined) {
      throw transitionError(
        prefix,
        `${CONTRIB_CARRIERS_PATH} owner ${owner} is not a release product`,
      );
    }
    const prefixes = tagPrefixes(product, prefix);
    if (readFileSync(path.join(state, owner + '.prefixes'), 'utf8') !== prefixes.join('\0') + '\0')
      throw transitionError(prefix, 'shared source tag prefixes changed after history capture');
    const baseRef = readFileSync(path.join(state, owner + '.tag'), 'utf8').trim() || EMPTY_TREE;
    if (baseRef === EMPTY_TREE) {
      throw transitionError(
        prefix,
        `${owner} has no release tag from which to derive shared contrib intent`,
      );
    }
    const commits = sharedCommits(state, owner);
    ...
    if (before === null) {
      throw transitionError(
        prefix,
        `${owner} shared contrib bridge cannot replace a first-release candidate`,
      );
    }
    ...
    const changelogSection = reasons.some(
      ({ type, intent }) => type === 'feat' || intent !== 'patch',
    )
      ? 'Features'
      : 'Bug Fixes';
    ...
    if (transition !== undefined) {
      ...
      candidates.push({
        ...
        after:
          INTENT_RANK[actualIntent] < INTENT_RANK[requiredIntent]
            ? requiredAfter
            : transition.after,
...
if (import.meta.main) {
  const [command, root, requestedRef, graphFile] = process.argv.slice(2);
  if (command !== '--snapshot-inputs') throw new Error('use release-please-state.sh');
  const state = process.env.OLIPHAUNT_RELEASE_PLEASE_STATE;
  const graph =
    graphFile === '@workspace' ? loadGraph() : JSON.parse(readFileSync(graphFile, 'utf8'));
  const headRef =
    requestedRef === '@sync'
      ? releasePleaseWorktreeTransitions(root).length > 0
        ? 'HEAD^'
        : 'HEAD'
      : requestedRef;
  const { owners, inputs } = carrierDescriptor(root, 'release-please-state');
  writeFileSync(path.join(state, 'shared-head'), headRef + '\n');
  writeFileSync(path.join(state, 'inputs'), inputs.join('\0') + '\0');
  for (const owner of owners) {
    if (!/^[a-zA-Z0-9_-]+$/u.test(owner)) throw new Error('invalid shared source owner');
    writeFileSync(
      path.join(state, owner + '.prefixes'),
      tagPrefixes(graph.products[owner]).join('\0') + '\0',
    );
  }
}
