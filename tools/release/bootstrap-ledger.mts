...
function publicationExpectation(carrier) {
  ...
  if (carrier.ecosystem === 'cargo') {
    if (!artifact.path.endsWith('.crate'))
      throw error(`${carrier.id} must freeze a .crate archive, not ${artifact.path}`);
    return { algorithm: 'sha256', digest: artifact.sha256, source: 'crates.io-version-checksum' };
  }
  if (carrier.ecosystem === 'npm') {
    if (!artifact.path.endsWith('.tgz'))
      throw error(`${carrier.id} must freeze an npm .tgz archive, not ${artifact.path}`);
    const file = path.resolve(ROOT, artifact.path);
    let stat;
    try {
      stat = statSync(file);
    } catch {
      throw error(`${carrier.id} locked npm archive is unavailable: ${artifact.path}`);
    }
    if (
      !stat.isFile() ||
      stat.size !== artifact.size ||
      fileDigest(file, 'sha256') !== artifact.sha256
    ) {
      throw error(`${carrier.id} local npm archive does not match its frozen byte envelope`);
    }
    return {
      algorithm: 'sha512',
      digest: fileDigest(file, 'sha512', 'base64'),
...
function publicationRows(lock, products) {
  ...
  return lockedCarriers(lock)
    .filter(
      (carrier) => selected.has(carrier.product) && ['cargo', 'npm'].includes(carrier.ecosystem),
    )
    .map((carrier) => ({
      id: carrier.id,
...
function checkpointBase(lock, products, publicationIds = undefined) {
  ...
  if (unknownIds.length > 0)
    throw error(
      `bootstrap publication IDs are absent from the selected lock: ${unknownIds.join(', ')}`,
    );
  const publications = allPublications.filter(({ id }) => selectedIds.has(id));
  if (publications.length === 0)
    throw error('selected products have no Cargo or npm publication identities to bootstrap');
  return {
...
export function buildBootstrapLedger(
  lock,
  products,
  { sequence = 0, previousCheckpointDigest = null, publicationIds = undefined, receipts = [] } = {},
) {
  const checkpoint = {
...
function validateReceipt(receipt, publication) {
  if (receipt === null || Array.isArray(receipt) || typeof receipt !== 'object')
    throw error('ledger receipt must be an object');
  for (const key of ['id', 'product', 'ecosystem', 'name', 'version']) {
    if (receipt[key] !== publication[key])
      throw error(`${publication.id} receipt ${key} does not match its frozen publication`);
  }
  ...
  if (
    ...
    !/^https?:\/\//u.test(proof.url)
  ) {
    throw error(
      `${publication.id} receipt does not prove the frozen archive digest at its registry`,
    );
  }
}
...
export function validateBootstrapLedger(checkpoint, lock, products) {
  if (checkpoint === null || Array.isArray(checkpoint) || typeof checkpoint !== 'object')
    throw error('ledger checkpoint must be an object');
  if (checkpoint.schema !== BOOTSTRAP_LEDGER_SCHEMA)
    throw error(`ledger schema must be ${BOOTSTRAP_LEDGER_SCHEMA}`);
  assertHash(checkpoint.checkpointDigest, 'ledger.checkpointDigest');
  if (checkpoint.checkpointDigest !== digest(withoutDigest(checkpoint)))
    throw error('ledger checkpoint digest mismatch');
  ...
  for (const key of [
    ...
    if (stableJson(checkpoint[key]) !== stableJson(expectedBase[key])) {
      throw error(
        `ledger ${key} is not bound to the active publication lock/source/package envelope`,
      );
    }
  }
  if (!Number.isSafeInteger(checkpoint.sequence) || checkpoint.sequence < 0)
    throw error('ledger sequence must be non-negative');
  if (
    !(
      checkpoint.previousCheckpointDigest === null ||
      /^[0-9a-f]{64}$/u.test(checkpoint.previousCheckpointDigest)
    )
  ) {
  ...
  for (const receipt of checkpoint.receipts) {
    if (seen.has(receipt?.id) || !byId.has(receipt?.id))
      throw error(`ledger has a duplicate or unknown receipt ${String(receipt?.id)}`);
    seen.add(receipt.id);
    validateReceipt(receipt, byId.get(receipt.id));
  }
  if (checkpoint.complete !== (checkpoint.receipts.length === checkpoint.publications.length))
    throw error('ledger complete flag is inconsistent');
  return checkpoint;
}
...
function checkpointFiles(directory) {
  ...
  return readdirSync(directory, { withFileTypes: true })
    .filter(
      (entry) => entry.isFile() && /^checkpoint-[0-9]{6}-[0-9a-f]{64}[.]json$/u.test(entry.name),
    )
    .map((entry) => path.join(directory, entry.name))
    .sort(compareText);
}

export function loadBootstrapLedger(
  directory,
  lock,
  products,
  { allowEmpty = false, requireComplete = false } = {},
) {
  ...
  for (const [index, file] of files.entries()) {
    ...
    try {
      ...
      throw error(`cannot read ${file}: ${cause.message}`);
    }
    validateBootstrapLedger(checkpoint, lock, products);
    if (
      checkpoint.sequence !== index ||
      checkpoint.previousCheckpointDigest !== (previous?.checkpointDigest ?? null)
    ) {
    ...
    if (path.basename(file) !== expectedName)
      throw error(`${file} name does not match its sequence/digest`);
    if (previous !== null) {
      const prior = new Map(previous.receipts.map((receipt) => [receipt.id, stableJson(receipt)]));
      const current = new Map(
        checkpoint.receipts.map((receipt) => [receipt.id, stableJson(receipt)]),
      );
      for (const [id, bytes] of prior) {
        if (current.get(id) !== bytes)
          throw error(`${file} rewrites or removes immutable receipt ${id}`);
      }
    }
    previous = checkpoint;
  }
  if (requireComplete && !previous.complete) {
    throw error(
      `bootstrap ledger is incomplete: ${previous.receipts.length}/${previous.publications.length} receipts`,
    );
  }
  return previous;
}
...
function writeCheckpoint(directory, checkpoint) {
  ...
  const file = path.join(
    directory,
    `checkpoint-${String(checkpoint.sequence).padStart(6, '0')}-${checkpoint.checkpointDigest}.json`,
  );
...
export function appendBootstrapCheckpoint(
  directory,
  lock,
  products,
  receipts,
  ...
  if (
    previous !== null &&
    publicationIds !== undefined &&
    stableJson([...publicationIds].sort(compareText)) !==
      stableJson([...activePublicationIds].sort(compareText))
  ) {
...
async function reverifyReceipts(checkpoint, lock) {
  const observed = await verifyLockedRegistryIntegrity(lock, {
    carrierIds: checkpoint.receipts.map(({ id }) => id),
  });
  const actual = new Map(observed.map((receipt) => [receipt.id, stableJson(receipt)]));
  for (const receipt of checkpoint.receipts) {
    if (actual.get(receipt.id) !== stableJson(receipt))
      throw error(`${receipt.id} registry proof changed since its immutable checkpoint`);
  }
}
...
function parseArgs(argv) {
  ...
  if (!['init', 'checkpoint', 'seal', 'verify'].includes(command) || !lockFile || !productsJson) {
    throw error(
      'usage: bootstrap-ledger.mts <init|checkpoint|seal|verify> --lock FILE --products-json JSON [--ledger DIR] [--product ID --ecosystem cargo|npm | --carrier-ids-json JSON] [--verify-registries]',
    );
  }
  let products;
  try {
  ...
  if (carrierIdsJson) {
    try {
      carrierIds = JSON.parse(carrierIdsJson);
    } catch (cause) {
      throw error(`--carrier-ids-json must be valid JSON: ${cause.message}`);
    }
    if (
  ...
  if (command === 'checkpoint') {
    const productCheckpoint =
      Boolean(product) && ['cargo', 'npm'].includes(ecosystem) && carrierIds.length === 0;
    const carrierCheckpoint = !product && !ecosystem && carrierIds.length > 0;
    if (!productCheckpoint && !carrierCheckpoint) {
      throw error(
        'checkpoint requires either --product with --ecosystem cargo|npm or --carrier-ids-json',
      );
    }
  }
...
if (import.meta.main) {
  try {
    ...
    if (args.command === 'init') {
      ...
      if (checkpoint === null)
        checkpoint = appendBootstrapCheckpoint(args.ledger, lock, args.products, []);
    } else if (args.command === 'checkpoint') {
      const receipts = await verifyLockedRegistryIntegrity(
        lock,
        args.carrierIds.length > 0
    ...
    console.log(
      `${args.command} bootstrap checkpoint ${checkpoint.sequence} (${checkpoint.receipts.length}/${checkpoint.publications.length}, ${checkpoint.checkpointDigest})`,
    );
  } catch (cause) {
