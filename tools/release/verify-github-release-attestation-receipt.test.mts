...
} from './verify_github_release_attestations.mts';
...
describe('GitHub release attestation receipt', () => {
  test('uses one release page plus one exact asset inventory per product in both phases', async () => {
    ...
    expect(calls.filter(({ url }) => url.endsWith('/releases?per_page=100&page=1'))).toHaveLength(
      2,
    );
    expect(
      calls.filter(({ url }) => /\/releases\/[1-9][0-9]*\/assets\?per_page=100&page=1$/u.test(url)),
    ).toHaveLength(98);
    expect(calls.every(({ options }) => options.headers.Authorization === `Bearer ${TOKEN}`)).toBe(
      true,
    );
  });
  ...
  test('paginates the repository release inventory and rejects duplicate selected tags', async () => {
    ...
    const historical = Array.from({ length: 100 }, (_, index) =>
      remoteRelease(
        { id: `historical-${index}`, version: '0.1.0' },
        {
          assets: [],
    ...
    expect(
      calls
        .filter((url) => new URL(url).pathname.endsWith('/releases'))
    ...
    await expect(
      queryLockedGithubReleases(lock, {
        authToken: TOKEN,
        fetchImpl: async (url) => {
          const page = Number(new URL(url).searchParams.get('page'));
          return Response.json(
            duplicatePages[page - 1],
            page === 1
              ? { headers: { Link: `<${url.replace('page=1', 'page=2')}>; rel="next"` } }
              : undefined,
          );
  ...
  test('exact 100- and 200-row release inventories do not request an empty trailing page', async () => {
    ...
    for (const totalRows of [100, 200]) {
      const historical = Array.from({ length: totalRows - lock.products.length }, (_, index) =>
        remoteRelease(
          { id: `historical-${totalRows}-${index}`, version: '0.1.0' },
          {
            assets: [],
            releaseId: index + 1_000,
          },
      ...
      const releases = await queryLockedGithubReleases(lock, {
        ...
        fetchImpl: async (url) => {
          ...
          if (parsed.pathname.endsWith('/releases')) {
            ...
            return Response.json(
              ...
              page < pages.length
                ? {
                    headers: {
                      Link: `<${url.replace(`page=${page}`, `page=${page + 1}`)}>; rel="next"`,
                    },
                  }
                : undefined,
            );
          ...
          return Response.json(
            parsed.pathname.endsWith('/releases/2/assets') ? [remoteAsset()] : [],
          );
        },
        repo: REPO,
      });
      expect(releases).toHaveLength(2);
      expect(releasePageCalls).toEqual(
        Array.from({ length: totalRows / 100 }, (_, index) => index + 1),
      );
    }
  });
  ...
  test('refuses to query draft releases without an explicit authenticated token', async () => {
    let calls = 0;
    await expect(
      queryLockedGithubReleases(lockFixture(), {
        authToken: '',
        fetchImpl: async () => {
          calls += 1;
          return Response.json([]);
  ...
  test('rejects a missing GitHub digest and contamination of a zero-asset release', async () => {
    ...
    await expect(
      queryLockedGithubReleases(zeroLock, {
        authToken: TOKEN,
  ...
  test('retries the whole exact snapshot beyond three seconds while GitHub digest metadata converges', async () => {
    ...
    const fetchImpl = async (url) => {
      ...
      return Response.json(
        parsed.pathname.endsWith('/releases/2/assets')
    ...
    expect(releases.find((release) => release.product === 'product-a').assets[0].sha256).toBe(
      ASSET_SHA,
    );
  });
  ...
  test('retries transient GitHub responses and honors bounded rate-limit waits', async () => {
    ...
    const value = await requestGithubJsonWithRetry('https://api.github.com/example', {
      ...
      fetchImpl: async () => {
        attempts += 1;
        return attempts === 1 ? new Response('busy', { status: 502 }) : Response.json({ ok: true });
      },
      nowImpl: () => now,
      sleepImpl: async (milliseconds) => {
        sleeps.push(milliseconds);
    ...
    await expect(
      requestGithubJsonWithRetry('https://api.github.com/example', {
        deadlineMs: 180_000,
        fetchImpl: async () =>
          Response.json(
            { message: 'API rate limit exceeded' },
  ...
  test('requires an exact non-overlapping signed subject union', () => {
    ...
    expect(() =>
      assertAttestationSubjectCoverage(assets, [
        ...receiptSubjects(),
    ...
    expect(assertAttestationSubjectCoverage(repeatedNameAssets, repeatedNameAttestation)).toEqual(
      repeatedNameAttestation,
    );
    expect(() =>
      assertAttestationSubjectCoverage(repeatedNameAssets, [
        {
  ...
  test('builds a deterministic lock/head/release-ID-bound receipt', async () => {
    ...
    const publisherReceipt = buildGithubAttestationReceipt({
      ...
      releases,
      repo: REPO,
      publisherSha: '4'.repeat(40),
    });
    expect(
      validateGithubAttestationReceipt(publisherReceipt, lock, { repo: REPO }).publisherSha,
    ).toBe('4'.repeat(40));
    expect(publisherReceipt.head).toBe(COMMIT);
    publisherReceipt.publisherSha = '5'.repeat(40);
    expect(() => validateGithubAttestationReceipt(publisherReceipt, lock, { repo: REPO })).toThrow(
      /digest mismatch/u,
    );
  ...
  test('verifies one locked local subject per bundle and checks the complete signed statement', async () => {
    ...
    await expect(
      verifyAttestationBundles(lock, [bundlePath], {
        repo: REPO,
...
`,
  ...
  test("accepts only gh's known empty signature key ID and RFC3161 protobuf defaults", () => {
    ...
    expect(() =>
      assertGhVerifiedBundleMatchesSupplied(unexpectedCanonicalization, supplied),
    ).toThrow('does not contain the supplied bundle');
  });
  ...
  test('publishes receipt files atomically, cleans interrupted temps, and permits only identical reruns', async () => {
    ...
    await expect(
      writeImmutableReceipt(output, receipt, {
        linkImpl: async () => {
    ...
    await expect(writeImmutableReceipt(output, receipt)).rejects.toThrow(
