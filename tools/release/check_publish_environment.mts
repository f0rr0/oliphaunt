...
function fail(message) {
  console.error(`check_publish_environment.mts: ${message}`);
  process.exit(1);
}
...
function parseArgs(argv) {
  ...
  if (productsJson === null) {
    fail('usage: tools/release/check_publish_environment.mts --products-json <json-string-list>');
  }
  return { productsJson };
}
...
async function productConfigs() {
  const releasePlease = JSON.parse(
    await fs.readFile(path.join(root, 'release-please-config.json'), 'utf8'),
  );
