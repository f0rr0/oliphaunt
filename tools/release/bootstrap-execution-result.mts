...
export function validateBootstrapExecutionResult(value, expected = {}) {
  ...
  exactKeys(
    value.lock,
    ['catalogDigest', 'lockDigest', 'packageEnvelopeDigest'],
    'execution result lock',
  );
  if (
    Object.values(value.lock).some((digest) => typeof digest !== 'string' || !DIGEST.test(digest))
  ) {
  ...
  const normalized = {
    ...
    admittedIds: uniqueStrings(value.admittedIds, 'execution result admittedIds', {
      ordered: true,
    }),
    completedIds: uniqueStrings(value.completedIds, 'execution result completedIds', {
      ordered: true,
    }),
    newlyCompletedIds: uniqueStrings(
      value.newlyCompletedIds,
      'execution result newlyCompletedIds',
      { ordered: true },
    ),
    remainingIds: uniqueStrings(value.remainingIds, 'execution result remainingIds', {
      ordered: true,
    }),
  };
  ...
  if (normalized.admittedIds.some((id) => !plan.has(id))) {
    throw error(
      'execution result admittedIds must be a projection of completedIds and remainingIds',
    );
  }
  if (
    normalized.newlyCompletedIds.some(
      (id) => !completed.has(id) || !normalized.admittedIds.includes(id),
    )
  ) {
    throw error(
      'execution result newlyCompletedIds must be a subset of completedIds and admittedIds',
    );
  }
  ...
  if (normalized.decision === 'complete') {
    if (
      ...
      throw error(
        'complete execution result must have no deferral mode, remaining IDs, or not-before time',
      );
    }
  } else {
    if (
      ...
      throw error(
        'deferred execution result requires remaining work and a positive not-before time',
      );
    }
    ...
    if (
      normalized.deferralMode === 'pre-mutation-capacity' &&
      (!zeroProgress || normalized.admittedIds.length !== 0)
    ) {
      throw error('pre-mutation capacity deferral must admit and mutate zero bootstrap operations');
    }
    if (
      normalized.deferralMode === 'rate-limit' &&
      (!zeroProgress ||
        !normalized.admittedIds.some((id) => remaining.has(id) && /^(?:carrier:)?cargo:/u.test(id)))
    ) {
      throw error(
        'rate-limit deferral requires admitted remaining Cargo work and no new completion',
      );
    }
    if (
      normalized.deferralMode === 'pre-mutation-deadline' &&
      (!zeroProgress || !normalized.admittedIds.some((id) => remaining.has(id)))
    ) {
      throw error(
    ...
    if (
      !['progress', 'pre-mutation-capacity', 'rate-limit', 'pre-mutation-deadline'].includes(
        normalized.deferralMode,
      )
    ) {
  ...
  if (
    expected.products !== undefined &&
    stable(normalized.products) !== stable(expected.products)
  ) {
