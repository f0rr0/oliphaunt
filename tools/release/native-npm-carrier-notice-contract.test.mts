...
function pack(directory, output) {
  const result = spawnSync('pnpm', ['pack', '--pack-destination', output, '--json'], {
    cwd: directory,
    encoding: 'utf8',
  });
  assert.equal(
...
test('every native npm payload carrier declares and physically packs its exact legal profile', (t) => {
  ...
  for (const [index, carrier] of CARRIERS.entries()) {
    ...
    const selectedLegalMembers = manifest.files.filter(
      (member) =>
        member === 'LICENSE' ||
    ...
    assert.deepEqual(
      selectedLegalMembers,
      noticeMembers,
      `${manifest.name} files must select no stale legal members`,
    );
    ...
    const packedManifest = JSON.parse(
      Buffer.from(readPortableArchiveEntries(tarball).get('package/package.json').data()).toString(
        'utf8',
      ),
    );
