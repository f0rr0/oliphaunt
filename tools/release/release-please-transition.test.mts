...
function withHistory(root, headRef, graph, callback) {
  const scratch = mkdtempSync(path.join(os.tmpdir(), 'release-please-test-context-'));
  try {
    ...
    writeFileSync(
      probe,
      'import * as api from ' +
        JSON.stringify(path.resolve(import.meta.dirname, 'release-please-transition.mts')) +
...
function releasePleaseWorktreeTransitions(root, options = {}) {
  return withHistory(
    root,
    options.headRef ?? 'HEAD',
    null,
    'api.releasePleaseWorktreeTransitions(' +
      JSON.stringify(root) +
      ', ' +
...
function sharedContribReleaseCandidates(root, graph, transitions, options = {}) {
  return withHistory(
    root,
    options.headRef ?? 'HEAD',
    graph,
    'api.sharedContribReleaseCandidates(' +
      [root, graph, transitions, options].map((value) => JSON.stringify(value)).join(',') +
      ')',
  );
}
...
function fixture(t, versions = null) {
  ...
  if (versions !== null) {
    for (const directory of Object.values(PRODUCT_PATHS))
...
function writeCarrierDescriptor(root) {
  ...
  writeFileSync(path.join(root, 'src/extensions/contrib/postgres18.toml'), contribManifest('18.4'));
  writeFileSync(
  ...
  writeFileSync(
    path.join(root, 'src/shared/extension-runtime-contract/contract.toml'),
    'schema = "v1"\n',
  );
}
...
function runtimeGraph(versions) {
  return {
    products: Object.fromEntries(
      ['liboliphaunt-native', 'liboliphaunt-wasix'].map((product) => [
        product,
        {
...
test('shared-only fixes with no Release Please transitions seed both runtime candidates', (t) => {
  ...
  assert.deepEqual(
    sharedContribReleaseCandidates(root, runtimeGraph(V1), [], { prefix: 'transition-test' }).map(
      ({ product, before, after, changelogSection }) => ({
        product,
        before,
        after,
        changelogSection,
...
test('a pre-1.0 breaking contrib commit requires a minor runtime bump', (t) => {
  ...
  assert.deepEqual(
    sharedContribReleaseCandidates(root, runtimeGraph(versions), [], {
      prefix: 'transition-test',
...
test('an insufficient runtime candidate is promoted to the shared-source intent', (t) => {
  ...
  assert.deepEqual(
    sharedContribReleaseCandidates(
      ...
      { prefix: 'transition-test' },
    ).map(({ product, before, after, changelogMode }) => ({
      product,
      before,
      after,
      changelogMode,
