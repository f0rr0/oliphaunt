...
import { requirePreMutationRegistryWindow } from './registry-publication-deferral.mts';
...
async function boundedSleep(milliseconds, { deadlineEpochSeconds, nowImpl, sleepImpl, context }) {
...
export async function inspectNpmVersionState({
  ...
  return {
    ...
    missingNames: identities
      .filter((_, index) => observed[index].state === 'missing-name')
      .map(({ name }) => name),
  };
}
...
export async function waitForNpmExactVersion({
  ...
  throw error(
    `${packageName}@${version} did not become visible with frozen SRI before the bounded visibility attempts ended`,
  );
}

export async function prepareFrozenNpmPublication({
  packageName,
  version,
  tarball,
  deadlineEpochSeconds,
  ...
  const registry = canonicalNpmRegistry(
    options.registry ?? process.env.NPM_REGISTRY ?? DEFAULT_NPM_REGISTRY,
  );
  const expectedIntegrity = frozenNpmIntegrity(tarball);
  const state = await inspectNpmExactVersion({
    packageName,
    version,
    expectedIntegrity,
  ...
  const timeout = state.published
    ...
    : Math.min(
        ...
        requirePreMutationRegistryWindow({
          deadlineEpochSeconds,
          nowEpochMilliseconds: (options.nowImpl ?? Date.now)(),
          minimumMilliseconds: MINIMUM_PUBLISH_ATTEMPT_MS,
          reserveMilliseconds: DEADLINE_RESERVE_MS,
          context: 'npm publish for ' + packageName + '@' + version,
        }),
      );
  return {
    packageName,
    ...
    expectedIntegrity,
    deadlineEpochSeconds,
    skipped: state.published,
    state,
    timeout,
  };
}

export async function reconcileFrozenNpmPublication(prepared, options = {}) {
  if (frozenNpmIntegrity(prepared.tarball) !== prepared.expectedIntegrity)
    throw error('npm tarball changed after publication admission');
  return await waitForNpmExactVersion({ ...prepared, ...options });
}
