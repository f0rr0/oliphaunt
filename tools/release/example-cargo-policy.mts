...
} from '../../src/shared/product-metadata/wasix-cargo-toolchain-policy.mts';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const TOOL = 'example-cargo-policy.mts';
const WASIX_PRODUCT_MANIFEST_PATH = 'src/bindings/wasix-rust/crates/oliphaunt-wasix/Cargo.toml';
...
export { REQUIRED_WASIX_CONSUMER_PINS, validateWasixConsumerDependencyPins };
...
export const EXAMPLE_CARGO_POLICIES = Object.freeze([
  exampleCargoPolicy({
    ...
    dependencyBindings: [
      ...
      dependencyBinding('liboliphaunt-native-linux-x64-gnu', [
        'target',
        LINUX_X64_GNU_TARGET,
        'dependencies',
      ]),
      dependencyBinding('oliphaunt-broker-linux-x64-gnu', [
        'target',
        LINUX_X64_GNU_TARGET,
        'dependencies',
      ]),
      dependencyBinding('oliphaunt-extension-contrib-pg18-linux-x64-gnu', [
        'target',
        LINUX_X64_GNU_TARGET,
        'dependencies',
      ]),
  ...
  exampleCargoPolicy({
    ...
    dependencyBindings: [
      ...
      dependencyBinding('liboliphaunt-wasix-aot-x86_64-unknown-linux-gnu', [
        'target',
        LINUX_X64_GNU_TARGET,
        'dependencies',
      ]),
      dependencyBinding('oliphaunt-wasix-tools-aot-x86_64-unknown-linux-gnu', [
        'target',
        LINUX_X64_GNU_TARGET,
        'dev-dependencies',
      ]),
...
function expectedProductVersion(product, context) {
  ...
  if (rows.length !== 1) {
    fail(
      `release product ${JSON.stringify(product)} must appear exactly once in the publication catalog`,
    );
  }
  ...
  for (const table of dependencyTables(manifest)) {
    for (const [name, spec] of Object.entries(table)) {
      ...
      if (expected !== undefined && actual !== expected) {
        failures.push(
          `${manifestLabel} ${name} uses ${JSON.stringify(actual)}; expected ${expected}`,
        );
      }
    }
  }
...
export function validateExampleManifests() {
  ...
  failures.push(
    ...validateWasixConsumerDependencyPins(readToml(path.join(ROOT, WASIX_PRODUCT_MANIFEST_PATH)), {
      toolchainVersions,
    }),
  );
  for (const policy of EXAMPLE_CARGO_POLICIES) {
...
export function validateResolvedPackagePolicy(
  ...
  failures.push(...validateResolvedWasixToolchainPolicy(lockfile, packages, { toolchainVersions }));
  return failures;
}

function main(argv) {
  if (argv.length > 1 || (argv.length === 1 && argv[0] !== '--check')) {
    fail('usage: tools/release/example-cargo-policy.mts [--check]');
  }
  const failures = validateExampleManifests();
  if (failures.length > 0) fail(failures.join('\n'));
  console.log(
    `example Cargo manifests are registry-neutral and ${EXAMPLE_CARGO_POLICIES.length} ephemeral lock policies are valid`,
  );
}
