...
import { extensionSqlNames } from '../../src/shared/product-metadata/release-artifact-targets.mts';
import {
  loadNativeComponentContract,
  resolveNativeComponentClosure,
} from '../../src/extensions/tools/native-component-contract.mts';
...
const WASIX_ASSET_PACKAGER = 'src/extensions/artifacts/wasix/tools/package-release-assets.mts';
...
`,
  ...
  const calls = existsSync(callsFile) ? readFileSync(callsFile, 'utf8').trimEnd().split('\n') : [];
...
test('full extension assembly validates exactly the planner-selected products and all targets', () => {
  ...
  assert.deepEqual(calls, [
    'src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts --all --require-native --require-wasix',
    'src/extensions/artifacts/packages/tools/check-carriers.mts --require-full-extension-targets oliphaunt-extension-postgis oliphaunt-extension-vector',
  ]);
});
...
test('full extension assembly requires every product when no focused selection exists', () => {
  ...
  assert.deepEqual(calls, [
    'src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts --all --require-native --require-wasix',
    'src/extensions/artifacts/packages/tools/check-carriers.mts --require-full-extension-targets all',
  ]);
});
...
test('extension assembly stops after producer failure and propagates validator failure', () => {
  const producerFailure = fixtureRun(RELEASE_SCRIPT, {
    failTool: 'src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts',
  });
  ...
  const validatorFailure = fixtureRun(RELEASE_SCRIPT, {
    failTool: 'src/extensions/artifacts/packages/tools/check-carriers.mts',
  });
...
test('mobile contrib assembly scopes both staging and validation to native carriers', () => {
  ...
  assert.deepEqual(calls, [
    'src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts --output-root target/mobile-extension-artifacts --family native oliphaunt-extension-contrib-pg18 --require-native-target android-arm64-v8a --require-native-target ios-xcframework',
    'src/extensions/artifacts/packages/tools/check-carriers.mts --family native oliphaunt-extension-contrib-pg18',
  ]);
});
...
test('WASIX release staging resolves runtime-owned contrib through its logical artifact product', () => {
  ...
  assert.match(
    execution.stdout,
    new RegExp(`staged ${sqlNames.length} WASIX exact-extension artifact`),
  );
