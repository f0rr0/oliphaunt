...
function assertQualifiedReplaySourceState({ repo, headRef, expectedSha }) {
  execFileSync(
    'bash',
    [path.resolve(import.meta.dirname, 'qualified-release-replay.sh'), headRef, expectedSha],
  ...
  try {
    assert.equal(
      execFileSync(
        'bash',
        [
          path.resolve(import.meta.dirname, 'qualified-release-replay.sh'),
          '--qualified-ci',
          'HEAD',
          sha,
        ],
...
test('qualified replay treats a newline-bearing untracked path as one complete dirty record', () => {
  ...
  try {
    ...
    assert.throws(
      () => assertQualifiedReplaySourceState({ repo, headRef: 'HEAD', expectedSha: sha }),
      (error) =>
        /clean source checkout/u.test(error.message) &&
