...
const REQUEST_ATTEMPTS = Math.max(
  1,
  Number.parseInt(process.env.OLIPHAUNT_REGISTRY_QUERY_ATTEMPTS || '8', 10) || 8,
);
const REQUEST_RETRY_DELAY_SECONDS = Math.max(
  0,
  Number.parseFloat(process.env.OLIPHAUNT_REGISTRY_QUERY_RETRY_DELAY || '1.0') || 0,
);
...
export function registryRequestTimeoutMilliseconds(context, { nowImpl = () => Date.now() } = {}) {
...
export async function boundedRegistrySleep(
  seconds,
  context,
  {
    nowImpl = () => Date.now(),
...
function fail(message) {
  console.error(`check_registry_publication.mts: ${message}`);
  process.exit(1);
}
...
async function productCrates(product) {
  const catalog = loadPublicationCatalog('check_registry_publication.mts', { products: [product] });
  ...
  const duplicates = [
    ...new Set(crates.filter((crate, index) => crates.indexOf(crate) !== index)),
  ].sort();
...
export function productRegistryPackagesFromLock(
  ...
  if (versionOverride !== undefined && versionOverride !== productRow.version) {
    fail(
      `${product} requested version ${versionOverride} does not match frozen publication-lock version ${productRow.version}`,
    );
  }
  const ecosystemByKind = new Map([
    ['crates', 'cargo'],
    ['npm', 'npm'],
    ['maven', 'maven'],
  ]);
...
export async function productRegistryPackages(
  product,
  { versionOverride = undefined, registryKind = undefined } = {},
) {
  ...
  if (stalePackages.length > 0) {
    fail(
      `${product} publication catalog contains entries without a matching registry publish target: ${stalePackages.join(', ')}`,
    );
  }
  ...
  if (duplicateIdentities.length > 0) {
    fail(
      `${product}.registry_packages contains duplicate identities: ${[...new Set(duplicateIdentities)].sort().join(', ')}`,
    );
  }
  const missingKinds = [];
  ...
  if (missingKinds.length > 0) {
    const selectedTargets = [...publishTargets]
      .filter((target) => REGISTRY_TARGETS.has(target))
      .sort();
    fail(
      `${product} publishes to ${JSON.stringify(selectedTargets)} but is missing registry_packages entries for: ${missingKinds.join(', ')}`,
    );
  }
    ...
    try {
      const response = await fetchImpl(url, {
        ...
        signal: AbortSignal.timeout(
          registryRequestTimeoutMilliseconds(`registry request for ${label}`),
        ),
      });
      if (response.ok) {
  ...
  for (let attempt = 0; attempt < REQUEST_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      const response = await fetchImpl(url, {
        ...
        redirect: 'follow',
        signal: AbortSignal.timeout(
          registryRequestTimeoutMilliseconds(`registry request for ${url}`, { nowImpl }),
        ),
      });
      if (response.ok) {
        await response.body?.cancel?.().catch(() => {});
...
export async function cratesioUrlExists(
  url,
  label,
  {
    fetchImpl = fetch,
...
export async function queryProductPublication(
  product,
  { versionOverride = undefined, registryKind = undefined, retries = 0, retryDelay = 0 } = {},
) {
  const packages = await productRegistryPackages(product, { versionOverride, registryKind });
  return queryRegistryPackages(packages, { retries, retryDelay });
}

export async function queryRegistryPackages(packages, { retries = 0, retryDelay = 0 } = {}) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    await boundedRegistrySleep(retryDelay, 'publication visibility retry');
  }
...
async function runQueryProductPublication(flags) {
  ...
  printJson(
    serializeQueryResult(
      await queryProductPublication(product, {
        versionOverride,
        registryKind,
        retries,
        retryDelay,
...
async function runProductRegistryPackages(flags) {
  ...
  const versionOverride = flagString(flags, 'version');
  printJson({
    packages: (await productRegistryPackages(product, { versionOverride, registryKind })).map(
      (pkg) => ({
        ...pkg,
  ...
  if (mode === 'require-identities') {
    ...
    for (const product of products) {
      ...
      if (status.packages.length === 0) {
        ...
        console.log(
          `${product} registry identity check passed: ${status.packages.map(identityLabel).join(', ')}`,
        );
      }
    }
  ...
  for (const product of products) {
    if (mode === 'report-identities') {
      ...
      if (status.present.length > 0) {
        console.log(
          `${product} registry identities present: ${status.present.map(identityLabel).join(', ')}`,
        );
      }
      if (status.missing.length > 0) {
        console.log(
          `${product} registry identities missing: ${status.missing.map(identityLabel).join(', ')}`,
        );
      }
      continue;
    }
    ...
    if (mode === 'report') {
      if (result.published.length > 0) {
        console.log(
          `${product} registry versions already present: ${result.published.map(packageLabel).join(', ')}`,
        );
      }
      if (result.missing.length > 0) {
        console.log(
          `${product} registry versions not yet present: ${result.missing.map(packageLabel).join(', ')}`,
        );
      }
      continue;
    }
    if (mode === 'require-published' && result.missing.length > 0) {
      fail(
        `${product} registry publication is missing: ${result.missing.map(packageLabel).join(', ')}`,
      );
    }
    if (mode === 'require-unpublished' && result.published.length > 0) {
      fail(
        `${product} version is already published in public registries: ${result.published.map(packageLabel).join(', ')}`,
      );
    }
    const state = mode === 'require-published' ? 'published' : 'unpublished';
    console.log(
      `${product} registry ${state} check passed: ${result.packages.map(packageLabel).join(', ')}`,
    );
  }
}

export async function checkRegistryPublication(argv) {
...
if (import.meta.main) {
  await checkRegistryPublication(Bun.argv.slice(2));
}
