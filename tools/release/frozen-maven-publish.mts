...
async function boundedSleep(milliseconds, { deadlineEpochSeconds, nowImpl, sleep, context }) {
...
export function stageFrozenMavenBundle({ lock, products, outputRoot }) {
  const carriers = lockedCarriers(lock, { products, ecosystem: 'maven' }).sort(
  ...
  for (const carrier of carriers) {
    ...
    for (const { artifact: envelope, file } of frozen.files) {
      if (/\.(?:asc|md5|sha1|sha256|sha512)$/u.test(file)) {
        throw error(
          `${carrier.id} lock must freeze primary Maven payloads, not generated signature/checksum ${envelope.path}`,
        );
      }
      ...
      payloads.push({
        carrier: carrier.id,
        frozenPath: envelope.path,
        staged,
        sha256: digestFile(staged, 'sha256'),
      });
    }
    ...
    validateMavenCentralPublication({
      ...
      files: [...names].map((name) => ({
        name,
        size: statSync(path.join(destination, name)).size,
      })),
      context: carrier.id,
    });
  }
...
// The preflight signs once. Publication reuses those bytes after binding the
// saved bundle to the current exact lock and selected Maven carrier set.
export function recordPreparedMavenBundle(prepared, lockDigest) {
  const size = statSync(prepared.bundle).size;
  assertMavenCentralBundleSize(size);
  const receipt = {
    lockDigest,
    carriers: prepared.carriers.map(({ id }) => id).sort(compareText),
    sha256: digestFile(prepared.bundle, 'sha256'),
    size,
  };
  writeFileSync(path.join(path.dirname(prepared.bundle), 'prepared.json'), JSON.stringify(receipt));
  return receipt;
}

export function loadPreparedMavenBundle({ lock, products, outputRoot }) {
  const receipt = JSON.parse(readFileSync(path.join(outputRoot, 'prepared.json'), 'utf8'));
  const carriers = lockedCarriers(lock, { products, ecosystem: 'maven' });
  const expected = carriers.map(({ id }) => id).sort(compareText);
  if (
    !expected.length ||
    receipt.lockDigest !== lock.lockDigest ||
    JSON.stringify(receipt.carriers) !== JSON.stringify(expected)
  )
    throw error('prepared Maven bundle does not match the exact publication lock and selection');
  const bundle = path.join(outputRoot, 'central-bundle.zip');
  const size = statSync(bundle).size;
  assertMavenCentralBundleSize(size);
  if (receipt.size !== size || receipt.sha256 !== digestFile(bundle, 'sha256'))
    throw error('prepared Maven bundle bytes changed after preflight');
  return { bundle, bundleSize: size, carriers };
}
...
export function assertMavenCentralBundleSize(size, maximum = MAX_CENTRAL_BUNDLE_BYTES) {
  ...
  if (!Number.isSafeInteger(maximum) || maximum < 1) {
    throw error(
      `Maven Central deployment bundle maximum must be a positive integer, got ${maximum}`,
    );
  }
  if (size >= maximum) {
    throw error(
      `Maven Central deployment bundle is ${size} bytes; the portal requires bundles smaller than ${maximum} bytes`,
    );
  }
}
...
async function centralRequest(
  ...
  if (!response.ok) {
    throw error(
      `Maven Central request returned HTTP ${response.status}${text ? `: ${boundedDetail(text)}` : ''}`,
    );
  }
  return text;
}
...
async function findDeployment({
  ...
  const matches =
    value?.deployments?.filter((deployment) => deployment?.deploymentName === deploymentName) ?? [];
  if (matches.length > 1) {
...
async function uploadBundle({
  ...
  if (existing !== null) {
    ...
    if (typeof id !== 'string' || !DEPLOYMENT_ID.test(id)) {
      throw error(
        `Maven Central deployment ${deploymentName} has invalid deployment ID ${JSON.stringify(id)}`,
      );
    }
...
async function waitForDeployment({
  ...
  for (let attempt = 0; attempt < 90; attempt += 1) {
    ...
    if (status?.deploymentState === 'FAILED') {
      throw error(
        `Maven Central deployment ${id} failed: ${boundedDetail(JSON.stringify(status.errors ?? status))}`,
      );
    }
    ...
    if (TERMINAL_STATES.has(status?.deploymentState)) {
      throw error(
        `Maven Central deployment ${id} reached unexpected state ${status.deploymentState}`,
      );
    }
  ...
  throw error(
    `Maven Central deployment ${id} did not reach ${[...acceptable].join(' or ')} within 15 minutes`,
  );
}
...
export async function publishFrozenMavenBundle({
  ...
  if (typeof deploymentScope !== 'string' || deploymentScope.length === 0) {
    throw error(
      'deploymentScope is required to distinguish product subsets from one publication lock',
    );
  }
  ...
  const published =
    validated.deploymentState === 'PUBLISHED'
      ? validated
