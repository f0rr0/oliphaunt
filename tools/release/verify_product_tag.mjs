...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (product !== null) {
      fail('usage: tools/release/verify_product_tag.mjs <product> [--target <commitish>] [--allow-missing]');
    }
    product = arg;
  }
  if (!product || !target) {
  ...
  return { allowMissing, product, target };
}
...
function refreshTagFromOrigin(tag) {
  ...
  if (advertised.exitCode === 2 || (advertised.exitCode === 0 && advertised.stdout.length === 0)) {
    return null;
  }
  if (advertised.exitCode !== 0) {
    fail(`could not inspect ${tag} on origin before verification${advertised.stderr ? `: ${advertised.stderr}` : ''}`);
  }
  ...
  if (commit === null) {
    fail(`${tag} was advertised by origin but could not be resolved to a commit`);
  }
  return commit;
}
...
const { allowMissing, product, target } = parseArgs(Bun.argv.slice(2));
const version = await currentProductVersion(product);
const tag = `${product}-v${version}`;
const targetCommit = commitForRef(target);
const existing = refreshTagFromOrigin(tag);
if (existing === null) {
  if (allowMissing) {
    console.log(`${tag} is absent and available for exact-SHA release creation`);
    process.exit(0);
  }
  fail(`${tag} does not exist. Stage the exact-SHA draft release before registry publish steps.`);
}
if (existing !== targetCommit) {
  fail(`${tag} points at ${existing}, not exact release commit ${targetCommit}`);
} else {
