...
async function acquireLock(file, { now, sleep }) {
  ...
  while (true) {
    ...
    try {
      ...
      if (cause?.code !== 'EEXIST')
        fail('could not acquire the core-request journal lock', { cause });
      if (now() - startedAt >= MAX_LOCK_WAIT_MS)
        fail('timed out waiting for the core-request journal lock');
      await sleep(100);
    }
  }
}
...
export async function reserveGitHubCoreRequest({
  ...
  mkdirSync(path.dirname(file), { recursive: true });
  const lock = await acquireLock(file, { now, sleep });
  try {
...
async function main(argv) {
  if (argv[0] === 'reserve' && argv.length === 3 && argv[1] === '--label') {
    const result = await reserveGitHubCoreRequest({ label: argv[2] });
    if (!result.enabled) fail('the core-request journal is not enabled');
    console.log(
      `reserved GitHub core request ${result.sequence} (${result.rollingCount} in the safety window)`,
    );
    return;
  }
  ...
  fail('usage: github-core-request-journal.mts <reserve --label LABEL|status>');
}
