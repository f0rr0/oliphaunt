...
test('runtime owners expose their complete contrib registry inventory', async () => {
  ...
  const contribPackages = async (owner) =>
    (await productRegistryPackages(owner)).filter(({ name }) => name.includes('contrib-pg18'));
...
test('publication-lock inventory includes dynamic Cargo payload-part carriers', () => {
  ...
  expect(
    productRegistryPackagesFromLock(publicationLock, product, {
      registryKind: 'crates',
    }).map(({ name }) => name),
  ).toEqual(['fixture-native-linux-x64-gnu-part-001', 'fixture-native-linux-x64-gnu']);
});
