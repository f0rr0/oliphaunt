...
const CACHE = path.join(ROOT, 'src/runtimes/liboliphaunt/native/bin/postgis-dependency-cache.sh');
...
describe('PostGIS native dependency cache', () => {
  ...
  test('fails closed without deleting data for an invalid fingerprint or root', () => {
    ...
    const invalidFingerprint = cacheOperation('prepare', dependencyRoot, 'not-a-hash', [buildRoot]);
