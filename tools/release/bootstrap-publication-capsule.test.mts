...
function cargoFixture(stageRoot, artifacts, name, version) {
  ...
  writeFileSync(
    ...
    `pub const NAME: &str = ${JSON.stringify(name)};\n`,
  );
...
function fixture() {
  ...
  const catalog = loadPublicationCatalog('bootstrap-publication-capsule.test', {
    products: PRODUCTS,
  });
...
function mutateTarPayload(source, destination, member, replacement) {
  ...
  for (;;) {
    ...
    const prefix = header
      .subarray(345, prefixNul === -1 || prefixNul > 500 ? 500 : prefixNul)
      .toString('utf8');
    ...
    if (fullName === member) {
      ...
      assert.equal(
        changed.length,
        payload.length,
        'payload mutation must preserve tar member size',
      );
...
test('packs every locked publication file deterministically and atomically installs it', () => {
  ...
  try {
    ...
    assert.equal(
      lstatSync(path.join(output, ...BOOTSTRAP_CAPSULE_MANIFEST_PATH.split('/'))).isFile(),
      true,
    );
    for (const source of [...value.cargo, value.npm]) {
      const artifact = manifest.files.find((file) => file.sha256 === sha256(source));
      assert.ok(artifact);
      assert.equal(sha256(path.join(output, ...artifact.path.split('/'))), artifact.sha256);
    }
  } finally {
