...
function member(product, sqlName) {
  ...
  return {
    ...
    iosNativeDependencies:
      nativeModuleStem === null
        ? []
        : (staticRows.get(sqlName)?.['ios-static-dependencies'] ?? '')
            .split(',')
            .filter(Boolean)
            .sort(),
    iosRegistration:
...
function manifest(product, version = '1.2.3') {
  ...
  const members = extensionSqlNames(product, 'verify-extension-release-identity.test').map(
    (sqlName) => member(product, sqlName),
  );
  const root = {
    product,
...
describe('canonical extension release identity', () => {
  ...
  test('rejects forged source, versioning, and semantic member metadata', () => {
    ...
    expect(() => assertCanonicalExtensionReleaseIdentity(product, value.version, value)).toThrow(
      /sourceIdentity differs/u,
    );

    const forgedVersioning = manifest(product);
    forgedVersioning.versioning = 'independent';
    expect(() =>
      assertCanonicalExtensionReleaseIdentity(product, forgedVersioning.version, forgedVersioning),
    ).toThrow(/versioning differs/u);

    const forgedInventory = manifest(product);
    forgedInventory.extensions[0].dataFiles = [
      ...forgedInventory.extensions[0].dataFiles,
      'undeclared/foreign.sql',
    ].sort();
    expect(() =>
      assertCanonicalExtensionReleaseIdentity(product, forgedInventory.version, forgedInventory),
    ).toThrow(/dataFiles differs/u);

    const unsortedInventory = manifest('oliphaunt-extension-postgis');
    unsortedInventory.extensionSqlFilePrefixes.reverse();
    expect(() =>
      assertCanonicalExtensionReleaseIdentity(
