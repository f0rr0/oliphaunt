...
const EXTENSION_EVIDENCE_PATHS = [
  path.join(ROOT, "src/extensions/evidence/matrix.toml"),
  path.join(ROOT, "src/extensions/evidence/runs/2026-06-07-transitional-catalog-smoke.json"),
  path.join(ROOT, "src/extensions/generated/docs/extension-evidence.json"),
];
const EXTENSION_EVIDENCE_STALE_RE =
  /([^:\n]+\.json) sourceDigest is stale; expected (sha256:[0-9a-f]{64}), got '([^']*)'/gu;
...
function cargoManifestPaths() {
  const ignoredRoots = new Set([".git", "target", "node_modules"]);
  const manifests = [];
  function walk(directory) {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      const file = path.join(directory, entry.name);
      const relativeParts = rel(file).split("/");
      if (relativeParts.some((part) => ignoredRoots.has(part))) {
        continue;
      }
...
function syncExtensionEvidence(changes, { write }) {
  ...
  const output = commandOutputForError(result);
  if (result.status !== 0) {
    const stale = [...output.matchAll(EXTENSION_EVIDENCE_STALE_RE)];
  ...
  for (const file of EXTENSION_EVIDENCE_PATHS) {
    if (before[file] !== readOptionalText(file)) {
      changes.push({ path: file, detail: "regenerated extension evidence" });
    }
  }
}
...
async function main(argv) {
  ...
  syncExtensionEvidence(changes, { write });
