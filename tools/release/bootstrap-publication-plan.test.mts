...
} from './bootstrap-publication-plan.mts';
...
describe('registry identity bootstrap publication plan', () => {
  test('uses frozen carrier order across product boundaries', () => {
    ...
    expect(bootstrapPublicationSchedule(plan, [])).toEqual([[], [0], []]);
    expect(() => bootstrapPublicationSchedule(plan.slice(1), [])).toThrow('unsatisfied dependency');
    expect(bootstrapPublicationSchedule(plan.slice(1), [plan[0].id])).toEqual([[], []]);
  });
  ...
  test('rejects a frozen order that precedes an internal dependency', () => {
    ...
    ).toThrow(/appears before bootstrap dependency/u);
  });
  ...
  test('rejects omitted and unknown locked Cargo/npm dependencies', () => {
    ...
    expect(() => bootstrapPublicationPlan(omitted, ['sdk'])).toThrow(
      /selection omits locked bootstrap dependency cargo:runtime/u,
    );
    ...
    expect(() => bootstrapPublicationPlan(unknown, ['sdk'])).toThrow(
      /refers to unknown locked dependency npm:@example\/missing/u,
    );
  });
  ...
  test('the complete real catalog closes over every Cargo/npm bootstrap carrier', () => {
    ...
    const splitParent = catalog.carriers.find(
      ({ ecosystem, role }) => ecosystem === 'cargo' && role === 'platform-leaf',
    );
    ...
    const expected = frozen.carriers.filter(
      ({ ecosystem }) => ecosystem === 'cargo' || ecosystem === 'npm',
    );
