...
function validateExecution(plan, execution) {
  ...
  const completedRows = plan.waves.slice(0, execution.completedWaves).flatMap(({ rows }) => rows);
  if (
...
export function githubReleaseAssetUploadEnvironment(parentEnvironment, { abortPath, windowMs }) {
  if (
    ...
    throw error(
      'upload environment requires a parent environment, abort path, and positive window',
      null,
    );
  }
  return {
