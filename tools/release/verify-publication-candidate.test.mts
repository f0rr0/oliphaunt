...
test('accepts an exact same-SHA rerun after its product tag exists', () => {
  ...
  assert.deepEqual(verifyPublicationCandidate({ repo, headRef: release, products: [PRODUCT] }), {
    mode: 'release-bump',
    publicationSha: release,
    releaseSha: release,
    products: [PRODUCT],
...
test('accepts a later release-control fix without changing the pending release', () => {
  ...
  assert.deepEqual(verifyPublicationCandidate({ repo, headRef: controller, products: [PRODUCT] }), {
    mode: 'release-bump',
