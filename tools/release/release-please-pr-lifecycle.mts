...
const TOOL = 'release-please-pr-lifecycle.mts';
...
function lifecycleBudget({ environment, now = Date.now, windowMs }) {
...
function requireBase(base) {
  if (base !== 'main') {
    throw lifecycleError(
      `the Release Please lifecycle base must be main, got ${JSON.stringify(base)}`,
    );
  }
  return base;
}
...
export function mergedPendingReleasePullRequests(pullRequests, { base = 'main', repository } = {}) {
  requireBase(base);
...
export async function assertCleanReleasePleaseState({
  base = 'main',
  ...
  const list =
    ...
    (async () => {
      ...
      return await requestGithubPages(`repos/${repository}/pulls?${query.toString()}`, {
        attemptTimeoutMs: LIFECYCLE_READ_ATTEMPT_TIMEOUT_MS,
        baseDelayMs: 500,
  ...
  const blockers = mergedPendingReleasePullRequests(await list(), { base, repository });
...
function exactReleasePullRequestIdentity(
  pullRequest,
  { base, number, releaseSha, repository } = {},
) {
...
export function selectExactReleasePullRequest(
  pullRequests,
  { base = 'main', releaseSha, repository } = {},
) {
  requireBase(base);
...
async function resolveExactReleasePleasePullRequest({
  base,
  budget,
  environment,
  ...
  const list =
    listAssociatedPullRequests ??
    (async () =>
      await requestGithubPages(`repos/${repository}/commits/${releaseSha}/pulls`, {
        ...lifecycleReadOptions(budget),
        itemsField: null,
        label: `pull requests associated with release ${releaseSha}`,
      }));
  const selected = selectExactReleasePullRequest(await list(), {
    base,
    releaseSha,
    repository,
  });
  ...
  const read =
    readPullRequest ??
    (async () =>
      await requestGithubRepositoryJson(`repos/${repository}/pulls/${selected.number}`, {
        ...lifecycleReadOptions(budget),
  ...
  const readLabel =
    readTaggedLabel ??
    (async () =>
      await requestGithubRepositoryJson(
        `repos/${repository}/labels/${encodeURIComponent(TAGGED_LABEL)}`,
        {
          ...lifecycleReadOptions(budget),
          label: `repository label ${TAGGED_LABEL}`,
        },
      ));
  assertTaggedRepositoryLabel(await readLabel());
  return {
    current,
    expectedIdentity,
...
export async function assertExactReleasePleasePullRequestMarkable({
  base = 'main',
  environment = process.env,
  listAssociatedPullRequests,
  now = Date.now,
  ...
  const resolved = await resolveExactReleasePleasePullRequest({
    base,
    budget,
...
export async function markExactReleasePleasePullRequestTagged({
  addTaggedLabel,
  base = 'main',
  environment = process.env,
  listAssociatedPullRequests,
  ...
  const reconciliation = {
    ...
    maxAttempts: Math.min(
      reconciliationOptions.maxAttempts ?? RELEASE_PLEASE_MARK_TAGGED_MAX_MUTATION_ATTEMPTS,
      RELEASE_PLEASE_MARK_TAGGED_MAX_MUTATION_ATTEMPTS,
    ),
  };
  const taggedResult = await reconcileGitHubMutation({
    inspect: async () => ensureTaggedState(await resolved.read(), resolved.expectedIdentity),
    label: `add ${TAGGED_LABEL} to Release Please PR #${resolved.number}`,
    mutate: async ({ deadlineMs, timeoutMs }) => {
      const input = `${JSON.stringify({ labels: [TAGGED_LABEL] })}\n`;
      if (addTaggedLabel !== undefined) {
        await addTaggedLabel({ deadlineMs, input, number: resolved.number, timeoutMs });
        return;
      }
      await requestGithubMutation(`repos/${resolved.repository}/issues/${resolved.number}/labels`, {
        method: 'POST',
        deadlineMs,
        environment: operationEnvironment,
        input,
  ...
  const pendingResult = await reconcileGitHubMutation({
    inspect: async () =>
      ensurePendingRemovedState(await resolved.read(), resolved.expectedIdentity),
    label: `remove ${PENDING_LABEL} from Release Please PR #${resolved.number}`,
    mutate: async ({ deadlineMs, timeoutMs }) => {
      if (removePendingLabel !== undefined) {
        await removePendingLabel({ deadlineMs, number: resolved.number, timeoutMs });
        return;
      }
...
function usage() {
  return (
    `${TOOL}: usage: ${TOOL} assert-clean --base main | ` +
...
async function main() {
  ...
  if (options.operation === 'assert-clean') {
    const result = await assertCleanReleasePleaseState(options);
    console.log(`Release Please lifecycle is clear for ${result.repository}:${result.base}`);
    return;
  }
  if (options.operation === 'assert-markable') {
    const result = await assertExactReleasePleasePullRequestMarkable(options);
    console.log(
  ...
  const result = await markExactReleasePleasePullRequestTagged(options);
  console.log(
