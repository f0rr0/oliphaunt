...
describe('registry publication deferral child boundary', () => {
  ...
  test('rejects lookalikes, duplicate markers, noncanonical bytes, extra keys, and schemas', () => {
    ...
    expect(() => decodeRegistryPublicationDeferral('ordinary failure only')).toThrow(
      /exactly one/u,
    );

    const valid = encodeRegistryPublicationDeferral(
      new RegistryPublicationDeferredError({
        reason: 'deadline',
        notBeforeEpochSeconds: 1_800_000_000,
    ...
    expect(() =>
      decodeRegistryPublicationDeferral(`${REGISTRY_PUBLICATION_DEFERRAL_PREFIX}***\n`),
    ).toThrow(/exactly one/u);
    ...
    for (const record of [
      ...
      expect(() => decodeRegistryPublicationDeferral(line)).toThrow(
        /keys must be exactly|schema must be/u,
      );
    }
  });
