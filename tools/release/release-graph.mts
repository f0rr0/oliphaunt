...
export function parseStableVersion(version, prefix = 'release-graph') {
  ...
  if (!match) {
    fail(
      prefix,
      `release version must be stable x.y.z for automated publish, got ${JSON.stringify(version)}`,
    );
  }
...
export function parseMoonProjects(result, prefix = 'release-graph') {
  if (!Array.isArray(result?.projects) || result.projects.length === 0) {
    fail(prefix, 'Moon snapshot must contain resolved projects');
  }
  return new Map(
    result.projects.map(({ id, source, dependencies = [], config }) => [
      id,
      {
        id,
        source,
        dependencies: [...dependencies].sort((left, right) => compareText(left.id, right.id)),
        config: { tags: [...(config.tags ?? [])].sort(compareText), project: config.project ?? {} },
      },
    ]),
  );
}
...
export function moonProjectsById(prefix = 'release-graph') {
  const file = process.env.OLIPHAUNT_MOON_PROJECTS_FILE;
  if (!file)
    fail(
      prefix,
      'resolved Moon projects are required; use the release-plan.sh or sync-release-pr.sh entry point',
    );
  return parseMoonProjects(JSON.parse(readFileSync(file, 'utf8')), prefix);
}
  ...
  for (const project of projects.values()) {
    ...
    if (!config.tags.includes('release-product')) {
      if (release !== undefined) {
        fail(
          prefix,
          `Moon project ${project.id} declares release metadata but is not tagged release-product`,
        );
      }
      continue;
    }
    ...
    if (release.component !== project.id) {
      fail(
        prefix,
        `Moon release product ${project.id} release.component must match the project id`,
      );
    }
...
export function loadProducts(prefix = 'release-graph') {
  const { byComponent } = releasePleasePackagesByComponent(prefix);
  const paths = new Map(
    [...byComponent].map(([component, { packagePath }]) => [component, packagePath]),
  );
  const manifest = readJson('.release-please-manifest.json', prefix);
  const products = {};
  for (const [product, packagePath] of [...paths.entries()].sort(([left], [right]) =>
...
export function wasixEvidenceProductsForRelease(
  ...
  const unknown = [...selectedProducts]
    .filter((product) => !(product in products))
    .sort(compareText);
  if (unknown.length > 0) {
...
export function compatibilityVersionEntries(
  ...
  const source = products ?? loadProducts(prefix);
  const knownProducts = new Set(Object.keys(source));
  const entries = [];
  for (const [product, config] of Object.entries(source).sort(([left], [right]) =>
    compareText(left, right),
  )) {
    const rawSpecs = config.compatibility_versions ?? {};
    assertObject(rawSpecs, `${product}.compatibility_versions`, prefix);
    for (const [specId, spec] of Object.entries(rawSpecs).sort(([left], [right]) =>
      compareText(left, right),
    )) {
      if (!specId) {
      ...
      if (requireSourceProduct) {
        if (typeof sourceProduct !== 'string' || sourceProduct.length === 0) {
          fail(
            prefix,
            `${product}.compatibility_versions.${specId}.source_product must be a non-empty string`,
          );
        }
        ...
        fail(
          prefix,
          `${product}.compatibility_versions.${specId}.source_product must be a string when present`,
        );
      }
      ...
      if (typeof parser !== 'string' || parser.length === 0) {
        fail(
          prefix,
          `${product}.compatibility_versions.${specId}.parser must be a non-empty string`,
        );
      }
      if (!existsSync(path.join(root, specPath))) {
        fail(
          prefix,
          `${product}.compatibility_versions.${specId} path does not exist: ${specPath}`,
        );
      }
      entries.push({
        id: specId,
...
export function compatibilityVersionValue(
  ...
  if (entry.parser.startsWith('rust-const:')) {
    ...
    const matches = [
      ...text.matchAll(
        new RegExp(`(?:pub\\s+)?const\\s+${name}\\s*:[^=]+?=\\s*"([^"]+)"\\s*;`, 'gu'),
      ),
    ];
    if (matches.length !== 1) {
...
export function latestProductTag(productConfig, headRef, prefix = 'release-graph', root = ROOT) {
  for (const candidatePrefix of tagPrefixes(productConfig, prefix)) {
    const tag = historyLatestTag(root, candidatePrefix, headRef);
    if (tag) {
      return tag;
    }
  }
...
const commitForRefAt = historyCommit;
...
export function changedFilesFromRefs(baseRef, headRef, prefix = 'release-graph', root = ROOT) {
  try {
    return historyChanges(root, baseRef, headRef).sort(compareText);
  } catch (error) {
    fail(
      prefix,
      `failed to read changed files between ${baseRef} and ${headRef}: ${error.message}`,
    );
  }
}
...
function manifestAtRef(root, ref, prefix) {
  let value;
  try {
    value = historyManifest(root, ref);
  } catch (error) {
    throw new Error(
      `${prefix}: cannot read .release-please-manifest.json at ${ref}: ${error.message}`,
    );
  }
  if (value === null || Array.isArray(value) || typeof value !== 'object') {
    throw new Error(
      `${prefix}: .release-please-manifest.json at ${ref} must contain a JSON object`,
    );
  }
  return value;
}
...
function canonicalVersionAtRef(root, ref, product, config, prefix) {
  ...
  let text;
  try {
    text = historyFile(root, ref, file);
    if (text === null) throw new Error('missing historical file');
  } catch (error) {
    throw new Error(
      `${prefix}: cannot read ${product} canonical version file ${file} at ${ref}: ${error.message}`,
    );
  }
  ...
  try {
    ...
    throw new Error(
      `${prefix}: cannot parse ${product} canonical version file ${file} at ${ref}: ${error.message}`,
    );
  }
...
function transitionVersion(value, context, prefix) {
  if (
    ...
    throw new Error(
      `${prefix}: ${context} must be a stable x.y.z version, got ${JSON.stringify(value)}`,
    );
  }
...
function manifestProductVersion(products, manifest, product, ref, prefix) {
  ...
  if (typeof packagePath !== 'string' || packagePath.length === 0) {
    throw new Error(
      `${prefix}: compatibility source product ${product} is missing its Release Please package path`,
    );
  }
  const version = manifest[packagePath];
...
function semanticDifferences(before, after, parts = []) {
  ...
  if (beforeObject && afterObject && Array.isArray(before) === Array.isArray(after)) {
    ...
    return [...keys].flatMap((key) =>
      semanticDifferences(before[key], after[key], [...parts, key]),
    );
  }
...
function compatibilityParser(entry) {
  ...
  if (
    (type === 'json' || type === 'toml') &&
    /^[A-Za-z0-9_-]+(?:[.][A-Za-z0-9_-]+)*$/u.test(expression)
  ) {
...
function compatibilityFileHasOnlyExpectedChanges({
  ...
  if (type === 'raw') {
    return (
      rules.length === 1 &&
      beforeText.trim() === rules[0].before &&
      afterText.trim() === rules[0].after
    );
  }
  if (type === 'rust-const') {
  ...
  for (const rule of rules) {
    ...
    if (
      previous !== undefined &&
      (previous.before !== rule.before ||
        previous.after !== rule.after ||
        previous.entry.sourceProduct !== rule.entry.sourceProduct)
    ) {
      return false;
    }
    if (
  ...
  return semanticDifferences(before, after).every((difference) => {
    ...
    return (
      rule !== undefined && difference.before === rule.before && difference.after === rule.after
    );
  });
}
...
export function productVersionTransitionStatus(
  ...
  const headParts = transitionVersion(
    headVersion,
    `${product} manifest version at ${headCommit}`,
    prefix,
  );
  ...
  if (baseRef !== EMPTY_TREE) {
    ...
    comparison = compareVersion(
      headParts,
      transitionVersion(baseVersion, `${product} base tag version`, prefix),
    );
    if (comparison < 0) {
      throw new Error(
  ...
  if (currentTagCommit !== null) {
    ...
    const taggedCanonicalVersion = canonicalVersionAtRef(
      root,
      currentTagCommit,
      product,
      config,
    ...
    const ancestor = historyAncestor(root, currentTagCommit, headCommit);
    if (!ancestor) {
      throw new Error(
  ...
  const matches = Object.values(projects)
    .filter(
      (project) =>
        ...
        candidate.startsWith(`${project.source}/`),
    )
    .sort((left, right) => right.source.length - left.source.length);
  return matches[0]?.id;
}

export function releaseOwnerProjectsForPath(
  products,
  projects,
  candidate,
  prefix = 'release-graph',
...
export function releaseProductProjectId(product, products, projects, prefix = 'release-graph') {
  ...
  const matches = Object.values(projects)
    .filter(
      (project) => packagePath === project.source || packagePath.startsWith(`${project.source}/`),
    )
    .sort((left, right) => right.source.length - left.source.length);
  if (matches.length === 0) {
...
export function releaseProductsForProjects(
  products,
  projects,
  projectIds,
  prefix = 'release-graph',
...
export function releaseOrder(products, _projects, selected, prefix = 'release-graph') {
  ...
  while (remaining.size > 0) {
    ...
    if (ready.length === 0) {
      fail(
        prefix,
        `release compatibility graph has a dependency cycle: ${JSON.stringify([...remaining].sort(compareText))}`,
      );
    }
...
export function buildPlan(graph, files, prefix = 'release-graph') {
  ...
  for (const file of files) {
    const sharedImpacts = (graph.shared_release_sources ?? []).filter(
      (impact) =>
        (impact.files ?? []).includes(file) ||
        (impact.source_paths ?? []).some(
...
export function buildPlanFromProductTags(
  ...
  for (const [product, config] of Object.entries(products)) {
    ...
    const productFiles =
      transition.eligible || baseRef !== EMPTY_TREE
    ...
    if (!transition.eligible) {
      if (baseRef !== EMPTY_TREE && productFiles.length > 0) {
        ...
        if (impactPlan.releaseProducts.includes(product)) {
          ...
          const suffix =
            relevantFiles.length > shown.length
              ? `, ... (${relevantFiles.length - shown.length} more)`
              : '';
          throw new Error(
...
if (import.meta.main) {
  const [mode, root, graphFile] = process.argv.slice(2);
  if (mode !== '--history-inputs')
    throw new Error('usage: release-graph.mts --history-inputs REPO GRAPH');
  const directory = process.env.OLIPHAUNT_PRODUCT_HISTORY;
  if (!directory) throw new Error('missing product history destination');

  const products =
    graphFile === '@workspace'
      ? loadProducts('release-history')
      : JSON.parse(readFileSync(graphFile, 'utf8')).products;
  const prefixes = new Set(Object.values(products).flatMap((config) => tagPrefixes(config)));
  const files = new Set([
    'release-please-config.json',
    '.release-please-manifest.json',
    ...Object.values(products).flatMap((config) => config.version_files ?? []),
    ...compatibilityVersionEntries(products, { root, requireSourceProduct: true }).map(
      (entry) => entry.path,
    ),
  ]);
  const currentConfig = path.join(root, 'release-please-config.json');
  writeFileSync(
    path.join(directory, 'current-config'),
    existsSync(currentConfig) ? readFileSync(currentConfig, 'utf8') : '{}',
  );
