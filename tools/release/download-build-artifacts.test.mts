...
const SCRIPT = path.resolve('.github/scripts/download-build-artifacts.sh');
const FETCH_FIXTURE = path.resolve('tools/test/download-build-artifacts-github-fixture.mts');
const CHECKSUM_MERGER = path.resolve('.github/scripts/merge-checksum-manifest.mts');
...
test('the workflow Node runtime reaches argument validation without a Bun global', () => {
  ...
  assert.match(result.stderr, /usage: download-build-artifacts[.]sh/u);
...
function invoke(f, mode, extra = [], environment = {}) {
  return spawnSync(
    ...
    ],
    {
      ...
      env: isolatedGitHubTestEnvironment({
        NODE_OPTIONS: `--import=${FETCH_FIXTURE}`,
        TMPDIR: f.root,
...
function temporarySiblings(f) {
  return readdirSync(f.root).filter(
...
test('generic per-target checksum manifests merge deterministically and reject conflicts', (t) => {
  ...
  const merged = spawnSync('node', [CHECKSUM_MERGER, linux, windows], {
    encoding: 'utf8',
  });
  assert.equal(merged.status, 0, merged.stderr);
  assert.equal(
    readFileSync(linux, 'utf8'),
    [
      `${linuxDigest}  ./liboliphaunt-1.2.3-linux-x64-gnu.tar.gz`,
      `${windowsDigest}  ./liboliphaunt-1.2.3-windows-x64-msvc.zip`,
      '',
  ...
  const rejected = spawnSync('node', [CHECKSUM_MERGER, linux, conflict], {
    encoding: 'utf8',
  });
  assert.equal(rejected.status, 1);
  assert.match(
    rejected.stderr,
    /conflicting checksum for liboliphaunt-1[.]2[.]3-linux-x64-gnu[.]tar[.]gz/u,
  );
...
test('different bytes at an existing path fail closed without changing the destination', (t) => {
  ...
  assert.match(
    result.stderr,
    /overwrite payload[.]txt with different bytes|conflicts with the durable destination/u,
  );
...
test('approved artifact metadata pins exact id, compressed size, and digest', (t) => {
  ...
  const rejected = invoke(f, 'success', [
    '--artifact-metadata-json',
    JSON.stringify([{ ...metadata[0], id: 999 }]),
  ]);
  assert.equal(rejected.status, 1);
...
test('one immutable run snapshot is reused while each requested artifact is downloaded by exact id', (t) => {
  ...
  assert.equal(
    endpoints.filter((endpoint) => /actions\/runs\/77\/artifacts/u.test(endpoint)).length,
    1,
  );
  assert.equal(
...
test('fallback discovery traverses exact-SHA workflow pages instead of truncating the latest runs', (t) => {
  ...
  const calls = readFileSync(f.log, 'utf8').trim().split(/\r?\n/u).map(JSON.parse);
  assert.equal(
    calls.filter((endpoint) => endpoint.includes('/actions/workflows/9/runs?')).length,
    2,
  );
});
...
test('Shell extraction rejects unsafe paths and corrupt contents before changing durable files', (t) => {
  const f = fixture(t, 'unsafe-zip');
  mkdirSync(f.destination);
  writeFileSync(path.join(f.destination, 'existing.txt'), 'preserve');
  for (const row of [
    { name: '../../escaped.txt', data: 'bad' },
    { name: 'payload.txt', data: 'bad', crc: 1 },
  ]) {
    writeFileSync(f.archive, zipArchive([row]));
    rmSync(f.snapshots, { recursive: true, force: true });
    const result = invoke(f, 'success');
    assert.notEqual(result.status, 0);
    assert.equal(readFileSync(path.join(f.destination, 'existing.txt'), 'utf8'), 'preserve');
    assert.equal(existsSync(path.join(f.root, 'escaped.txt')), false);
    assert.deepEqual(temporarySiblings(f), []);
  }
});
