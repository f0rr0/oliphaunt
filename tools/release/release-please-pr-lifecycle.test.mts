...
} from './release-please-pr-lifecycle.mts';
...
test('merged pending preflight uses exact current REST state', async () => {
  ...
  await assert.rejects(
    async () =>
      await assertCleanReleasePleaseState({
        base: 'main',
...
test('preflight ignores unmerged, other-base, cross-repository, and cleared PRs', async () => {
  const cases = [
...
test('lifecycle state preserves unrelated labels and converges to tagged only', () => {
  ...
  assert.deepEqual(
    releasePleaseLabelState(pullRequest({ labels: labels('reviewed', 'autorelease: tagged') }), {
      base: 'main',
      releaseSha: RELEASE_SHA,
      repository: REPOSITORY,
    }),
  ...
  assert.equal(
    releasePleaseLabelState(pullRequest({ labels: labels('reviewed') }), {
      base: 'main',
      releaseSha: RELEASE_SHA,
      repository: REPOSITORY,
    }).kind,
...
test('assert-markable proves exact identity, current lifecycle state, and tagged-label existence', async () => {
  const result = await assertExactReleasePleasePullRequestMarkable({
    environment: ENVIRONMENT,
  ...
  await assert.rejects(
    async () =>
      await assertExactReleasePleasePullRequestMarkable({
        environment: ENVIRONMENT,
        listAssociatedPullRequests: () => [pullRequest()],
        now: () => 0,
        readPullRequest: () => pullRequest(),
...
test('mark-tagged adds then removes labels without replacing concurrent unrelated labels', async () => {
  let current = pullRequest({ labels: labels('reviewed', 'autorelease: pending') });
  const mutations = [];
  const result = await markExactReleasePleasePullRequestTagged({
  ...
  const rerun = await markExactReleasePleasePullRequestTagged({
...
test('ambiguous mutation success is recovered by exact fresh state', async () => {
...
test('lifecycle mutation retries are capped at the exact admission bound', async () => {
  let current = pullRequest();
  let addCalls = 0;
  let removeCalls = 0;
  const result = await markExactReleasePleasePullRequestTagged({
...
test('missing lifecycle labels and changed exact identity fail closed', async () => {
  for (const current of [
    ...
    await assert.rejects(
      async () =>
        await markExactReleasePleasePullRequestTagged({
