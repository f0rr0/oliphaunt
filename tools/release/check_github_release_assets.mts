...
function fail(message) {
  console.error(`check_github_release_assets.mts: ${message}`);
  process.exit(1);
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === '--asset') {
      ...
      console.log(
        'usage: tools/release/check_github_release_assets.mts <product> [--version VERSION] [--default-assets] [--asset NAME...]',
      );
      process.exit(0);
