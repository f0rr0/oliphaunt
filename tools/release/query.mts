...
function output(rows, format, field) {
  if (format === 'lines') {
    for (const value of new Set(rows.map((row) => row[field]))) console.log(value);
  } else if (format === 'json') {
    printJson(rows);
  } else {
...
function orderedReleaseProducts(raw) {
  ...
  return releaseOrder(products, undefined, selected, TOOL);
}
...
function runCiArtifactNames(argv) {
  ...
  if (values.family === 'release-assets') {
    ...
    if (rows.length !== 1) fail(`${values.product} does not declare a source package artifact`);
  } else {
...
function runCiProducts(argv) {
  ...
  if (
    ...
    (carrierFamily !== undefined || values.field !== 'product')
  ) {
  ...
  const selectedReleaseProducts =
    values['products-json'] === undefined
      ? undefined
      : orderedReleaseProducts(values['products-json']);
  const products =
    selectedReleaseProducts === undefined
  ...
  if (values.field === 'product') {
    output(
      products.map((product) => rowsByProduct.get(product)),
      values.format,
      'product',
    );
    return;
  ...
  const selectedSet =
    selectedReleaseProducts === undefined ? null : new Set(selectedReleaseProducts);
  const roots = products.flatMap((product) => {
    const families = carrierFamily === undefined ? ['native', 'wasix'] : [carrierFamily];
    return families
      .filter(
        (family) =>
          selectedSet === null || selectedSet.has(extensionReleaseProduct(product, family, TOOL)),
      )
      .map((family) =>
        extensionArtifactProductRoot(product, family, 'target/extension-artifacts', TOOL),
      );
  });
...
function runExtensionArtifactRoot(argv) {
  ...
  console.log(
    extensionArtifactProductRoot(values.product, values.family, 'target/extension-artifacts', TOOL),
  );
}
...
function usage() {
  return `usage: tools/release/query.mts <command> [options]
