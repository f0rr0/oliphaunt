...
$RepositoryExtensionMaterializerRelativePath =
    "tools/release/extension-registry-carrier-materializer.mjs"
$RepositoryExtensionDiscoveryProofRelativePath =
    "tools/release/extension-manifest-discovery-proof.mjs"
$ExtensionManifestDiscoveryProofArgument = "--windows-standard-user-discovery-proof"
$ExtensionManifestDiscoveryProof =
...
$RepositoryImmutableInputRelativePaths = @(
...
function Test-PathsOverlap([string]$Left, [string]$Right) {
    return (
        ...
        (Test-PathInside $Left $Right) -or
        (Test-PathInside $Right $Left)
    )
}

function Get-ImmutableInputFiles([string]$Root, [string]$Label) {
    $resolvedRoot = Resolve-Directory $Root $Label
    $reparsePoint = [System.IO.FileAttributes]::ReparsePoint
    if (([System.IO.File]::GetAttributes($resolvedRoot) -band $reparsePoint) -ne 0) {
        Fail "$Label root must not be a reparse point"
    }
    ...
    while ($directories.Count -gt 0) {
        ...
        foreach ($entry in [System.IO.Directory]::EnumerateFileSystemEntries($directory)) {
            $entries.Add([System.IO.Path]::GetFullPath($entry)) | Out-Null
        }
        $entries.Sort([System.StringComparer]::Ordinal)
        foreach ($entry in $entries) {
            $attributes = [System.IO.File]::GetAttributes($entry)
            if (($attributes -band $reparsePoint) -ne 0) {
                Fail "$Label contains a reparse point: $entry"
            }
...
function Read-And-ValidateProof(
    ...
    if (
        ...
        $proof.repositoryAccess.immutableInputRootsBound -isnot [bool] -or
        $proof.repositoryAccess.immutableInputRootsBound -ne $true -or
        [int]$proof.repositoryAccess.immutableInputRootCount -ne
            @($Manifest.immutableInputRoots).Count -or
        [int]$proof.repositoryAccess.immutableInputFileCount -lt
            @($Manifest.immutableInputRoots).Count -or
        $proof.repositoryAccess.immutableInputCreateDenied -isnot [bool] -or
        $proof.repositoryAccess.immutableInputCreateDenied -ne $true -or
        $proof.repositoryAccess.immutableInputFileWriteDenied -isnot [bool] -or
        $proof.repositoryAccess.immutableInputFileWriteDenied -ne $true -or
        $proof.repositoryAccess.immutableInputFileAppendDenied -isnot [bool] -or
        $proof.repositoryAccess.immutableInputFileAppendDenied -ne $true -or
        $proof.repositoryAccess.extensionManifestDiscoveryVerified -isnot [bool] -or
    ...
    if ($Manifest.operation -eq "self-test") {
        if (
            $proof.repositoryAccess.extensionManifestDiscoveryVerified -ne $true -or
            $proof.repositoryAccess.extensionManifestPath -cne
                $Manifest.extensionManifestDiscovery.expectedManifest -or
            $proof.repositoryAccess.extensionManifestSha256 -cne
                $Manifest.extensionManifestDiscovery.sha256 -or
            $proof.repositoryAccess.extensionMaterializerSha256 -cne
                $Manifest.extensionManifestDiscovery.materializerSha256 -or
            $proof.repositoryAccess.extensionDiscoveryProofToolSha256 -cne
                $Manifest.extensionManifestDiscovery.proofToolSha256 -or
            [int]$proof.repositoryAccess.immutableInputRootCount -ne 1 -or
            [int]$proof.repositoryAccess.immutableInputFileCount -ne 1
        ) {
            Fail "standard-user self-test proof is not bound to exact manifest discovery"
        }
    } elseif (
        $proof.repositoryAccess.extensionManifestDiscoveryVerified -ne $false -or
        $null -ne $proof.repositoryAccess.extensionManifestPath -or
        $null -ne $proof.repositoryAccess.extensionManifestSha256 -or
        $null -ne $proof.repositoryAccess.extensionMaterializerSha256 -or
        $null -ne $proof.repositoryAccess.extensionDiscoveryProofToolSha256
    ) {
        Fail "standard-user consumer proof contains self-test-only discovery evidence"
    }
    $manifestProbeNames = @(
...
function Invoke-ChildMode([string]$ManifestPath) {
    ...
    if (
        ...
        $manifest.immutableInputRoots -isnot [System.Array] -or
    ...
    if ($manifest.operation -eq "self-test") {
        if (
            $null -eq $manifest.extensionManifestDiscovery -or
            $manifest.extensionManifestDiscovery.expectedManifest -isnot [string] -or
            $manifest.extensionManifestDiscovery.sha256 -notmatch '^[0-9a-f]{64}$' -or
            $manifest.extensionManifestDiscovery.materializerSha256 -notmatch '^[0-9a-f]{64}$' -or
            $manifest.extensionManifestDiscovery.proofToolSha256 -notmatch '^[0-9a-f]{64}$'
        ) {
            Fail "self-test manifest has an invalid extension-discovery contract"
        }
    } elseif ($null -ne $manifest.extensionManifestDiscovery) {
    ...
    $expectedImmutableInputRoots = [System.Collections.Generic.List[string]]::new()
    if ($manifest.operation -eq "consumer") {
        foreach ($relativeInputRoot in $RepositoryImmutableInputRelativePaths) {
            $expectedImmutableInputRoots.Add(
                (Resolve-Directory `
                    (Join-Path $repository $relativeInputRoot) `
                    "expected immutable exact-candidate input root")
            ) | Out-Null
        }
    } else {
        $sandboxNonce = Split-Path -Leaf $sandbox
        if ($sandboxNonce -notmatch '^[0-9a-f]{16}$') {
            Fail "self-test sandbox does not carry the generated launcher nonce"
        }
        $expectedImmutableInputRoots.Add(
            (Resolve-Directory `
                (Join-Path $repository "target/windows-standard-user-input/$sandboxNonce") `
                "expected immutable self-test input root")
        ) | Out-Null
    }
    $manifestImmutableInputRoots = @($manifest.immutableInputRoots)
    if ($manifestImmutableInputRoots.Count -ne $expectedImmutableInputRoots.Count) {
        Fail "child manifest immutable-input-root count does not match the operation contract"
    }
    $seenImmutableInputRoots = [System.Collections.Generic.HashSet[string]]::new(
        [System.StringComparer]::OrdinalIgnoreCase
    )
    $immutableInputFileCount = 0
    for ($index = 0; $index -lt $expectedImmutableInputRoots.Count; $index += 1) {
        if ($manifestImmutableInputRoots[$index] -isnot [string]) {
            Fail "child manifest immutable input root is not a string"
        }
        $immutableInputRoot = Resolve-Directory `
            ([string]$manifestImmutableInputRoots[$index]) `
            "child immutable input root"
        if (
            -not [string]::Equals(
                $immutableInputRoot,
                $expectedImmutableInputRoots[$index],
                [System.StringComparison]::OrdinalIgnoreCase
            ) -or
            -not (Test-PathInside $repository $immutableInputRoot) -or
            (Test-PathsOverlap $immutableInputRoot $sandbox) -or
            (Test-PathsOverlap $immutableInputRoot $toolExecutionRoot)
        ) {
            Fail "child immutable input root escaped, overlapped child state, or was duplicated"
        }
        foreach ($previousImmutableInputRoot in $seenImmutableInputRoots) {
            if (Test-PathsOverlap $immutableInputRoot $previousImmutableInputRoot) {
                Fail "child immutable input roots overlap"
            }
        }
        if (-not $seenImmutableInputRoots.Add($immutableInputRoot)) {
            Fail "child manifest contains a duplicate immutable input root"
        }
        foreach ($writableRoot in $expectedWritableRoots) {
            if (Test-PathsOverlap $immutableInputRoot $writableRoot) {
                Fail "child immutable input root overlaps an authorized writable root"
            }
        }
        Assert-InheritedRepositoryMutationDeny `
            $immutableInputRoot `
            $identity.Sid `
            "immutable input root $index"
        Assert-DirectoryCreateDenied $immutableInputRoot "immutable input root $index"
        $immutableFiles = @(Get-ImmutableInputFiles `
            $immutableInputRoot `
            "immutable input root $index")
        if ($immutableFiles.Count -eq 0) {
            Fail "immutable input root $index contains no files to prove"
        }
        foreach ($immutableFile in $immutableFiles) {
            if (-not (Test-PathInside $immutableInputRoot $immutableFile)) {
                Fail "immutable input file escaped its declared root"
            }
            Assert-FileWriteOpenDenied $immutableFile "immutable input file"
            Assert-FileAppendOpenDenied $immutableFile "immutable input file"
            $immutableInputFileCount += 1
        }
    }
    ...
    $extensionManifestDiscoveryVerified = $false
    $extensionManifestDiscoveryPath = $null
    $extensionManifestDiscoverySha256 = $null
    if ($manifest.operation -eq "self-test") {
        $expectedDiscoveryRoot = $expectedImmutableInputRoots[0]
        $expectedDiscoveryManifest = Resolve-File `
            (Join-Path `
                $expectedDiscoveryRoot `
                $ExtensionManifestDiscoveryRelativePath) `
            "expected immutable self-test extension manifest"
        $declaredDiscoveryManifest = Resolve-File `
            $manifest.extensionManifestDiscovery.expectedManifest `
            "declared immutable self-test extension manifest"
        if (
            -not [string]::Equals(
                $declaredDiscoveryManifest,
                $expectedDiscoveryManifest,
                [System.StringComparison]::OrdinalIgnoreCase
            ) -or
            $manifest.extensionManifestDiscovery.sha256 -notmatch '^[0-9a-f]{64}$' -or
            (Get-FileSha256 $expectedDiscoveryManifest "self-test extension manifest") -cne
                $manifest.extensionManifestDiscovery.sha256
        ) {
            Fail "self-test extension-discovery path or digest disagrees with the derived contract"
        }
        $materializer = Resolve-File `
            (Join-Path $repository $RepositoryExtensionMaterializerRelativePath) `
            "extension carrier materializer"
        $discoveryProofTool = Resolve-File `
            (Join-Path $repository $RepositoryExtensionDiscoveryProofRelativePath) `
            "extension manifest discovery proof tool"
        foreach ($discoveryCode in @(
            @{ path = $materializer; label = "extension carrier materializer" },
            @{ path = $discoveryProofTool; label = "extension manifest discovery proof tool" }
        )) {
            Assert-InheritedRepositoryMutationDeny `
                $discoveryCode.path `
                $identity.Sid `
                $discoveryCode.label
            Assert-FileWriteOpenDenied $discoveryCode.path $discoveryCode.label
            Assert-FileAppendOpenDenied $discoveryCode.path $discoveryCode.label
        }
        if (
            (Get-FileSha256 $materializer "extension carrier materializer") -cne
                $manifest.extensionManifestDiscovery.materializerSha256 -or
            (Get-FileSha256 $discoveryProofTool "extension manifest discovery proof tool") -cne
                $manifest.extensionManifestDiscovery.proofToolSha256
        ) {
            Fail "active extension discovery code digest disagrees with the parent contract"
        }
        $discoveryResult = Invoke-BoundedToolProbe `
            $bunPath `
            @(
                $discoveryProofTool,
                $ExtensionManifestDiscoveryProofArgument,
                $expectedDiscoveryRoot,
                $expectedDiscoveryManifest,
                $manifest.extensionManifestDiscovery.sha256
            ) `
        ...
        $expectedDiscoveryOutput =
            $ExtensionManifestDiscoveryProof + "`t" +
            $ExtensionManifestDiscoveryRelativePath + "`t" +
            $manifest.extensionManifestDiscovery.sha256
        if (
            -not $discoveryResult.started -or
            $discoveryResult.timedOut -or
            $discoveryResult.error -or
            $discoveryResult.exitCode -ne 0 -or
            $discoveryResult.stdout.Trim() -cne $expectedDiscoveryOutput -or
            -not [string]::IsNullOrWhiteSpace($discoveryResult.stderr)
        ) {
            $discoveryStdout = ConvertTo-Json (
                ConvertTo-SanitizedDiagnosticTail $discoveryResult.stdout "" $false
            ) -Compress
            $discoveryStderr = ConvertTo-Json (
                ConvertTo-SanitizedDiagnosticTail $discoveryResult.stderr "" $false
            ) -Compress
            Fail (
                "active extension-manifest discovery failed under the standard-user token: " +
                "exitCode=$($discoveryResult.exitCode) " +
                "timedOut=$($discoveryResult.timedOut) " +
                "error=$($discoveryResult.error) " +
                "stdout=$discoveryStdout stderr=$discoveryStderr"
            )
        }
        $postDiscoveryFiles = @(Get-ImmutableInputFiles `
            $expectedDiscoveryRoot `
            "post-discovery immutable self-test input root")
        if (
            $postDiscoveryFiles.Count -ne 1 -or
            -not [string]::Equals(
                $postDiscoveryFiles[0],
                $expectedDiscoveryManifest,
                [System.StringComparison]::OrdinalIgnoreCase
            ) -or
            (Get-FileSha256 $expectedDiscoveryManifest "post-discovery extension manifest") -cne
                $manifest.extensionManifestDiscovery.sha256 -or
            (Get-FileSha256 $materializer "post-discovery extension carrier materializer") -cne
                $manifest.extensionManifestDiscovery.materializerSha256 -or
            (Get-FileSha256 $discoveryProofTool "post-discovery extension proof tool") -cne
                $manifest.extensionManifestDiscovery.proofToolSha256
        ) {
            Fail "active extension-manifest discovery changed its immutable input contract"
        }
        $extensionManifestDiscoveryVerified = $true
        $extensionManifestDiscoveryPath = $expectedDiscoveryManifest
        $extensionManifestDiscoverySha256 = $manifest.extensionManifestDiscovery.sha256
    }
    $repositoryAccessEvidence = [ordered]@{
        ...
        immutableInputRootsBound = $true
        immutableInputRootCount = $expectedImmutableInputRoots.Count
        immutableInputFileCount = $immutableInputFileCount
        immutableInputCreateDenied = $true
        immutableInputFileWriteDenied = $true
        immutableInputFileAppendDenied = $true
        extensionManifestDiscoveryVerified = $extensionManifestDiscoveryVerified
        extensionManifestPath = $extensionManifestDiscoveryPath
        extensionManifestSha256 = $extensionManifestDiscoverySha256
        extensionMaterializerSha256 = if ($extensionManifestDiscoveryVerified) {
            $manifest.extensionManifestDiscovery.materializerSha256
        } else {
            $null
        }
        extensionDiscoveryProofToolSha256 = if ($extensionManifestDiscoveryVerified) {
            $manifest.extensionManifestDiscovery.proofToolSha256
        } else {
            $null
        }
    ...
    foreach ($relativeInputRoot in $RepositoryImmutableInputRelativePaths) {
        $inputRootArgument = if (
            $relativeInputRoot -ceq "target/js-exact-candidate-input/ios-extensions"
        ) {
            "--ios-extension-artifact-root"
        } else {
            "--artifact-root"
        }
        $arguments += @($inputRootArgument, $relativeInputRoot)
    }
    $arguments += @("--output-root", $manifest.outputRoot)
    & $bunPath @arguments
...
function Invoke-ParentMode {
    ...
    $immutableInputRoots = [System.Collections.Generic.List[string]]::new()
    ...
    if (-not $SelfTest) {
        ...
        foreach ($relative in $RepositoryImmutableInputRelativePaths) {
            $immutableInputRoots.Add(
                (Resolve-Directory `
                    (Join-Path $repository $relative) `
                    "exact-candidate artifact root")
            ) | Out-Null
        }
    }
    ...
    $selfTestInputRoot = $null
    $selfTestExtensionManifest = $null
    $selfTestExtensionManifestSha256 = $null
    $selfTestExtensionMaterializer = $null
    $selfTestExtensionMaterializerSha256 = $null
    $selfTestExtensionDiscoveryProofTool = $null
    $selfTestExtensionDiscoveryProofToolSha256 = $null
    $extensionManifestDiscovery = $null
    ...
    $selfTestInputRootRemoved = -not $SelfTest
    $toolStagingEvidence = @()
    ...
    try {
        if ($SelfTest) {
            ...
            $selfTestInputRoot = [System.IO.Path]::GetFullPath(
                (Join-Path $repository "target/windows-standard-user-input/$nonce")
            )
            if (
                -not (Test-PathInside $repository $selfTestInputRoot) -or
                (Test-PathsOverlap $selfTestInputRoot $sandbox) -or
                (Test-PathsOverlap $selfTestInputRoot $toolExecutionRoot) -or
                (Test-Path -LiteralPath $selfTestInputRoot)
            ) {
                Fail "generated immutable self-test input root is unsafe or already exists"
            }
            $selfTestExtensionManifest = Join-Path `
                $selfTestInputRoot `
                $ExtensionManifestDiscoveryRelativePath
            Write-JsonFile $selfTestExtensionManifest ([ordered]@{
                product = "oliphaunt-extension-standard-user-fixture"
                version = "1.0.0"
                sqlName = "standard_user_fixture"
            })
            $selfTestInputRoot = Resolve-Directory `
                $selfTestInputRoot `
                "immutable self-test input root"
            $selfTestExtensionManifest = Resolve-File `
                $selfTestExtensionManifest `
                "immutable self-test extension manifest"
            $selfTestExtensionManifestSha256 = Get-FileSha256 `
                $selfTestExtensionManifest `
                "immutable self-test extension manifest"
            $selfTestExtensionMaterializer = Resolve-File `
                (Join-Path $repository $RepositoryExtensionMaterializerRelativePath) `
                "self-test extension carrier materializer"
            $selfTestExtensionMaterializerSha256 = Get-FileSha256 `
                $selfTestExtensionMaterializer `
                "self-test extension carrier materializer"
            $selfTestExtensionDiscoveryProofTool = Resolve-File `
                (Join-Path $repository $RepositoryExtensionDiscoveryProofRelativePath) `
                "self-test extension manifest discovery proof tool"
            $selfTestExtensionDiscoveryProofToolSha256 = Get-FileSha256 `
                $selfTestExtensionDiscoveryProofTool `
                "self-test extension manifest discovery proof tool"
            $immutableInputRoots.Add($selfTestInputRoot) | Out-Null
            $extensionManifestDiscovery = [ordered]@{
                expectedManifest = $selfTestExtensionManifest
                sha256 = $selfTestExtensionManifestSha256
                materializerSha256 = $selfTestExtensionMaterializerSha256
                proofToolSha256 = $selfTestExtensionDiscoveryProofToolSha256
            }
        }
        ...
        $manifest = [ordered]@{
            ...
            immutableInputRoots = @($immutableInputRoots)
            extensionManifestDiscovery = $extensionManifestDiscovery
        ...
        if (
            ...
            (Get-RepositoryControlReadSetSha256 $repository) -cne
                $consumerControlReadSetSha256 -or
            (
                $SelfTest -and
                (
                    (Get-FileSha256 `
                        $selfTestExtensionManifest `
                        "parent-side immutable self-test extension manifest") -cne
                        $selfTestExtensionManifestSha256 -or
                    (Get-FileSha256 `
                        $selfTestExtensionMaterializer `
                        "parent-side extension carrier materializer") -cne
                        $selfTestExtensionMaterializerSha256 -or
                    (Get-FileSha256 `
                        $selfTestExtensionDiscoveryProofTool `
                        "parent-side extension discovery proof tool") -cne
                        $selfTestExtensionDiscoveryProofToolSha256
                )
            )
        ) {
                ...
                if (
                    ...
                    (
                        ...
                        (
                            (Get-FileSha256 `
                                $selfTestExtensionManifest `
                                "cleanup immutable self-test extension manifest") -cne
                                $selfTestExtensionManifestSha256 -or
                            (Get-FileSha256 `
                                $selfTestExtensionMaterializer `
                                "cleanup extension carrier materializer") -cne
                                $selfTestExtensionMaterializerSha256 -or
                            (Get-FileSha256 `
                                $selfTestExtensionDiscoveryProofTool `
                                "cleanup extension discovery proof tool") -cne
                                $selfTestExtensionDiscoveryProofToolSha256
                        )
                    )
                ) {
        ...
        if ($canDestroyChildState) {
            ...
            if ($SelfTest) {
                try {
                    if ($selfTestInputRoot -and (Test-Path -LiteralPath $selfTestInputRoot)) {
                        Remove-Item `
                            -LiteralPath $selfTestInputRoot `
                            -Recurse `
                            -Force `
                            -ErrorAction Stop
                    }
                    $selfTestInputRootRemoved = (
                        -not $selfTestInputRoot -or
                        -not (Test-Path -LiteralPath $selfTestInputRoot)
                    )
                    if (-not $selfTestInputRootRemoved) {
                        Fail "immutable self-test input root still exists after recursive removal"
                    }
                } catch {
                    Add-CleanupFailure $cleanupErrors "immutable self-test input root removal" $_
                }
            }
        }
        try {
            if (
                ...
                (
                    ...
                    (
                        (Get-FileSha256 `
                            $selfTestExtensionMaterializer `
                            "post-cleanup extension carrier materializer") -cne
                            $selfTestExtensionMaterializerSha256 -or
                        (Get-FileSha256 `
                            $selfTestExtensionDiscoveryProofTool `
                            "post-cleanup extension discovery proof tool") -cne
                            $selfTestExtensionDiscoveryProofToolSha256
                    )
                )
            ) {
