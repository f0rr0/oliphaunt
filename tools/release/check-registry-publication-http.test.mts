...
describe('registry publication HTTP response boundary', () => {
  test('parses a response only within the configured byte limit', async () => {
    await expect(
      readBoundedRegistryJson(Response.json({ present: true }), 'registry', 64),
    ...
    ).rejects.toThrow('registry response exceeds 64 bytes');
  });

  test('rejects streamed overflow and malformed JSON deterministically', async () => {
    await expect(readBoundedRegistryJson(new Response('12345'), 'registry', 4)).rejects.toThrow(
      'registry response exceeds 4 bytes',
    );
    await expect(readBoundedRegistryJson(new Response('not-json'), 'registry', 64)).rejects.toThrow(
  ...
  test('clamps requests and all retry sleeps before the shared mutation deadline reserve', async () => {
    ...
    try {
      ...
      expect(registryRequestTimeoutMilliseconds('registry request', { nowImpl: () => now })).toBe(
        5_000,
      );
      await expect(
        boundedRegistrySleep(4, 'HTTP Retry-After', {
          nowImpl: () => now,
      ...
      await expect(
        boundedRegistrySleep(5, 'outer publication retry', {
          nowImpl: () => now,
          sleepImpl: async () => {
      ...
      expect(() =>
        registryRequestTimeoutMilliseconds('registry request', { nowImpl: () => now }),
  ...
  test('retains the ordinary read-only timeout and sleep behavior when no mutation deadline is present', async () => {
    ...
    try {
      ...
      expect(
        registryRequestTimeoutMilliseconds('registry request', { nowImpl: () => 1_000_000 }),
      ).toBe(20_000);
  ...
  test('uses a conservative bounded crates.io 429 fallback and never retries before an excessive Retry-After', async () => {
    ...
    await expect(
      cratesioUrlExists('https://crates.example.test/api/v1/crates/absent/1.0.0', 'absent 1.0.0', {
        nowImpl: () => now,
        sleepImpl: async (milliseconds) => {
    ...
    await expect(
      cratesioUrlExists('https://crates.example.test/api/v1/crates/later/1.0.0', 'later 1.0.0', {
        nowImpl: () => 1_000_000,
