...
function compatibilityVersionLinks() {
  return compatibilityVersionEntries(graphProducts(), {
...
function setRustConstString(file, constName, expected, context) {
  ...
  const pattern = new RegExp(
    `^(\\s*(?:pub\\s+)?const\\s+${escapeRegExp(constName)}\\s*:\\s*&str\\s*=\\s*)"([^"]*)"(;.*)$`,
    'u',
  );
...
function syncExtensionRegistryMetadata(changes, { write }) {
  ...
  for (const product of exactExtensionReleaseProducts(PREFIX)) {
    ...
    let updated = replaceTopLevelArrayAssignment(
      text,
      'publish_targets',
      expectedPublishTargets,
      product,
    );
    updated = replaceTopLevelArrayAssignment(
      updated,
      'registry_packages',
      expectedRegistryPackages,
      product,
    );
    if (updated !== text) {
      writeTextIfChanged(
        ...
        updated,
        changes,
        'synced explicit extension registry metadata',
        { write },
      );
    }
  }
...
function iterDependencyTables(manifest) {
  const tables = [];
  for (const [parts, owner] of [
    [[], manifest],
    ...Object.entries(manifest.target ?? {}).map(([target, value]) => [['target', target], value]),
  ]) {
    for (const tableName of DEPENDENCY_TABLES) {
      const table = owner?.[tableName];
      if (table && !Array.isArray(table) && typeof table === 'object')
        tables.push([[...parts, tableName], table]);
    }
  }
  return tables;
}
...
export function priorCargoPathDependencyVersions(manifestPath, { root = ROOT } = {}) {
  const file = path.relative(root, manifestPath).split(path.sep).join('/');
  const state = releasePleaseState(root, 'HEAD');
  if (file === '..' || file.startsWith('../') || path.isAbsolute(file))
    fail('prior Cargo manifest must remain inside its checkout');
  if (readFileSync(path.join(state, 'ancestry'), 'utf8').trim().split(/\s+/u).length !== 2)
    fail('prior Cargo manifest requires one parent commit');
  const prior = path.join(state, 'prior-cargo', file);
  if (!existsSync(prior)) return new Map();
  const versions = new Map();
  for (const [parts, table] of iterDependencyTables(Bun.TOML.parse(readFileSync(prior, 'utf8')))) {
    for (const [name, dependency] of Object.entries(table)) {
      if (
        ...
        versions.set(JSON.stringify([...parts, name]), dependency.version);
      }
    }
  }
  return versions;
...
export function cargoPathDependencyBindings(
  text,
  manifestPath,
  localPackages,
  priorVersions = new Map(),
) {
  const manifest = Bun.TOML.parse(text);
  const desired = [];
  for (const [parts, table] of iterDependencyTables(manifest)) {
    for (const [dependencyName, dependency] of Object.entries(table)) {
      ...
      const entryParts = [...parts, dependencyName];
      desired.push({
        kind: 'dependency',
        name: dependencyName,
        entryParts,
        expected: desiredCargoPathDependencyVersion(
          priorVersions.get(JSON.stringify(entryParts)) ?? versionValue,
          packageVersion,
        ),
      });
    }
...
function syncCargoPathDependencyPins(changes, { write, transitions }) {
  ...
  for (const manifestPath of cargoManifestPaths()) {
    ...
    const result = syncExampleCargoManifestText(source, {
      ...
      bindings,
      label: rel(manifestPath),
    });
    if (result.details.length)
      writeTextIfChanged(manifestPath, result.text, changes, result.details.join('; '), { write });
  }
}
...
function replaceUniqueDependencyVersion(text, binding, label) {
  ...
  if (matches.length === 0) {
    return replaceUniqueStringAssignment(
      text,
      { ...binding, entryParts: [...binding.entryParts, 'version'] },
      label,
    );
  }
  if (matches.length !== 1)
  ...
  if (first === '"' || first === "'") return replaceUniqueStringAssignment(text, binding, label);
  if (first !== '{') {
    throw new Error(
      `${label} ${binding.name} must use a string or inline-table dependency specification`,
    );
  }
  ...
  if (versions.length !== 1) {
    throw new Error(
      `${label} ${binding.name} inline table must declare version exactly once, found ${versions.length}`,
    );
  }
...
function replaceUniqueStringAssignment(text, binding, label) {
  ...
  if (matches.length !== 1) {
    throw new Error(
      `${label} must declare ${binding.entryParts.join('.')} exactly once, found ${matches.length}`,
    );
  }
  const { valueStart } = matches[0];
...
export function syncExampleCargoManifestText(
  text,
  { policy, bindings, label = `${policy.crateDir}/Cargo.toml` },
) {
  const manifest = Bun.TOML.parse(text);
  const details = [];
  let updated = text;
  for (const binding of bindings) {
    if (valueAt(manifest, binding.entryParts) === undefined) {
      throw new Error(
        `${label} is missing release-bound TOML path ${binding.entryParts.join('.')}`,
      );
    }
    const result =
...
function syncExtensionEvidenceSummary(changes, { write }) {
  const identity = {
    commit: process.env.OLIPHAUNT_EVIDENCE_COMMIT,
    tree: process.env.OLIPHAUNT_EVIDENCE_TREE,
  };
  if (![identity.commit, identity.tree].every((value) => /^[0-9a-f]{40}$/.test(value ?? ''))) {
    fail('run bash tools/release/sync-release-pr.sh to supply the checkout identity');
  }
  const expected = currentEvidenceTable(readJson(catalogPath), identity);
  writeTextIfChanged(
...
async function main(argv) {
  ...
  if (args.check) {
    console.error('release PR derived files are stale; run `tools/release/sync-release-pr.mts`');
    process.exit(1);
  }
