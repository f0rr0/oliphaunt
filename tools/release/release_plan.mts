...
function printGithubOutput(plan) {
  ...
  const extensionProducts = products
    .filter((product) => product.startsWith('oliphaunt-extension-'))
    .sort(compareText);
  const extensionArtifactProducts = extensionArtifactProductsForReleaseProducts(products, {
    prefix: TOOL,
  });
  ...
  console.log(
    `requires_wasix_release_regression_evidence=${String(wasixEvidenceProducts.length > 0).toLowerCase()}`,
  );
}
...
function planForArgs(args) {
  ...
  if (args.changedFiles.length > 0) {
    ...
    plan = buildPlan(
      graph,
      normalizeFiles(changedFilesFromRefs(args.baseRef, args.headRef, TOOL)),
      TOOL,
    );
  } else {
...
function main(argv) {
  if (argv[0] === '--history-inputs') {
    const directory = argv[1];
    const args = parseArgs(argv.slice(2));
    const needsHistory = args.changedFiles.length === 0 && (args.fromProductTags || args.baseRef);
    writeFileSync(
      path.join(directory, 'refs'),
      [needsHistory ? args.headRef : '', args.fromProductTags ? '' : (args.baseRef ?? '')]
        .map((value) => value + '\0')
        .join(''),
    );
    writeFileSync(
      path.join(directory, 'graph.json'),
      JSON.stringify({ products: args.fromProductTags ? loadGraph(TOOL).products : {} }),
    );
    return;
  }
