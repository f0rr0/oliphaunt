...
function sharedDeadlineMilliseconds({ env = process.env, deadlineEpochMs } = {}) {
  ...
  if (!Number.isSafeInteger(deadline))
    throw error(`${REGISTRY_MUTATION_DEADLINE_VARIABLE} exceeds the safe timestamp range`);
  return deadline;
}
...
export async function acquireCratesIoTrustedPublishingToken({
  ...
  const publicationDeadlineEpochMs =
    sharedDeadline === null
      ? acquiredAt + TOKEN_LIFETIME_MS
      : Math.min(
          acquiredAt + TOKEN_LIFETIME_MS,
          sharedDeadline - CRATES_IO_TRUSTED_REVOKE_RESERVE_MS,
        );
  if (publicationDeadlineEpochMs <= acquiredAt) {
    const reserveError = error(
      'temporary token was acquired without the mandatory revocation reserve intact',
    );
    try {
  ...
  return {
    token,
    acquiredAt,
    expiresAt: acquiredAt + TOKEN_LIFETIME_MS,
    publicationDeadlineEpochMs,
  };
}

export async function revokeCratesIoTrustedPublishingToken(
  token,
  {
    env = process.env,
    fetchImpl = fetch,
