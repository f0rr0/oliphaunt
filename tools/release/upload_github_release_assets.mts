...
import {
  ...
  readReleaseById as githubReleaseById,
...
function error(message, options = {}) {
  return new Error(`upload_github_release_assets.mts: ${message}`, options);
}
...
export function assertSharedUploadMutationAllowed(environment = process.env) {
  ...
  const cause = error(
    'a peer product upload lane failed; refusing a new GitHub release asset mutation',
  );
...
function usageError() {
  return error(
    'usage: upload_github_release_assets.mts <product> [--tag TAG] [--repo OWNER/NAME] ' +
...
export function assertExactFrozenUploadSelection(frozen, requestedPaths, product) {
  if (!Array.isArray(frozen) || !(requestedPaths instanceof Set)) {
    throw new TypeError(
      'frozen GitHub release assets and requested paths must be canonical collections',
    );
  }
  ...
  if (!sameSet(requestedPaths, frozenPaths)) {
    throw error(
      'requested GitHub release assets do not exactly match the frozen product asset set',
    );
  }
}
...
export function frozenUploadPlan({ assets, product, publicationLock, repo, tag }) {
  ...
  const productMetadata = loadProducts('github-release-assets');
  ...
  const frozen = lockedProductArtifactPaths(lock, product).filter(({ artifact }) =>
    GITHUB_ASSET_ROLES.has(artifact.role),
  );
...
export async function withStagedFrozenAsset(asset, operation, dependencies = {}) {
...
function pendingError(message, options = {}) {
  return new RetryableReadError(`upload_github_release_assets.mts: ${message}`, options);
}
...
function assertRemoteAssetInventory(value, plan) {
  if (!(value instanceof Map)) {
    throw permanentError(
      `${plan.product} GitHub release ${plan.tag} returned a non-canonical asset inventory`,
    );
  }
  const ids = new Set();
  for (const [name, asset] of value) {
    if (
      ...
      throw permanentError(
        `${plan.product} GitHub release ${plan.tag} returned malformed asset metadata`,
      );
    }
    ids.add(asset.id);
  }
...
export async function readExactReleaseAssetSnapshot(
  ...
  const readReleaseMap =
    dependencies.readReleaseMap ?? (async () => await githubReleaseMap(plan.repo, readOptions()));
  const readReleaseById =
    dependencies.readReleaseById ??
    (async (releaseId) => await githubReleaseById(plan.repo, releaseId, readOptions()));
  const readAssets =
    dependencies.readAssets ??
    (async (releaseId) => await readReleaseAssets(plan.repo, releaseId, readOptions()));

  let release;
  if (expectedReleaseId === undefined) {
    const releasesByTag = await readReleaseMap(context);
    ...
    release = await readReleaseById(expectedReleaseId, context);
  }
  if (release === null) {
    throw permanentError(
  ...
  const assets = assertRemoteAssetInventory(await readAssets(releaseId, context), plan);
...
function inspectFrozenReleaseAssetSnapshot({
  ...
  for (const [name, remote] of snapshot.assets) {
    ...
    if (remote.size !== asset.size) {
      throw permanentError(
        `${name} remote size ${remote.size} conflicts with frozen size ${asset.size}`,
      );
    }
  ...
  if (
    ...
    if (pendingMetadata.length > 0)
      details.push(`pending asset metadata: ${pendingMetadata.sort().join(', ')}`);
    if (requiredMissing.length > 0)
      details.push(`required asset absent: ${requiredMissing.map(({ name }) => name).join(', ')}`);
    if (!allowMissing && missing.length > 0)
      details.push(`frozen asset absent: ${missing.map(({ name }) => name).join(', ')}`);
    throw pendingError(`${plan.tag} exact asset snapshot is not ready (${details.join('; ')})`);
  }
  return { ...snapshot, missing };
}

export async function requireExactReleaseAssetSnapshot(
  {
    allowMissing,
    budget,
    expectedReleaseId,
  ...
  return await retryReadOperation(
    ...
    async () =>
      inspectFrozenReleaseAssetSnapshot({
        ...
        plan,
        requiredNames,
        snapshot: await readExactReleaseAssetSnapshot(
          { budget, expectedReleaseId, phase, plan },
          dependencies,
        ),
      }),
...
export async function uploadFrozenReleaseAssets(plan, dependencies = {}) {
  const environment = dependencies.environment ?? process.env;
  const uploadTimeoutMs =
    dependencies.uploadTimeoutMs ?? DEFAULT_GITHUB_RELEASE_ASSET_UPLOAD_TIMEOUT_MS;
  if (!Number.isSafeInteger(uploadTimeoutMs) || uploadTimeoutMs <= 0 || uploadTimeoutMs > 120_000) {
    throw error('GitHub release asset upload timeout must be between 1 and 120000 milliseconds');
  }
  let budget = dependencies.budget;
  if (budget === undefined) {
    const requiredWindowMs = githubReleaseAssetUploadWindowMs(plan.assets.length, {
      uploadTimeoutMs,
    });
    ...
    const requiredMs =
      plan.assets.length * (GITHUB_CONTENT_WRITE_INTERVAL_MS + uploadTimeoutMs) +
  ...
  const initial = await requireExactReleaseAssetSnapshot(
    {
      allowMissing: true,
      budget,
  ...
  const stage = dependencies.withStagedAsset ?? withStagedFrozenAsset;
  let recoveredUploads = 0;
  let uploadedAssets = 0;
  const completedMutationNames = new Set();
  const reconcileCompletedMutationsBeforeAbort = async () => {
    if (completedMutationNames.size === 0) return;
    await requireExactReleaseAssetSnapshot(
      {
        allowMissing: true,
        budget,
  ...
  for (const asset of initial.missing) {
    try {
      ...
      if (cause?.code === SHARED_UPLOAD_ABORT_CODE) {
        await reconcileCompletedMutationsBeforeAbort();
      }
      throw cause;
    }
    let mutationFailure;
    await stage(asset, async (stagedFile) => {
      ...
      try {
        if (dependencies.uploadAsset === undefined) {
          await requestGithubMutation(
            `https://uploads.github.com/repos/${plan.repo}/releases/${releaseId}/assets?name=${encodeURIComponent(asset.name)}`,
            {
              method: 'POST',
              file: stagedFile,
    ...
    try {
      ...
      const mutationDetail =
        mutationFailure instanceof Error ? mutationFailure.message : String(mutationFailure);
    ...
    completedMutationNames.add(asset.name);
    console.log(
      `${asset.name} became exact after an ambiguous upload response; the mutation was not replayed.`,
    );
  }
