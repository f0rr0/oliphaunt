...
pub(super) fn load_sources_manifest() -> Result<SourcesManifest> {
    let toolchain_path = Path::new("src/sources/toolchains/wasix.toml");
    let toolchain_text = fs::read_to_string(toolchain_path)
        .with_context(|| format!("read {}", toolchain_path.display()))?;
    let wasix: WasixToolchainManifest = toml::from_str(&toolchain_text)
        .with_context(|| format!("parse {}", toolchain_path.display()))?;
