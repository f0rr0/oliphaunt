...
mod tests {
    ...
    impl CurrentDirGuard {
        fn repo_root() -> Result<Self> {
            let previous = std::env::current_dir().context("read current test directory")?;
    ...
    #[test]
    fn postgis_build_spec_uses_wasix_target_recipe_metadata() -> Result<()> {
        let _cwd = CurrentDirGuard::repo_root()?;
        let specs = promoted_build_specs()?;
        let postgis = specs
            .iter()
