...
fn write_generated_extension_api(catalog: &ExtensionCatalog) -> Result<()> {
    ...
    let all = extensions
        .iter()
        .map(|extension| format!("        Self::{},", extension.rust_constant))
        .collect::<Vec<_>>()
        .join("\n");
    text.push_str(&format!(
    ...
    let mut contrib =
        String::from("// @generated by `cargo run -p xtask -- extensions generate`\n\n");
    for extension in extensions
        .iter()
        .filter(|extension| extension.source_kind == "postgres-contrib")
    {
        contrib.push_str(&format!(
            "/// Select `{}`.\npub const {}: crate::ExtensionDescriptor = crate::ExtensionDescriptor {{ sql_name: {:?}, package: None }};\n",
            extension.sql_name, extension.rust_constant, extension.sql_name
        ));
    }
    let contrib_path = Path::new(
        "src/bindings/wasix-rust/crates/oliphaunt-wasix/src/oliphaunt/generated_contrib_descriptors.rs",
    );
    fs::write(contrib_path, contrib)?;
    format_rust_source(contrib_path)
}
...
fn validate_wasix_sdk_extension_features(catalog: &ExtensionCatalog) -> Result<()> {
    ...
    let features = manifest["features"]
        .as_table()
        .context("WASIX SDK features missing")?;
    ensure!(
        !features.keys().any(|name| name.starts_with("extension-")),
        "WASIX extensions must be selected through package descriptors, not SDK features"
    );
    let selected = manifest["dependencies"]["liboliphaunt-wasix-portable"]["features"]
        .as_array()
        .context("WASIX base must include contrib")?
        .iter()
    ...
    let expected = catalog
        ...
        .iter()
        .filter(|extension| extension.source_kind == "postgres-contrib")
        .map(wasix_extension_feature)
        .collect::<BTreeSet<_>>();
    ensure!(
        selected == expected.iter().map(String::as_str).collect(),
        "WASIX base must include exactly the contrib artifact features"
    );
    Ok(())
}
