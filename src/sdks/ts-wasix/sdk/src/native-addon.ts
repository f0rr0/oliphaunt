...
export type NativeWasixDatabaseHandle = {
  ...
  backup(): Uint8Array;
  pgDump(args: string[], assets: NativeWasixToolAssets): NativeWasixToolResult;
  psql(
    args: string[],
...
export type NativeWasixActorDatabaseHandle = {
  ...
  pgDump(args: string[], assets: NativeWasixToolAssets): Promise<NativeWasixToolResult>;
  psql(
    args: string[],
    assets: NativeWasixToolAssets,
    command?: string,
    script?: string,
  ): Promise<NativeWasixToolResult>;
  close(): Promise<void>;
};
