...
import { nodeDirectAddonReleaseAssetUrl } from '../native/node-addon.js';
...
function releaseMetadataMatchesLiboliphauntAssets(): void {
  const target = liboliphauntReleaseTarget('0.1.0', 'darwin', 'aarch64');
  assert.equal(target.id, 'macos-arm64');
  assert.equal(target.assetName, 'liboliphaunt-0.1.0-macos-arm64.tar.gz');
  assert.equal(target.libraryRelativePath, 'lib/liboliphaunt.dylib');
  assert.equal(target.runtimeRelativePath, 'runtime');
  assert.equal(
    liboliphauntReleaseAssetUrl('0.1.0', target.assetName),
    'https://github.com/f0rr0/oliphaunt/releases/download/liboliphaunt-native-v0.1.0/liboliphaunt-0.1.0-macos-arm64.tar.gz',
  );
  const linuxTarget = liboliphauntReleaseTarget('0.1.0', 'linux', 'x64');
  assert.equal(linuxTarget.id, 'linux-x64-gnu');
  assert.equal(linuxTarget.assetName, 'liboliphaunt-0.1.0-linux-x64-gnu.tar.gz');
  assert.equal(linuxTarget.libraryRelativePath, 'lib/liboliphaunt.so');
  const linuxArmTarget = liboliphauntReleaseTarget('0.1.0', 'linux', 'arm64');
  assert.equal(linuxArmTarget.id, 'linux-arm64-gnu');
  assert.equal(linuxArmTarget.assetName, 'liboliphaunt-0.1.0-linux-arm64-gnu.tar.gz');
  assert.equal(linuxArmTarget.libraryRelativePath, 'lib/liboliphaunt.so');
  const windowsTarget = liboliphauntReleaseTarget('0.1.0', 'win32', 'x64');
  assert.equal(windowsTarget.id, 'windows-x64-msvc');
  assert.equal(windowsTarget.assetName, 'liboliphaunt-0.1.0-windows-x64-msvc.zip');
  assert.equal(windowsTarget.libraryRelativePath, 'bin/oliphaunt.dll');
  assert.equal(windowsTarget.runtimeRelativePath, 'runtime');
}

function checksumManifestsAreStrict(): void {
  const checksums = parseReleaseChecksumManifest(
    'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa  ./liboliphaunt-0.1.0-macos-arm64.tar.gz\n',
  );
  assert.equal(
    checksumForReleaseAsset(checksums, 'liboliphaunt-0.1.0-macos-arm64.tar.gz'),
    'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
  );
  assert.throws(() => parseReleaseChecksumManifest('not-a-checksum  ./asset.tar.gz'), /malformed/);
}
...
async function nodeResolverInstallsVerifiedReleaseAsset(): Promise<void> {
  const root = await mkdtemp(join(tmpdir(), 'oliphaunt-js-assets-'));
  const assetDir = join(root, 'assets');
  const cacheDir = join(root, 'cache');
  const version = packageMetadataVersion(await readTypeScriptPackageJson(), 'liboliphauntVersion');
  const target = liboliphauntReleaseTarget(version, platform(), arch());
  const archive = Uint8Array.from(
    gzipSync(
      tarArchive([
        { path: 'lib', mode: 0o755, directory: true },
        { path: target.libraryRelativePath, mode: 0o755, bytes: utf8('native-lib') },
        { path: 'runtime', mode: 0o755, directory: true },
        { path: 'runtime/bin', mode: 0o755, directory: true },
        { path: 'runtime/bin/initdb', mode: 0o755, bytes: utf8('#!/bin/sh\n') },
      ]),
    ),
  );
  try {
    await mkdir(assetDir, { recursive: true });
    await writeFile(join(assetDir, target.assetName), archive);
    await writeFile(
      join(assetDir, liboliphauntChecksumAssetName(version)),
      `${sha256Hex(archive)}  ./${target.assetName}\n`,
    );

    const previousAssetDir = process.env.OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR;
    const previousCacheDir = process.env.OLIPHAUNT_CACHE_DIR;
    const previousLibraryPath = process.env.LIBOLIPHAUNT_PATH;
    const previousRuntimeDir = process.env.OLIPHAUNT_RUNTIME_DIR;
    process.env.OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR = assetDir;
    process.env.OLIPHAUNT_CACHE_DIR = cacheDir;
    delete process.env.LIBOLIPHAUNT_PATH;
    delete process.env.OLIPHAUNT_RUNTIME_DIR;
    try {
      const install = await resolveNodeNativeInstall();
      assert.equal(await readFile(install.libraryPath, 'utf8'), 'native-lib');
      assert.ok(install.runtimeDirectory?.endsWith('runtime'));
      assert.equal(
        await readFile(join(install.runtimeDirectory ?? '', 'bin/initdb'), 'utf8'),
        '#!/bin/sh\n',
      );
      assert.ok((await stat(install.libraryPath)).mode & 0o100);
      assert.deepEqual(await resolveNodeNativeInstall(), install);
    } finally {
      restoreEnv('OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR', previousAssetDir);
...
async function nodeDirectAddonReleaseMetadataMatchesTypeScriptAssets(): Promise<void> {
  ...
  const version = packageMetadataVersion(packageJson, 'nodeDirectAddonVersion');
  ...
  assert.deepEqual(Object.keys(packageJson.optionalDependencies ?? {}).sort(), optionalDependencyNames);
  for (const packageName of optionalDependencyNames) {
    assert.equal(packageJson.optionalDependencies?.[packageName], `workspace:${version}`);
  }
  assert.equal(
    nodeDirectAddonReleaseAssetUrl(version, `oliphaunt-node-direct-${version}-macos-arm64.tar.gz`),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-node-direct-v${version}/oliphaunt-node-direct-${version}-macos-arm64.tar.gz`,
  );
  assert.equal(
    nodeDirectAddonReleaseAssetUrl(version, `oliphaunt-node-direct-${version}-linux-x64-gnu.tar.gz`),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-node-direct-v${version}/oliphaunt-node-direct-${version}-linux-x64-gnu.tar.gz`,
  );
  assert.equal(
    nodeDirectAddonReleaseAssetUrl(version, `oliphaunt-node-direct-${version}-windows-x64-msvc.zip`),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-node-direct-v${version}/oliphaunt-node-direct-${version}-windows-x64-msvc.zip`,
  );
  assert.throws(
    () => nodeDirectAddonReleaseAssetUrl(version, '../evil.tar.gz'),
    /invalid Oliphaunt Node direct release asset name/,
  );
}

async function brokerSupportInstallsVerifiedHelperAndNativeAssets(): Promise<void> {
  const version = packageMetadataVersion(await readTypeScriptPackageJson(), 'brokerVersion');
  const brokerAsset = `oliphaunt-broker-${version}-macos-arm64.tar.gz`;
  assert.equal(
    oliphauntBrokerReleaseAssetUrl(version, brokerAsset),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-broker-v${version}/oliphaunt-broker-${version}-macos-arm64.tar.gz`,
  );
  assert.equal(
    oliphauntBrokerReleaseAssetUrl(version, `oliphaunt-broker-${version}-linux-x64-gnu.tar.gz`),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-broker-v${version}/oliphaunt-broker-${version}-linux-x64-gnu.tar.gz`,
  );
  assert.equal(
    oliphauntBrokerReleaseAssetUrl(version, `oliphaunt-broker-${version}-windows-x64-msvc.zip`),
    `https://github.com/f0rr0/oliphaunt/releases/download/oliphaunt-broker-v${version}/oliphaunt-broker-${version}-windows-x64-msvc.zip`,
  );
  ...
  const nativeArchive = Uint8Array.from(
    gzipSync(
      tarArchive([
        { path: 'lib', mode: 0o755, directory: true },
        { path: nativeTarget.libraryRelativePath, mode: 0o755, bytes: utf8('native-lib') },
        { path: 'runtime', mode: 0o755, directory: true },
        { path: 'runtime/bin', mode: 0o755, directory: true },
        { path: 'runtime/bin/postgres', mode: 0o755, bytes: utf8('#!/bin/sh\n') },
      ]),
    ),
  );
  const brokerArchive = Uint8Array.from(
    gzipSync(
      tarArchive([
        { path: 'bin', mode: 0o755, directory: true },
        { path: 'bin/oliphaunt-broker', mode: 0o755, bytes: utf8('#!/bin/sh\n') },
        { path: 'manifest.properties', mode: 0o644, bytes: utf8('schema=test\n') },
      ]),
    ),
  );
  try {
    await mkdir(nativeAssetDir, { recursive: true });
    ...
    await writeFile(
      join(brokerAssetDir, brokerChecksumAsset),
      `${sha256Hex(brokerArchive)}  ./${brokerAsset}\n`,
    );

    const previousNativeAssetDir = process.env.OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR;
    const previousBrokerAssetDir = process.env.OLIPHAUNT_BROKER_ASSET_DIR;
    const previousCacheDir = process.env.OLIPHAUNT_CACHE_DIR;
    const previousLibraryPath = process.env.LIBOLIPHAUNT_PATH;
    const previousRuntimeDir = process.env.OLIPHAUNT_RUNTIME_DIR;
    const previousBroker = process.env.OLIPHAUNT_BROKER;
    process.env.OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR = nativeAssetDir;
    process.env.OLIPHAUNT_BROKER_ASSET_DIR = brokerAssetDir;
    process.env.OLIPHAUNT_CACHE_DIR = cacheDir;
    delete process.env.LIBOLIPHAUNT_PATH;
    delete process.env.OLIPHAUNT_RUNTIME_DIR;
    ...
    try {
      ...
      restoreEnv('OLIPHAUNT_LIBOLIPHAUNT_ASSET_DIR', previousNativeAssetDir);
      restoreEnv('OLIPHAUNT_BROKER_ASSET_DIR', previousBrokerAssetDir);
      restoreEnv('OLIPHAUNT_CACHE_DIR', previousCacheDir);
      restoreEnv('LIBOLIPHAUNT_PATH', previousLibraryPath);
      restoreEnv('OLIPHAUNT_RUNTIME_DIR', previousRuntimeDir);
