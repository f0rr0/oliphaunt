...
test('parseQueryResponse surfaces PostgreSQL errors and malformed backend traffic', () => {
  ...
  assert.throws(() => parseQueryResponse(Uint8Array.from([...backend(0x49, [])])), /before ReadyForQuery/);
});

test('assertSuccessfulQueryResponse and row decoding reject invalid payloads', () => {
  assertSuccessfulQueryResponse(Uint8Array.from([...backend(0x43, cstring('CREATE 1')), ...backend(0x5a, [0x49])]));
  assert.throws(
