...
if (pkg.dependencies || JSON.stringify(optional) !== JSON.stringify(expected.sort())) {
  throw new Error('TypeScript SDK normal installs may only declare Node direct optional platform packages');
}
" "$package_dir/package.json"
require_source_text "$package_dir/jsr.json" '"./deno": "./src/native/deno.ts"' \
  "TypeScript SDK must publish a Deno-native JSR entrypoint"
...
require_source_text "$package_dir/src/native/common.ts" "liboliphaunt-native-v" \
  "TypeScript SDK must know the product-scoped liboliphaunt release tag prefix"
require_source_text "$package_dir/src/native/assets-node.ts" "liboliphauntReleaseAssetUrl" \
  "TypeScript Node/Bun native binding must resolve compatible liboliphaunt release assets"
require_source_text "$package_dir/src/native/node-addon.ts" "oliphaunt-node-direct" \
  "TypeScript Node native-direct binding must resolve compatible prebuilt Node-API adapter release assets"
require_source_text "$package_dir/src/native/node-addon.ts" "OLIPHAUNT_NODE_ADDON_ASSET_DIR" \
  "TypeScript Node native-direct adapter resolver must support local release-asset fixtures for tests and release validation"
...
require_source_text "$package_dir/src/native/assets-deno.ts" "liboliphauntReleaseAssetUrl" \
  "TypeScript Deno native binding must resolve compatible liboliphaunt release assets"
...
require_source_text "$package_dir/src/__tests__/asset-resolver.test.ts" "nodeResolverInstallsVerifiedReleaseAsset" \
  "TypeScript SDK tests must cover verified liboliphaunt release asset installation"
require_source_text "$package_dir/src/__tests__/asset-resolver.test.ts" "nodeDirectAddonReleaseMetadataMatchesTypeScriptAssets" \
  "TypeScript SDK tests must cover verified Node.js native-direct adapter release asset metadata"
...
require_source_text "$package_dir/src/runtime/broker.ts" "oliphauntBrokerReleaseAssetUrl" \
  "TypeScript broker mode must resolve the published Rust broker helper release asset"
require_source_text "$package_dir/src/runtime/broker.ts" "OLIPHAUNT_BROKER_ASSET_DIR" \
