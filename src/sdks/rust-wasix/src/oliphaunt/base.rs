...
static RUNTIME_CACHE: OnceLock<Mutex<BTreeMap<String, Arc<CachedRuntime>>>> = OnceLock::new();
...
pub(crate) struct DatabasePlan {
    ...
    pub(crate) catalog_profile: CatalogProfile,
    pub(crate) seed: Option<ClusterSeed>,
    pub(crate) icu_data: Option<IcuData>,
}
...
fn runtime_cache_key(profile: CatalogProfile, icu_data: Option<&IcuData>) -> Result<String> {
    build_runtime_cache_key(profile, icu_data)
}

fn build_runtime_cache_key(profile: CatalogProfile, icu_data: Option<&IcuData>) -> Result<String> {
    ensure!(
