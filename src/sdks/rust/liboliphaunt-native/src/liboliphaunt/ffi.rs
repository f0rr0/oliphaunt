...
impl NativeSymbols {
    ...
    pub(super) fn register_selected_extensions(&self) -> Result<()> {
        ...
        if selected.is_none() {
            ...
            if let Ok(library) = load_native_library(Path::new("liboliphaunt_extensions.so")) {
                selected = unsafe { library.get::<Selected>(NAME).ok().map(|s| *s) };
        ...
        if selected.is_none() {
            ...
            selected = unsafe { process.get::<Selected>(NAME).ok().map(|s| *s) };
        }
    ...
    pub(super) fn load_path(path: &Path) -> Result<Self> {
        Self::from_library(load_native_library(path)?)
    }

    pub(super) fn load_current_process() -> Result<Self> {
        #[cfg(unix)]
        {
