...
fn resolve_broker_executable_from_asset_dir() -> Option<PathBuf> {
    ...
    for candidate in target.unpacked_executable_candidates(&root) {
        if candidate.is_file() {
            return Some(candidate);
        }
    }
    ...
    fn broker_release_targets_match_published_artifact_layout() {
        let cases = [
            (
                ...
                "aarch64",
                "macos-arm64",
                "oliphaunt-broker-0.1.0-macos-arm64.tar.gz",
            ...
            (
                ...
                "x86_64",
                "linux-x64-gnu",
                "oliphaunt-broker-0.1.0-linux-x64-gnu.tar.gz",
                "bin/oliphaunt-broker",
            ),
            (
                "linux",
                "aarch64",
                "linux-arm64-gnu",
                "oliphaunt-broker-0.1.0-linux-arm64-gnu.tar.gz",
                "bin/oliphaunt-broker",
            ),
            (
                "windows",
                "x86_64",
                "windows-x64-msvc",
                "oliphaunt-broker-0.1.0-windows-x64-msvc.zip",
        ...
        for (os, arch, target_id, asset, executable) in cases {
            ...
            assert_eq!(target.asset_name(), asset);
    ...
    fn broker_release_asset_dir_candidates_cover_package_shapes() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                PathBuf::from(
                    "/cache/broker/oliphaunt-broker-0.1.0-windows-x64-msvc/bin/oliphaunt-broker.exe"
                ),
            ]
        );
    }
