...
run_android_release_asset_fixture() {
  liboliphaunt_version="$(cat src/runtimes/liboliphaunt/native/VERSION)"
  fixture_assets="$(prepare_scratch_dir kotlin-liboliphaunt-release-assets)"
  run python3 tools/test/create-liboliphaunt-release-fixture.py \
    --asset-dir "$fixture_assets" \
    --version "$liboliphaunt_version"
  rm -rf \
    "$kotlin_build_dir/generated/oliphaunt-release-assets" \
    "$kotlin_build_dir/oliphaunt/release-asset-cache"
  run "$gradle_cmd" -p "$project_dir" :oliphaunt:resolveOliphauntAndroidReleaseAssets \
    "-PoliphauntLiboliphauntVersion=$liboliphaunt_version" \
    "-PoliphauntAssetBaseUrl=file://$fixture_assets" \
    "-PoliphauntAndroidAbiFilters=arm64-v8a" \
    $gradle_scratch_args \
    $gradle_smoke_cache_args
  resolved_release_root="$kotlin_build_dir/generated/oliphaunt-release-assets"
  if [ ! -f "$resolved_release_root/runtime-resources/oliphaunt/runtime/manifest.properties" ]; then
    echo "Kotlin Android release asset resolver did not unpack runtime resources" >&2
    exit 1
  fi
  if [ ! -f "$resolved_release_root/jniLibs/arm64-v8a/liboliphaunt.so" ]; then
    echo "Kotlin Android release asset resolver did not unpack the selected Android liboliphaunt runtime" >&2
    exit 1
  fi
}
