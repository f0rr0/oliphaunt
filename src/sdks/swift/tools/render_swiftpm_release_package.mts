...
async function prepareIcuResourceTree(assetDir, version, generatedTree) {
  ...
  try {
    ...
    fail(
      `SwiftPM ICU data asset is not a strict portable tar archive: ${archivePath}: ${error.message}`,
    );
  }
  ...
  try {
    ...
    fail(
      `SwiftPM ICU resource product did not extract its exact data receipt from ${archivePath}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  await fs.writeFile(
