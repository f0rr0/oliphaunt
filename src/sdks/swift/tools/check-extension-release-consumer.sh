...
tools/dev/bun.sh -e '
  await Bun.write(Bun.argv[1], JSON.stringify({
    plan: JSON.parse(Bun.argv[2]), productsFile: Bun.argv[3],
    releasePackage: Bun.argv[4], carrier: Bun.argv[5], cache: Bun.argv[6],
    output: Bun.argv[7], extensionCarriers: Bun.argv.slice(8),
  }));
' "$scratch/consumer.json" "$carrier_plan" "$selected_package/extension-products.json" \
  "$release_package" "$source_carrier" "$cache" "$consumer" "${extension_carriers[@]}"
node src/sdks/swift/tools/prepare-extension-release-consumer.mjs "$scratch/consumer.json"
