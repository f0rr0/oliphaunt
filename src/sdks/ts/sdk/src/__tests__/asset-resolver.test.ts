...
async function withConsumer(run: () => Promise<void>): Promise<void> {
...
async function explicitRuntimeExtensionValidationUsesPreparedFiles(): Promise<void> {
  ...
  try {
    ...
    const direct = await nodeAssets.validatePreparedNativeRuntimeExtensions(
    ...
    const releaseShaped = await nodeAssets.prepareExtensionInstall(
...
async function nodeExtensionMaterializationCopiesPackagePayloads(): Promise<void> {
  ...
  try {
    ...
    const cached = await nodeAssets.materializeExtensionInstall(
      { libraryPath, runtimeDirectory: installRuntime },
      ['hstore'],
    );
