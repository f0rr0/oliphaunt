...
function insertIosPodfileBlock(contents) {
  const block = iosPodfileBlock();
...
function withOliphaunt(config, options = {}) {
  ...
  config = plugin.withDangerousMod(config, [
    ...
    (modConfig) => {
      ...
      patchIosPodfile(path.join(iosRoot, 'Podfile'));
      return modConfig;
    },
  ]);
