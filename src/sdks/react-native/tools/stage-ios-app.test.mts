...
import {
  ...
  zipArchive,
  fixtureFiles,
} from '../../../../tools/test/zip-fixture.mts';
import { craftedTar, tarArchive } from '../../../../tools/test/tar-fixture.mts';
...
import { parseProperties, validateNativeRuntimeClosure } from './native-resource-closure.mts';
...
const SHARED_NATIVE_SEED_FIXTURES = Object.freeze({
  standard: await fs.readFile(
    new URL(
      '../../../shared/cluster-seed-contract/fixtures/native-standard.valid.properties',
      import.meta.url,
    ),
    'utf8',
  ),
  icu: await fs.readFile(
    new URL(
      '../../../shared/cluster-seed-contract/fixtures/native-icu.valid.properties',
      import.meta.url,
    ),
    'utf8',
  ),
...
async function highCardinalityIcuTar(archive, localeCount, legalRoot) {
  await fs.mkdir(path.dirname(archive), { recursive: true });
  const rows = [
    { name: 'share/icu/icudt77l.dat', data: Buffer.from('fixture') },
    ...Array.from({ length: localeCount }, (_, index) => ({
      name: 'share/icu/locale-' + String(index).padStart(4, '0') + '.res',
  ...
  for (const { name, data } of rows)
    digest
      .update(name.slice('share/icu/'.length))
      .update('\0')
  ...
  rows.push(
    ...(await fixtureFiles(legalRoot)).filter(
      (row) => row.name !== 'manifest.properties' && !row.name.startsWith('share/icu/'),
    ),
  );
...
async function main() {
  ...
  try {
    ...
    run(process.execPath, [
      path.join(import.meta.dirname, 'verify-ios-package.mts'),
      '--payload-dir',
      output,
    ]);
    ...
    highCardinalityIcu.base.assets = await Promise.all(
      highCardinalityIcu.base.assets.map(async (row) =>
        row.role === 'icu-data' ? asset('icu-data', highCardinalityIcuArchive, 'tar.gz', '.') : row,
      ),
    );
    await expectCarrierFailure(
    ...
    highCardinalityRuntime.base.assets = await Promise.all(
      highCardinalityRuntime.base.assets.map(async (row) =>
        row.role === 'runtime-resources'
          ? asset('runtime-resources', highCardinalityRuntimeArchive, 'tar.gz', 'share/icu')
          : row,
      ),
    );
    await expectCarrierFailure(
