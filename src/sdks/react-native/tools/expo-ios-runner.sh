...
validate_ios_static_extension_linkage() {
  ...
  local pods_support="$example_dir/ios/Pods/Target Support Files/OliphauntReactNative"
  local input_file="$pods_support/OliphauntReactNative-xcframeworks-input-files.xcfilelist"
  local output_file="$pods_support/OliphauntReactNative-xcframeworks-output-files.xcfilelist"
  [ -f "$input_file" ] ||
...
ios_resource_root_for_package() {
  printf '%s\n' "$1/ios/resources/OliphauntReactNativeResources.bundle/oliphaunt"
}

copy_ios_library_artifact_to_package() {
  local package_root="$1"
  local artifact="$2"
  local resource_root
  resource_root="$(ios_resource_root_for_package "$package_root")"
  case "$artifact" in
    *.xcframework)
      liboliphaunt_pod_mode="vendored-framework"
      mkdir -p "$package_root/ios/frameworks"
      rsync -a --delete "$artifact" "$package_root/ios/frameworks/"
      ;;
    *.framework)
      liboliphaunt_pod_mode="vendored-framework"
      mkdir -p "$package_root/ios/frameworks"
      ...
      ;;
    *.dylib)
      liboliphaunt_pod_mode="runtime-resource"
      mkdir -p "$resource_root/lib"
      rsync -a "$artifact" "$resource_root/lib/liboliphaunt.dylib"
      ;;
  esac
}

copy_ios_library_artifact() {
  copy_ios_library_artifact_to_package "$package_work" "$1"
}

stage_ios_static_registry_source_to_package() {
  local package_root="$1"
  local runtime_resources="$2"
  local selected_extensions="$3"
  local generated_root="$package_root/ios/generated/static-registry"
  local resource_source
  resource_source="$(ios_resource_root_for_package "$package_root")/static-registry/oliphaunt_static_registry.c"
  local stems
  ...
  local source="$runtime_resources/oliphaunt/static-registry/oliphaunt_static_registry.c"
  [ -f "$source" ] ||
    fail "selected iOS extension(s) require generated static-registry source, but it is missing: $source"
  mkdir -p "$generated_root"
  rsync -a "$source" "$generated_root/oliphaunt_static_registry.c"
}
...
install_ios_mobile_assets_into_react_native_package() {
  local installed_package="$1"
  local runtime_resources="$2"
  local library_artifact="$3"
  [ -d "$installed_package" ] ||
    fail "installed React Native SDK package is missing after artifact install: $installed_package"
  rm -rf \
    "$installed_package/ios/resources" \
    "$installed_package/ios/frameworks" \
    "$installed_package/ios/extension-frameworks"
  local installed_resource_root
  installed_resource_root="$(ios_resource_root_for_package "$installed_package")"
  mkdir -p "$installed_resource_root"
  rsync -a --delete \
    --exclude '/static-registry/archives/***' \
    "$runtime_resources/oliphaunt/" "$installed_resource_root/"
  local selected_extensions selected_module_extensions
  selected_extensions="$(normalize_mobile_extensions)"
  stage_ios_static_registry_source_to_package "$installed_package" "$runtime_resources" "$selected_extensions"
  selected_module_extensions="$(oliphaunt_dev_mobile_module_extensions_for_selection "$selected_extensions")"
  if [ -n "$selected_module_extensions" ]; then
    oliphaunt_dev_unpack_ios_extension_frameworks_for_selection \
      "$selected_module_extensions" \
      "$installed_package/ios/extension-frameworks"
  fi
  copy_ios_library_artifact_to_package "$installed_package" "$library_artifact"
}

pack_react_native_sdk() {
  local runtime_resources="$1"
  local library_artifact="$2"

  case "$library_artifact" in
    *.dylib)
      liboliphaunt_pod_mode="runtime-resource"
      ;;
    *)
  ...
  if expo_requires_sdk_artifacts; then
    ...
    install_ios_mobile_assets_into_react_native_package "$installed_package" "$runtime_resources" "$library_artifact"
    return
  fi
  ...
  if [ "${OLIPHAUNT_EXPO_IOS_REPACK_RN:-0}" != "1" ] &&
    [ -f "$tarball" ] &&
    [ -f "$package_stamp" ] &&
    [ ! "$runtime_resources/.prepared" -nt "$package_stamp" ] &&
    [ ! "$library_artifact" -nt "$package_stamp" ] &&
    [ -z "$(
      find "$rn_dir" \
  ...
  local package_resource_root
  package_resource_root="$(ios_resource_root_for_package "$package_work")"
  mkdir -p "$package_resource_root"
  rsync -a --delete \
    --exclude '/static-registry/archives/***' \
    "$runtime_resources/oliphaunt/" "$package_resource_root/"
  local selected_extensions
  selected_extensions="$(normalize_mobile_extensions)"
  stage_ios_static_registry_source_to_package "$package_work" "$runtime_resources" "$selected_extensions"
  local selected_module_stems
  selected_module_stems="$(oliphaunt_dev_mobile_module_stems_for_selection "$selected_extensions")"
  if [ -n "$selected_module_stems" ]; then
    local selected_module_extensions
    selected_module_extensions="$(oliphaunt_dev_mobile_module_extensions_for_selection "$selected_extensions")"
    oliphaunt_dev_unpack_ios_extension_frameworks_for_selection \
      "$selected_module_extensions" \
      "$package_work/ios/extension-frameworks"
  fi
  copy_ios_library_artifact "$library_artifact"
...
ensure_ios_project() {
  if [ ! -f "$example_dir/ios/Podfile" ]; then
    echo "Generating Expo iOS project for smoke validation"
    (
      cd "$example_dir"
...
main() {
  ...
  local runtime_resources library_artifact static_registry_source app
  library_artifact="$(find_ios_library_artifact)"
  validate_ios_library_artifact "$library_artifact"
  static_registry_source="$(mobile_static_registry_source_for_library "$library_artifact")"
