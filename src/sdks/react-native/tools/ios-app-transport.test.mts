...
const CLI = path.join(import.meta.dirname, 'ios-app-transport.sh');
...
function runCliSuccess(args) {
  return runSuccess('bash', [CLI, ...args]);
}
...
test('fails clearly when the required Apple transport tools are unavailable', {
  skip: IS_MACOS,
}, async (t) => {
  const root = await fixtureRoot(t, 'unsupported');
  expectCliFailure(
    ['pack', '--app-dir', path.join(root, 'app'), '--transport-dir', path.join(root, 'transport')],
    /required Apple command ditto was not found; run this operation on macOS/u,
  );
});

test('packs deterministically and restores executable and symlink fidelity', {
  skip: !IS_MACOS,
}, async (t) => {
  ...
  assert.deepEqual(
    firstManifest,
    secondManifest,
    'unchanged app input must produce a stable manifest',
  );
  assert.equal(
  ...
  runCliSuccess(['verify-extract', '--transport-dir', firstTransport, '--output-dir', output]);
  ...
  assert.equal(
    await fs.readFile(path.join(extractedApp, 'Resources', 'Payload.txt'), 'utf8'),
    'payload\n',
  );
});

test('rejects a checksum-bound traversal member before ditto can write outside extraction', {
  skip: !IS_MACOS,
}, async (t) => {
...
test('pack retries remove only owned stale transport temporaries', {
  skip: !IS_MACOS,
}, async (t) => {
...
test('preserves case-distinct resources when the test volume supports them', {
...
test('rejects archive checksum tampering', { skip: !IS_MACOS }, async (t) => {
  const { root, transport } = await makeTransport(t, 'checksum');
  await fs.appendFile(path.join(transport, ARCHIVE_NAME), 'tamper');
  expectCliFailure(
    ['verify-extract', '--transport-dir', transport, '--output-dir', path.join(root, 'output')],
    /transport archive (byte count|checksum) mismatch/u,
  );
});

test('rejects transported build-report tampering', { skip: !IS_MACOS }, async (t) => {
...
test('rejects unrelated top-level archive payloads', { skip: !IS_MACOS }, async (t) => {
  ...
  await fs.writeFile(
    path.join(payload, 'unrelated.txt'),
...
test('pack and verifier reject multiple direct app bundles', { skip: !IS_MACOS }, async (t) => {
...
test('pack and verifier reject a non-executable CFBundleExecutable', {
  skip: !IS_MACOS,
}, async (t) => {
