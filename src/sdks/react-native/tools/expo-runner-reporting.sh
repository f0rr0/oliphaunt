...
configure_mobile_catalog_profile_probe() {
  ...
  export EXPO_PUBLIC_OLIPHAUNT_CATALOG_PROFILE_PROBE_SQL="$({
    node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" profile "$fixture" "$profile" sql
  })"
  export EXPO_PUBLIC_OLIPHAUNT_CATALOG_PROFILE_PROBE_EXPECTED="$({
    node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" profile "$fixture" "$profile" expected
  })"
}
...
require_nonempty_json_file() {
  ...
  local json_status=0
  node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" json-object "$file" >/dev/null || json_status=$?
...
write_runner_report() {
  ...
  OLIPHAUNT_EXPO_LOG_TAG="$success_tag" \
    ...
    node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" parse-pass >"$report_tmp" || parse_status=$?
...
verify_mobile_extension_smoke_receipt() {
  ...
  node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" extension-receipt "$report" "$metadata" "$platform" "$candidate_sha" "$candidate_tree" >"$receipt_tmp" || receipt_status=$?
...
verify_mobile_e2e_smoke_receipt() {
  ...
  node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" verify-receipt "$report" "$receipt" "$metadata" "$platform" "$candidate_sha" "$candidate_tree" >/dev/null || verify_status=$?
...
write_maestro_runner_report() {
  ...
  OLIPHAUNT_MAESTRO_PLATFORM="$platform" \
    ...
    node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" maestro-report >"$reports_dir/$runner-report.json"
  OLIPHAUNT_MAESTRO_PLATFORM="$platform" \
    OLIPHAUNT_MAESTRO_APP_ID="$app_id" \
    OLIPHAUNT_MAESTRO_FLOW="$maestro_flow" \
    node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" maestro-pass >"$reports_dir/$runner-pass.log"
}
...
write_mobile_package_size_report() {
  ...
  local reports_dir="$scratch_root/reports"
  mkdir -p "$reports_dir"
  node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" package-sizes "$reports_dir/$runner-package-sizes.json" "$artifact_size_key" "$artifact_bytes" "$rn_package_bytes"
}
...
write_mobile_build_artifact_report_json() {
  ...
  shift 8
  node "$root/src/sdks/react-native/tools/expo-runner-reporting.mts" build-artifact "$report" "$platform" "$artifact" "$artifact_bytes" "$rn_package" "$rn_package_bytes" "$selected_extensions" "$report_scratch_root" "$@"
}
