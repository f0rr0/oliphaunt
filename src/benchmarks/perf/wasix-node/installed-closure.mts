...
import { findPackageManifest, sha256, stableJson } from './plan.mts';
...
export async function installedPackageClosure(entry, expectedName) {
  ...
  while (pending.length > 0) {
    ...
    for (const dependency of dependencies) {
      let dependencyEntry;
      // A dependency may expose only subpaths, with no root or package.json export.
      for (const directory of require.resolve.paths(dependency.name) ?? []) {
        const manifest = resolve(directory, dependency.name, 'package.json');
        try {
          await access(manifest);
          dependencyEntry = manifest;
          break;
        } catch (error) {
