...
export const defaultPlanFile = resolve(
  repositoryRoot,
  'src/benchmarks/wasix/node-pglite-memory-v2.json',
);
...
export async function loadPlan(file = defaultPlanFile) {
...
function validateEngines(plan) {
  ...
  exactRecord(
    comparisonSurfaces.worker,
    {
      ...
      entrypoint: 'src/benchmarks/perf/wasix-node/pglite-node-worker.mts',
      executionBoundary: 'node-worker-thread',
...
export function validateComparisonIdentity(comparison) {
  equal(comparison.package, COMPARISON_PACKAGE, 'plan.engines.comparison.package');
  if (typeof comparison.version !== 'string' || !/^\d+\.\d+\.\d+$/u.test(comparison.version)) {
    fail('plan.engines.comparison.version must be an exact release version');
  }
  if (
    typeof comparison.integrity !== 'string' ||
    !/^sha512-[A-Za-z0-9+/]{86}==$/u.test(comparison.integrity)
  ) {
  ...
  if (!/^[0-9a-f]{64}$/u.test(comparison.installedTreeSha256 ?? '')) {
    fail('plan.engines.comparison.installedTreeSha256 must be a SHA-256 digest');
  }
}
