...
const OLIPHAUNT_BENCHMARK_SQL_DIR: &str = "src/benchmarks/native/sql";
...
pub(crate) fn perf(args: Vec<String>) -> Result<()> {
    match args.first().map(String::as_str) {
        ...
        None => bail!(
            "usage: cargo run -p oliphaunt-perf -- <native-postgres|native-liboliphaunt|sqlite|diagnose-speed-cases>"
        ),
    }
}
