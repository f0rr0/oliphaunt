...
import { compatibilityVersionSource } from '../../../tools/test/product-history-fixture.mts';

import { requireCompatibilityVersionBinding } from './compatibility-version-policy.mts';
...
test('an unchanged independently versioned external is bound to its immutable current tag', (t) => {
  ...
  assert.deepEqual(
    compatibilityVersionSource(
      EXTERNAL_ENTRY,
      products(released),
      pending(released, NATIVE, CONTRIB),
      {
        root,
        prefix: 'compatibility-test',
      },
...
test('a newly bumped SDK without its not-yet-created tag follows the current source', (t) => {
  ...
  assert.deepEqual(
    compatibilityVersionSource(
      { ...EXTERNAL_ENTRY, product: SDK },
      products(released),
      pending(released, SDK),
      {
        root,
        prefix: 'compatibility-test',
      },
