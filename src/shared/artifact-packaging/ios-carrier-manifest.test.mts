...
import {
  ...
  extensionSqlNames,
} from '../product-metadata/release-artifact-targets.mts';
...
function archive(root, name, member, format, legal = undefined) {
  ...
  if (format === 'zip') {
    execFileSync('zip', ['-qry', output, legal?.insideMember === false ? '.' : member], {
      cwd: staging,
    });
  } else {
...
test('produces exact local and GitHub carrier envelopes without consulting truncated tar/unzip output', () => {
  ...
  try {
    ...
    const postgisPrimary = archive(
      root,
      'postgis-primary.zip',
      'liboliphaunt_extension_postgis-3.xcframework',
      'zip',
    );
    const postgisGeos = archive(
      root,
      'postgis-geos.zip',
    ...
    const local = withTruncatedArchiveTools(root, () =>
      buildIosCarrierManifest({
        baseAssetDir: base,
    ...
    assert.deepEqual(
      ...
      ['base-xcframework', 'runtime-resources', 'icu-data'],
    );
    assert.deepEqual(
    ...
    assert.deepEqual(
      ...
      ['MIT AND PostgreSQL AND Unicode-3.0', 'MIT AND PostgreSQL', 'MIT AND Unicode-3.0'],
    );
    assert.deepEqual(
...
test('bundle carriers verify exact nested bytes without consulting truncated tar output', () => {
  ...
  try {
    ...
    const carrier = withTruncatedArchiveTools(root, () =>
      buildSwiftExtensionCarrierManifest({
