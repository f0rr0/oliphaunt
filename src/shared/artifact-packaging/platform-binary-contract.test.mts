...
} from './platform-binary-contract.mts';
import {
...
describe('Mach-O platform compatibility', () => {
  test('accepts a thin arm64 direct macOS binary at the 11.0 floor', () => {
    const result = inspectPlatformBinaryBuffer(macho(), {
      target: 'macos-arm64',
      label: 'lib.dylib',
    });
  ...
  test('rejects x64, missing build metadata, and truncated load commands', () => {
    expect(() =>
      inspectPlatformBinaryBuffer(macho({ cpu: 0x01000007 }), {
        target: 'macos-arm64',
        label: 'wrong.dylib',
      }),
    ).toThrow(/not arm64/u);
    ...
    expect(() =>
      inspectPlatformBinaryBuffer(macho({ commandSize: 12 }), {
        target: 'macos-arm64',
        label: 'short.dylib',
      }),
  ...
  test('bounds-checks fat slices and validates every embedded slice', () => {
    ...
    expect(
      inspectPlatformBinaryBuffer(valid, { target: 'macos-arm64', label: 'fat' }).slices,
    ).toHaveLength(1);
    const outside = Buffer.from(valid);
    outside.writeUInt32BE(outside.length - 4, 16);
    expect(() =>
      inspectPlatformBinaryBuffer(outside, { target: 'macos-arm64', label: 'fat' }),
    ).toThrow(/outside the .*file/u);
    const highFloor = fatMacho([macho({ minos: [14, 0, 0] })]);
    expect(() =>
      inspectPlatformBinaryBuffer(highFloor, { target: 'macos-arm64', label: 'fat' }),
    ).toThrow(/exceeds.*11\.0/u);
    expect(() =>
  ...
  test('requires iOS device and simulator and permits macOS only through 14.0', () => {
    ...
    expect(inspectPlatformBinaryEntries(entries, { target: 'ios-xcframework' }).platforms).toEqual([
      1, 2, 7,
    ]);
    expect(() =>
      inspectPlatformBinaryEntries(entries.slice(0, 1), { target: 'ios-xcframework' }),
    ).toThrow(/missing macOS and iOS Simulator/u);
    expect(() =>
      inspectPlatformBinaryEntries(
  ...
  test('inspects Mach-O object members in static XCFramework archives', () => {
    ...
    expect(() =>
      inspectPlatformBinaryEntries(
        [entries[0], entry('device/libextension.a', malformed), entries[2]],
        {
          target: 'ios-xcframework',
          requiredApplePlatforms: [1, 2, 7],
        },
    ...
    expect(() =>
      inspectPlatformBinaryEntries(
        [
          entries[0],
          entry('device/libextension.a', ar([['readme', Buffer.from('not an object')]])),
          entries[2],
        ],
...
describe('ELF platform and GNU symbol-version compatibility', () => {
  ...
  test('rejects wrong architecture, class, byte order, and truncated tables', () => {
    ...
    expect(() =>
      inspectPlatformBinaryBuffer(elf({ littleEndian: false }), {
        target: 'linux-x64-gnu',
        label: 'be.so',
      }),
...
describe('PE32+ architecture and self-contained runtime imports', () => {
  ...
  test('requires app-local production MSVC runtime closure and rejects debug CRT', () => {
    ...
    expect(() =>
      inspectPlatformBinaryEntries([entry('bin/oliphaunt.dll', main)], {
        target: 'windows-x64-msvc',
      }),
  ...
  test('rejects x86, PE32, malformed import descriptors, and truncated files', () => {
    expect(() =>
      inspectPlatformBinaryBuffer(pe({ machine: 0x14c }), {
        target: 'windows-x64-msvc',
        label: 'x86.exe',
      }),
    ).toThrow(/not x64/u);
    ...
    expect(() =>
      inspectPlatformBinaryBuffer(Buffer.from('MZ'), {
        target: 'windows-x64-msvc',
        label: 'short.exe',
      }),
  ...
  test('validates the exported Windows contract through the standalone CLI', async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [
        path.join(import.meta.dir, 'platform-binary-contract.mts'),
        '--target',
        'windows-x64-msvc',
        '--root',
        root,
  ...
  test('keeps PostGIS COPYING.LIB legal text out of Windows binary discovery without admitting stray libraries', () => {
    ...
    expect(() =>
      ...
      /files\/share\/uncontracted\/COPYING\.LIB.*expected native binary is malformed or truncated/u,
    );
    expect(() =>
      inspectPlatformBinaryEntries(
  ...
  test('rejects malformed, wrong-machine, wrong-DLL, and arbitrary Windows libraries', () => {
    ...
    expect(() =>
      inspectImportLibrary(
        windowsImportLibraryFixture({
          importSymbols: ['oliphaunt_init', 'oliphaunt_logical_generation'],
        }),
      ),
    ...
    expect(() => inspectImportLibrary(invalidOffset)).toThrow(
      /refers to non-object archive offset 0/u,
    );
    ...
    expect(() =>
      inspectPlatformBinaryEntries(
        [
          entry('bin/oliphaunt.dll', pe()),
          entry('lib/arbitrary.lib', windowsImportLibraryFixture()),
        ],
        { target: 'windows-x64-msvc' },
      ),
    ).toThrow(/only the exact lib\/oliphaunt\.lib runtime import library is permitted/u);
    ...
    expect(() =>
      inspectPlatformBinaryEntries(
        [
          entry('bin/oliphaunt.dll', pe()),
          entry('lib/development.a', windowsImportLibraryFixture()),
        ],
        { target: 'windows-x64-msvc' },
      ),
...
describe('staged-tree discovery', () => {
  test('requires a binary and rejects malformed expected binary names', () => {
    expect(() =>
      inspectPlatformBinaryEntries([entry('README.md', Buffer.from('text'))], {
        target: 'linux-x64-gnu',
      }),
    ).toThrow(/no ELF binaries/u);
    expect(() =>
      inspectPlatformBinaryEntries(
  ...
  test('fails closed on symbolic links and non-regular archive entries', async () => {
    expect(() =>
      inspectPlatformBinaryEntries(
        [
          entry('lib/libok.so', elf()),
          { name: 'lib/redirect.so', isFile: false, isSymbolicLink: true },
        ],
        { target: 'linux-x64-gnu' },
      ),
