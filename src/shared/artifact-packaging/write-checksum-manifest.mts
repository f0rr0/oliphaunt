...
function fail(message) {
  console.error(`write_checksum_manifest.mts: ${message}`);
  process.exit(2);
}
...
function parseArgs(argv) {
  ...
  if (
    ...
    !output ||
    patterns.length === 0 ||
    patterns.some((pattern) => pattern.length === 0)
  ) {
    fail(
      'usage: src/shared/artifact-packaging/write-checksum-manifest.mts --asset-dir <dir> --output <file> --pattern <glob> [--pattern <glob>...]',
    );
  }
  return {
...
export async function writeChecksumManifest(argv) {
...
if (import.meta.main) {
  await writeChecksumManifest(Bun.argv.slice(2));
}
