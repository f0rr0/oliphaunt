...
} from '../product-metadata/platform-compatibility-policy.mts';
import {
...
function isWindowsExtensionLegalTextLibrary(name, buffer, format) {
  return (
    format === null && WINDOWS_EXTENSION_LEGAL_TEXT_LIBRARY_PATH.test(name) && isPlainText(buffer)
  );
}
...
function parseArArchiveMembers(buffer, label) {
  ...
  while (cursor < buffer.length) {
    ...
    if (rawName.startsWith('#1/')) {
      ...
      if (!/^[0-9]+$/u.test(nameLengthText))
        fail(label, `ar BSD member name length is invalid: ${rawName}`);
      ...
      if (longNames === null)
        fail(label, `ar member ${rawName} refers to a missing long-name table`);
      ...
      if (nameEnd === undefined)
        fail(label, `ar member long name at offset ${nameOffset} is unterminated`);
...
function parseWindowsSecondLinkerMember(member, objectMembers, label) {
  ...
  requireRange(
    ...
    memberCount * 4 + 4,
    memberLabel,
    'member-offset and symbol-count tables',
  );
  const offsets = [];
  ...
  for (let index = 1; index < names.length; index += 1) {
    if (
      Buffer.compare(
        Buffer.from(names[index - 1], 'latin1'),
        Buffer.from(names[index], 'latin1'),
      ) >= 0
    ) {
...
function parseCoffObjectMember(buffer, label, contract) {
  ...
  requireRange(
    buffer,
    sectionTable,
    sectionCount * COFF_SECTION_HEADER_SIZE,
    label,
    'COFF section table',
  );
  ...
  if (symbolCount === 0) {
    ...
    while (symbolIndex < symbolCount) {
      ...
      if (auxiliaryCount > symbolCount - symbolIndex - 1) {
        fail(
          label,
          `COFF symbol ${symbolIndex} has ${auxiliaryCount} out-of-range auxiliary record(s)`,
        );
      }
...
function parseCoffImportObjectMember(buffer, label, contract) {
  ...
  if (version !== 0) {
    fail(
      label,
      `unsupported anonymous COFF object version ${version}; expected a short import object`,
    );
  }
  ...
  if (machine !== contract.pe.machine) {
    fail(
      label,
      `COFF import-object machine 0x${machine.toString(16)} is not ${contract.architecture}`,
    );
  }
...
function parseWindowsRuntimeImportLibrary(buffer, label, contract) {
  ...
  if (objectMembers.length === 0)
    fail(label, 'MSVC import library contains no COFF object members');
  ...
  if (unexpectedSpecial !== undefined) {
    fail(
      label,
      `MSVC import library contains unsupported special member ${JSON.stringify(unexpectedSpecial.rawName)}`,
    );
  }
  ...
  if (imports.length === 0)
    fail(label, 'MSVC import library contains no short import-object members');
...
function parseMachoSlice(buffer, sliceOffset, sliceSize, label, contract) {
  ...
  if (commandCount > 65_536)
    fail(label, `Mach-O declares unreasonable load-command count ${commandCount}`);
  ...
  if (cursor !== commandsEnd) {
    fail(
      label,
      `Mach-O load commands consume ${cursor - (sliceOffset + 32)} bytes, expected ${commandsSize}`,
    );
  }
  if (buildVersions.length !== 1) {
    fail(
      label,
      `Mach-O slice must contain exactly one LC_BUILD_VERSION, found ${buildVersions.length}`,
    );
  }
...
function parseMacho(buffer, label, contract) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (alignment > 31)
      fail(label, `fat Mach-O slice ${index} has unsafe alignment exponent ${alignment}`);
    if (offset < tableEnd) fail(label, `fat Mach-O slice ${index} overlaps its architecture table`);
    if (offset % 2 ** alignment !== 0)
      fail(label, `fat Mach-O slice ${index} offset is not aligned`);
    ...
    if (slice.cpuType !== cpuType || slice.cpuSubtype !== cpuSubtype) {
      fail(
        label,
        `fat Mach-O slice ${index} architecture table identity does not match its thin header`,
      );
    }
    results.push(slice);
  }
...
function validateElfTable(buffer, offset, entrySize, count, minimumSize, label, name) {
  ...
  if (entrySize < minimumSize)
    fail(label, `ELF ${name} entry size ${entrySize} is below ${minimumSize}`);
...
function androidApiNotes(buffer, sectionOffset, sectionEntrySize, sectionCount, label) {
  ...
  for (let index = 0; index < sectionCount; index += 1) {
    ...
    const noteOffset = safeNumber(
      buffer.readBigUInt64LE(section + 24),
      label,
      `ELF note section ${index} offset`,
    );
    const noteSize = safeNumber(
      buffer.readBigUInt64LE(section + 32),
      label,
      `ELF note section ${index} size`,
    );
    ...
    while (cursor < end) {
      ...
      const owner = buffer
        .subarray(nameOffset, nameOffset + nameSize)
        .toString('ascii')
        .replace(/\x00+$/u, '');
      if (owner === 'Android' && type === 1) {
        if (descriptionSize < 4)
          fail(label, '.note.android.ident NT_VERSION description is truncated');
...
function parseElf(buffer, label, contract) {
  ...
  if (headerSize < 64 || headerSize > buffer.length)
    fail(label, `ELF header size ${headerSize} is invalid`);
  ...
  if (sectionCount === 0 && sectionOffset !== 0)
    fail(label, 'ELF extended section-header counts are not accepted');
  ...
  if (Number.isSafeInteger(contract.elf.androidApiLevel)) {
    ...
    if (elfType === ELF_TYPE_EXEC || elfType === ELF_TYPE_DYN) {
      const apiNotes = androidApiNotes(
        buffer,
        sectionOffset,
        sectionEntrySize,
        sectionCount,
      ...
      if (apiNotes.length !== 1) {
        fail(
          label,
          `Android ELF must contain exactly one .note.android.ident API record, found ${apiNotes.length}`,
        );
      }
...
function parsePe(buffer, label, contract) {
  ...
  if (sectionCount === 0 || sectionCount > 96)
    fail(label, `PE section count ${sectionCount} is invalid`);
  ...
  if (sizeOfHeaders === 0 || sizeOfHeaders > buffer.length)
    fail(label, `PE SizeOfHeaders ${sizeOfHeaders} is invalid`);
  ...
  for (let index = 0; index < sectionCount; index += 1) {
    ...
    if (rawSize > 0)
      requireRange(buffer, rawOffset, rawSize, label, `PE section ${index} raw data`);
...
export function inspectPlatformBinaryBuffer(input, { target, label = 'binary' }) {
  ...
  if (format === 'ar') {
    ...
    return {
      ...
      slices,
      platforms:
        contract.format === 'macho' ? [...new Set(slices.map(({ platform }) => platform))] : [],
    };
  }
  if (format !== contract.format) {
    fail(
      label,
      `${format.toUpperCase()} content does not match target ${target} (${contract.format.toUpperCase()})`,
    );
  }
  if (format === 'macho') {
  ...
  if (format === 'elf')
    return { format, slices: [parseElf(buffer, label, contract)], platforms: [] };
...
function finalizeInspection(
  ...
  if (inspected.length === 0) {
    fail(
      ...
      `no ${contract.format.toUpperCase()} binaries were found for ${target}`,
    );
  }
...
export function inspectPlatformBinaryEntries(
  ...
  for (const entry of entries) {
    ...
    const windowsExtensionLegalTextLibrary =
      target === 'windows-x64-msvc' && isWindowsExtensionLegalTextLibrary(name, buffer, format);
    const msvcLibraryPath =
      target === 'windows-x64-msvc' &&
      MSVC_LIBRARY_PATH.test(name) &&
      !windowsExtensionLegalTextLibrary;
    const expectedPath =
      EXPECTED_BINARY_PATH.test(name) || STATIC_ARCHIVE_PATH.test(name) || msvcLibraryPath;
    if (format === null && !expectedPath) continue;
    if (format === null)
      fail(name || rootLabel, 'expected native binary is malformed or truncated');
    const label = name ? `${rootLabel}/${name}` : rootLabel;
    if (!windowsRuntimeImportLibrary && msvcLibraryPath) {
      fail(
        label,
        `only the exact ${WINDOWS_RUNTIME_IMPORT_LIBRARY_PATH} runtime import library is permitted`,
      );
    }
    ...
    if (windowsRuntimeImportLibrary) {
      ...
      if (format !== 'ar')
        fail(label, 'required MSVC import library is not an ar-format COFF archive');
...
async function walkTree(root, relative = '') {
  ...
  for (const name of names) {
    ...
    if (stat.isSymbolicLink()) {
      ...
      entries.push({
        name: childRelative.split(path.sep).join('/'),
...
function usage() {
  return 'usage: src/shared/artifact-packaging/platform-binary-contract.mts --target TARGET --root STAGED_RELEASE_TREE [--required-apple-platforms macos,ios,ios-simulator] [--require-windows-runtime-import-library] [--windows-vc-runtime-profile direct|provider]\n';
}
  ...
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === '--target') {
      ...
      if (names.length === 0 || new Set(names).size !== names.length) {
        fail(
          'platform-binary-contract.mts',
          '--required-apple-platforms must be a nonempty unique CSV',
        );
      }
      requiredApplePlatforms = names.map((name) => {
        const platform = APPLE_PLATFORM_BY_CLI_NAME.get(name);
        if (platform === undefined) {
          fail('platform-binary-contract.mts', `unknown Apple platform ${JSON.stringify(name)}`);
        }
        return platform.id;
      });
      ...
      fail('platform-binary-contract.mts', `unknown argument ${JSON.stringify(argv[index])}`);
    }
  }
  if (!target || !root) {
...
if (import.meta.main) {
  try {
    ...
    console.error(`platform-binary-contract.mts: ${error.message}`);
    process.exit(1);
  }
}
