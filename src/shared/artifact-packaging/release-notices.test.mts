...
test('validates exact archive members and canonical bytes', (t) => {
  ...
  const result = spawnSync(
    ...
    ['src/shared/artifact-packaging/archive-directory.mts', '--keep-parent', stage, archive],
...
test('rejects archive byte and mode drift', (t) => {
  ...
  let result = spawnSync(
    path.join(ROOT, 'tools/dev/bun.sh'),
    ['src/shared/artifact-packaging/archive-directory.mts', '--keep-parent', stage, byteArchive],
    { cwd: ROOT, encoding: 'utf8' },
  );
  assert.equal(result.status, 0, result.stderr);
  assert.throws(
  ...
  result = spawnSync(
    path.join(ROOT, 'tools/dev/bun.sh'),
    ['src/shared/artifact-packaging/archive-directory.mts', '--keep-parent', stage, modeArchive],
    { cwd: ROOT, encoding: 'utf8' },
  );
  assert.equal(result.status, 0, result.stderr);
  assert.throws(
