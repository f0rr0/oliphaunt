...
export const RELEASE_CARRIER_PROFILES = Object.freeze({
  ...
  'node-direct-addon': Object.freeze({
  ...
  'native-runtime': Object.freeze({
    products: Object.freeze(['native']),
    components: Object.freeze(['postgresql', 'icu']),
  }),
  'native-tools': Object.freeze({
    products: Object.freeze(['native']),
    components: Object.freeze(['postgresql']),
  }),
  'native-runtime-resources': Object.freeze({
    products: Object.freeze(['native']),
    components: Object.freeze(['postgresql']),
  }),
  'native-icu-data': Object.freeze({
    products: Object.freeze(['native']),
    components: Object.freeze(['icu']),
  }),
  'wasix-runtime': Object.freeze({
    products: Object.freeze(['wasix']),
    components: Object.freeze(['postgresql', 'icu']),
  }),
  'wasix-tools': Object.freeze({
    products: Object.freeze(['wasix']),
    components: Object.freeze(['postgresql', 'icu']),
  }),
  'wasix-aot': Object.freeze({
    products: Object.freeze(['wasix']),
    components: Object.freeze(['postgresql', 'icu']),
  }),
  'wasix-icu-data': Object.freeze({
    products: Object.freeze(['wasix']),
    components: Object.freeze(['postgresql', 'icu']),
  }),
  'wasix-icu-data-crate': Object.freeze({
    products: Object.freeze(['wasix']),
    components: Object.freeze(['icu']),
  }),
  ...
  'contrib-native-openssl': Object.freeze({
    products: Object.freeze([]),
    components: Object.freeze(['postgresql', 'openssl']),
  }),
  'contrib-wasix': Object.freeze({
    products: Object.freeze([]),
    components: Object.freeze(['postgresql']),
  }),
  'contrib-wasix-openssl': Object.freeze({
    products: Object.freeze([]),
    components: Object.freeze(['postgresql', 'openssl']),
  }),
...
const LICENSE_COMPONENT_ROWS = Object.freeze({
  ...
  icu: Object.freeze({
    ...
    licenseUrl:
      'https://github.com/unicode-org/icu/blob/8eca245c7484ac6cc179e3e5f7c1ea7680810f39/LICENSE',
  }),
  openssl: Object.freeze({
    ...
    licenseUrl:
      'https://github.com/openssl/openssl/blob/286ddeaac037533bbdce65b3c689e3f7ffebf0f6/LICENSE.txt',
  }),
});
...
function checkedSelection({ profile, products, components } = {}) {
  if (profile !== undefined) {
    if (products !== undefined || components !== undefined) {
      throw new Error(
        'release carrier profile cannot be combined with explicit products or components',
      );
    }
...
function requireCanonicalSource(row) {
  ...
  if (row.sha256) {
    ...
    if (actual !== row.sha256) {
      throw new Error(
        `canonical release license digest changed for ${row.source}: expected ${row.sha256}, got ${actual}`,
      );
    }
  }
  return bytes;
...
function validateRuntimeLicenseSource(row) {
  ...
  if (row.id === 'postgresql') {
    ...
    if (!path.basename(row.source).startsWith(`postgresql-${source.version}-`)) {
      throw new Error(
        `PostgreSQL runtime license snapshot name does not carry pinned version ${source.version}`,
      );
    }
  } else {
    if (
    ...
    if (!path.basename(row.source).includes(row.sourceVersion)) {
      throw new Error(
        `${row.id} runtime license snapshot name does not carry pinned version ${row.sourceVersion}`,
      );
    }
  }
...
function archiveNoticeNamespaceEntries(entries, prefix) {
  ...
  for (const [archiveMember, entry] of entries) {
    ...
    if (PRODUCT_NOTICE_NAMESPACE_PATTERN.test(member)) {
      ...
      member === LICENSE_NAMESPACE_ROOT ||
      member.startsWith(`${LICENSE_NAMESPACE_ROOT}/`)
    ) {
      namespaceEntries.push({ archiveMember, entry, member, namespace: 'release license' });
    }
  }
  return namespaceEntries.sort((left, right) =>
    compareText(left.archiveMember, right.archiveMember),
  );
}
...
function unexpectedNamespaceEntry(namespaceEntry, expectedMembers) {
  ...
  if (
    ...
    return (
      namespaceEntry.entry?.isDirectory !== true || namespaceEntry.entry?.isSymbolicLink === true
    );
  }
  return true;
}
...
export function releaseNoticeRows(options = {}) {
  ...
  const rows = [
    ...
    ...selection.components.map((component) =>
      validateRuntimeLicenseSource(LICENSE_COMPONENT_ROWS[component]),
    ),
  ];
  return Object.freeze(rows.map((row) => Object.freeze({ ...row })));
}

export function releaseNoticeInputPaths(
  options = { products: RELEASE_NOTICE_PRODUCTS, components: RELEASE_LICENSE_COMPONENTS },
) {
...
export function releaseLicenseComponents(ids) {
  return Object.freeze(
    checkedComponents(ids).map((id) => validateRuntimeLicenseSource(LICENSE_COMPONENT_ROWS[id])),
  );
}
...
export function releaseMavenLicenses({
  product,
  version,
  components = [],
  includeOliphaunt = true,
} = {}) {
  ...
  return Object.freeze(
    releasePackageLicense({ components, includeOliphaunt }).entries.map((entry) =>
      Object.freeze({
        name: entry.name,
        url:
          entry.id === 'oliphaunt'
...
export function releaseProfileMavenLicenses(
  profile,
  { product, version, includeOliphaunt = true } = {},
) {
  const selection = releaseCarrierProfile(profile);
...
export function stageReleaseNotices(destination, options = {}) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (cause?.code !== 'ENOENT') {
        throw new Error(
          `release notice destination ${destinationFile} cannot be inspected: ${cause.message}`,
        );
      }
    }
...
export function assertReleaseNoticesInEntries(entries, options = {}) {
  const { prefix = '', exact = true, label = 'archive' } = options;
  ...
  for (const row of rows) {
    ...
    if (!actual.equals(canonical)) {
      throw new Error(
        `${label} release notice member ${member} differs byte-for-byte from ${row.source}`,
      );
    }
  }
  if (exact) {
    for (const entry of archiveNoticeNamespaceEntries(entries, checkedArchivePrefix)) {
      if (!unexpectedNamespaceEntry(entry, expected)) continue;
      throw new Error(
        `${label} contains unexpected ${entry.namespace} member ${entry.archiveMember}`,
      );
    }
  }
...
function usage() {
  return [
    'usage:',
    '  src/shared/artifact-packaging/release-notices.mts stage <directory> --profile <carrier-profile>',
    '  src/shared/artifact-packaging/release-notices.mts check-directory <directory> --profile <carrier-profile>',
    '  src/shared/artifact-packaging/release-notices.mts check-archive <archive> --profile <carrier-profile> [--prefix <member-prefix>]',
...
function parseCli(argv) {
  ...
  return {
    ...
    noticeOptions:
      profile === undefined
        ? {
            products: checkedProductValues,
            components: checkedComponents(componentsSupplied ? components : []),
          }
        : { profile },
    target,
  };
