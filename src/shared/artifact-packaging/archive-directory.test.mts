...
const ARCHIVER = path.resolve(import.meta.dirname, 'archive-directory.mts');
...
test('writes deterministic canonical ustar directory markers', () => {
  ...
  try {
    ...
    assert.equal(
      readFileSync(path.join(extracted, 'nested', 'child', 'payload.txt'), 'utf8'),
      'payload\n',
    );
    ...
    assert.match(
      runFailure(process.execPath, [
        ARCHIVER,
        unsplittable,
        path.join(root, 'unsplittable.tar.gz'),
      ]),
