...
import { ROOT } from '../product-metadata/release-graph.mts';

const RUST_IMAGE = 'rust@sha256:5b9332190bb3b9ece73b810cd1f1e9f06343b294ce184bcb067f0747d7d333ea';
const FEDORA_IMAGE =
  'fedora@sha256:d63d63fe593749a5e8dbc8152427d40bbe0ece53d884e00e5f3b44859efa5077';
...
test('Linux broker build is exact, isolated, offline, and non-privileged', () => {
  ...
  try {
    const result = spawnSync(
      'bash',
      [path.join(ROOT, 'src/runtimes/broker/tools/build-linux-broker-baseline.sh'), targetDir],
      {
        cwd: ROOT,
        encoding: 'utf8',
        env: {
...
test('Linux WASIX Node-API build uses the pinned baseline with exact payload mounts', () => {
  ...
  const log = path.join(fixture, 'docker.log');
  const rustHost = os.arch() === 'arm64' ? 'aarch64-unknown-linux-gnu' : 'x86_64-unknown-linux-gnu';
  const inputs = {
  ...
  try {
    const result = spawnSync(
      'bash',
      [
        path.join(ROOT, 'src/runtimes/wasix-napi/tools/build-linux-wasix-napi-baseline.sh'),
        targetDir,
        rustHost,
        'release',
      ],
...
test('Linux ABI rehearsal pins Fedora and executes without network or privilege', () => {
  ...
  try {
    const result = spawnSync(
      'bash',
      [
        path.join(ROOT, 'src/shared/artifact-packaging/check-linux-consumer-baseline.sh'),
        '--target',
...
test('baseline scripts reject arbitrary host mounts', () => {
  ...
  for (const [script, args] of [
    [
      'src/runtimes/broker/tools/build-linux-broker-baseline.sh',
      [path.join(os.tmpdir(), 'oliphaunt-outside-build')],
    ],
    [
