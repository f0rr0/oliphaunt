...
installer="$root/src/shared/mobile-tools/setup-maestro.sh"
...
run_case() {
  ...
  mkdir -p "$CASE_REPO/src/shared/mobile-tools" "$CASE_REPO/tools/dev" "$CASE_REPO/src/sources/toolchains" "$CASE_HOME/.maestro"
  cp "$installer" "$CASE_REPO/src/shared/mobile-tools/setup-maestro.sh"
  cp "$root/src/shared/mobile-tools/extract-maestro.mts" "$CASE_REPO/src/shared/mobile-tools/extract-maestro.mts"
  cp "$root/tools/dev/bun.sh" "$CASE_REPO/tools/dev/bun.sh"
  cp "$root/.prototools" "$CASE_REPO/.prototools"
  mkdir -p "$CASE_REPO/src/shared/artifact-packaging"
  cp "$root/src/shared/artifact-packaging/portable-archive.mts" "$CASE_REPO/src/shared/artifact-packaging/portable-archive.mts"
  ...
  if (
    ...
    env \
      ...
      bash "$CASE_REPO/src/shared/mobile-tools/setup-maestro.sh"
  ) >"$CASE_LOG" 2>&1; then
    CASE_STATUS=0
  else
    CASE_STATUS=$?
