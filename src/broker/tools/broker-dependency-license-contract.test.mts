...
const CONTRACT = path.join(ROOT, 'src/broker/dependency-licenses.json');
...
if (!packageFixture)
  throw new Error('Run bash src/broker/tools/broker-dependency-license-contract.test.sh');
...
test('broker local dependency licenses follow Cargo workspace membership', () => {
  const row = {
    ...
    source: null,
    license: 'MIT',
    manifest_path: path.join(ROOT, 'src/sdks/rust/liboliphaunt-native/Cargo.toml'),
  };
