...
contract=src/broker/tools/broker-dependency-license-contract.mts
...
"$bun" src/broker/tools/create-release-fixture.mts --asset-dir "$scratch/assets" --version "$version"
package() {
  PATH="$scratch/empty-path" CARGO_HOME="$scratch/empty-cargo-home" \
    "$bun" src/broker/tools/package_broker_cargo_artifacts.mts \
