...
function generateSdkMatrix(sdkManifest) {
  const rows = sdkRows(sdkManifest).map(([id, sdk]) => {
    ...
    return `| ${escapeMarkdown(id)} | ${escapeMarkdown(sdk.package_name)} | ${escapeMarkdown((sdk.primary_targets ?? []).join(', '))} | ${escapeMarkdown(sdk.runtime_boundary)} | ${escapeMarkdown(modes)} |`;
  });
  return `---
title: SDK Matrix
---
...
Use this matrix to compare package names, primary targets, runtime boundaries,
and advertised modes.

| SDK | Package | Targets | Runtime boundary | Advertised modes |
| --- | --- | --- | --- | --- |
${rows.join('\n')}
`;
}
...
function generatePlatformMatrix(sdkManifest) {
  const rows = sdkRows(sdkManifest).flatMap(([id, sdk]) =>
    (sdk.primary_targets ?? []).map(
      (target) =>
        `| ${escapeMarkdown(target)} | ${escapeMarkdown(id)} | ${escapeMarkdown(sdk.package_name)} |`,
    ),
  );
  return `---
...
function generateVersionMatrix(releaseGraph) {
  const rows = releaseProducts(releaseGraph).map(([id, product]) => {
    return `| ${escapeMarkdown(id)} | ${escapeMarkdown((product.publish_targets ?? []).join(', ') || 'none')} | ${escapeMarkdown(product.tag_prefix)} |`;
  });
  return `---
...
| Product | Publish targets | Tag prefix |
| --- | --- | --- |
${rows.join('\n')}
`;
}
