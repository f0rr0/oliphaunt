...
export const sdkSurfaces: SdkSurface[] = [
  ...
  },
  {
    ...
    owns: 'Rust WASIX hosting, storage and initialization, server mode, and dump/restore tooling.',
  ...
  },
  {
    ...
    href: '/docs/sdk/wasix/browser-typescript',
    packageName: '@oliphaunt/wasix',
    install: 'pnpm add @oliphaunt/wasix',
    target: 'Cross-origin-isolated browser and Node.js applications',
    startWith: 'A host Worker with memory storage by default',
    owns: 'Browser and Node Worker hosting, pgwire recovery, selective extension imports, and optional browser IndexedDB snapshots.',
    modes: ['WASIX browser', 'WASIX Node'],
    verifyFirst: 'Open memory storage, recover from a SQL error, and close the host Worker cleanly.',
    guideOutcomes: [
      'Run PostgreSQL in a browser or Node Worker without importing the native TypeScript SDK.',
      'Import only the WASIX extension descriptors the application uses.',
      'Opt into IndexedDB explicitly when checkpoint and clean-close persistence fits the app.',
    ],
    icon: Braces,
  },
  {
...
export const runtimeModes: RuntimeMode[] = [
  ...
  },
  {
    ...
    boundary: 'Rust-specific storage, extensions, capabilities, archive, and dump APIs.',
    icon: Boxes,
  },
  {
    name: 'wasixTypeScript',
    label: 'WASIX TypeScript',
    href: '/docs/sdk/wasix/browser-typescript',
    useWhen: 'A browser Worker or Node worker thread owns one portable PostgreSQL instance.',
    boundary: 'Memory by default, optional browser IndexedDB snapshots, and no native fallback.',
    icon: Boxes,
  },
];
