...
[`liboliphaunt-wasix-postmaster`](../../runtimes/liboliphaunt/wasix-postmaster/README.md)
...
in [`src/docs/maintainers/wasix-postmaster.md`](../maintainers/wasix-postmaster.md).
...
`src/docs/internal/PG18_WASIX_PERF_STATUS.md`.
...
first int4 leaf comparison—are shared in `src/third-party/postgres/patches/wasix`.
...
each source directory is represented in `src/sources/src/third-party/**`, and, when the
