...
- `pnpm --dir src/sdks/react-native/src/examples/expo run smoke:android` passes on an Android
...
- `pnpm moon run liboliphaunt-native:build-runtime-ios-xcframework` produces current
  ...
  symbols.
- `pnpm --dir src/sdks/react-native/src/examples/expo run smoke:ios` passes on an iOS
  simulator/device with current native iOS artifacts.
- Every row in `src/docs/maintainers/sdk-parity-policy.md` has SDK-specific tests or a documented
