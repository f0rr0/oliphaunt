...
> Archived completion log; non-normative. Start with
> `src/docs/maintainers/README.md` and executable configuration.

This was the chronological status log for implementation work completed during
the original build-out. It is retained as maintainer-facing history and is
intentionally separate from current policy and end-user documentation.
...
- `src/benchmarks/perf/matrix/run_native_speed_diagnostics.sh` runs repeated
...
- `target/perf/native-liboliphaunt-20260524T090412Z/report.md` is a complete
  ...
  `src/benchmarks/perf/check-native-perf-report.sh` provenance
...
- source pins live in `src/sources/src/third-party/**`;
...
- extension discovery now merges Oliphaunt src/docs/REPL exports, Oliphaunt package
...
- `xtask assets fetch` now clones/fetches every pinned source from
  `src/sources/src/third-party/**` into ignored `target/oliphaunt-sources/checkouts/**` directories,
...
- `src/docs/maintainers/sdk-parity-policy.md` links the inventory next to `src/docs/products/sdk-manifest.toml`, so
...
- `src/runtimes/liboliphaunt/native/tools/check-patch-stack.mts --write` generates
  `src/docs/internal/OLIPHAUNT_PATCH_STACK.md` from `src/runtimes/liboliphaunt/native/postgres18/source.toml`
...
- `src/benchmarks/perf/check-native-perf-report.sh` passes
...
- `OLIPHAUNT_REACT_NATIVE_ANDROID_ABI_FILTERS=all` or
  `OLIPHAUNT_KOTLIN_ANDROID_ABI_FILTERS=all` restores every locally buildable
  ABI; the published Android support envelope remains `arm64-v8a,x86_64`;
...
- `src/sdks/react-native/src/examples/expo` is an Expo SDK 56 development-build
...
- the smoke generates the ignored Expo `android/` project on demand, so a clean
  ...
  validation can run;
- `pnpm --dir src/sdks/react-native/src/examples/expo run smoke:android` exposes the same
...
- the React Native iOS Swift adapter now discovers bundled `liboliphaunt` from the
  ...
  packaged builds;
- `src/sdks/react-native/src/examples/expo` installs `expo-mcp` and exposes
...
- `src/benchmarks/perf/matrix/run_mobile_footprint_matrix.sh` enumerates the requested
