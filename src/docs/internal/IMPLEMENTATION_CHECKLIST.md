...
`src/docs/architecture/final-product-source-architecture.md`. Its checked state is
...
particular, derive CI selection from [`tools/graph/ci_plan.mts`](../../../tools/graph/ci_plan.mts),
extension membership from the
[`src/extensions/catalog`](../../extensions/catalog) and product target
manifests, artifact/package targets from
[`src/shared/product-metadata/release-artifact-targets.mts`](../../shared/product-metadata/release-artifact-targets.mts),
...
- [x] Stable CI job names are derived from Moon task `ci-*` tags. Evidence:
  `tools/graph/ci_plan.mts` and `tools/policy/check-moon-product-graph.mjs`.
...
- [x] Moon dependency scopes encode source/qualification impact; product-local compatibility metadata encodes published dependencies.
  Evidence: `bash tools/release/release-plan.sh --changed-file ... --format json`
...
- [x] Builds workflow has a builder-only aggregate. Evidence:
  `tools/graph/ci_plan.mts` emits `builder_jobs`, and the `Builds` GitHub job
...
- [x] Mobile-focused extension artifact builders are target-scoped. Evidence:
  direct `tools/graph/ci_plan.mts` probes show Android mobile builds select
...
- [x] Mobile build jobs inspect the produced app artifact for selected-extension
  ...
  `tools/dev/bun.sh tools/release/check-staged-artifacts.mts --require-mobile android
...
- [x] iOS selected-extension artifact inspection is link-aware instead of
  ...
  `tools/dev/bun.sh tools/release/check-staged-artifacts.mts --require-mobile ios
...
- [x] Android selected-extension artifact inspection is link-aware at the
  ...
  `bun test src/sdks/react-native/tools/validate-android-link-evidence.test.mts` asserts that
...
- [x] Swift SDK package artifacts render the public SwiftPM release manifest
  ...
  `oliphaunt-swift:release-package` fails closed unless that
...
- [x] Full non-PR Builds runs are deliverable builders by default. Evidence:
  `tools/graph/ci_plan.mts::planForFullRun()` starts from `BUILDER_JOBS`
...
- [x] Mobile app builders consume staged SDK, runtime, and exact-extension
  ...
  `check-staged-artifacts.mts --require-mobile-*-prebuilt-extensions`
...
- [x] Product-local `release.toml` files own registry/package metadata.
  Release code imports it directly from `release-graph.mts` and
  `release-artifact-targets.mts`; the workflow query wrapper exposes only the
...
- [x] `bash tools/release/release-plan.sh` uses Moon project ownership and dependency
...
- [x] WASIX runtime release download filters same-SHA CI runs by the `Builds`
  ...
  `.github/scripts/download-wasix-runtime-build-artifacts.mts` invokes
...
- [x] Exact-extension package assembly has no broad native-index fallback.
  Evidence: `src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts` now requires
...
- [x] Exact-extension release packages emit JSON manifest, ecosystem-friendly
  ...
  `src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts oliphaunt-extension-vector
...
- [x] SDK package checks prove wrapper packages do not ship runtime or
  ...
  `tools/dev/bun.sh tools/release/check-staged-artifacts.mts --inspect-present` validates staged
  ...
  runs `check-staged-artifacts.mts --require-sdk-product "$product"` for every
...
- [x] Node direct optional npm packages are built in the Builds workflow and
  ...
  `src/runtimes/node-direct/tools/package-node-direct-runtime.sh` emits both
...
- [x] WASIX Rust binding package artifacts stay binding-scoped and do not force
  ...
  `tools/dev/bun.sh tools/release/check-staged-artifacts.mts --require-sdk-product
...
- [x] `tools/dev/bun.sh tools/graph/ci_plan.mts --help`
...
- [x] `tools/dev/bun.sh tools/graph/ci_plan.mts` for a full run now selects only
...
- [x] `GITHUB_EVENT_NAME=workflow_dispatch NATIVE_TARGET=all
  ...
  tools/dev/bun.sh tools/graph/ci_plan.mts` now selects only
  ...
  `linux-x64-gnu`.
- [x] `bash tools/release/release-plan.sh`
- [x] `bash tools/release/release-check.sh`
- [x] `bash tools/release/release-dry-run.sh --products-json
...
- [x] `GITHUB_EVENT_NAME=workflow_dispatch NATIVE_TARGET=all
  WASM_TARGET=all MOBILE_TARGET=ios tools/dev/bun.sh tools/graph/ci_plan.mts`
- [x] `GITHUB_EVENT_NAME=workflow_dispatch NATIVE_TARGET=all
  WASM_TARGET=all MOBILE_TARGET=android tools/dev/bun.sh tools/graph/ci_plan.mts`
- [x] `tools/graph/ci_plan.mts` direct probe for
...
- [x] `tools/graph/ci_plan.mts` direct probes for
...
- [x] `tools/graph/ci_plan.mts` direct probe for a single
  ...
  WASIX row.
- [x] `tools/dev/bun.sh tools/release/check-staged-artifacts.mts
...
- [x] `tools/dev/bun.sh tools/release/check-staged-artifacts.mts --require-mobile ios
  --require-mobile-prebuilt-extensions` passes after rebuilding
  `pnpm --dir src/examples/react-native-expo run mobile-build:ios` with
...
- [x] `GITHUB_EVENT_NAME=workflow_dispatch NATIVE_TARGET=ios-xcframework
  WASM_TARGET=all MOBILE_TARGET=all tools/dev/bun.sh tools/graph/ci_plan.mts`
...
- [x] `bash src/runtimes/node-direct/tools/package-node-direct-runtime.sh`
- [x] `tools/dev/bun.sh src/extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts
...
- [x] `bash tools/release/release-dry-run.sh`
...
- [x] Local PR 38 CI hardening checks passed after fixing the observed builder
  ...
  touched native scripts, `sh -n src/runtimes/node-direct/tools/package-node-direct-runtime.sh`,
...
- [x] GitHub Builds run `27383810080` on `d7ad6eca` proved the next CI-only
  ...
  `BE_DLLLIBS=-bundle_loader ../../backend/postgres` won and failed when
  ...
  `python3 src/extensions/tools/check-extension-model.sh --check`,
  `python3 tools/release/check_artifact_targets.py`,
  `python3 tools/policy/check-release-policy.py`,
  `moon run extensions:lint`,
...
- [x] GitHub Builds run `27388349669` on `274f86a6` proved the Android
  ...
  selected dependencies/src/extensions/stems and adds a mobile policy assertion for
...
- [x] Local release version freshness no longer blocks the selected product
  closure. `tools/dev/bun.sh tools/release/check_release_versions.mts --products-json
...
- [x] The WASIX Rust publishing surface now uses the WASIX product name instead
  ...
  packages, `bash tools/release/release-check.sh` passes, and
  `bash tools/release/release-check-registries.sh --products-json "$(cat
...
- [x] GitHub Builds run `27448574605` on `927457d3` proved the native
  ...
  later because `src/extensions/postgis_extension_helper.sql` was not generated; the
  ...
  `bash src/sources/tools/fetch-sources.sh native-runtime --force`,
  `bash src/sources/tools/fetch-sources.sh wasix-runtime --force`,
  `node tools/policy/check-moon-product-graph.mjs`,
  `bash tools/policy/check-tooling-stack.sh`,
...
- [ ] Release workflow dry-run green for selected products. Historical note:
  an earlier local check used `release-check-registries.sh
...
- [x] Windows native exact-extension coverage has a producer path for all nine
  ...
  `python3 src/extensions/tools/check-extension-model.sh --write-evidence`,
  `python3 src/extensions/tools/check-extension-model.sh --check`,
...
- [x] Native mobile CI target staging now uses the Bun
  `build-ci-target.mts` wrapper from Moon. The retired shell wrapper is blocked
