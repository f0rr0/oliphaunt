...
> [`src/docs/maintainers/release.md`](../maintainers/release.md),
> [`src/docs/maintainers/release-setup.md`](../maintainers/release-setup.md), the
...
- extensions: extensions are opt-in in the Rust model, and the packaged
  ...
  physical restore checks across src/broker/direct-C-ABI and server paths; pgGraph
...
- The gated native extension matrix now creates or loads every currently
  release-ready exact extension through src/broker/direct-C-ABI and server paths, reopens the
...
src/benchmarks/perf/check-native-perf-report.sh
```
