...
> `src/docs/maintainers/README.md`, and executable product metadata describe the
...
[`Capability Matrix`](../content/reference/capabilities.mdx),
...
- `src/benchmarks/`: benchmark plans and future cross-engine harnesses.
...
- `src/docs/`: architecture, release, development, maintainer, and internal source
  material.
- `src/docs/internal/`: maintainer-only progress notes and patch-stack
  audits.
...
- the gated native extension matrix creates or loads release-ready PostgreSQL 18
  ...
  through src/broker/direct-C-ABI and server paths;
...
```sh
src/benchmarks/perf/matrix/run_native_oliphaunt_matrix.sh
```
