...
direct `pg_dump`/non-interactive `psql`; TypeScript keeps sockets absent in
browsers while its direct tools remain available there through a private
in-process pgwire bridge. The TypeScript binding also owns worker-isolated host
adapters, pgwire error recovery, callback-scoped transactions, and
host-specific persistence (IndexedDB or OPFS in browsers, directories on
Node/Bun/Deno). Shared concepts and carriers do not force identical host code
or language signatures.
