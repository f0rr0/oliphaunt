...
runtime assets and the WASIX physical archive contract. Rust alone owns server
hosting and optional logical dump tooling. The TypeScript binding owns
direct and worker-isolated browser, Node, Bun, and Deno adapters, pgwire error
recovery, callback-scoped transactions, optional host-specific persistence
(IndexedDB or OPFS in browsers, directories on Node/Bun/Deno), and a narrow asynchronous API.
