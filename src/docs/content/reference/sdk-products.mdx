...
| WASIX TypeScript | `@oliphaunt/wasix-ts` | Caller-realm root; explicit `/worker` entry point on browser, Node, Bun, or Deno | Memory filesystem |
...
in browsers: `pgDump` supports direct or Worker handles, while `psql` requires
a Worker handle and uses a private pgwire bridge. The
