...
| WASIX TypeScript | `@oliphaunt/wasix-ts` | Browser, Node, Bun, or Deno direct/Worker placement | Memory filesystem |
...
The native TypeScript npm package carries native runtime integration.
The JSR distribution is deliberately limited to portable protocol and query helpers.
WASIX TypeScript is a separate product: npm supports every declared
host, while `jsr:@oliphaunt/wasix-ts` exposes the full Deno WASIX facade and its
Deno directory-storage subpath.
...
direct and worker-isolated browser, Node, Bun, and Deno adapters, pgwire error
recovery, callback-scoped transactions, optional host-specific snapshots
(IndexedDB in browsers, directories on Node/Bun/Deno), and a narrow asynchronous API.
