...
| `pg_dump` / `psql` | Optional endpoint-oriented `oliphaunt-tools` / `@oliphaunt/tools` products on Rust and desktop TypeScript | Optional direct-handle `tools` feature | Optional direct-handle `@oliphaunt/wasix-tools` package on every host |
