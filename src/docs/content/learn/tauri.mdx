...
Use `oliphaunt::worker::Oliphaunt` with direct topology when async Tauri
...
Store the resulting worker handle with `tauri::State`. Its clones share one
FIFO owner thread and one PostgreSQL session. The synchronous crate-root
`oliphaunt::Oliphaunt` is deliberately `!Send + !Sync`; use it only when one
ordinary Rust thread owns the database directly. Server mode is the path for
