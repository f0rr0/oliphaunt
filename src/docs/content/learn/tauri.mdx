...
Configure the [native runtime resources](/docs/sdk/rust#configure-the-native-runtime) before starting the Tauri process. Packaged applications must ship the complete runtime resources and configure the library/resource paths in their launcher; the database-state examples below do not perform that packaging.
