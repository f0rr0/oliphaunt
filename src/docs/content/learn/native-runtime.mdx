...
native SDKs. Rust WASIX and WASIX TypeScript are documented separately
under [`WASIX`](/docs/sdk/wasix).
...
- choose `NativeBroker` when a desktop app wants the database runtime outside
  the UI process or needs to manage several database instances deliberately;
...
| --- | --- | --- | --- | --- |
| `NativeDirect` | in-process | one serialized physical session | one resident instance per process | same-instance logical reopen; WAL recovery after process relaunch |
...
process exits, the next launch reopens the same persistent storage and PostgreSQL performs WAL
...
Native live storage is a PostgreSQL directory. It contains PGDATA,
