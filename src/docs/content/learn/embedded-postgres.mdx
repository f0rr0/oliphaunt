...
| WASIX TypeScript `@oliphaunt/wasix-ts` | Browser, Node, Bun, or Deno direct/Worker placement with memory; browsers may opt into IndexedDB or OPFS persistence and Node, Bun, and Deno hosts may opt into directory persistence |
...
| Checkpoint | Request a PostgreSQL checkpoint; WASIX TypeScript then publishes the durable PGDATA delta through its selected provider |
...
Applications decide when platform lifecycle events trigger checkpoint,
