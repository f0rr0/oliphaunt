...
| WASIX TypeScript `@oliphaunt/wasix-ts` | Package-owned Worker by default or explicit `/blocking` entry point on browser, Node, Bun, or Deno; browsers may opt into IndexedDB or OPFS persistence and Node, Bun, and Deno hosts may opt into directory persistence |
