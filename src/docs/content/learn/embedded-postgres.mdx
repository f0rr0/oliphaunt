...
| WASIX TypeScript `@oliphaunt/wasix-ts` | Caller-realm root or explicit `/worker` entry point on browser, Node, Bun, or Deno; browsers may opt into IndexedDB or OPFS persistence and Node, Bun, and Deno hosts may opt into directory persistence |
