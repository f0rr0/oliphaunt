...
Use `@oliphaunt/wasix-ts/browser` for an explicit browser import. Its `open` and
`restore` types reject directory storage; native entrypoints reject IndexedDB
and OPFS. Runtime validation also protects JavaScript callers. The `/browser`
entrypoint fails early on Node.js, Bun, and Deno. One package installation still
includes both hosts, but native entrypoints do not load the browser engine.
