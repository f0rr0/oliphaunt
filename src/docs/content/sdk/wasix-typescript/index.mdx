...
`@oliphaunt/wasix-tools` adds direct `pg_dump` and non-interactive `psql` on
every host. Explicit server subpaths add a local endpoint on Node, Bun, and
Deno. Cancellation and a dedicated typed COPY API remain absent.
...
- Keep tools on worker-backed databases. Use the lightweight local endpoint for
  one compatibility client, or the separate postmaster product for concurrency.
