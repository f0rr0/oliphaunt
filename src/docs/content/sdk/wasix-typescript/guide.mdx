...
Install `@oliphaunt/extension-vector-wasix` separately. Contrib ships with the
base SDK; select both descriptors explicitly:

```ts
import { Oliphaunt, extensions } from '@oliphaunt/wasix-ts';
import vector from '@oliphaunt/extension-vector-wasix';

const database = await Oliphaunt.open({ extensions: [vector, extensions.hstore] });
await database.execute('CREATE EXTENSION vector; CREATE EXTENSION hstore');
```
...
the addon receives the installed package payloads and validates their owner,
runtime compatibility, target, and hashes before loading them. External bytes
are owned by their packages; the base addon includes the runtime and contrib.
