...
Tools work in browsers, Node.js, Bun, and Deno against a worker-backed database.
`pgDump()` returns PostgreSQL's ordinary plain UTF-8 dump, including its normal
COPY statements. It does not force inserts or rewrite valid SQL. `psql()` is
...
Direct tools exclusively own and reset the embedded PostgreSQL session. Do not
