...
With the default Worker entry point, OPFS uses browser synchronous access
handles for exact-range PostgreSQL file I/O when available. The blocking entry
point and browsers without that facility use the portable journal against the
same opaque OPFS format. This selection is automatic and adds no public mode or
tuning option.
...
Use the root entry point for the default package-owned Worker. Import the
explicit blocking entry point only when blocking the caller's realm during
PostgreSQL work is acceptable:

```ts
import OliphauntBlocking from '@oliphaunt/wasix-ts/blocking';

const database = await OliphauntBlocking.open();
```

Both entry points return the same `OliphauntDatabase` contract. Blocking open
and operations remain promise-shaped for loading and durable publication, but
synchronous guest execution occurs before the caller's realm can process other
work. There is no runtime fallback, capability profile, or host-mode
negotiation.
...
After it expires, close waits for forced Worker termination to settle before
releasing other owned resources, so storage cleanup never races a live Worker.
...
Tools work in browsers, Node.js, Bun, and Deno. `pgDump()` accepts a default
Worker or explicit blocking handle and returns PostgreSQL's ordinary plain UTF-8 dump, including
its normal COPY statements. It does not force inserts or rewrite valid SQL.
`psql()` requires a default Worker handle because COPY restore is full duplex; it is
...
or reenter the same database or transaction. Typed query results remain
buffered.
