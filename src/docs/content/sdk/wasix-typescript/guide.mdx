...
New browser storage requires a seed. The imports above use a browser bundler's
asset URL support; seed packages are independently installed development
...
For ICU, use `@oliphaunt/seed-wasix-icu` instead and pass
...
required when reopening an ICU database; the seed does not.
