...
Configure storage, runtime mode, selected exact extensions, startup identity,
and durability through the Swift configuration API. Storage defaults to
...
formats and destination directories before calling into the native runtime.
`OliphauntRestoreRequest` defaults to `.failIfExists`; call
