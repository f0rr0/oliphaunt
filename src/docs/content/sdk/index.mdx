...
- Choose WASIX TypeScript (`@oliphaunt/wasix`) when portable PostgreSQL runs
  directly or in a module Worker in a browser, or in a Node worker thread.
  Memory is the default; extensions are selective imports and IndexedDB is a
  browser-only opt-in.
...
`open`, `query`, `execute`, raw protocol, `checkpoint`, and `close`.
...
- Use [WASIX TypeScript](/docs/sdk/wasix/browser-typescript) for the browser and
  Node Worker, error-recovery, IndexedDB, and selective-extension contract.
