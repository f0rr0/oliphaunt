...
- Choose WASIX TypeScript (`@oliphaunt/wasix-ts`) when portable PostgreSQL runs
  directly in the importing realm or through the explicit package-owned Worker
...
optional tools package adds `pgDump` for direct or Worker handles and `psql`
for Worker handles, while explicit server subpaths add a local
...
- Use [WASIX TypeScript](/docs/sdk/wasix-typescript) for direct caller-realm or
  explicit Worker-owned browser, Node, Bun, and Deno execution, error recovery,
