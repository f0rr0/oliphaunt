...
- Choose WASIX TypeScript (`@oliphaunt/wasix-ts`) when portable PostgreSQL runs
  directly or in a module Worker in a browser, or in a Node worker thread.
...
| Which host runs PostgreSQL? | Native process, Rust WASIX host, browser caller realm or Worker, and Node worker thread have different lifecycle behavior. |
...
- Use [WASIX TypeScript](/docs/sdk/wasix/browser-typescript) for direct or
  worker-isolated browser execution, Node worker execution, error recovery,
