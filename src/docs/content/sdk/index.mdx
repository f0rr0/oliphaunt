...
- Choose WASIX TypeScript (`@oliphaunt/wasix`) when a browser module Worker or
  Node worker thread owns portable PostgreSQL. Memory is the default;
  extensions are selective imports and IndexedDB is a browser-only opt-in.
...
| Which host runs PostgreSQL? | Native process, Rust WASIX host, browser Worker, and Node worker thread have different lifecycle behavior. |
