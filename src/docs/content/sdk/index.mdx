...
engine modes. Rust WASIX adds Rust host, server, physical archive, and
optional dump tooling. WASIX TypeScript exposes `open`, typed and raw query
operations, callback transactions, `checkpoint`, physical `backup`, static
`restore`, and `close`; it does not expose Rust's server or PostgreSQL tool APIs.
