...
- Choose WASIX TypeScript (`@oliphaunt/wasix-ts`) when portable PostgreSQL runs
  in the package-owned Worker default or through the explicit caller-blocking
  entrypoint in a browser, Node.js, Bun, or Deno.
...
optional tools package adds `pgDump` for blocking or default Worker handles
and `psql` for default Worker handles, while explicit server subpaths add a local
...
- Use [WASIX TypeScript](/docs/sdk/wasix-typescript) for default Worker-owned
  browser, Node, Bun, and Deno execution or its explicit blocking entry point,
  error recovery,
