...
<Callout type="warn" title="Linux initialization issue in 0.2.0">
The published native package currently fails to initialize a fresh database on Linux x64 under Node.js and Bun with a PostgreSQL data-directory permission error. The query example below reflects the API, but does not bypass this release defect. Use [WASIX TypeScript](/docs/sdk/wasix-typescript) if you need a verified Linux setup with the current releases.
</Callout>
