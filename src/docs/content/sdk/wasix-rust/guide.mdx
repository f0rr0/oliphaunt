...
Contrib ships with the base SDK. Install external packages independently and
select descriptors for both. Selection makes resources available; migrations
still own `CREATE EXTENSION` and `LOAD`.
...
let mut database = Oliphaunt::builder()
    ...
    .open()?;
database.execute("CREATE EXTENSION vector; CREATE EXTENSION hstore")?;
```
...
cluster seed and exact runtime/AOT asset pair, selected extension packages,
