...
database methods take `&mut self`. Applications that need to keep an async
executor responsive can import the cloneable `oliphaunt_wasix::worker`
...
For a dedicated owner thread, import the worker type explicitly:
...
The Rust binding exposes the default direct caller-thread API, the explicit
worker API, optional `pg_dump`/`psql`, and a local
PostgreSQL-compatible URL. WASIX TypeScript makes the same calling choice at
the package boundary: its root entry point runs in the importing realm, while
`@oliphaunt/wasix-ts/worker` is the explicit package-owned Worker surface.
Optional tools and Node, Bun, and Deno server subpaths remain language-specific.
