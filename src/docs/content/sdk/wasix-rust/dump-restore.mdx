...
core database API stays focused. They exclusively borrow the direct database
for the complete invocation and cannot run inside a callback transaction. The
tool connection resets PostgreSQL session state before and after it runs, so
...
script to the database `execute` method: plain `pg_dump` output can include
