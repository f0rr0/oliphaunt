...
Enable the `tools` feature for the optional `tools` namespace:
...
The tools are free functions rather than database or server methods, so the
core database API stays focused. On the root async API they take
`&Oliphaunt`, then queue one exclusive operation on its owner; no `&mut` handle
is required. They cannot run inside a callback transaction. The tool connection
resets PostgreSQL session state before and after it runs, so
prepared statements and session settings on the direct handle do not survive.
`tools::pg_dump()` returns UTF-8 SQL text and `tools::psql()` returns UTF-8
...
The explicit blocking surface provides synchronous equivalents when running
the tool on the current thread is intentional:

```rust,no_run
use oliphaunt_wasix::blocking::{Oliphaunt, tools};

fn run_blocking() -> oliphaunt_wasix::Result<()> {
    let mut database = Oliphaunt::open()?;
    let sql = tools::pg_dump(&mut database, tools::PgDumpOptions::new())?;
    assert!(!sql.is_empty());
    database.close()
}
```
...
`Oliphaunt::restore(destination, bytes)`; both are awaited on the root API:

```rust,no_run
use oliphaunt_wasix::Oliphaunt;

async fn clone_database() -> oliphaunt_wasix::Result<()> {
    let source = Oliphaunt::open().await?;
    let archive = source.backup().await?;
    source.close().await?;
    Oliphaunt::restore("./restored", archive).await
}
```
...
Once its restore thread starts, dropping the future does not promise to cancel
filesystem publication.

For a logical restore, run
`tools::psql(&database, PsqlOptions::new().script(sql)).await`. Do not pass the
script to `database.execute(...).await`: plain `pg_dump` output can include
`psql` meta commands. Oliphaunt does not expose a second logical-restore
abstraction. Use `blocking::tools::psql(&mut database, ...)` only with a handle
explicitly imported from `oliphaunt_wasix::blocking`.
...
2. Create a logical dump with `tools::pg_dump(&database, ...).await`, or use
   `oliphaunt-wasix-dump`.
3. Open a fresh database with the new crate/runtime.
4. Feed the script through
   `tools::psql(&database, PsqlOptions::new().script(sql)).await`.
