...
the root uses contrib extensions, repeat `--extension NAME` for each required
contrib member. External descriptors are supplied through the Rust API:

```sh
oliphaunt-wasix-dump --directory ./.oliphaunt \
  --database app --username owner \
  --extension hstore --extension pg_trgm
```
