...
Open a PGDATA directory, send raw protocol bytes, read backend messages, and close the
handle.
...
directories, username, database, PostgreSQL startup arguments, and reserved
flags. Higher-level SDKs translate their storage, durability, and engine-mode
settings into this direct-runtime record.
...
`oliphaunt_capabilities()` returns `OLIPHAUNT_CAP_*` bits for the loaded
artifact. The C ABI itself is the native-direct boundary and owns one serialized
embedded session; choose broker or server through a language SDK when process
isolation or independent client sessions are required.
