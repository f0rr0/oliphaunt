...
| --- | --- | --- |
| Initialization | `oliphaunt_init`, `OliphauntConfig` | Open a native direct backend for a PGDATA directory |
| Versioning | `oliphaunt_version` | Report the runtime and PostgreSQL build identity |
| Capabilities | `oliphaunt_capabilities`, `OLIPHAUNT_CAP_*` | Read the runtime's capability bitmask |
...
| Data movement | `oliphaunt_backup`, `OliphauntBackupOptions`, `oliphaunt_restore`, `OliphauntRestoreOptions` | Move PGDATA through validated physical archives |
...
ABI v7 places the optional embedded module directory at
