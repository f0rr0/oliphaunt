...
Configure storage, mode, selected exact extensions, durability, and startup
identity through the JS API and config plugin. Build-time extension selection
...
artifacts in the installed app. `Oliphaunt.open()` controls storage, mode,
durability, and extension activation for that app run. Omit `storage` for an
...
mobile mode can logically reopen the same instance inside one resident app process.
...
Omit `destinationPolicy` to fail when the destination exists; use
`destinationPolicy: 'replaceExisting'` only when replacement is deliberate.
