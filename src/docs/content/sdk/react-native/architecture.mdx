...
That keeps platform behavior consistent. Storage validation, extension selection,
...
session. It can logically reopen the same instance inside that process. Broker and
...
The bridge owns at most one native direct open. It rejects another open while
the current one is pending, active, or closing, then permits logical reopen
after `close()` completes; it never aliases two JavaScript database objects to
one native handle.
