...
| macOS | Swift SDK where supported | Same API shape as iOS |
...
server modes add a process boundary on targets that advertise those modes.
...
Mode requests outside advertised capabilities fail with clear errors. Direct
mode remains one physical session; use a server-capable platform runtime when an
app needs independent PostgreSQL client sessions.
