...
from the callback, rejects the stream. `cancel()` remains independently
admitted so it can interrupt active native work. It remains callable after
close admission while earlier FIFO work drains and stops at native teardown.
