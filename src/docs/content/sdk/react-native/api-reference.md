...
| Raw protocol | `execProtocolRaw`, `execProtocolRawStream` | Send PostgreSQL protocol bytes as one result or synchronous callback chunks through JSI `ArrayBuffer`; callbacks cannot return thenables or reenter the same handle (`cancel` remains out of band) |
