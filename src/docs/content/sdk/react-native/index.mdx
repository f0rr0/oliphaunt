...
are queued through the platform SDK. `OpenConfig.engine` currently accepts
`nativeDirect` only; broker and server mode entries in capability reports are
discovery signals until the React Native bridge exposes those open paths.
