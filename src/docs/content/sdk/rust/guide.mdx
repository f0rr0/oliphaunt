...
```rust
use oliphaunt::{extensions, DatabaseStorage, Oliphaunt};
...
Select explicit extension descriptors before open. There are no selection aliases or
implicit selectors. Selecting `vector` makes its package resources and declared
dependencies available to that database. The base SDK also carries supported contrib
resources; select their descriptors separately when a database needs them.
