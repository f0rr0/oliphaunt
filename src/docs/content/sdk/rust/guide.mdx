...
SDK-session work queues fairly on one owner executor in every native mode.
Transactions pin the physical session until commit or rollback. `close()` establishes an ordered
cutoff: work already admitted drains, later application work is rejected, and
then the selected runtime closes or detaches. If pre-cutoff work leaves a
transaction active, close returns `TransactionActive`; finish that transaction
and retry. A required `COMMIT` or `ROLLBACK` remains admissible for a transaction
whose `BEGIN` crossed the cutoff first. Once runtime teardown starts, the handle
is terminal even when teardown fails: `is_closed()` is true, later work is
rejected, and repeated close calls return the same result. Use explicit
cancellation for long-running SQL. Cancellation remains out of band and
admissible while pre-cutoff work drains; it is rejected when destructive
teardown begins.
