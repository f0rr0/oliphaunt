...
Raw-stream callbacks are synchronous backpressure boundaries: same-handle
database and transaction work is rejected from their scope, except for
out-of-band `cancel()`. A callback failure is returned only after confirmed
...
`ABORT`, `PREPARE TRANSACTION`, or `AND CHAIN`. Nested `SAVEPOINT`, `RELEASE
...
transactional ready status, so `AND CHAIN` is unsupported contract misuse that
cannot be reliably distinguished before or after execution. Raw protocol APIs
stay on the database for callers that explicitly own transaction lifecycle.
