...
- `bun run --cwd src/benchmarks/perf/wasix-node bench:streaming`: quick local WASIX
...
[`src/docs/maintainers/sdk-parity-policy.md`](./sdk-parity-policy.md) for the SDK ownership contract. `full` enables
...
progress notes under `src/docs/internal/` are archived and non-normative; they are
