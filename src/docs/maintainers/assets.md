...
`src/third-party/**`, extension-owned source pins under
...
`src/third-party/icu/source.toml`, while the canonical official
little-endian data archive is independently pinned in
`src/database-resources/icu/source.toml`. Native and WASIX builders compile
