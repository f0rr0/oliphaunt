...
src/benchmarks/perf/run-native.sh target/perf/direct-rtt native-liboliphaunt --engine direct --suite rtt --iterations 100
src/benchmarks/perf/run-native.sh target/perf/postgres-rtt native-postgres --suite rtt --iterations 100
src/benchmarks/perf/run-native.sh target/perf/sqlite-speed sqlite --suite speed
```
...
bun run --cwd src/benchmarks/perf/wasix-node bench:streaming
```
...
`src/benchmarks/` remain historical evidence; they do not qualify current builds.
