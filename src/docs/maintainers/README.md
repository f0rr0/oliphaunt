...
`consumer-dx-release-blueprint.md` is archived design history. Files under `src/docs/internal/` are implementation history, investigations, or evidence snapshots unless a current maintainer document links to a specific section. They are not policy and must not be used to override the executable sources above.
