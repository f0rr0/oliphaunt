...
patches in `src/third-party/postgres/patches/wasix` and product-owned patches.
...
`target/`. Benchmark harnesses belong under `src/benchmarks/perf/` and durable benchmark
results under `src/benchmarks/`; neither belongs in the runtime product source.
