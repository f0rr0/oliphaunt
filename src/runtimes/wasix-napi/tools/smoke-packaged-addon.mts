...
const PGWIRE_CLIENT = pathToFileURL(
  path.join(WORKSPACE_ROOT, 'src/bindings/wasix-ts/tools/pgwire-client.mts'),
).href;
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!['--package-manager', '--runtime', '--target'].includes(argument) || !value) {
      throw new Error(
        'usage: smoke-packaged-addon.mts --target TARGET --runtime node|bun|deno|electron [--package-manager npm|pnpm]',
      );
    }
