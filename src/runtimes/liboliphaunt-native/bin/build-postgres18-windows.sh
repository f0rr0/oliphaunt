...
desired_hash="$(
  set -e
  {
    ...
    while IFS= read -r patch; do
      ...
      sha256 "$patch"
    done <src/runtimes/liboliphaunt-native/postgres/series
    while IFS= read -r -d '' input; do
      printf '%s\n' "$input"
      sha256 "$input"
    done < <(find src/runtimes/liboliphaunt-native/src src/runtimes/liboliphaunt-native/include src/runtimes/liboliphaunt-native/portable-uuid src/extensions/artifacts/native/tools -type f ! -name '*.test.*' ! -path '*/fixtures/*' ! -name '*.md' -print0 | sort -z)
    sha256 "${BASH_SOURCE[0]}"
    sha256 src/runtimes/liboliphaunt-native/bin/common.sh
