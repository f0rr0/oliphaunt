...
extractor="${OLIPHAUNT_NODE_HEADERS_EXTRACTOR:-$root/src/runtimes/node-direct/tools/extract-node-headers.mts}"
...
require node
runtime_version="$(node --version)" || die "could not read the active Node runtime version"
