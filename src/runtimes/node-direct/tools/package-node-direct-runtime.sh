...
version="$(node tools/dev/node-info.mts package-version src/runtimes/node-direct/package.json)"
node_include="${NODE_INCLUDE_DIR:-}"
if [ -z "$node_include" ]; then
  node_include="$(node src/runtimes/node-direct/tools/native-build-data.mts headers 2>/dev/null || true)"
fi
...
case "$platform" in
  ...
  windows)
    ...
    if [ -z "$node_lib" ]; then
      node_exec="$(to_shell_path "$(node tools/dev/node-info.mts executable)")"
    ...
    if [ -z "$node_lib" ]; then
      case "$arch" in
        ...
        *)
          echo "unsupported Node direct Windows architecture for node.lib: $arch" >&2
          exit 2
          ;;
      esac
      node_lib="$(
...
bash src/shared/artifact-packaging/strip-native-binaries.sh "$addon_file"

node "$root/src/runtimes/node-direct/tools/native-build-data.mts" check-addon "$addon_file" "$lifecycle_test_addon_file"
...
tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts stage "$asset_stage" --profile source-sdk
tools/dev/bun.sh src/shared/artifact-packaging/platform-binary-contract.mts --target "$target" --root "$asset_stage"
if [ "$platform" = "linux" ]; then
  src/shared/artifact-packaging/check-linux-consumer-baseline.sh --target "$target" --root "$asset_stage"
fi
src/shared/artifact-packaging/archive-directory.mts "$asset_stage" "$asset_dir/$asset"
...
src/shared/artifact-packaging/write-checksum-manifest.mts \
  --asset-dir "$asset_dir" \
...
tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts stage "$package_work" --profile source-sdk
...
tarball="$(
  PACK_JSON="$pack_json" PACK_DIR="$npm_package_dir" node "$root/src/runtimes/node-direct/tools/native-build-data.mts" pack-tarball
)"
...
tools/dev/bun.sh src/runtimes/node-direct/tools/check-release-assets.mts \
  --asset-dir "$asset_dir" \
  --allow-partial \
  --npm-package "$tarball"
