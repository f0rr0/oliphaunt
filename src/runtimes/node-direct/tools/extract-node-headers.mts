...
const REQUIRED_HEADERS = ['include/node/node_api.h', 'include/node/node.h', 'include/node/v8.h'];
...
function parseArchive(expanded, expectedRoot) {
  ...
  while (offset + BLOCK_SIZE <= expanded.length) {
    ...
    const relativePath = safeRelativePath(
      pendingLongName ?? archivePath(block),
      expectedRoot,
      type,
    );
    ...
    if (paths.has(collisionKey)) {
      fail(
        `tar archive contains a duplicate or case-colliding path: ${relativePath || expectedRoot}`,
      );
    }
...
function main() {
  ...
  if (
    ...
    fail(
      'usage: src/runtimes/node-direct/tools/extract-node-headers.mts <archive.tar.gz> <empty-destination> <expected-root>',
    );
  }
