...
impl ArtifactArgs {
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self> {
        ...
        while let Some(arg) = args.next() {
            ...
            if inline.is_some() && matches!(flag, "--help" | "--force") {
            ...
            match flag {
                ...
                "--runtime" => {
                    parsed.runtime = Some(PathBuf::from(value()?));
                }
                "--sql-name" => {
                    parsed.sql_name = Some(value()?);
                }
                "--native-runtime-version" | "--liboliphaunt-native-version" => {
                    parsed.native_runtime_version = Some(value()?);
                }
                "--license-profile" => {
                    parsed.license_profile =
                        Some(NativeExtensionArtifactLicenseProfile::parse(&value()?)?);
                }
                "--legal-files-root" => {
                    parsed.legal_files_root = Some(PathBuf::from(value()?));
                }
                "--license-file" | "--license-files" => {
                    push_paths(&mut parsed.license_files, &value()?);
                }
                "--format" => {
                    parsed.format = parse_format(&value()?)?;
                }
                "--creates-extension" => {
                    parsed.creates_extension = parse_bool(&value()?)?;
                }
                "--native-module-stem" => {
                    parsed.native_module_stem = Some(value()?);
                }
                "--native-module-file" => {
                    parsed.native_module_file = Some(value()?);
                }
                "--native-target" | "--target" => {
                    parsed.native_target = Some(value()?);
                }
                "--embedded-module-root" => {
                    parsed.embedded_module_root = Some(PathBuf::from(value()?));
                }
                "--dependency" | "--dependencies" => {
                    push_strings(&mut parsed.dependencies, &value()?);
                }
                "--data-file" | "--data-files" => {
                    push_paths(&mut parsed.data_files, &value()?);
                }
                "--extension-sql-file-name" | "--extension-sql-file-names" => {
                    push_strings(&mut parsed.extension_sql_file_names, &value()?);
                }
                "--extension-sql-file-prefix" | "--extension-sql-file-prefixes" => {
                    push_strings(&mut parsed.extension_sql_file_prefixes, &value()?);
                }
                "--shared-preload-library" | "--shared-preload-libraries" => {
                    push_strings(&mut parsed.shared_preload_libraries, &value()?);
                }
                "--mobile-static-archive" | "--mobile-static-archives" => {
                    push_mobile_static_archives(&mut parsed.mobile_static_archives, &value()?)?;
                }
                ...
                "--static-symbol-prefix" => {
                    parsed.static_symbol_prefix = Some(value()?);
                }
                "--static-symbol-alias" | "--static-symbol-aliases" => {
                    push_static_symbol_aliases(&mut parsed.static_symbol_aliases, &value()?)?;
                }
                _ => {
                    return Err(Error::InvalidConfig(format!("unknown argument '{arg}'")));
                }
...
mod tests {
    ...
    fn embedded_module_root_accepts_separate_and_equals_forms() {
        assert!(ArtifactArgs::parse(["--force=false".to_owned()]).is_err());
        assert!(ArtifactArgs::parse(["--embedded-module-root".to_owned()]).is_err());
