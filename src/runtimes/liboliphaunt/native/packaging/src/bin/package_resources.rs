        ...
        while let Some(arg) = args.next() {
            ...
            if inline.is_some()
                && matches!(
                    ...
                    "--help"
                        ...
                        | "--resolve-liboliphaunt-release"
                        | "--resolve-broker-release-assets"
                        | "--resolve-oliphaunt-broker-release"
                        | "--force"
                        | "--require-mobile-static-registry"
                        | "--require-signed-extension-index"
                        | "--require-signed-extension-indexes"
                )
            {
            ...
            match flag {
                ...
                "--liboliphaunt-native-version" => {
                    parsed.liboliphaunt_version = Some(value()?);
                }
                "--broker-version" | "--oliphaunt-broker-version" => {
                    parsed.broker_version = Some(value()?);
                }
                "--release-asset-base-url" => {
                    parsed.release_asset_base_url = Some(value()?);
                }
                "--broker-release-asset-base-url" => {
                    parsed.broker_release_asset_base_url = Some(value()?);
                }
                "--release-asset-cache" | "--release-asset-cache-dir" => {
                    parsed.release_asset_cache_dir = Some(PathBuf::from(value()?));
                }
                "--broker-release-asset-cache" | "--broker-release-asset-cache-dir" => {
                    parsed.broker_release_asset_cache_dir = Some(PathBuf::from(value()?));
                }
                "--release-asset-target" | "--release-target" => {
                    parsed.release_asset_target = Some(value()?);
                }
                "--broker-release-target" | "--broker-release-asset-target" => {
                    parsed.broker_release_asset_target = Some(value()?);
                }
                "--release-asset" => {
                    parsed.release_assets.push(value()?);
                    parsed.resolve_release_assets = true;
                }
                "--mode" => {
                    let value = value()?;
                ...
                "--extension-cache" | "--extension-artifact-cache" => {
                    parsed.extension_cache_dir = Some(PathBuf::from(value()?));
                }
                "--trusted-extension-index-key" => {
                    let (key_id, key) = parse_key_value(&value()?)?;
                    parsed
                ...
                "--trusted-extension-index-key-file" => {
                    let (key_id, key) = read_key_file_value(&value()?)?;
                    parsed
                        .trusted_extension_index_keys
                        .push(NativeExtensionArtifactIndexTrustRoot::new(key_id, key));
                    parsed.require_signed_extension_indexes = true;
