...
oliphaunt_icu_target_stamp() {
  ...
  local ldflags="${11}"
  {
    printf 'schema=oliphaunt-icu-target-v4\n'
    printf 'source=%s\n' "$(oliphaunt_icu_source_commit "$source_dir")"
    printf 'script=%s\n' "$(oliphaunt_icu_script_sha256 "$script_dir")"
    printf 'target=%s\n' "$target_label"
    printf 'host=%s\n' "$host"
    ...
    printf 'cxxflags=%s\n' "$cxxflags"
    printf 'ldflags=%s\n' "$ldflags"
    printf 'configure=static-data-static-libs-static-consumer-no-extra-target-tools-real-data-archive\n'
  } | shasum -a 256 | awk '{print $1}'
}
...
oliphaunt_icu_static_data_ready() {
  local data_archive="${1:?ICU data archive is required}"
  [ -f "$data_archive" ] || return 1
  local members
  members="$(ar -t "$data_archive")" || return 1
  if grep -Eq '^stubdata\.ao/?$' <<< "$members"; then
    return 1
  fi
...
oliphaunt_icu_artifacts_ready() {
  ...
  oliphaunt_icu_static_data_ready "$prefix/lib/libicudata.a"
}
...
oliphaunt_icu_linked_symbols_ready() {
  ...
  grep -Eq '(^|[[:space:]])_?udata_setCommonData(_[0-9]+)?($|[[:space:]])' <<< "$symbols" || return 1
  grep -Eq '(^|[[:space:]])_?ucol_open(_[0-9]+)?($|[[:space:]])' <<< "$symbols" || return 1
  grep -Eq '(^|[[:space:]])_?icudt[0-9]+[a-z]*_dat($|[[:space:]])' <<< "$symbols"
}
...
oliphaunt_icu_build_target() {
  ...
  if [ -f "$stamp_file" ] &&
     [ "$(cat "$stamp_file")" = "$stamp" ] &&
     [ -f "$prefix/include/unicode/ucol.h" ] &&
     [ -f "$prefix/lib/libicui18n.a" ] &&
     [ -f "$prefix/lib/libicuuc.a" ] &&
     oliphaunt_icu_static_data_ready "$prefix/lib/libicudata.a"; then
    return 0
  fi
  ...
  (
    ...
    make -j"$jobs"
    make install
    make -j"$jobs" -C data packagedata
    make -C data install-local DESTDIR=
  )
