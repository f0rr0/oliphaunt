...
build_postgis_libiconv_dependency() {
  ...
  if [ ! -f "$source_dir/configure" ] || [ ! -f "$source_dir/.oliphaunt-source-pin" ]; then
    oliphaunt_postgis_fail \
      "pinned libiconv source checkout is missing; run bash src/sources/tools/fetch-sources.sh native-runtime --force"
  fi
  bash \
    "$repo_root/src/sources/tools/fetch-sources.sh" extensions --verify-only >/dev/null
  rm -rf "$build_root" "$dependency_dir"
