...
mkdir -p "$owner/tools" "$owner/bin" src/postgres/versions/18 bin
cp "$root/$owner/tools/release-runtime.sh" "$root/$owner/tools/build-ci-target.sh" "$owner/tools/"
cp "$root/src/postgres/versions/18/source.toml" src/postgres/versions/18/
...
STUB
chmod +x bin/*
for name in build-postgres18-linux build-postgres18-macos build-postgres18-android-arm64 build-postgres18-android-x86_64 build-ios-xcframework; do cp bin/product-build "$owner/bin/$name.sh"; done
for platform in linux macos; do cp bin/product-build "$owner/tools/package-liboliphaunt-$platform-assets.sh"; done
...
for pair in Linux:linux-x64-gnu Darwin:macos-arm64 MINGW64_NT:windows-x64-msvc; do
  ...
  if [ "$TEST_HOST" = MINGW64_NT ]; then grep -q '\.ps1' "$CAPTURE"; else grep -q "desktop-release-assets/$OLIPHAUNT_CI_TARGET 0 0" "$CAPTURE"; fi
done
: >"$CAPTURE"
