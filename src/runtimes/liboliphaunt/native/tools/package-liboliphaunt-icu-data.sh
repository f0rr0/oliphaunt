...
[ -n "$source_dir" ] && [ -n "$out_dir" ] ||
  fail "usage: src/runtimes/liboliphaunt/native/tools/package-liboliphaunt-icu-data.sh SOURCE_DIR OUTPUT_DIR"
...
tools/dev/bun.sh src/shared/cluster-seed-contract/icu-data.mts \
  "$stage/share/icu" \
  "$stage/manifest.properties"
tools/dev/bun.sh src/runtimes/liboliphaunt/native/tools/write-icu-package-size-report.mts \
  "$stage/share/icu" \
  "$stage/package-size.tsv"

tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts stage "$stage" --profile native-icu-data

src/shared/artifact-packaging/archive-directory.mts "$stage" "$partial"
tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts check-archive "$partial" --profile native-icu-data
