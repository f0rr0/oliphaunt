...
} from '../../../../shared/artifact-packaging/native-mobile-abi-contract.mts';
...
import { validateNativeIcuDataManifestRows } from '../../../../shared/cluster-seed-contract/icu-data.mts';
import {
  parseProperties,
...
import { NATIVE_RUNTIME_CARRIER_SCHEMA } from './native-runtime-carrier-contract.mts';
...
function validateIcuDataArtifactContents(file) {
  ...
  if (unexpected.length > 0) {
    fail(
      `${file} must contain only ICU data and its receipt, found: ${unexpected.slice(0, 5).join(', ')}`,
    );
  }
