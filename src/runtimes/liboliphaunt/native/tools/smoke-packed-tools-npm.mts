...
async function prepareConsumer(assetDir, scratch) {
  ...
  const { currentProductVersionSync } = await import(
    pathToFileURL(
      path.join(repositoryRoot, 'src/shared/product-metadata/release-artifact-targets.mts'),
    ).href
  );
  const { liboliphauntToolsNpmTarballs } = await import(
    pathToFileURL(
      path.join(repositoryRoot, 'src/runtimes/liboliphaunt/native/tools/package-carriers.mts'),
    ).href
  );
