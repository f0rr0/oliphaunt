...
archive_staged_dir() {
  ...
  src/shared/artifact-packaging/archive-directory.mts "$staged" "$out_dir/${name}.tar.gz"
  tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts check-archive \
    "$out_dir/${name}.tar.gz" \
    --profile "$profile"
}
...
package_android() {
  ...
  tools/dev/bun.sh src/shared/artifact-packaging/native-mobile-abi-contract.mts write \
  ...
  bash src/shared/artifact-packaging/strip-native-binaries.sh --target "$target_id" "$stage"
  ...
  tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts stage \
    "$stage" \
...
package_ios() {
  ...
  tools/dev/bun.sh src/shared/artifact-packaging/native-mobile-abi-contract.mts compare \
    --domain ios-datum64 \
  ...
  bash src/shared/artifact-packaging/strip-native-binaries.sh --target "$target_id" "$stage_ios"
  echo "==> Verifying staged liboliphaunt iOS binary compatibility"
  tools/dev/bun.sh src/shared/artifact-packaging/platform-binary-contract.mts --target "$target_id" --root "$stage_ios"
  ...
  src/runtimes/liboliphaunt/native/tools/package-liboliphaunt-icu-data.sh "$icu_source" "$out_dir"
}
