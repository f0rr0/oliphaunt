...
test('release archive validation requires canonical producer markers', () => {
  ...
  assert.match(
    canonicalTarEntryMarkerError('runtime', '5'),
    /directory member must use a trailing slash/u,
  );
  assert.match(
    canonicalTarEntryMarkerError('runtime\/manifest.properties/', '0'),
    /regular-file member must not use a trailing slash/u,
  );
});
...
test('aggregate validation reads Apple notices from the canonical XCFramework member root', (t) => {
  ...
  const result = spawnSync(
    ...
    ['src/shared/artifact-packaging/archive-directory.mts', '--keep-parent', framework, archive],
  ...
  const missingNoticeResult = spawnSync(
    path.join(ROOT, 'tools/dev/bun.sh'),
    [
      'src/shared/artifact-packaging/archive-directory.mts',
      '--keep-parent',
      framework,
