...
build_dir="$work_root/postgresql-$pg_version-build"
...
if [ "$smoke_only" = 0 ]; then
  ...
  if [ "$platform" = macos ]; then
    ...
    clang="$(xcrun --sdk iphonesimulator --find clang 2>/dev/null || true)"
    if [ -n "$sdk_path" ] && [ -n "$clang" ]; then
      for source in liboliphaunt_native liboliphaunt_error liboliphaunt_runtime liboliphaunt_protocol liboliphaunt_config liboliphaunt_process liboliphaunt_trace liboliphaunt_fs liboliphaunt_backup_state liboliphaunt_archive liboliphaunt_archive_tar liboliphaunt_static_extensions liboliphaunt_builtin_extensions; do
        "$clang" -std=c11 -Wall -Wextra -Werror -Wpedantic -Werror=unguarded-availability-new -fsyntax-only \
          -target arm64-apple-ios17.0-simulator -mios-simulator-version-min=17.0 -isysroot "$sdk_path" \
          -I "$include_dir" -I "$source_dir" "$source_dir/$source.c"
      done
    else echo 'skipping iOS C source syntax check: iPhoneSimulator SDK is unavailable' >&2; fi
  fi
fi
[ "$abi_only" = 0 ] || exit 0
require_file "$initdb"
...
archive_fixture="$root/src/shared/fixtures/storage/physical-archive-native-v1.properties"
