...
const EXPECTED_TOUCHPOINTS = new Map([
  ...
  ['src/bin/initdb/initdb.c', 'Skips ICU collation setup when optional ICU data is absent during WASIX initdb bootstrap.'],
  ...
  ['src/port/pg_strong_random.c', 'Batches checked WASI entropy reads for the single-backend runtime.'],
...
const REQUIRED_AUDIT_CHECKS = [
  ...
  },
  {
    ...
    posture: 'The embedded backend amortizes host entropy calls without changing failure handling or any concurrent PostgreSQL build.',
  },
  {
