...
} from '../../../../shared/artifact-packaging/npm-trusted-publishing.mts';
import {
  canonicalGzipSync,
...
function assertCoreManifestContract(manifest, runtimeEntries, standardSeedEntries, seedManifest) {
  ...
  if (runtimeModuleSha256 !== standardSeedRuntimeModuleSha256) {
    fail(
      'frozen WASIX core manifest runtime and standard cluster seed identify different runtime modules',
    );
  }
  if (sourceFingerprint !== standardSeedFingerprint) {
    fail(
      'frozen WASIX core manifest runtime and standard cluster seed identify different source fingerprints',
    );
  }
  if (runtimePostgresMajor !== standardSeedPostgresMajor) {
    fail(
      'frozen WASIX core manifest runtime and standard cluster seed identify different PostgreSQL majors',
    );
  }
  ...
  const pgVersion = Buffer.from(
    requireRegularEntry(
      standardSeedEntries,
      'PG_VERSION',
      'frozen WASIX standard cluster seed archive',
    ).data(),
  )
    .toString('utf8')
    .trim();
...
export function wasixRuntimeNpmInputs({ portableReleaseArchive }) {
  ...
  if (
    standardSeedBytes.length !== standardSeed.size ||
    sha256Bytes(standardSeedBytes) !== standardSeed.sha256
  ) {
