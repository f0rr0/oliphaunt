...
import { RUST_BUILD_SCRIPT_SHA256 } from '../../../../shared/artifact-packaging/rust-build-script-sha256.mts';
...
import { compareText } from '../../../../shared/product-metadata/release-graph.mts';
import {
...
function validateToolsPayload(root) {
  ...
  if (!sameSet(actual, expected)) {
    fail(
      `WASIX tools Cargo payload file set mismatch for ${rel(root)}: expected ${JSON.stringify([...expected].sort(compareText))}, got ${JSON.stringify([...actual].sort(compareText))}`,
    );
  }
}
...
function splitAotToolsPayload(aotRoot, extractRoot, targetId) {
  ...
  const missing = TOOLS_AOT_ARTIFACTS.filter(
    (name) => !toolsArtifacts.some((item) => item.name === name),
  ).sort(compareText);
  if (missing.length > 0) {
  ...
  for (const [targetRoot, targetArtifacts] of [
    [coreRoot, coreArtifacts],
    [toolsRoot, toolsArtifacts],
  ]) {
...
function rewriteCargoManifest(
  manifest,
  { packageName, version, extensionSources, extensionAotSources, noticeProfile },
) {
  let text = readFileSync(manifest, 'utf8');
...
function cargoPackage(crateDir, targetDir) {
  const manifest = path.join(crateDir, 'Cargo.toml');
...
function packageSpec(
  spec,
  { version, sourceRoot, outputDir, cargoTargetDir, extensionSources, extensionAotSources },
) {
  const crateDir = copyPackageSource(
    spec,
    sourceRoot,
    version,
    extensionSources,
