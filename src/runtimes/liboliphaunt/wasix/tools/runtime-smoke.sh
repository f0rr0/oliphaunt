...
if [ "$asset_mode" = "full" ]; then
  cargo_features=(--features extensions)
fi
...
cargo test -p oliphaunt-wasix --locked "${cargo_features[@]}" \
  --test runtime_smoke \
  --test proxy_smoke \
  --test cli_smoke \
...
if [ "$asset_mode" = "full" ]; then
  if [ "$mode" = "regression" ]; then
    cargo test -p oliphaunt-wasix --locked "${cargo_features[@]}" --test client_compat -- --nocapture --test-threads=1
  fi
  cargo test -p oliphaunt-wasix --locked "${cargo_features[@]}" --lib pg_dump -- --nocapture
else
