...
`,
  ...
  test("streams nested portable archives larger than spawnSync's default buffer", () => {
    ...
    run(
      'tar',
      ['--format', 'ustar', '-czf', carrier, '-C', path.join(root, 'stage'), carrierRoot],
      {
  ...
  test('extension packaging rejects AOT raw-digest tampering before Cargo packaging', () => {
    ...
    const result = spawnSync(
      'bun',
      [
        'src/runtimes/liboliphaunt/wasix/tools/package_liboliphaunt_wasix_cargo_artifacts.mts',
        '--extensions-only',
  ...
  test('splits from part-001 and a single carrier feature selects earthdistance plus cube only', {
    ...
    expect(
      Object.keys(runtimeManifest.dependencies).filter((name) => name === carrierName),
    ).toEqual([carrierName]);
