...
import {
  ICU_DATA_FORM,
  ICU_DATA_VERSION,
} from '../../../../shared/cluster-seed-contract/icu-data.mts';
...
export function validatePortableReleaseAsset(archive) {
  ...
  if (missingToolPayloads.length > 0) {
    fail(
      `${rel(archive)} must include split WASIX tool payloads for registry tools crates: ${missingToolPayloads.join(', ')}`,
    );
  }
  ...
  const seed = readArchiveJsonEntry(
    entries,
    WASIX_PORTABLE_RELEASE_MEMBERS.icuSeedManifest,
    archive,
  );
  return validateIcuSeedIdentity(manifest, seed).dataTreeSha256;
}

export function validateIcuSeedIdentity(manifest, seed) {
  const data = seed.icu;
  const closure = manifest['cluster-seeds']?.icu;
  if (
    ...
    !LOWER_SHA256.test(data?.dataTreeSha256 ?? '') ||
    closure?.manifest !== 'cluster-seeds/icu.json' ||
    closure['icu-data-tree-sha256'] !== data.dataTreeSha256
  )
    fail('inconsistent ICU seed identity metadata');
  return data;
}

export function validateIcuReleaseAsset(archive, expectedDigest) {
  const entries = checkedWasixArchiveEntries(archive, 'wasix-icu-data');
  const members = new Set(entries.keys());
  ...
  const expectedMembers = new Set([...icuEntries, ...expectedReleaseNoticeFiles('wasix-icu-data')]);
  ...
  const digest = logicalTreeSha256(archiveLogicalTreeRows(entries, archive, icuRoot + '/'));
  if (expectedDigest !== undefined && digest !== expectedDigest)
    fail(rel(archive) + ': ICU data does not match the runtime seed identity');
  return digest;
}
...
function validateAssetContents(assetDir, version) {
  const icuDigest = validatePortableReleaseAsset(
    path.join(assetDir, `${PRODUCT}-${version}-runtime-portable.tar.zst`),
  );
  validateIcuReleaseAsset(path.join(assetDir, `${PRODUCT}-${version}-icu-data.tar.zst`), icuDigest);
