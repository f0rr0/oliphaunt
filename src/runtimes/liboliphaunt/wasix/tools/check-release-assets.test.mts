...
test('downloader selects the same archive for a target id and its compilation triple', () => {
  const cli = path.join(import.meta.dirname, 'wasix-cargo-artifact-contract.mts');
  for (const [id, triple] of Object.entries(AOT_TARGET_TRIPLES)) {
    for (const selector of [id, triple]) {
      const result = spawnSync(process.execPath, [cli, selector], { encoding: 'utf8' });
      assert.equal(result.status, 0, result.stderr);
      assert.equal(
        result.stdout.trim(),
        [triple, 'liboliphaunt-wasix-runtime-aot-' + id, id].join('\t'),
      );
    }
  }
  assert.notEqual(spawnSync(process.execPath, [cli, 'unsupported']).status, 0);
});
...
function stageIcuPayload(stage, profile = 'wasix-icu-data') {
  const payload = path.join(stage, 'target/oliphaunt-wasix/icu/share/icu/icudt76l/data.res');
...
const icuDigest = logicalTreeSha256([
  { path: 'icudt76l/data.res', bytes: Buffer.from('icu-data\n') },
]);
...
function stagePortablePayload(stage, runtimeBytes) {
    ...
    `${JSON.stringify(
      {
        'format-version': 2,
        'cluster-seeds': {
          icu: { manifest: 'cluster-seeds/icu.json', 'icu-data-tree-sha256': icuDigest },
        },
        runtime: {
  ...
  mkdirSync(path.join(root, 'cluster-seeds'));
  writeFileSync(
    path.join(root, 'cluster-seeds/icu.json'),
    JSON.stringify({
...
test('WASIX release binds the ICU seed identity to actual sidecar bytes', (t) => {
  const root = fixture(t);
  const portableStage = path.join(root, 'portable');
  stagePortablePayload(portableStage, runtimeArchive(root));
  const portable = archiveStage(portableStage, path.join(root, 'portable.tar.zst'), '.');
  const digest = validatePortableReleaseAsset(portable);
  const icuStage = path.join(root, 'icu');
  stageIcuPayload(icuStage);
  const icuArchive = path.join(root, 'icu.tar.zst');
  archiveStage(icuStage, icuArchive, '.');
  assert.doesNotThrow(() => validateIcuReleaseAsset(icuArchive, digest));
  writeFileSync(
    path.join(icuStage, 'target/oliphaunt-wasix/icu/share/icu/icudt76l/data.res'),
    'changed',
  );
  archiveStage(icuStage, icuArchive, '.');
  assert.throws(() => validateIcuReleaseAsset(icuArchive, digest), /ICU data does not match/);
  const seedPath = path.join(portableStage, 'target/oliphaunt-wasix/assets/cluster-seeds/icu.json');
  const seed = JSON.parse(readFileSync(seedPath, 'utf8'));
  seed.icu.dataTreeSha256 = '0'.repeat(64);
  writeFileSync(seedPath, JSON.stringify(seed));
  archiveStage(portableStage, portable, '.');
  assert.throws(() => validatePortableReleaseAsset(portable), /inconsistent ICU seed identity/);
});
