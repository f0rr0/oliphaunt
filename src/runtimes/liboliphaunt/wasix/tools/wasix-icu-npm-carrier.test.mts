...
import { packWasixIcuNpmCarrier, stageWasixIcuNpmCarrier } from './wasix-icu-npm-carrier.mts';
...
function fixture() {
  ...
  const tree = createHash('sha256')
    .update(dataPath)
    .update(Buffer.of(0))
    .update(String(dataBytes.length))
    .update(Buffer.of(0))
    .update(dataBytes)
    .update('\n')
    .digest('hex');
  ...
  const seedManifest = {
    ...
    runtime: {
      ...
      consumerSha256: moduleSha,
      producerSha256: moduleSha,
      initdbSha256: '2'.repeat(64),
    },
    source: { fingerprint: source, catalogVersion: '1', lane: 'stable', producer: 'wasix-initdb' },
    initProfile: 'fixture',
    archive: {
      path: 'cluster-seeds/icu.tar.zst',
      sha256: digest(seedBytes),
      compressedBytes: seedBytes.length,
      expandedBytes: 1,
      regularFiles: 1,
      directories: 1,
    },
    requiredRuntimeFeatures: ['icu'],
...
test('packs one exact ICU data and ICU cluster-seed closure', () => {
  const value = fixture();
  const staged = packWasixIcuNpmCarrier({
    version: '7.8.9',
    ...
    tarballRoot: path.join(value.root, 'tarballs'),
  });
  const packageJson = JSON.parse(
    readFileSync(path.join(staged.packageDir, 'package.json'), 'utf8'),
  );
  expect(packageJson.name).toBe('@oliphaunt/wasix-icu');
  expect(staged.descriptor.compatibility.dataTreeSha256).toBe(value.tree);
  expect(readFileSync(path.join(staged.packageDir, 'index.js'), 'utf8')).toContain(
    'oliphaunt-wasix-icu-v1',
  );
  expect(readFileSync(path.join(staged.packageDir, 'assets/cluster-seed-icu.tar.zst'))).toEqual(
    Buffer.from('seed\n'),
  );
  const entries = readPortableArchiveEntries(staged.tarball);
  expect(entries.get('package/assets/cluster-seed-icu.tar.zst').data()).toEqual(
    Buffer.from('seed\n'),
  );
  expect(JSON.parse(entries.get('package/package.json').data().toString('utf8'))).toEqual(
    packageJson,
  );
});
