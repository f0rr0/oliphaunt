...
- Split WASIX runtime artifacts into the product-scoped `liboliphaunt-wasix`
  release with portable assets, AOT crates, and GitHub release assets.
- Add exact-extension WASIX artifact build metadata for the product release
  graph.

### Changed

- Harden WASIX builder setup, source fetches, and release packaging for
  same-SHA artifact consumption.
- Rename internal WASIX runtime payload crates to `oliphaunt-wasix-assets` and
  `oliphaunt-wasix-aot-*`.
...
- Rename project to Oliphaunt
- Organize polyglot release tooling
...
- Publish release crates from the validated staged workspace so the
  `oliphaunt-wasix-assets` and target-specific AOT crates include their generated
...
- Pivoted `oliphaunt-wasix` to a new runtime architecture built around
...
- Refreshed example lockfiles automatically after release version bumps so
...
- `OliphauntRuntimeOptions::default` now selects the optimized embedded-template
  startup path.
...
- Repo hooks for Conventional Commit validation and lightweight formatting/file
  hygiene checks.
...
- modernize embedded Oliphaunt API and OSS tooling ([#3](https://github.com/f0rr0/oliphaunt/pull/3))

- Added the high-level `Oliphaunt` and `OliphauntServer` APIs for direct embedded use
