...
if [ ! -f "$LIBICONV_SOURCE_DIR/configure" ] || [ ! -f "$LIBICONV_SOURCE_PIN" ]; then
  echo "pinned libiconv source checkout is missing; run bash src/sources/tools/fetch-sources.sh wasix-runtime --force" >&2
  exit 1
fi
source_tree_sha256="$(bash "$REPO_ROOT/tools/dev/bun.sh" "$REPO_ROOT/src/sources/tools/verify-source-tree.mts" \
