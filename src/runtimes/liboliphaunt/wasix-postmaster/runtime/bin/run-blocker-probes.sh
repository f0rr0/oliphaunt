...
usage() {
	cat <<EOF
usage: $0 [--wasmer-bin PATH] [--wasmer-arg ARG] [--compile|--no-compile] [--strict] [--probe NAME]
...
Examples:
  ...
  $0 --probe exec-shared-latch-sigurg --wasmer-arg --disable-non-volatile-memops --strict
EOF
}
...
fail_sysroot() {
	printf 'run-blocker-probes: patched sysroot rejected: %s\n' "$*" >&2
	exit 2
}
	...
	for probe in "$@"; do
		case "$probe" in
			...
			setitimer-epoll-one-shot)
				[ -f "$BUILD_DIR/setitimer_epoll_one_shot.pic.wasm" ] || return 1
				;;
...
compile_probes() {
	...
	"$DOCKER_BIN" run --rm \
		...
		bash -lc '
			...
			if selected_probe setitimer-epoll-one-shot; then
				WASIXCC_WASM_EXCEPTIONS="$WASIXCC_EXCEPTION_MODE" wasixcc -O0 -g3 -fPIC -pthread \
					"$PROBE_SOURCE_DIR/setitimer_epoll_one_shot_probe.c" \
					-o "$PROBE_BUILD_DIR/setitimer_epoll_one_shot.pic.wasm"
			fi
...
write_header() {
...

	{
		...
		printf -- '- sysroot variant manifest SHA-256: `%s`\n\n' "$PATCHED_SYSROOT_MANIFEST_SHA256"
			printf -- '- Selected research process model: `EH/PIC dynamic-main + side-module dlopen + posix_spawn/proc_spawn2 + MAP_FIXED shared mmap reattach`\n'
...
run_selected_diagnostic_probe() {
	local name="$1"
	local wasm="$2"
	local flags="$3"

	if probe_selected "$name"; then
		run_probe "$name" "$wasm" "$flags" "$report" || diagnostic_failures=$((diagnostic_failures + 1))
	fi
}
...
run_selected_diagnostic_probe "setitimer-epoll-one-shot" "$BUILD_DIR/setitimer_epoll_one_shot.pic.wasm" "--enable-exceptions --enable-threads"
...

{
	...
	printf -- '- Failed diagnostic probes (non-gating): `%s`\n' "$diagnostic_failures"
