...
fresh_require_managed_generated_path "$matrix_report_dir" "profile matrix report root"
fresh_require_managed_generated_path "$matrix_run_root" "profile matrix run root"
...
write_header() {
  {
    ...
    for profile in "${profiles[@]}"; do
      ...
      printf '  - suppress implicit wasm-opt defaults: `%s`\n' "$FRESH_WASIX_CORE_PROFILE_WASM_OPT_SUPPRESS_DEFAULT"
...
bench_native() {
  ...
  fresh_require_managed_generated_path "$pgdata" "native profile matrix PGDATA"
...
bench_wasix_profile() {
  ...
  fresh_require_managed_generated_path "$pgdata" "WASIX profile matrix PGDATA"
  fresh_require_managed_generated_path "$dev_shm" "WASIX profile matrix shared-memory root"
