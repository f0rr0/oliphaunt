...
async function main() {
  const [phase, workRoot, ...argv] = Bun.argv.slice(2);
  if (!workRoot || !['prepare', 'finish'].includes(phase))
    fail('internal usage: prepare|finish WORKDIR [options]');
  const planFile = path.join(workRoot, 'plan.json');
  if (phase === 'finish') {
    if (argv.length) usage();
    const { targets, version, outputDir } = JSON.parse(await readFile(planFile, 'utf8'));
    for (const target of targets) {
      const packaged = path.join(
        workRoot,
        'cargo-target/package',
        `${target.packageName}-${version}.crate`,
      );
      await validateCrate(
        packaged,
        target.packageName,
  ...
  for (const target of TARGETS.filter(
    ...
    targets.push(
      await prepareTarget(targetFromSource(target, args.version), { ...args, sourceRoot }),
    );
  }
  await writeFile(
    planFile,
    JSON.stringify({ targets, version: args.version, outputDir: args.outputDir }),
  );
  await writeFile(
    path.join(workRoot, 'manifests'),
    targets.map((target) => path.join(target.crateDir, 'Cargo.toml') + '\0').join(''),
  );
}
