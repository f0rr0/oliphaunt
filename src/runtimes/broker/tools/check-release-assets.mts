...
import { finalizeHelperAssets } from '../../../shared/artifact-packaging/finalize-helper-assets.mts';
import { inspectPlatformBinaryEntries } from '../../../shared/artifact-packaging/platform-binary-contract.mts';
import {
  assertFileExists,
  checksumManifest,
  readArchiveEntries,
  sha256,
} from '../../../shared/artifact-packaging/release-asset-validation.mts';
import {
...
import { assertBrokerDependencyLicensesInEntries } from './broker-dependency-license-contract.mts';

const PREFIX = 'check-broker-release-assets.mts';
...
async function validateArchive(file, target) {
  ...
  if (target.target === 'windows-x64-msvc') {
    ...
    const pending = windowsVcRuntimeImports(
      Buffer.from(broker.data()),
      `${path.basename(file)}:${executable}`,
    ...
    while (pending.length > 0) {
      ...
      let inspected;
      try {
        inspected = inspectPortableExecutable(
          Buffer.from(entry.data()),
          `${path.basename(file)}:${member}`,
        );
      } catch (error) {
        fail(PREFIX, error instanceof Error ? error.message : String(error));
      }
    ...
    if (actual.join('\0') !== [...required].sort(compareText).join('\0')) {
      fail(
        PREFIX,
        `${path.basename(file)} app-local VC runtime members must exactly match its PE import closure`,
      );
    }
    ...
    const expectedReceipt = [...required]
      .sort(compareText)
      .map((name) => {
        const digest = createHash('sha256')
          .update(Buffer.from(entries.get(`bin/${name}`).data()))
          .digest('hex');
        return `${digest}  ${name}\n`;
      })
      .join('');
    if (receipt !== expectedReceipt) {
      fail(
        PREFIX,
        `${path.basename(file)} ${receiptMember} does not exactly bind its app-local VC runtime bytes`,
      );
    }
...
export async function checkBrokerReleaseAssets(argv = Bun.argv.slice(2)) {
...
if (import.meta.main) await checkBrokerReleaseAssets();
