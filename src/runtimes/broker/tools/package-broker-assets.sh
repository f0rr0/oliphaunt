...
version="$(tools/dev/bun.sh src/shared/product-metadata/product-version.mts version oliphaunt-broker)"
...
if [[ "$target_id" == linux-*-gnu ]]; then
  ...
  broker_bin="$cargo_target_dir/release/$broker_stage_name"
  src/runtimes/broker/tools/build-linux-broker-baseline.sh "$cargo_target_dir"
else
...
bash src/shared/artifact-packaging/strip-native-binaries.sh "$stage"
vc_runtime_dlls=""
if [ "$target_id" = "windows-x64-msvc" ]; then
  vc_runtime_dlls="$(bun src/shared/artifact-packaging/windows-vc-runtime-closure.mts stage \
    --root "$stage" \
    --destination "$stage/bin" \
    --print-required)"
  bun src/shared/artifact-packaging/windows-vc-runtime-closure.mts verify \
    --root "$stage" \
...
tools/dev/bun.sh src/shared/artifact-packaging/release-notices.mts stage "$stage" --profile broker
tools/dev/bun.sh src/runtimes/broker/tools/broker-dependency-license-contract.mts stage \
  "$stage" \
  --target "$target_id"

src/shared/artifact-packaging/archive-directory.mts "$stage" "$out_dir/$asset"
tools/dev/bun.sh src/runtimes/broker/tools/broker-dependency-license-contract.mts check-archive \
  "$out_dir/$asset" \
  --target "$target_id"
...
bun src/runtimes/broker/tools/check-release-assets.mts "${check_args[@]}"
