...
import { requireSafeDirectoryChain as requireReleaseDirectoryChain } from '../../../shared/artifact-packaging/release-directory-safety.mts';
import { readPortableArchiveEntries } from '../../../shared/artifact-packaging/portable-archive.mts';
import { assertReleaseNoticesInEntries } from '../../../shared/artifact-packaging/release-notices.mts';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../../..');
const TOOL = 'broker-dependency-license-contract.mts';
...
export const BROKER_PAYLOAD_LICENSE = 'MIT AND ISC AND Unicode-3.0';
...
const PAYLOAD_LICENSE_ATOMS = Object.freeze(['MIT', 'ISC', 'Unicode-3.0']);
...
function canonicalBlobBytes(digest, expectedBytes) {
  ...
  if (
    ...
    fail(
      `canonical broker dependency license blob does not match ${digest}/${expectedBytes}: ${file}`,
    );
  }
  return content;
}
...
function readCargoSnapshot(graphDirectory, file) {
  ...
  const metadata = requireRealFile(snapshot, 'broker Cargo snapshot');
  if (metadata.size > 128 * 1024 * 1024) fail('broker Cargo snapshot exceeds 128 MiB');
  return new TextDecoder('utf-8', { fatal: true }).decode(readFileSync(snapshot));
}

function cargoTreePackageKeys(cargoTarget, metadataPackages, graphDirectory) {
...
function validateContractShape(contract, contractBytes) {
  ...
  if (
    !sameStrings([...selectedAtoms].sort(compareText), [...PAYLOAD_LICENSE_ATOMS].sort(compareText))
  ) {
    fail(
...
function validateCanonicalBlobs(contract) {
  ...
  if (!sameStrings(actualNames, expectedNames)) {
    fail(
      `canonical broker dependency license blobs differ: expected=${JSON.stringify(expectedNames)}, actual=${JSON.stringify(actualNames)}`,
    );
  }
...
function validateGraph(contract, packages, graphDirectory) {
  requireSafeDirectoryChain(graphDirectory, 'broker Cargo snapshot directory');
...
function targetExpectedFiles(contractState, target) {
  ...
  const blobMembers = targetBlobMembers(contractState, target);
  files.set(
    `${BROKER_DEPENDENCY_LICENSE_ROOT}/DEPENDENCIES.json`,
...
export function brokerDependencyLicenseMembers(target, { prefix = '' } = {}) {
  ...
  const checkedPrefix = prefix
    ? safeMember(prefix.replace(/\/$/u, ''), 'broker dependency archive prefix')
    : '';
...
export function stageBrokerDependencyLicenses(destination, target) {
  ...
  ensureSafeDirectoryChain(
    ...
    'broker dependency license namespace parent',
  );
  let prior;
  try {
    ...
    if (cause?.code !== 'ENOENT')
      fail(`cannot inspect prior broker dependency license root: ${cause.message}`);
  }
  if (prior) {
  ...
  ensureSafeDirectoryChain(
    ...
    'broker dependency license staging root',
  );
...
export function assertBrokerDependencyLicensesInDirectory(directory, { target } = {}) {
  ...
  for (const member of expectedMembers) {
    ...
    if (expected.has(member)) {
      ...
      if (!hasCanonicalBrokerFilesystemMode(entry.stat.mode, 0o644))
      ...
      if (!hasCanonicalBrokerFilesystemMode(entry.stat.mode, 0o755))
        fail(`${member} must have mode 0755`);
    }
  }
  for (const member of actual.keys()) {
    if (!expectedMembers.has(member))
      fail(`broker dependency license carrier has unexpected member ${member}`);
  }
...
function checkedArchivePrefix(prefix) {
  ...
  return safeMember(
    ...
    'broker dependency archive prefix',
  );
}

export function assertBrokerDependencyLicensesInEntries(
  entries,
  ...
  for (const [member, entry] of entries) {
    ...
    if (member !== prefixed(BROKER_DEPENDENCY_LICENSE_ROOT) && !member.startsWith(namespace))
      continue;
...
function usage() {
  return [
    'usage:',
    `  ${TOOL} check-contract`,
    `  bash src/runtimes/broker/tools/audit-dependency-licenses.sh`,
...
function main() {
  try {
    ...
    if (args.command === 'audit-targets') {
      ...
      const state = loadBrokerDependencyLicenseContract();
      console.log(
        `${TOOL}: canonical self-contained license contract passed (${state.contract.packages.length} packages)`,
      );
    } else if (args.command === 'audit-contract') {
      const state = loadBrokerDependencyLicenseContract({ graphDirectory: args.graphDirectory });
      console.log(
      ...
      assertBrokerDependencyLicensesInArchive(args.subject, {
        target: args.target,
        prefix: args.prefix,
      });
      console.log(`${TOOL}: checked ${args.target} dependency licenses in ${args.subject}`);
