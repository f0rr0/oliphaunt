...
import { currentProductVersionSync } from '../../../shared/product-metadata/release-artifact-targets.mts';
...
function archive(directory, extension) {
  ...
  const result = spawnSync(
    path.join(ROOT, 'src/shared/artifact-packaging/archive-directory.mts'),
    [directory, output],
...
test('broker contract pins the complete package-specific legal inventory', {
  timeout: TIMEOUT,
}, () => {
  ...
  assert.ok(
    contract.packages.some((row) => row.name === 'libloading' && row.selectedLicense === 'ISC'),
  );
  assert.ok(
    contract.packages.some((row) => row.name === 'serde_json' && row.version === '1.0.150'),
  );
  assert.ok(
    contract.packages.some(
      (row) => row.name === 'unicode-ident' && row.selectedLicense === 'MIT AND Unicode-3.0',
    ),
  );
  assert.ok(
    contract.packages.some(
...
test('selected-target carrier staging is self-contained without Cargo or registry sources', (t) => {
  ...
  const result = spawnSync(
    process.execPath,
    [
      path.join(ROOT, 'src/runtimes/broker/tools/broker-dependency-license-contract.mts'),
      'stage',
      directory,
      '--target',
      'linux-x64-gnu',
...
test('concurrent real Cargo payload packagers are isolated and reopen exact target closures', {
  ...
  assert.equal(fixture.status, 0, fixture.stderr);
  const packagedRuns = await Promise.all(
    outputDirs.map((outputDir, index) =>
      spawnResult(
        'bash',
        [
          'src/runtimes/broker/tools/package-broker-cargo-artifacts.sh',
          '--asset-dir',
          assetDir,
          '--output-dir',
          outputDir,
