...
try {
  ...
  if (operation === 'audit') {
    ...
    throw new Error('source planning is internal; use bash src/sources/tools/fetch-sources.sh');
} catch (error) {
  fail(error.message);
}
...
function parseArgs(args) {
  ...
  for (const arg of args) {
    ...
    if (arg === '--help' || arg === '-h') {
      console.log(
        `usage: bash src/sources/tools/fetch-sources.sh [${sourceScopes.join('|')}] [--force|--validate-only|--verify-only]`,
      );
      process.exit(0);
    }
    if (sawScope) {
