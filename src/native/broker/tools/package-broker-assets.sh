...
version="$(tools/dev/bun.sh tools/release/product-version.mts version oliphaunt-broker)"
...
if [[ "$target_id" == linux-*-gnu ]]; then
  ...
  broker_bin="$cargo_target_dir/release/$broker_stage_name"
  src/native/broker/tools/build-linux-broker-baseline.sh "$cargo_target_dir"
else
...
bash tools/packaging/strip-native-binaries.sh "$stage"
vc_runtime_dlls=""
if [ "$target_id" = "windows-x64-msvc" ]; then
  vc_runtime_dlls="$(bun tools/packaging/windows-vc-runtime-closure.mts stage \
    --root "$stage" \
    --destination "$stage/bin" \
    --print-required)"
  bun tools/packaging/windows-vc-runtime-closure.mts verify \
    --root "$stage" \
    --search-root "$stage/bin"
fi
tools/dev/bun.sh tools/packaging/platform-binary-contract.mts --target "$target_id" --root "$stage"
if [[ "$target_id" == linux-*-gnu ]]; then
  tools/packaging/check-linux-consumer-baseline.sh --target "$target_id" --root "$stage"
fi
...
tools/dev/bun.sh tools/packaging/release-notices.mts stage "$stage" --profile broker
tools/dev/bun.sh src/native/broker/tools/broker-dependency-license-contract.mts stage \
  "$stage" \
  --target "$target_id"

tools/packaging/archive-directory.mts "$stage" "$out_dir/$asset"
tools/dev/bun.sh src/native/broker/tools/broker-dependency-license-contract.mts check-archive \
  "$out_dir/$asset" \
  --target "$target_id"
...
bun src/native/broker/tools/check-release-assets.mts "${check_args[@]}"
