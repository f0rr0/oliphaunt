...
version="$(bun tools/dev/node-info.mts package-version src/native/node-addon/package.json)"
...
bash src/native/node-addon/tools/build-native.sh "$out_dir"
...
tools/dev/bun.sh src/native/node-addon/tools/dependency-license-contract.mts stage "$asset_stage" --target "$target"
if [ "$platform" = "windows" ]; then
...
package_source="$root/src/native/node-addon/packages/$optional_package"
...
tools/dev/bun.sh src/native/node-addon/tools/dependency-license-contract.mts stage "$package_work" --target "$target"
...
tools/dev/bun.sh src/native/node-addon/tools/check-release-assets.mts \
  --asset-dir "$asset_dir" \
  --allow-partial \
  --npm-package "$tarball"
