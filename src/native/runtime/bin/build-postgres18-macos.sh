...
macos_module_nm_audit="$repo_root/src/native/runtime/tools/audit-macos-module-nm.awk"
macos_provider_collision_audit="$repo_root/src/native/runtime/tools/audit-macos-provider-collisions.awk"
...
liboliphaunt_build_fingerprint() {
  {
    ...
    shasum -a 256 "$repo_root/src/native/runtime/include/oliphaunt.h"
    shasum -a 256 "$repo_root/src/native/runtime/src/liboliphaunt_internal.h"
    local source
...
extension_build_fingerprint() {
...

  {
    ...
    hash_extension_source_tree "$repo_root/src/extensions/contrib/portable-uuid"
  } | shasum -a 256 | awk '{print $1}'
}
...
if [ "$script_mode" != "build" ] && [ "$script_mode" != "--runtime-only" ]; then
  cat >&2 <<'MSG'
usage: src/native/runtime/bin/build-postgres18-macos.sh [--runtime-only|--print-required-extension-artifacts|--check-oliphaunt-current|--check-extension-artifacts-current]
MSG
  exit 2
fi
