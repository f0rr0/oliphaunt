...
sources=("$repo_root"/src/native/runtime/src/*.c)
...
public_exports() {
  sed -nE 's/^OLIPHAUNT_API[[:space:]]+.*[[:space:]*](oliphaunt_[a-z0-9_]+)\(.*/\1/p' src/native/runtime/include/oliphaunt.h
}
artifact_ready() {
...
link_embedded() {
  ...
  for source in "${sources[@]}"; do
    ...
    logged "compile-$stem.log" native cl.exe /nologo /std:c11 /O2 /Zi /MD /DOLIPHAUNT_EMBEDDED /DOLIPHAUNT_BUILTIN_PLPGSQL /DOLIPHAUNT_BUILDING_DLL /D_CRT_SECURE_NO_WARNINGS "/I$(native_path "$repo_root/src/native/runtime/include")" "/I$(native_path "$repo_root/src/native/runtime/src")" /c "$(native_path "$source")" "/Fo$(native_path "$object")"
...
desired_hash="$(
  set -e
  {
    ...
    for input in src/third-party/postgres/{source.toml,source.mts,fetch-source.sh} src/native/runtime/postgres/series src/native/runtime/sources/icu-windows.toml src/extensions/generated/pgxs-build.tsv src/extensions/catalog/native-components.toml src/extensions/tools/native-component-contract.mts src/native/runtime/tools/liboliphaunt-extension-guard.sh "$vc_runtime_tool" tools/packaging/windows-vc-runtime-policy.json src/native/runtime/tools/native-runtime-payload-policy.json "$catalog"; do sha256 "$input"; done
    while IFS= read -r patch; do
      case "$patch" in '' | '#'*) continue ;; esac
      sha256 "$patch"
    done <src/native/runtime/postgres/series
    while IFS= read -r -d '' input; do
      printf '%s\n' "$input"
      sha256 "$input"
    done < <(find src/native/runtime/src src/native/runtime/include src/extensions/contrib/portable-uuid src/extensions/artifacts/native/tools -type f ! -name '*.test.*' ! -path '*/fixtures/*' ! -name '*.md' -print0 | sort -z)
    sha256 "${BASH_SOURCE[0]}"
    sha256 src/native/runtime/bin/common.sh
    if [ "$build_extensions" != 0 ]; then
      while IFS= read -r -d '' input; do
        printf '%s\n' "$input"
        sha256 "$input"
      done < <(find extensions/external src/third-party/icu src/third-party/openssl -type f ! -name '*.test.*' ! -path '*/fixtures/*' ! -name '*.md' -print0 | sort -z)
    fi
