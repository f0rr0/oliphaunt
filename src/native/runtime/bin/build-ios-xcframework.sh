...
runtime_version_file="$repo_root/src/native/runtime/VERSION"
...
simulator_script="$repo_root/src/native/runtime/bin/build-postgres18-ios-simulator.sh"
device_script="$repo_root/src/native/runtime/bin/build-postgres18-ios-device.sh"
macos_script="$repo_root/src/native/runtime/bin/build-postgres18-macos.sh"
public_header="$repo_root/src/native/runtime/include/oliphaunt.h"
...
usage() {
  cat >&2 <<'MSG'
usage: src/native/runtime/bin/build-ios-xcframework.sh [--check-current]
MSG
}
...
build_xcframework() {
  ...
  rsync -a --delete "$repo_root/src/native/runtime/include/" "$headers_dir/"
