...
receipt() {
  bun src/native/runtime/tools/native-mobile-abi-contract.mts write --build-root "$root/$1/postgresql-$pg_version" --target "$2" --output "$root/$3"
}
case "$target" in
  android-*)
    if [ "$target" = android-arm64-v8a ]; then
      mobile_root=target/liboliphaunt-pg18-android-arm64
      OLIPHAUNT_ANDROID_ABI=arm64-v8a OLIPHAUNT_ANDROID_ARM64_ROOT="$root/$mobile_root" bash src/native/runtime/bin/build-postgres18-android-arm64.sh
      host_args=(--runtime-only)
    else
      mobile_root=target/liboliphaunt-pg18-android-x86_64
      OLIPHAUNT_ANDROID_ABI=x86_64 OLIPHAUNT_ANDROID_X86_64_ROOT="$root/$mobile_root" bash src/native/runtime/bin/build-postgres18-android-x86_64.sh
      host_args=()
    fi
    OLIPHAUNT_LINUX_WORK_ROOT="$root/$host_root" bash src/native/runtime/bin/build-postgres18-linux.sh "${host_args[@]}"
    ...
    ;;
  ios-xcframework)
    OLIPHAUNT_WORK_ROOT="$root/$host_root" bash src/native/runtime/bin/build-ios-xcframework.sh
    OLIPHAUNT_BUILD_EXTENSIONS="${OLIPHAUNT_BUILD_EXTENSIONS-0}" OLIPHAUNT_WORK_ROOT="$root/$host_root" bash src/native/runtime/bin/build-postgres18-macos.sh --runtime-only
    ...
    bun src/native/runtime/tools/native-mobile-abi-contract.mts compare --domain ios-datum64 --receipt "$root/$device/out/native-mobile-abi.properties" --receipt "$root/$simulator/out/native-mobile-abi.properties" --receipt "$root/$framework/out/native-mobile-abi-producer.properties"
    stage "$framework/out"
    stage "$simulator/out"
    stage "$device/out"
    stage "$host_root/install"
