...
pg_version="$(bun src/native/runtime/tools/native-smoke-data.mts postgres-version)"
build_dir="$work_root/postgresql-$pg_version"
include_dir="$root/src/native/runtime/include"
source_dir="$root/src/native/runtime/src"
smoke_dir="$root/src/native/runtime/smoke"
...
if [ ! -e "$smoke_root/.oliphaunt.json" ]; then
  ...
  bun src/native/runtime/tools/native-smoke-data.mts managed-root "$smoke_root"
fi
archive_fixture="$root/src/native/runtime/smoke/fixtures/physical-archive-native-v1.properties"
...
for profile in standard icu; do
  ...
  bun src/native/runtime/tools/native-smoke-data.mts managed-root "$cluster_root/$profile"
  probe="$(bun src/native/runtime/tools/native-smoke-data.mts profile "$profile")"
  sql="${probe%$'\n'*}"
  expected="${probe##*$'\n'}"
  for _attempt in 1 2; do
...
bun src/native/runtime/tools/native-smoke-data.mts managed-root "$cluster_root/standard-icu-import"
for _attempt in 1 2; do
