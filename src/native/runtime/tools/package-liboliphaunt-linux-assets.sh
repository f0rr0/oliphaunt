...
if [ "${OLIPHAUNT_RELEASE_BUILD_RUNTIME:-1}" = "1" ]; then
  ...
  src/native/runtime/bin/build-postgres18-linux.sh >/tmp/liboliphaunt-release-"$target_id".log
fi
...
tools/dev/bun.sh src/native/runtime/tools/native-runtime-payload.mts "$stage" --target "$target_id" --tool-set runtime
