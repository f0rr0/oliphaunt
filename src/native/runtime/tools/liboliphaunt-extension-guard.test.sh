...
bun src/native/runtime/tools/liboliphaunt-extension-guard.fixture.mts "$scratch/payload"
bun src/native/runtime/tools/native-runtime-payload.mts "$scratch/payload" --target linux-x64-gnu --tool-set runtime
rm "$scratch/payload/runtime/share/postgresql/tsearch_data/english.stop"
if bun src/native/runtime/tools/native-runtime-payload.mts "$scratch/payload" \
