...
if [ "${OLIPHAUNT_RELEASE_BUILD_RUNTIME:-1}" != 0 ]; then
  OLIPHAUNT_CI_TARGET="$target_id" bash src/native/runtime/tools/release-runtime.sh build
fi
...
cp -R src/native/runtime/include/. "$stage/include/"
...
bun src/native/runtime/tools/native-runtime-payload.mts "$stage" --target "$target_id" --tool-set runtime
