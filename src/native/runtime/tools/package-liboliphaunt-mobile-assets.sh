...
source "$root/src/native/runtime/tools/liboliphaunt-extension-guard.sh"
...
case "$target_id" in
  android-arm64-v8a|android-x86_64|ios-xcframework)
    ;;
  *)
    fail "usage: src/native/runtime/tools/package-liboliphaunt-mobile-assets.sh [android-arm64-v8a|android-x86_64|ios-xcframework]"
    ;;
esac
...
headers_dir="$root/src/native/runtime/include"
...
package_ios() {
  ...
  OLIPHAUNT_MACOS_RUNTIME_RESOURCES_ROOT="$macos_runtime_stage/oliphaunt" \
    ...
    src/native/runtime/bin/build-ios-xcframework.sh >/tmp/liboliphaunt-release-ios-xcframework-resources.log
  mkdir -p "$stage_ios"
