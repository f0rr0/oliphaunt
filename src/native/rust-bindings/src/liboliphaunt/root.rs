...
impl PreparedNativeRoot {
    pub fn prepare(config: &OpenConfig, extensions: &[Extension]) -> Result<Self> {
        Self::prepare_inner(
            config,
            extensions,
            true,
    ...
    pub fn prepare_for_server(config: &OpenConfig, extensions: &[Extension]) -> Result<Self> {
        Self::prepare_inner(
            config,
            extensions,
            true,
    ...
    pub fn root_key(&self) -> Result<PathBuf> {
...
pub fn native_root_key(root: &Path) -> Result<PathBuf> {
