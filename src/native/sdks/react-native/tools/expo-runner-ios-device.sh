...
select_ios_simulator_udid() {
  ...
  booted="$(
    xcrun simctl list devices booted -j |
      bun "$root/src/native/sdks/react-native/tools/expo-runner-ios-device.mts" booted-simulator
  )"
  if [ -n "$booted" ]; then
    printf '%s\n' "$booted"
    return
  ...
  xcrun simctl list devices available -j |
    OLIPHAUNT_EXPO_IOS_DEVICE_NAME="$simulator_name" bun "$root/src/native/sdks/react-native/tools/expo-runner-ios-device.mts" available-simulator
}
...
select_ios_physical_device_id() {
  ...
  xcrun devicectl list devices --timeout 10 --json-output "$json" >/dev/null 2>&1 ||
    return 1
  bun "$root/src/native/sdks/react-native/tools/expo-runner-ios-device.mts" physical-device "$json"
}
...
preflight_physical_ios_device() {
  ...
  bun "$root/src/native/sdks/react-native/tools/expo-runner-ios-device.mts" preflight "$json" || exit $?
}
