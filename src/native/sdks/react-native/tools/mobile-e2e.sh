...
case "$platform" in
  android|ios)
    ;;
  *)
    echo "usage: src/native/sdks/react-native/tools/mobile-e2e.sh [android|ios]" >&2
    exit 2
    ;;
esac
...
case "$platform" in
  android)
    ...
    "$root/src/native/sdks/react-native/tools/expo-android-runner.sh"
    if [ "$mobile_runner" = "smoke" ]; then
  ...
  ios)
    ...
    "$root/src/native/sdks/react-native/tools/expo-ios-runner.sh"
    if [ "$mobile_runner" = "smoke" ]; then
