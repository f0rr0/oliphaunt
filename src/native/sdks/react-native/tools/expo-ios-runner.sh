...
cd "$root"
. "$root/src/native/runtime/bin/build-output.bash"
. "$root/src/native/sdks/react-native/tools/expo-runner-common.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-metro.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-reporting.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-workspace.sh"
. "$root/src/native/sdks/react-native/tools/mobile-extension-runtime.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-runtime-resources.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-ios-device.sh"
. "$root/src/native/sdks/react-native/tools/expo-runner-ios-installed-app.sh"

source_example_dir="$root/src/examples/native/react-native-expo"
rn_dir="$root/src/native/sdks/react-native"
mobile_platform="ios"
...
example_dir="${OLIPHAUNT_EXPO_IOS_EXAMPLE_DIR:-$scratch_root/src/examples/native/react-native-expo}"
...
package_work="$scratch_root/src/native/sdks/react-native"
...
find_ios_library_artifact() {
  ...
  if [ -z "$artifact" ] && [ "$sdk" = "iphonesimulator" ]; then
    ...
    artifact="$(oliphaunt_capture_build_artifact_path \
      ...
      env OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$static_extensions" src/native/runtime/bin/build-postgres18-ios-simulator.sh)"
  fi
  [ -n "$artifact" ] ||
...
install_ios_resource_carriers() {
  ...
  if [ "$example_dir" = "$scratch_root/src/examples/native/react-native-expo" ]; then
    workspace_manifest="$scratch_root/package.json"
  fi
  bun "$root/src/native/sdks/react-native/tools/expo-ios-runner.mts" configure-resource-dependencies \
...
configure_ios_carrier_inputs() {
  ...
  bun "$root/src/native/sdks/react-native/tools/expo-ios-runner.mts" configure-plugin "$example_dir/app.json" "$selected_extensions" "$icu_enabled"
}
...
validate_app_owned_payload_pod_source() {
  ...
  "$root/tools/dev/bun.sh" "$root/src/native/sdks/react-native/tools/expo-ios-runner.mts" validate-pod-source "$lockfile" "$expected_root"
}
...
patch_expo_modules_jsi_for_host_toolchain() {
  ...
  "$root/tools/dev/bun.sh" "$root/src/native/sdks/react-native/tools/expo-ios-runner.mts" patch-weak-references "$package_dir"
