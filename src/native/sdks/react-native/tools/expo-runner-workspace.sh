...
patch_expo_example_react_native_dependency() {
  ...
  if expo_requires_sdk_artifacts; then
    ...
    bun "$root/src/native/sdks/react-native/tools/expo-runner-common.mts" check-query-dependency \
      "${dependency_spec#file:}" "$query_artifact" || return
  fi
  bun "$root/src/native/sdks/react-native/tools/expo-runner-common.mts" patch-dependency "$example_dir/package.json" "$dependency_spec"
}
...
write_scratch_bun_workspace() {
  ...
  bun "$root/src/native/sdks/react-native/tools/expo-runner-common.mts" workspace \
...
install_react_native_package_dependencies() {
  if [ "$package_work" = "$scratch_root/src/native/sdks/react-native" ]; then
