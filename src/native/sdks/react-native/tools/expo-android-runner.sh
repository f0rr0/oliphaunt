...
. "$root/src/native/sdks/react-native/tools/expo-android-gradle-limits.sh"
...
example_dir="${OLIPHAUNT_EXPO_ANDROID_EXAMPLE_DIR:-$scratch_root/src/examples/native/react-native-expo}"
...
android_build_script_for_abi() {
  case "$android_abi" in
    arm64-v8a) printf '%s\n' "$root/src/native/runtime/bin/build-postgres18-android-arm64.sh" ;;
    x86_64) printf '%s\n' "$root/src/native/runtime/bin/build-postgres18-android-x86_64.sh" ;;
    *) fail "unsupported Android ABI: $android_abi" ;;
  esac
}
...
publish_local_kotlin_sdk() {
  ...
  mkdir -p "$local_maven_repo"
  run "$root/src/native/sdks/kotlin/gradlew" -p "$root/src/native/sdks/kotlin" \
...
build_apk() {
  ...
  if [ "${OLIPHAUNT_EXPO_ANDROID_SKIP_BUILD:-0}" = "1" ] && [ -f "$apk" ]; then
    ...
    if [ -n "$module_stems" ]; then
      run bun "$root/src/native/sdks/react-native/tools/validate-android-link-evidence.mts" \
  ...
  run "$root/src/native/sdks/react-native/tools/verify-android-apk.sh" "$apk"
