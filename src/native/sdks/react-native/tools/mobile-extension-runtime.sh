...
. "$root/src/native/runtime/bin/mobile-static-extensions.sh"
...
oliphaunt_dev_normalize_mobile_extensions() {
  ...
  bun "$root/src/native/sdks/react-native/tools/mobile-extension-runtime.mts" normalize "$(oliphaunt_dev_sdk_extension_json)" "$raw" "$platform"
}
...
oliphaunt_dev_mobile_createable_extensions_for_selection() {
  ...
  bun "$root/src/native/sdks/react-native/tools/mobile-extension-runtime.mts" createable "$(oliphaunt_dev_sdk_extension_json)" "$selected_extensions"
}

oliphaunt_dev_mobile_static_extensions_for_selection() {
  local selected_extensions="$1"
  [ -n "$(printf '%s' "$selected_extensions" | tr -d '[:space:],')" ] || return 0
  bun "$root/src/native/sdks/react-native/tools/mobile-extension-runtime.mts" static \
    "$(oliphaunt_dev_sdk_extension_json)" \
...
oliphaunt_dev_prebuilt_extension_asset_paths_for_selection() {
  ...
  "$root/tools/dev/bun.sh" "$root/src/native/sdks/react-native/tools/mobile-extension-artifact-paths.mts" \
    --root "$root" \
...
oliphaunt_dev_prepare_prebuilt_mobile_runtime_resource_package() {
  ...
  need_cmd cargo
  native_runtime_version="$(tr -d '\r\n' <"$root/src/native/runtime/VERSION")"
...
oliphaunt_dev_unpack_ios_extension_frameworks_for_selection() {
  ...
  while IFS= read -r archive; do
    [ -n "$archive" ] || continue
    index=$((index + 1))
    if ! bun "$root/src/native/sdks/swift/tools/extract-verified-zip.mts" \
      --archive "$archive" \
...
oliphaunt_dev_mobile_registry_data_files() {
  ...
  bun "$root/src/native/sdks/react-native/tools/mobile-extension-runtime.mts" data-files "$(oliphaunt_dev_mobile_registry_json)" "$mode" "$selected_extensions"
}
...
oliphaunt_dev_assert_runtime_extension_tree() {
  ...
  local selected_extensions="$2"
  local platform="$3"
  bun "$root/src/native/sdks/react-native/tools/validate-mobile-runtime-files.mts" \
...
oliphaunt_dev_assert_runtime_file_list() {
  ...
  cat >"$file_list"
  if ! bun "$root/src/native/sdks/react-native/tools/validate-mobile-runtime-files.mts" \
    --metadata "$(oliphaunt_dev_sdk_extension_json)" \
    --registry "$(oliphaunt_dev_mobile_registry_json)" \
    --selected "$selected_extensions" \
    --platform "$platform" \
