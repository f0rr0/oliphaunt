...
switch (command) {
  ...
  case 'workspace': {
    ...
    const packages = queryArtifact
      ...
      : ['src/native/sdks/react-native', 'src/examples/native/react-native-expo', 'src/query/ts'];
    ...
    if (queryArtifact) {
      fs.rmSync(path.join(scratch, 'src/query/ts'), { recursive: true, force: true });
    } else {
      fs.cpSync(path.join(root, 'src/query/ts'), path.join(scratch, 'src/query/ts'), {
        recursive: true,
