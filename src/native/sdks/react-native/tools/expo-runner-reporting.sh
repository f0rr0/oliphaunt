...
configure_mobile_catalog_profile_probe() {
  ...
  export EXPO_PUBLIC_OLIPHAUNT_CATALOG_PROFILE_PROBE_SQL="$({
    bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" profile "$fixture" "$profile" sql
  })"
  export EXPO_PUBLIC_OLIPHAUNT_CATALOG_PROFILE_PROBE_EXPECTED="$({
    bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" profile "$fixture" "$profile" expected
  })"
}
...
require_nonempty_json_file() {
  ...
  local json_status=0
  bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" json-object "$file" >/dev/null || json_status=$?
...
write_runner_report() {
  ...
  OLIPHAUNT_EXPO_LOG_TAG="$success_tag" \
    ...
    bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" parse-pass >"$report_tmp" || parse_status=$?
...
write_mobile_package_size_report() {
  ...
  local reports_dir="$scratch_root/reports"
  mkdir -p "$reports_dir"
  bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" package-sizes "$reports_dir/$runner-package-sizes.json" "$artifact_size_key" "$artifact_bytes" "$rn_package_bytes"
}
...
write_mobile_build_artifact_report_json() {
  ...
  shift 8
  bun "$root/src/native/sdks/react-native/tools/expo-runner-reporting.mts" build-artifact "$report" "$platform" "$artifact" "$artifact_bytes" "$rn_package" "$rn_package_bytes" "$selected_extensions" "$report_scratch_root" "$@"
}
