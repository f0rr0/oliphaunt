...
bun src/native/sdks/ts/tools/prepare-consumer.mts "$consumer" "${1:-}"
...
cp src/native/sdks/ts/tools/entrypoint-consumer.mts "$consumer/entrypoints.mts"
bun src/native/sdks/ts/node_modules/typescript/bin/tsc --ignoreConfig --noEmit --strict --skipLibCheck \
  --target ES2022 --module NodeNext --types node \
  --typeRoots "$root/src/native/sdks/ts/node_modules/@types" "$consumer/entrypoints.mts"
bun "$consumer/entrypoints.mts"
export OLIPHAUNT_SMOKE_SDK="$consumer/node_modules/@oliphaunt/ts/lib/index.js"
if [[ "${1:-}" == --published-dependencies ]]; then
  bun src/native/sdks/ts/tools/published-consumer.mts "$consumer"
...
bash src/native/sdks/ts/tools/test-native.sh
