...
for host in node bun deno; do
  ...
  for OLIPHAUNT_SMOKE_PHASE in source restored; do
    ...
    case "$host" in
      node) node --experimental-strip-types src/native/sdks/ts/src/__tests__/native-smoke.mts ;;
      bun) bun src/native/sdks/ts/src/__tests__/native-smoke.mts ;;
      deno) deno run --no-config --node-modules-dir=manual --allow-all src/native/sdks/ts/src/__tests__/native-smoke.mts ;;
    esac
  done
done
bun src/native/sdks/ts/src/__tests__/native-server-smoke.ts
