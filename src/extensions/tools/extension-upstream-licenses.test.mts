...
import { hasCanonicalReleaseStagingMode } from '../../shared/artifact-packaging/release-notices.mts';
import {
...
test('every active external release has an exact upstream license contract', () => {
  ...
  assert.deepEqual(
    rows.map((row) => row.sqlName),
    ['pg_hashids', 'pg_ivm', 'pg_textsearch', 'pg_uuidv7', 'pgtap', 'postgis', 'vector'],
  );
  ...
  assert.deepEqual(
    extensionUpstreamLicenseSources().map((source) => source.id),
    [
      'geos',
      'json-c',
      'libiconv',
...
test('committed legal bytes stage every active external release without source checkouts', (t) => {
  ...
  const script = `
    ...
    import path from "node:path";
    import { createDeterministicTar } from ${JSON.stringify(path.join(ROOT, 'src/shared/artifact-packaging/cargo-source-package.mts'))};
    import { canonicalGzipSync } from ${JSON.stringify(path.join(ROOT, 'src/shared/artifact-packaging/portable-archive.mts'))};
    import {
      assertExtensionUpstreamLicensesInArchive,
      assertExtensionUpstreamLicensesInDirectory,
      externalReleaseExtensionSqlNames,
      stageExtensionUpstreamLicenses,
    } from ${JSON.stringify(path.join(ROOT, 'src/extensions/tools/extension-upstream-licenses.mts'))};
