...
GENERATED_RUST_CONTRIB_MODULE = ROOT / "src/sdks/rust/src/extensions.rs"
...
GENERATED_SWIFT_SDK_MODULE = ROOT / "src/sdks/swift/Sources/Oliphaunt/OliphauntExtensions.swift"
    ...
    extension_plan = [
        {
            ...
            "descriptor": {
                ...
                "packageName": row["npm-package"],
                **({} if row["runtime-bound"] else {"version": (ROOT / "src/extensions/external" / row["id"] / "VERSION").read_text().strip()}),
            },
        }
...
def generated_wasix_contrib_shim(metadata: dict) -> str:
    return "// Generated by check-extension-model.py.\n" + "\n".join(
        "export declare const " + row["id"] + ": import('./types.js').WasixExtensionDescriptor;"
        for row in sorted(metadata["extensions"], key=lambda row: row["id"]) if row["runtime-bound"]
    ) + "\n"


def generated_rust_contrib_module(metadata: dict) -> str:
    body = "\n".join(
        "/// Select the `" + row["sql-name"] + "` contrib extension.\n"
    ...
    return f"// This file is generated by {CHECK_EXTENSION_MODEL_PATH}.\n// Do not edit by hand.\n\n" + body


def generated_swift_extension_module(metadata: dict) -> str:
    rows = sorted(metadata.get("extensions", []), key=lambda row: str(row["sql-name"]))
    body = "\n".join(
        "    public static let " + row["id"].split("_")[0]
        + "".join(part.title() for part in row["id"].split("_")[1:])
        + " = OliphauntExtension(sqlName: " + json.dumps(row["sql-name"])
        + ", product: " + json.dumps(row["artifact-product"]) + ")"
        for row in rows if row["runtime-bound"]
    )
    contracts = "\n".join(
        "    " + json.dumps(row["sql-name"]) + ": (dependencies: "
        + json.dumps(row["selected-extension-dependencies"]) + ", sharedPreloads: "
        + json.dumps(row["shared-preload-libraries"]) + ", module: "
        + (json.dumps(row["native-module-stem"]) if row["native-module-stem"] is not None else "nil") + "),"
        for row in rows
    )
    return (
        f"// This file is generated by {CHECK_EXTENSION_MODEL_PATH}.\n"
...
def generated_kotlin_extension_module(metadata: dict) -> str:
    ...
    body = "\n".join(
        ...
        + ", dependencies = listOf(" + ", ".join(json.dumps(name) for name in row["selected-extension-dependencies"]) + ")"
        + ", sharedPreloads = listOf(" + ", ".join(json.dumps(name) for name in row["shared-preload-libraries"]) + ")"
        + "),"
        for row in rows
    )
    contrib = "\n\n".join(
        "    @JvmField public val " + row["id"].upper() + " = ExtensionDescriptor("
        + json.dumps(row["sql-name"]) + ", " + json.dumps(row["artifact-product"]) + ")"
        for row in rows if row["runtime-bound"]
    )
    return (
        f"// This file is generated by {CHECK_EXTENSION_MODEL_PATH}.\n"
...
def validate_generated_sdk_metadata(catalog: dict, write: bool) -> None:
    ...
    validate_generated_text_file(
        ROOT / "src/bindings/wasix-ts/src/contrib-carrier-shim.d.ts",
        generated_wasix_contrib_shim(metadata), write,
    )
    ...
    validate_generated_text_file(
        GENERATED_RUST_CONTRIB_MODULE,
        generated_rust_contrib_module(metadata),
        write,
    )
    validate_generated_text_file(
        GENERATED_SWIFT_SDK_MODULE,
        generated_swift_extension_module(metadata),
        write,
    )
    validate_generated_text_file(
        GENERATED_KOTLIN_GRADLE_PLUGIN_CATALOG,
