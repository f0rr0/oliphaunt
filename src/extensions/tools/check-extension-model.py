...
def validate_evidence(catalog: dict, require_current: bool = False) -> dict:
    ...
    for run_file in run_files:
        ...
        for result in results:
            ...
            if not statuses or any(
                ...
                or not mode
                or status not in SMOKE_STATUSES
    ...
    qualified = all(row["qualification-status"] == "qualified" for row in claim_rows)
    return {
        "format-version": 1,
...
def record_wasix_evidence_run(catalog: dict, run_id: str, observed_at: str) -> None:
    ...
    results = []
    for extension in public_extensions(catalog):
        results.append(
            {
    ...
    run = {
        ...
        "notes": (
            "Recorded only after the full WASIX public-extension direct, server, "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="fail unless every public claim has passing evidence for the current semantic source digest",
    )
