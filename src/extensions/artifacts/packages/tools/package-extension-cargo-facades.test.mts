...
`,
    ...
    const wasixOnly = spawnSync(
      ...
      [
        ...
        'metadata',
        '-o',
        path.join(output, 'wasix-only.rmeta'),
        forcedUnsupportedSource,
      ],
      { encoding: 'utf8' },
    );
  ...
  test('the native-owned contrib facade exposes every SQL member without WASIX carriers', () => {
    ...
    const source = readFileSync(
      path.join(output, 'sources/oliphaunt-extension-contrib-pg18/src/lib.rs'),
      'utf8',
    );
    const manifest = Bun.TOML.parse(readFileSync(pkg.manifestPath, 'utf8'));
  ...
  test('real Cargo metadata relays exact bundle and external manifests into an app build', {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'extension-facade-integration-'));
...
[build-dependencies]
oliphaunt-build = { path = ${JSON.stringify(path.resolve(import.meta.dir, '../../../../sdks/rust/crates/oliphaunt-build'))} }
