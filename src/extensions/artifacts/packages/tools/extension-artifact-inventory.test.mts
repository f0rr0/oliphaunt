...
} from '../../../../shared/artifact-packaging/extension-artifact-archive-policy.mts';
...
import { extensionProductForSqlName } from '../../../../shared/product-metadata/release-artifact-targets.mts';
...
async function main() {
  ...
  try {
    ...
    const producer = spawnSync(
      'bash',
      [
        'src/extensions/artifacts/native/tools/create-artifact.sh',
        '--runtime',
        producerRuntime,
        '--sql-name',
        'pgtap',
    ...
    assert.equal(
      producerValidated.entries.has('files/share/postgresql/extension/pgtap-core-evil.control'),
      false,
    );
    assert.equal(
      producerValidated.entries.has('files/share/postgresql/extension/foreign.control'),
      false,
    );
    ...
    const sourceSkewedProducer = spawnSync(
      ...
      [
        ...
        'tar-gz',
        '--output',
        path.join(root, 'producer-source-version-skew.tar.gz'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    assert.equal(
      updateChainProducer.status,
      0,
      updateChainProducer.stderr || updateChainProducer.stdout,
    );
    ...
    const disconnectedUpdateProducer = spawnSync(
      ...
      [
        ...
        'tar-gz',
        '--output',
        path.join(root, 'producer-disconnected-update.tar.gz'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    const ancillaryOnlyProducer = spawnSync(
      ...
      [
        ...
        'tar-gz',
        '--output',
        path.join(root, 'producer-ancillary-only.tar.gz'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    const letterLeadingProducer = spawnSync(
      ...
      [
        ...
        'tar-gz',
        '--output',
        path.join(root, 'producer-letter-leading-only.tar.gz'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    const streamedProducer = spawnSync(
      ...
      [
        ...
        '1.2.3',
        '--data-files',
        streamedDataFiles.join(','),
        '--format',
        'tar-gz',
        '--output',
        streamedProducerArchive,
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    const missingEmbeddedRootProducer = spawnSync(
      ...
      [
        ...
        '1.2.3',
        '--output',
        path.join(root, 'producer-auto-explain-missing-embedded'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
    ...
    const sqlOnlyEmbeddedRootProducer = spawnSync(
      ...
      [
        ...
        '1.2.3',
        '--output',
        path.join(root, 'producer-pgtap-unexpected-embedded'),
        '--force',
      ],
      { cwd: REPOSITORY, encoding: 'utf8' },
    );
