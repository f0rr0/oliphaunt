...
import { WINDOWS_VC_RUNTIME_DLLS } from '../../../../shared/artifact-packaging/windows-vc-runtime-closure.mts';
import { elfFixture, machoFixture } from '../../../../../tools/test/release-fixture-utils.mts';
...
describe('Windows exact-extension binary-contract staging', () => {
  test('validates an explicitly selected desktop extension set while excluding every development/archive class', async () => {
    ...
    for (const relative of installedDevelopmentArchives) {
      await writeRuntimeFile(runtime, relative, Buffer.from('!<arch>\n', 'ascii'));
    }
