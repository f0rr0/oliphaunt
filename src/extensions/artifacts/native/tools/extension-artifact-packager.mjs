...
export function selectMobileStaticModuleSqlNames(rows, sqlNames) {
  if (!Array.isArray(rows)) {
    fail('mobile static catalog rows must be an array');
  }
  const selected = new Set(sortedDeduped(sqlNames));
  return sortedDeduped(
    rows
      .filter((row) => {
        if (row === null || Array.isArray(row) || typeof row !== 'object') {
          fail('mobile static catalog rows must be objects');
        }
        return selected.has(row.sqlName)
          && row.mobilePrebuilt === true
          && row.mobileStaticRequired === true
...
function mobileStaticSpecTable(text, label) {
  const lines = text.replace(/\r\n/g, '\n').replace(/\n+$/u, '').split('\n');
  if (lines.length < 2 || !lines[0].startsWith('# @generated by ')) {
    fail(`${label} mobile static registry is missing its generated marker`);
  }
  const header = lines[1];
  const columns = header.split('\t');
  if (columns[0] !== 'sql-name' || columns.length !== 16) {
    fail(`${label} mobile static registry has an invalid header`);
  }
  const rows = new Map();
  for (const [offset, line] of lines.slice(2).entries()) {
    ...
    if (rows.has(fields[0])) {
      fail(`${label} mobile static registry repeats SQL name '${fields[0]}'`);
    }
    rows.set(fields[0], line);
  }
  return { generatedMarker: lines[0], header, rows };
}

export function mergeMobileStaticSpecRegistries(publicText, qualificationText) {
  const publicTable = mobileStaticSpecTable(publicText, 'public');
  const qualificationTable = mobileStaticSpecTable(qualificationText, 'qualification');
  if (publicTable.header !== qualificationTable.header) {
    fail('public and qualification mobile static registry headers disagree');
  }
  const rows = new Map(publicTable.rows);
  for (const [sqlName, row] of qualificationTable.rows) {
    if (rows.has(sqlName)) {
      fail(`qualification mobile static registry overlaps public SQL name '${sqlName}'`);
    }
  ...
  const body = [...rows]
    ...
    .map(([, row]) => row);
  return [publicTable.generatedMarker, publicTable.header, ...body, ''].join('\n');
}
...
async function mobileStaticModuleSqlNames(args = []) {
  if (args.length !== 4 || args[0] !== '--target' || args[2] !== '--sql-names') {
    fail('mobile-static-module-sql-names requires --target <native-target> --sql-names <csv>');
  }
  const target = args[1];
  if (!MOBILE_NATIVE_TARGETS.has(target)) {
    fail(`mobile-static-module-sql-names requires a mobile native target, got '${target}'`);
  }
  const qualificationSqlNames = qualificationCandidateSqlNamesForTarget(target, { family: 'native' });
  const rows = await catalogRows({ qualificationSqlNames, qualificationTarget: target });
  console.log(selectMobileStaticModuleSqlNames(rows, splitCsv(args[3])).join(','));
}

async function mergeMobileStaticSpecs(args = []) {
  if (args.length !== 3) {
    fail('merge-mobile-static-specs requires <public-tsv> <qualification-tsv> <output-tsv>');
  }
  const [publicPath, qualificationPath, outputPath] = args;
  const merged = mergeMobileStaticSpecRegistries(
    await readText(publicPath),
    await readText(qualificationPath),
  );
  await fs.mkdir(path.dirname(path.resolve(outputPath)), { recursive: true });
  await fs.writeFile(path.resolve(outputPath), merged);
}
...
async function main() {
  ...
  switch (command) {
    ...
    case 'selected-sql-names':
      ...
      break;
    case 'mobile-static-module-sql-names':
      await mobileStaticModuleSqlNames(args);
      break;
    case 'merge-mobile-static-specs':
      await mergeMobileStaticSpecs(args);
      break;
    ...
    case 'create-artifact':
      ...
      break;
    default:
      fail('usage: extension-artifact-packager.mjs <list-catalog|selected-sql-names|mobile-static-module-sql-names|merge-mobile-static-specs|product-version|create-artifact> [options]');
  }
}
