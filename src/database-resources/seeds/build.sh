...
case "$family" in
  native)
    ...
    if [[ "$target" == *-datum64 && "$profile" == icu ]]; then
      ...
      source src/database-resources/icu/tools/data.sh
...
if [ "$family" = native ]; then
  bash src/database-resources/seeds/native/tools/stage-native-cluster-seed.sh \
...
bash tools/ci/with-projects.sh src/database-resources/seeds/package.mts --family "$family" --profile "$profile" --target "$target" \
