...
[ -n "$source_dir" ] && [ -n "$out_dir" ] ||
  fail "usage: src/database-resources/icu/tools/package-liboliphaunt-icu-data.sh SOURCE_DIR OUTPUT_DIR"
[ -d "$source_dir" ] || fail "missing portable ICU data directory: $source_dir"

require mktemp
require bun

source "$root/src/database-resources/icu/tools/data.sh"
...
tools/dev/bun.sh src/database-resources/contracts/icu-data.mts \
  "$stage/share/icu" \
  "$stage/manifest.properties"
tools/dev/bun.sh src/database-resources/icu/tools/write-icu-package-size-report.mts \
  "$stage/share/icu" \
