...
import { nativeIcuDataManifest } from '../../contracts/icu-data.mts';
import {
...
const ICU_PACKAGE_ROOT = path.join(ROOT, 'src/database-resources/icu/npm');
...
if (process.argv[2] === 'extract-config') {
  const root = process.argv[3];
  const entries = readPortableArchiveEntries(path.join(root, 'packed/package.tgz'));
...
test('ICU npm pack includes one canonical bundle and preserves both native descriptors byte-for-byte', () => {
  const root = process.env.OLIPHAUNT_ICU_NPM_TEST_ROOT;
  if (!root)
    throw new Error('Run bash src/database-resources/icu/tools/icu-npm-carrier-contract.test.sh');
  const tarball = path.join(root, 'packed/package.tgz');
  const entries = readPortableArchiveEntries(tarball);
  assert.deepEqual(JSON.parse(readFileSync(path.join(root, 'config.json'), 'utf8')), {
