...
import { filesystemTreeRows, logicalTreeSha256, parseProperties } from './native-manifest.mts';
...
export function parseNativeIcuDataIdentity(bytes, label = 'native ICU data manifest') {
  ...
  for (const [key, value] of expected) {
    if (actual.get(key) !== value) {
      throw new Error(
        `${label}: ${key} must be ${JSON.stringify(value)}, got ${JSON.stringify(actual.get(key))}`,
      );
    }
  }
...
if (import.meta.main) {
  ...
  if (!icuData || !output || process.argv.length !== 4) {
    throw new Error('usage: icu-data.mts ICU_DATA_DIR OUTPUT');
  }
