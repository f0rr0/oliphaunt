...
test('keeps process-argument profile probes independent of the host code page', () => {
  const probe = readFileSync(new URL('./profile-probe.json', import.meta.url));
...
test('validates independent standard and ICU native cluster seed roles', () => {
  ...
  expect(() =>
    validateNativeClusterSeedManifest(fixture('native-standard.valid.properties'), 'standard', {
      target,
    }),
  ).not.toThrow();
  expect(() =>
    validateNativeClusterSeedManifest(fixture('native-icu.valid.properties'), 'icu', {
      target,
...
test('rejects the shared malformed, extra-field, cache-key, target, and profile vectors', () => {
  for (const name of [
    ...
    expect(
      () =>
        validateNativeClusterSeedManifest(fixture(name), 'standard', {
...
test('canonicalizes producer manifests and rejects extra public fields', () => {
  ...
  expect(() =>
    validateNativeClusterSeedManifest(canonical, 'standard', {
      target: 'ios-datum64',
    }),
  ).not.toThrow();
  ...
  expect(() =>
    validateNativeClusterSeedManifest(
      Buffer.concat([canonical, Buffer.from('extra=value\n')]),
      'standard',
      {
        target: 'ios-datum64',
      },
...
test('logical ICU digest is metadata-independent and path-sensitive', () => {
  ...
  expect(() => logicalTreeSha256([{ path: '../outside', bytes: Buffer.from('x') }])).toThrow(
    /unsafe path/u,
  );
});
...
test('requires a complete regular native PGDATA seed tree', () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      fixture('native-standard.valid.properties'),
    );
    expect(() =>
      validateNativeClusterSeedDirectory(seed, 'standard', {
        target: 'linux-x64-gnu',
      }),
    ).not.toThrow();
    ...
    for (const directory of ['pg_notify', 'pg_wal/archive_status', 'pg_multixact/offsets']) {
      ...
      expect(() =>
        validateNativeClusterSeedDirectory(seed, 'standard', { target: 'linux-x64-gnu' }),
      ).toThrow();
