...
if (!scratch || !['--prepare', '--run'].includes(phase))
  throw new Error('use bash src/bindings/wasix-ts/tools/integration/smoke-browser.sh [options]');
if (phase === '--prepare') {
  ...
  let packedConsumer;
  if (packageOnly) {
    const packageScratch = resolve(scratch, 'package');
    await mkdir(packageScratch);
    packedConsumer = await stagePackedBrowserConsumer(packageScratch);
  }
