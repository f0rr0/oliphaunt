...
for value in "$wasmer_js_url" "$wasmer_js_commit" "$wasmer_wasix_url" "$wasmer_wasix_version" "$wasmer_wasix_sha256"; do
  if [[ -z "$value" ]]; then
...
input_hash="$({
  for input in "$source_manifest" "$wasmer_js_patch" "$wasmer_wasix_patch" "$wasmer_js_filesystem_patch" "$wasmer_wasix_recovery_patch" "$wasmer_js_init_patch" "${BASH_SOURCE[0]}"; do
...
for command_name in awk curl git patch pnpm sha256sum tar wasm-pack; do
...
# The upstream npm lock predates its package metadata, so even the recorded npm
# source release cannot install frozen. Keep this private source build on the
# repository-pinned pnpm path. Import the upstream lock first so its tested
# dependency versions remain the baseline, then resolve only the missing build
# metadata in the disposable exact-commit checkout.
pnpm --dir "$wasmer_js_dir" import
pnpm --dir "$wasmer_js_dir" install --no-frozen-lockfile --ignore-scripts
...
printf '{\n  "wasmerJsCommit": "%s",\n  "wasmerWasixVersion": "%s",\n  "inputsSha256": "%s"\n}\n' \
  "$wasmer_js_commit" "$wasmer_wasix_version" "$input_hash" > "$staging_dir/provenance.json"
