...
export const OPFS_POOL_ROOT = '.oliphaunt-wasix-v3';

const LEGACY_RAW_ROOT = '.oliphaunt-wasix-v2';
const STATE_FILE = 'state.json';
const DATA_DIRECTORY = 'data';
const STATE_SCHEMA = 'oliphaunt-wasix-opfs-pool-v2';
...
export class DirectOpfsPool {
  ...
  static async open(
    ...
    let stored = await readPoolState(database, name, compatibility);
    const state = stored?.phase === 'ready' ? 'existing' : 'new';
    if (stored === undefined) {
      stored = await initializePoolState(database, data, name, template, compatibility);
    }
    const pool = new DirectOpfsPool(name, database, data, compatibility, state);
    try {
  ...
  async completeInitialization(): Promise<void> {
    ...
    await writePoolState(
      this.#database,
      serializePoolState(this.#name, this.#compatibility, this.#entries, 'ready'),
    );
    ...
    if (this.#namespaceDirty) {
      ...
      await writePoolState(
        ...
        serializePoolState(
          ...
          this.#compatibility,
          this.#entries,
          this.#initializationComplete ? 'ready' : 'initializing',
        ),
      );
  ...
  #flushNewlyMappedFiles(): void {
    for (const entry of this.#entries.values()) {
      if (entry.type === 'file' && entry.record.dirty) this.#flushRecord(entry.record);
    }
  }
  ...
  #validateEssentials(): void {
    ...
    const expected = this.#compatibility.runtime.postgresVersion.split('.')[0] ?? '';
    if (actual !== expected) {
      throw corrupt(
        this.#name,
...
export async function readPooledOpfsDatabase(
  ...
  return (await inspectPooledOpfsDatabase(database, name, compatibility)).snapshot;
}
...
export async function inspectPooledOpfsDatabase(
  ...
  const state = await readPoolState(database, name, compatibility);
...
export async function completePooledOpfsInitialization(
  database: FileSystemDirectoryHandle,
  name: string,
  compatibility: WasixStorageCompatibility,
): Promise<void> {
  const state = await readPoolState(database, name, compatibility);
  if (state === undefined) {
    throw corrupt(name, 'lost state while completing first-open setup');
  }
  if (state.phase === 'ready') return;
  await writePoolState(database, { ...state, phase: 'ready' });
}
...
export async function applyPooledOpfsDelta(
  ...
  const data = await database.getDirectoryHandle(DATA_DIRECTORY, { create: true });
  const existing = await readPoolState(database, name, compatibility);
...
export async function openPooledDatabaseDirectory(name: string): Promise<FileSystemDirectoryHandle> {
  ...
  if (await hasLegacyRawDatabase(origin, name)) {
    throw new WasixStorageError(
      `OPFS storage ${JSON.stringify(name)} uses the retired raw-PGDATA layout; export it with a compatible release before opening this pooled format`,
...
async function hasLegacyRawDatabase(
  origin: FileSystemDirectoryHandle,
  name: string,
): Promise<boolean> {
  try {
    const root = await origin.getDirectoryHandle(LEGACY_RAW_ROOT);
    const database = await root.getDirectoryHandle(name);
    for await (const _entry of database.keys()) return true;
    return false;
  } catch (error) {
...
async function readPoolState(
  ...
  validateStoredCompatibility(parsed.compatibility, compatibility, {
    label: poolLabel(name),
    corrupt: (detail, cause) => corrupt(name, detail, cause),
  });
  ...
  const entries: StoredEntry[] = parsed.entries.map((entry: unknown) => {
    ...
    if (entry.type === 'directory') return { path: entry.path, type: 'directory' };
  ...
  return {
    ...
    name,
    phase: parsed.phase,
    compatibility: parsed.compatibility as WasixStorageCompatibility,
    entries,
  };
}
...
async function requireOpfsRoot(): Promise<FileSystemDirectoryHandle> {
  ...
  if (storage?.getDirectory === undefined) {
    throw new WasixStorageError('OPFS is unavailable in this @oliphaunt/wasix-ts host', {
      code: 'unavailable', durability: 'unchanged',
    });
  }
...
function corrupt(name: string, detail: string, cause?: unknown): WasixStorageError {
  return new WasixStorageError(`${poolLabel(name)} ${detail}`, {
    code: 'corrupt', durability: 'unchanged', ...(cause === undefined ? {} : { cause }),
  });
}
