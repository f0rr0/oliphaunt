...
impl BenchState {
    ...
    pub async fn profile_queries(&self, fresh: bool, row_count: u32) -> Result<BenchReport> {
        ...
        if fresh && guard.is_some() {
            bail!(
                "fresh profile requires restarting the app so the existing oliphaunt proxy can exit"
            );
        }
...
impl DatabaseHarness {
    pub async fn start(root: PathBuf, fresh: bool) -> Result<Self> {
        ...
        let pool = time_async(&mut startup, "sqlx pool connect", async {
            let options =
                pg_connect_options(&server)?.application_name("oliphaunt-wasix-tauri-sqlx-profile");
            ...
            PgPoolOptions::new()
                ...
                .await
                .context("connect SQLx pool to oliphaunt proxy")
        })
        .await?;
    ...
    pub async fn profile(&self, row_count: u32) -> Result<BenchReport> {
        ...
        let notes = vec![
            ...
            "SQLx is configured with one PostgreSQL connection because the embedded oliphaunt runtime is single-process."
                .to_string(),
...
fn pg_connect_options(server: &OliphauntServer) -> Result<PgConnectOptions> {
    ...
    let addr = server
        .tcp_addr()
        .ok_or_else(|| anyhow!("Oliphaunt server did not expose a TCP address"))?;
