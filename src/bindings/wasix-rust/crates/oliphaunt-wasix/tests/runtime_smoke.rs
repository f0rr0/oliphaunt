...
fn first_row(result: &oliphaunt_wasix::Results) -> anyhow::Result<&serde_json::Map<String, Value>> {
    result
        .rows
        .first()
...
fn raw_backend_message_name(message: &oliphaunt_wasix::BackendMessage) -> &'static str {
    match message {
        oliphaunt_wasix::BackendMessage::RowDescription(_) => "rowDescription",
        oliphaunt_wasix::BackendMessage::DataRow(_) => "dataRow",
        oliphaunt_wasix::BackendMessage::CommandComplete(_) => "commandComplete",
        oliphaunt_wasix::BackendMessage::ReadyForQuery(_) => "readyForQuery",
        oliphaunt_wasix::BackendMessage::Error(_) => "error",
        oliphaunt_wasix::BackendMessage::ParseComplete { .. } => "parseComplete",
        oliphaunt_wasix::BackendMessage::BindComplete { .. } => "bindComplete",
        _ => "other",
    }
}
...
fn startup_args_are_passed_to_postgres() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(&json!("oliphaunt-wasix-test"))
    );
    db.close()?;
    Ok(())
}
...
fn direct_raw_protocol_api_matches_oliphaunt_exec_protocol_cases() -> anyhow::Result<()> {
    ...
    assert_eq!(
        simple
            ...
            .iter()
            .filter(|message| {
                !matches!(message, oliphaunt_wasix::BackendMessage::ParameterStatus(_))
            })
...
fn pg18_embedded_backend_uses_sync_io_method() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(&json!("sync")),
        "PostgreSQL 18 worker AIO requires postmaster-managed IO workers; embedded WASIX uses sync AIO"
    );
    pg.close()?;
    Ok(())
}
...
fn persistent_root_lock_rejects_cross_process_open() -> anyhow::Result<()> {
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_oliphaunt-wasix-proxy"))
        ...
        .stderr(Stdio::piped())
        .spawn()?;
    let mut child = ChildGuard::new(child, "oliphaunt-wasix-proxy")?;

    let stdout = child
        .child_mut()
        .stdout
        .take()
        .ok_or_else(|| anyhow::anyhow!("missing oliphaunt-wasix-proxy stdout"))?;
...
fn runtime_smoke() -> anyhow::Result<()> {
    ...
    let icu_available = pg.query(
        "SELECT (count(*) > 0) AS available FROM pg_collation WHERE collprovider = 'i'",
        &[],
        None,
    )?;
    assert_eq!(
        first_row(&icu_available)?.get("available"),
        Some(&json!(true))
    );
    pg.exec(
        "CREATE COLLATION und_numeric \
            (provider = icu, locale = 'und-u-kn-true', deterministic = false)",
        None,
    )?;
    let icu_ordered = pg.query(
        "SELECT string_agg(value, ',' ORDER BY value COLLATE und_numeric) AS values \
         FROM (VALUES ('10'), ('2'), ('1')) AS input(value)",
        &[],
        None,
    )?;
    assert_eq!(
        first_row(&icu_ordered)?.get("values"),
        Some(&json!("1,2,10"))
    );
