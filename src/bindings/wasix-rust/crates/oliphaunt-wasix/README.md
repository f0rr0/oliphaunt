...
`oliphaunt-wasix` brings the packaged WASIX Oliphaunt/Postgres runtime to Rust with a
small API. Open a database directly with `Oliphaunt`, or hand `OliphauntServer` to
SQLx and any standard Postgres client. The packaged runtime is PostgreSQL 18.4.
No local Postgres install, no Docker, no runtime build toolchain.
...
[extensions guide](https://github.com/f0rr0/oliphaunt/blob/main/src/docs/content/reference/extensions.mdx)
