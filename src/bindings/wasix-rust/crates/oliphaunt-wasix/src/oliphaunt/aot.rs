...
fn target_manifest_artifact(name: &str) -> Result<AotManifestArtifact> {
    ...
    #[cfg(feature = "bundled")]
    validate_aot_manifest_metadata(&manifest)?;
...
#[cfg(feature = "bundled")]
fn validate_aot_manifest_metadata(manifest: &AotManifest) -> Result<()> {
    let metadata = super::assets::asset_manifest_metadata()?;
    let asset_source_lane = metadata
        .source_lane
        .as_deref()
        .context("asset manifest is missing source-lane metadata")?;
    let aot_source_lane = manifest
        .source_lane
        .as_deref()
        .context("AOT manifest is missing source-lane metadata")?;
    ensure!(
        aot_source_lane == asset_source_lane,
        "AOT manifest source lane mismatch: manifest={} assets={asset_source_lane}",
        aot_source_lane
    );
    ...
    let postgres_version = manifest
        .postgres_version
        .as_deref()
        .context("AOT manifest is missing postgres-version metadata")?;
    ensure!(
        postgres_version == metadata.postgres_version,
        "AOT manifest PostgreSQL version mismatch: manifest={postgres_version} assets={}",
...
fn target_artifact_bytes(_name: &str) -> Option<&'static [u8]> {
    ...
    {
        return oliphaunt_wasix_aot_aarch64_apple_darwin::artifact_bytes(_name);
    }
    #[cfg(all(feature = "bundled", target_os = "linux", target_arch = "x86_64"))]
    {
        return oliphaunt_wasix_aot_x86_64_unknown_linux_gnu::artifact_bytes(_name);
    }
    #[cfg(all(feature = "bundled", target_os = "linux", target_arch = "aarch64"))]
    {
        return oliphaunt_wasix_aot_aarch64_unknown_linux_gnu::artifact_bytes(_name);
    }
    #[cfg(all(feature = "bundled", target_os = "windows", target_arch = "x86_64"))]
    {
        return oliphaunt_wasix_aot_x86_64_pc_windows_msvc::artifact_bytes(_name);
    }
    #[allow(unreachable_code)]
    None
}
...
fn target_aot_manifest_json() -> Option<&'static str> {
    #[cfg(all(feature = "bundled", target_os = "macos", target_arch = "aarch64"))]
    {
        return Some(oliphaunt_wasix_aot_aarch64_apple_darwin::MANIFEST_JSON);
    }
    #[cfg(all(feature = "bundled", target_os = "linux", target_arch = "x86_64"))]
    {
        return Some(oliphaunt_wasix_aot_x86_64_unknown_linux_gnu::MANIFEST_JSON);
    }
    #[cfg(all(feature = "bundled", target_os = "linux", target_arch = "aarch64"))]
    {
        return Some(oliphaunt_wasix_aot_aarch64_unknown_linux_gnu::MANIFEST_JSON);
    }
    #[cfg(all(feature = "bundled", target_os = "windows", target_arch = "x86_64"))]
    {
        return Some(oliphaunt_wasix_aot_x86_64_pc_windows_msvc::MANIFEST_JSON);
    }
    #[allow(unreachable_code)]
    None
}
