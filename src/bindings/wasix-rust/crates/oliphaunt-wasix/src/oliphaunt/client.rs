...
use crate::oliphaunt::interface::{
    ...
    ExecProtocolOptions, ExecProtocolResult, NoticeCallback, ParserMap, QueryOptions, Results,
    SerializerMap,
};
...
enum ExecTransportResult {
    CommandOnly(Vec<usize>),
    Raw(Vec<u8>),
}
...
impl Oliphaunt {
    ...
    pub(crate) fn new_prepared_with_config(
        ...
        if session_startup_config.username != "postgres" {
            let sql = format!(
                "SET ROLE {}",
                crate::oliphaunt::templating::quote_identifier(&session_startup_config.username)
            );
            instance
                .exec(&sql, None)
    ...
    fn restore_session_state_after_backend_restart(&mut self) -> Result<()> {
        ...
        if username != "postgres" {
            let sql = format!(
                "SET ROLE {}",
                crate::oliphaunt::templating::quote_identifier(&username)
            );
        ...
        for channel in channels {
            let quoted_channel = crate::oliphaunt::templating::quote_identifier(&channel);
    ...
    fn exec_internal(&mut self, sql: &str, options: Option<&QueryOptions>) -> Result<Vec<Results>> {
        ...
        let transport_result = {
            ...
            self.backend
                .with_buffered(&message, exec_opts.data_transfer_container, |data| {
                    if let Some(affected_rows) = parse_command_only_result_counts(data) {
                        Ok(ExecTransportResult::CommandOnly(affected_rows))
                    } else {
                        Ok(ExecTransportResult::Raw(data.to_vec()))
                    }
                })
        };
        ...
        let data = match transport_result {
            ExecTransportResult::CommandOnly(affected_rows) => {
                return self.finish_exec_command_only(affected_rows, options);
            }
            ExecTransportResult::Raw(data) => data,
        };
        let ExecProtocolResult { messages, .. } =
        ...
        let has_row_description = messages
            .iter()
            .any(|message| matches!(message, BackendMessage::RowDescription(_)));
        collected_messages.extend(messages);

        self.finish_exec(collected_messages, options, has_row_description)
    }
    ...
    pub fn unlisten(&mut self, handle: ListenerHandle) -> Result<()> {
        if let Some(listeners) = self.notify_listeners.get_mut(&handle.normalized_channel) {
            ...
            if listeners.is_empty() {
                ...
                let quoted_channel =
                    crate::oliphaunt::templating::quote_identifier(&handle.channel);
    ...
    fn finish_exec_command_only(
        ...
        let _ = options;
        let mut results = Vec::with_capacity(affected_rows.len().max(1));
        for count in affected_rows {
    ...
    pub fn exec_protocol(
        ...
        self.parse_protocol_data(data, throw_on_error, on_notice)
    }

    fn parse_protocol_data(
        &mut self,
        data: Vec<u8>,
        throw_on_error: bool,
    ...
    fn refresh_array_types_internal(&mut self) -> Result<()> {
        ...
        ";
        let results = {
            let _phase = timing::phase("oliphaunt.array_type_catalog_query");
        ...
        {
            let _phase = timing::phase("oliphaunt.array_type_register");
    ...
    fn try_register_array_type_by_array_oid(&mut self, array_oid: i32) -> Result<bool> {
        ...
        let results = {
            let _phase = timing::phase("oliphaunt.array_type_targeted_lookup");
            self.exec_internal(&sql, None)?
        };
    ...
    fn check_ready(&self) -> Result<()> {
        if self.closing {
            bail!("Oliphaunt instance is closing");
        }
        if self.closed {
            bail!("Oliphaunt instance is closed");
        }
        if !self.ready {
            bail!("Oliphaunt instance is not ready");
        }
        Ok(())
    }
}
...
fn parse_command_only_result_counts(data: &[u8]) -> Option<Vec<usize>> {
    ...
    while offset + 5 <= data.len() {
        ...
        if frame_len > data.len() - offset {
            return None;
        }
        let body_start = offset + 5;
        let body_end = offset + frame_len;
        match tag {
            b'C' => {
                let command_tag = data[body_start..body_end]
                    .strip_suffix(&[0])
                    .unwrap_or(&data[body_start..body_end]);
                affected_total = affected_total.saturating_add(command_tag_row_count(command_tag));
                affected_rows.push(affected_total);
            }
            b'Z' => {}
            _ => return None,
        }
    ...
    (offset == data.len()).then_some(affected_rows)
}
...
impl<'a> Transaction<'a> {
    fn new(client: &'a mut Oliphaunt) -> Self {
        Self {
            client,
            closed: false,
        }
