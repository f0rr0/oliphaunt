...
const PACKAGED_QUERY_CORE: &str = "src/oliphaunt/query_core.rs";
const CHECKOUT_QUERY_CORE: &str = "../../../../../src/shared/rust-query-core/query_core.rs";
...
mod tests {
    ...
    fn query_core_fixture() -> (PathBuf, PathBuf, PathBuf) {
        ...
        let manifest = root.join("src/bindings/wasix-rust/crates/oliphaunt-wasix");
        let canonical = root.join("src/shared/rust-query-core/query_core.rs");
        let out = root.join("out");
        fs::create_dir_all(canonical.parent().unwrap()).unwrap();
        fs::create_dir_all(manifest.join("src/oliphaunt")).unwrap();
