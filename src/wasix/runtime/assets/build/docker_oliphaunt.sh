...
JOBS="${JOBS:-4}"
CONTAINER_ROOT="${CONTAINER_ROOT:-/work/src/wasix/runtime/assets/build}"
...
"$DOCKER" run --rm \
  ...
  bash -lc '
    set -euo pipefail
    . ./src/wasix/runtime/assets/build/docker_wasix_env.sh
    . ./src/wasix/runtime/assets/build/profile_flags.sh
    oliphaunt_wasix_apply_wasix_profile configure
    profile_signature="$(oliphaunt_wasix_wasix_profile_signature)"
    configure_script=./src/wasix/runtime/assets/build/configure_wasix_dl.sh
    icu_prefix="$(./src/wasix/runtime/assets/build/build_wasix_icu.sh)"
    ...
    if [ "$needs_configure" = "1" ]; then
      ...
      sha256sum ./src/wasix/runtime/assets/build/wasix_shim/oliphaunt_wasix_bridge.c \
    ...
    ./src/wasix/runtime/assets/build/link_wasix_runtime.sh \
      "$BUILD_DIR" \
      "$ICU_PREFIX" \
      "$CONTAINER_GENERATED_ROOT/build/wasix-oliphaunt/oliphaunt_wasix_bridge.o" \
      ./src/wasix/runtime/assets/generated/wasix-dl.exports \
