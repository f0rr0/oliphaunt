...
. "$root/src/wasix/runtime/tools/runtime-preflight.sh"
. "$root/src/wasix/runtime/tools/cargo-test-filter.sh"
...
case "$mode" in
  smoke|regression|core-smoke)
    ;;
  *)
    echo "usage: src/wasix/runtime/tools/runtime-smoke.sh [smoke|regression|core-smoke]" >&2
    exit 2
    ;;
esac
...
if [ "$asset_mode" = "full" ]; then
  ...
  full_evidence_features="$(
    tools/dev/bun.sh src/wasix/runtime/tools/wasix-extension-features.mts \
