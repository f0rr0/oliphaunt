...
if [[ "$host_system" == "Linux" ]]; then
  linux_libc="$(node src/wasix/node-addon/tools/detect-linux-libc.mts)"
  case "$linux_libc" in
    glibc) ;;
    musl)
...
tools/dev/bun.sh src/wasix/node-addon/tools/check-build-inputs.mts \
  "${build_input_args[@]}" \
...
build_addon() {
  ...
  if [[ "$target_id" == linux-*-gnu ]]; then
    src/wasix/node-addon/tools/build-linux-wasix-napi-baseline.sh \
      "$cargo_target_dir" \
      "$cargo_target" \
      release
  else
...
if [[ "$target_id" == "$host_target" ]]; then
  node "$workspace_root/src/wasix/node-addon/tools/native-build-data.mts" check-addon \
    "$addon" \
...
OLIPHAUNT_WASIX_NAPI_ARTIFACT_SOURCE_SHA="$artifact_source_sha" \
  bash src/wasix/node-addon/tools/package-platform.sh \
  --target "$target_id" \
...
for runtime_and_manager in \
  ...
  bash src/wasix/node-addon/tools/smoke-packaged-addon.sh \
    --target "$target_id" \
