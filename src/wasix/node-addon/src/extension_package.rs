...
use super::NativeExtensionPackage;
...
pub(super) fn load(selection: NativeExtensionPackage) -> Result<Extension> {
    ...
    if archive_hash != selection.archive_sha256
        || carrier["size"].as_u64() != Some(u64::from(selection.archive_size))
    {
        return Err(fail(
            "extension descriptor archive identity differs from its installed package",
        ));
    }
