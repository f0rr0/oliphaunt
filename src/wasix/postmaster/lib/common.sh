...
_fresh_project_source_id_prefix="src/wasix/postmaster"
...
export WASIX_TOOLCHAIN_ROOT="${WASIX_TOOLCHAIN_ROOT:-$REPO_ROOT/src/wasix/runtime/assets/build}"
...
_fresh_wasmer_tag="$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/wasmer.toml" branch)" || return 2
...
FRESH_WASMER_SOURCE_COMMIT="${FRESH_WASMER_SOURCE_COMMIT:-$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/wasmer.toml" commit)}" || return 2
export FRESH_WASMER_SOURCE_COMMIT
FRESH_WASMER_NAPI_COMMIT="${FRESH_WASMER_NAPI_COMMIT:-$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/wasmer-napi.toml" commit)}" || return 2
export FRESH_WASMER_NAPI_COMMIT
FRESH_WASMER_TEST_FILES_COMMIT="${FRESH_WASMER_TEST_FILES_COMMIT:-$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/wasmer-test-files.toml" commit)}" || return 2
export FRESH_WASMER_TEST_FILES_COMMIT
FRESH_WASMER_SPEC_COMMIT="${FRESH_WASMER_SPEC_COMMIT:-$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/webassembly-testsuite.toml" commit)}" || return 2
export FRESH_WASMER_SPEC_COMMIT
FRESH_WASIX_LIBC_SOURCE_COMMIT="${FRESH_WASIX_LIBC_SOURCE_COMMIT:-$(fresh_source_scalar "$REPO_ROOT/src/wasix/postmaster/sources/wasix-libc.toml" commit)}" || return 2
