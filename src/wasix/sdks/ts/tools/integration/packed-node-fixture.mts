...
import { WASIX_RUNTIME_NPM_ASSET_PATHS } from '../../../../runtime/tools/wasix-runtime-npm-contract.mts';
import {
  renderWasixRuntimeDescriptorModule,
  renderWasixRuntimeDescriptorTypes,
} from '../../../../runtime/tools/wasix-runtime-npm-descriptor.mts';
...
import { packWasixToolsAotNpmCarriers } from '../../../../postgres-tools/tools/wasix-tools-aot-npm.mts';
...
const packageRoot = resolve(repositoryRoot, 'src/wasix/sdks/ts');
...
export async function stagePackedWasixConsumer({
  ...
  const nativeCarrier = includeNative
    ...
    : undefined;
  const toolsVersion = releaseVersions['src/wasix/postgres-tools'];
...
async function packStubRuntime({ scratch, tarballs, runtimeVersion }) {
  requireReleaseVersion(runtimeVersion, 'src/wasix/runtime');
...
async function packToolsCarrier({ scratch, tarballs, toolsVersion }) {
  requireReleaseVersion(toolsVersion, 'src/wasix/postgres-tools');
