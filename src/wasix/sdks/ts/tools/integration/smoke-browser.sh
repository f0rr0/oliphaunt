...
cd "$root"
tool="$root/src/wasix/sdks/ts/tools/integration/smoke-browser.mts"
...
if [ -n "$packed_consumer" ]; then
  ...
  bun "$root/src/wasix/sdks/ts/tools/stage-host.mts"
fi
vite_port="$(jq -r '.vitePort' "$configuration")"
chrome_port="$(jq -r '.chromePort' "$configuration")"
(cd "$root/src/wasix/sdks/ts" && OLIPHAUNT_WASIX_BROWSER_SMOKE=1 bun x --no-install vite \
  --config "$root/src/examples/wasix/browser/vite.config.ts" --host 127.0.0.1 \
