...
import { runWasixPgDumpProcess, WASIX_PROTOCOL_CALLBACK_CHUNK_BYTES } from '../core/database.js';
import { runWasixToolProcess } from '../internal.node.js';
import type { WorkerResponse } from '../workers/rpc.js';
import { createWorkerSessionDispatcher } from '../workers/worker-dispatch.js';
import { openWorkerDatabase, WorkerRpc } from '../workers/worker-rpc.js';
...
describe('WASIX worker RPC', () => {
  ...
  it('preserves deferred execution and explicit sync boundaries across worker RPC', async () => {
    const events: string[] = [];
    const responses: Extract<WorkerResponse, { ok: boolean }>[] = [];
