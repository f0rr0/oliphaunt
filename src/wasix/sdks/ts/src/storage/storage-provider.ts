...
import type { SerializedWasixStorage } from './types.js';
import type { StoredSnapshot } from './storage-snapshot.js';
import { validateIndexedDbDatabaseName, validateOpfsDatabaseName } from './types.js';
...
export async function restoreWasixStorage(
  ...
  switch (storage.kind) {
    ...
    case 'indexed-db': {
      ...
      const { restoreIndexedDbStorage } = await import('./indexed-db-provider.js');
      return restoreIndexedDbStorage(storage.name, snapshot, identity);
    }
    case 'opfs': {
      validateOpfsDatabaseName(storage.name);
      const { restoreOpfsStorage } = await import('./opfs-provider.js');
...
export async function acquireWasixStorage(
  ...
  switch (storage.kind) {
    ...
    case 'indexed-db': {
      validateIndexedDbDatabaseName(storage.name);
      const { acquireIndexedDbStorage } = await import('./indexed-db-provider.js');
      return acquireIndexedDbStorage(storage.name, loadClusterSeed, identity);
    }
    case 'opfs': {
      validateOpfsDatabaseName(storage.name);
      const { acquireOpfsStorage } = await import('./opfs-provider.js');
