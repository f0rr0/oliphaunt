...
import {
  ...
  taskCapabilities,
  taskLabel,
} from './moon-task-capabilities.mts';
...
describe('Moon task capabilities', () => {
  ...
  test('fails closed for incomplete or cyclic dependency metadata', () => {
    ...
    assert.throws(() => taskCapabilities(incomplete.get('repo:root'), incomplete), /repo:missing/u);

    const cyclic = tasks(
      { target: 'repo:first', deps: ['repo:second'] },
      { target: 'repo:second', deps: ['repo:first'] },
    );
    assert.throws(() => taskCapabilities(cyclic.get('repo:first'), cyclic), /dependency cycle/u);
  });
