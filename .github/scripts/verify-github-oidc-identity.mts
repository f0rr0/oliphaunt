...
export function expectedOidcIdentity(environment = process.env) {
  ...
  if (ref !== expectedRef) {
    throw new Error(`trusted publication ref mismatch: expected ${expectedRef}, got ${ref}`);
  }
...
export function verifyOidcClaims(claims, expected) {
  ...
  if (Object.hasOwn(claims, 'job_workflow_sha') && !Object.hasOwn(claims, 'job_workflow_ref')) {
    throw new Error('GitHub OIDC claim job_workflow_sha requires claim job_workflow_ref');
  }
