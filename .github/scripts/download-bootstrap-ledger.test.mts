...
import { isolatedGitHubTestEnvironment } from '../../tools/test/isolated-github-test-environment.mts';

import {
  selectEarlierAttemptArtifact,
  validateAttemptMetadata,
} from './download-bootstrap-ledger.mts';

const SCRIPT = path.join(import.meta.dirname, 'download-bootstrap-ledger.mts');
...
function invoke(
  ...
  return spawnSync(
    'node',
    ['--import', path.join(import.meta.dirname, 'bootstrap-ledger-github-fixture.mts'), SCRIPT],
    {
      encoding: 'utf8',
      timeout: BOOTSTRAP_LEDGER_PROCESS_TIMEOUT_MS,
...
function downloadedArtifactIds(fixture) {
  return calls(fixture)
    .map((args) => /\/artifacts\/([0-9]+)\/zip$/u.exec(args)?.[1])
    .filter(Boolean);
}
...
test('the current run artifact must retain its exact-SHA binding', async (t) => {
  ...
  const result = invoke(f, {
    artifactsByRun: {
      900: [
        artifact(5_000, 900, '2026-07-14T10:05:00Z', undefined, {
...
test('attempt boundary uses both creation and update time and metadata validation fails closed', () => {
  ...
  assert.throws(
    () =>
      selectEarlierAttemptArtifact([artifact(104, 900, 'not-a-timestamp')], {
        runId: '900',
...
test('artifact transport retries transient failure and restores only a complete checkpoint envelope', async (t) => {
  ...
  assert.equal(
    readdirSync(f.root).some((name) => name.startsWith('.destination.')),
    false,
  );
});
...
test('a valid-looking ZIP with the wrong immutable digest is never restored', async (t) => {
  ...
  assert.match(
    result.stderr,
    /transport identity mismatch.*retry budget exhausted|retry budget exhausted.*transport identity mismatch/su,
  );
  assert.equal(readFileSync(f.downloadState, 'utf8'), '4');
  assert.deepEqual(restoredNames(f), [existing]);
  assert.equal(
...
test('checkpoint collision is validated before atomic promotion and preserves prior bytes', async (t) => {
  ...
  assert.equal(
    readdirSync(f.root).some((entry) => entry.startsWith('.destination.')),
    false,
  );
});
