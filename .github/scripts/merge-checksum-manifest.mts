...
function fail(message) {
  throw new Error(`merge-checksum-manifest.mts: ${message}`);
}
...
export async function mergeChecksumManifest(existing, incoming) {
  if (existing === undefined || incoming === undefined) {
    fail('usage: merge-checksum-manifest.mts <existing> <incoming>');
  }
...
if (import.meta.main) {
  try {
    await mergeChecksumManifest(...process.argv.slice(2));
  } catch (error) {
