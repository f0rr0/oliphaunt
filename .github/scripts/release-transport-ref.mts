...
import { reserveGitHubContentWrite } from '../../tools/release/github-content-write-pacer.mts';
import { reserveGitHubCoreRequest } from '../../tools/release/github-core-request-journal.mts';
import { assertPublicationController } from '../../tools/release/publication-controller.mts';
...
const CONTENT_WRITE_ADMISSIONS = new Set(['self-paced', 'pre-reserved', 'isolated-bootstrap']);
...
export function validateReleaseTransportRef(value, commit) {
  ...
  if (
    ...
    throw error(
      `transport ref ${expectedRef} does not point directly to exact release commit ${expectedCommit}`,
    );
  }
  return {
...
function rootAdmission(contentWriteAdmission, commit, environment) {
  ...
  const expectedOperation =
    contentWriteAdmission === 'isolated-bootstrap' ? 'publish-bootstrap' : 'publish';
  ...
  if (workflowSha !== commit) {
    try {
      assertPublicationController({ source: commit, controller: workflowSha, environment });
    } catch (cause) {
      throw error(
        `${contentWriteAdmission} admission requires the exact root ${expectedOperation} GitHub run with a verified publication controller: ${cause.message}`,
      );
    }
  }
...
export async function proveCurrentMain({
  commit,
  ...
  const expected = normalizeReleaseTransportCommit(commit);
  if (environment.GITHUB_REF !== 'refs/heads/main')
    throw error('release operations must be dispatched from main');
  const safeRepo = safeRepository(repo);
  const secret = safeToken(token);
  await reserveGitHubCoreRequest({ environment, label: 'release current main proof' });
  const response = await request(
    fetchImpl,
    apiUrl(safeRepo, 'git/ref/heads/main'),
    {
  ...
  if (response.status !== 200) {
    await boundedText(response, 'current main proof failure');
    throw error(`current main proof returned HTTP ${response.status}`);
  }
  const ref = await responseJson(response, 'current main proof');
  if (
    ref?.ref !== 'refs/heads/main' ||
    ref?.object?.type !== 'commit' ||
    ref.object.sha !== expected
  ) {
    throw error('main moved after this workflow was dispatched or returned an invalid commit ref');
  }
}
...
async function boundedText(response, context) {
  ...
  if (declared !== null && declared !== undefined) {
    ...
    if (
      ...
      length < 0 ||
      length > RELEASE_TRANSPORT_MAX_RESPONSE_BYTES
    ) {
...
async function request(fetchImpl, url, options, context) {
  try {
    ...
    throw error(
      `${context} failed before a complete response: ${cause instanceof Error ? cause.message : String(cause)}`,
    );
  }
}
...
export async function ensureReleaseTransportRef({
  ...
  environment = process.env,
  fetchImpl = fetch,
  proveCurrentMain: proveMain = proveCurrentMain,
  repo = environment.GH_REPO,
  token = environment.GH_TOKEN,
} = {}) {
  if (!CONTENT_WRITE_ADMISSIONS.has(contentWriteAdmission)) {
    throw error('contentWriteAdmission must be self-paced, pre-reserved, or isolated-bootstrap');
  }
  const normalizedCommit = normalizeReleaseTransportCommit(commit);
  const admission = rootAdmission(contentWriteAdmission, normalizedCommit, environment);
  const safeRepo = safeRepository(repo);
  const secret = safeToken(token);
  ...
  await proveMain({
    commit: admission.workflowSha ?? normalizedCommit,
    environment,
    fetchImpl,
    repo: safeRepo,
    token: secret,
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  await main(process.argv.slice(2)).catch((cause) => {
