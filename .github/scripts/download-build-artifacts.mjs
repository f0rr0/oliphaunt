...
function runMatchesRequest(repo, runId, workflow, sha) {
  let data;
  try {
    data = JSON.parse(
      gh(["api", `repos/${repo}/actions/runs/${runId}`], { capture: true }),
    );
  } catch {
    return false;
  }
  if (!(
    ...
    data.head_sha.toLowerCase() === sha.toLowerCase() &&
  ...
  try {
    const workflowData = JSON.parse(
      gh(["api", `repos/${repo}/actions/workflows/${data.workflow_id}`], { capture: true }),
    );
    return workflowData?.name === workflow;
  } catch {
    return false;
  }
}
...
function selectRunId(repo, args) {
  if (args.selectedRunId !== "") {
    ...
    if (!runMatchesRequest(repo, runId, args.workflow, args.sha)) {
  ...
  for (const candidate of candidateRunIds(repo, args.workflow, args.sha, args.requiredJob)) {
    if (!runMatchesRequest(repo, candidate, args.workflow, args.sha)) {
      continue;
    }
