...
set -euo pipefail
[ "$#" = 0 ] || {
  echo 'usage: write-affected-moon-target-matrices.sh' >&2
  exit 2
}
...
base="${MOON_BASE:-}"
head="${MOON_HEAD:-}"
if [[ ${CI_QUALIFICATION_MODE:-} != selected-products && -n "${base//[[:space:]]/}" && -n "${head//[[:space:]]/}" ]]; then
  query_args+=(--affected --upstream none --downstream deep)
fi
"$moon_bin" "${query_args[@]}" </dev/null >"$query_dir/selected.json"
"$moon_bin" task-graph --json >"$query_dir/graph.json"
bun .github/scripts/write-affected-moon-target-matrices.mts "$query_dir/selected.json" "$query_dir/graph.json"
