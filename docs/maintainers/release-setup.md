...
2. Run the WASIX Cargo artifact packager against staged `liboliphaunt-wasix`
   release assets and use `packages.json` as the exact crate identity list for
   WASIX public artifact crates. The packager creates only the listed public
   artifact crates, including the separate `oliphaunt-icu` data crate, and
   fails if any generated `.crate` exceeds the crates.io 10 MB package limit.
3. Perform the first publication manually for each new crate. crates.io trusted
   publishing cannot create brand-new crates; it can publish later versions
   after the crate exists.
4. Add all maintainers who should have owner access.
5. Configure trusted publishing for every crate:
   - owner: `f0rr0`
   - repository: `oliphaunt`
   - workflow filename: `release.yml`
   - environment: `release-publish`
6. Do not add `CARGO_REGISTRY_TOKEN` to the repository. Cargo publishing runs
...
other native packages to behave: the published package pins compatible
`@oliphaunt/liboliphaunt-*`, `@oliphaunt/broker-*`, and
`@oliphaunt/node-direct-*` optional platform packages. The package manager
selects the package for the current `os`, `cpu`, and `libc`; runtime startup
loads the installed package-local artifacts and performs no GitHub release
download. Consumers must not install `@oliphaunt/ts` with optional dependencies
disabled. `libraryPath`, `runtimeDirectory`, `LIBOLIPHAUNT_PATH`, and
`OLIPHAUNT_RUNTIME_DIR` remain development overrides, not the public install
story.
