...
- Swift Package Manager expects each package to own a `Package.swift`, products,
  ...
  under `src/sdks/swift` as a normal Swift package instead of as ad hoc root files:
...
- `src/*/moon.yml` is the canonical product graph. `tools/policy/sdk-manifest.toml`
  remains a small SDK parity registry during the migration and must agree with
  moon metadata.
...
- `src/sdks/kotlin` is a Gradle multi-project Kotlin Multiplatform build for
  Android apps. It owns the common suspend API, cinterop metadata, wrapper, and
  Kotlin tests.
