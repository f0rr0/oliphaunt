...
| Optional ICU data stays optional during initdb | `0033-oliphaunt-wasix-skip-icu-collation-setup-without-icu-data.patch` | `getenv("ICU_DATA")`, `pg_collation_actual_version`, `pg_import_system_collations` | WASIX initdb skips ICU-backed collation setup until the optional ICU data package is present. |
