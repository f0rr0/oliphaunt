...
| Browser-worker hosts can own one blocking stdio pgwire lifecycle | `0039-oliphaunt-wasix-add-stdio-pgwire-lifecycle.patch` | `OLIPHAUNT_WASIX_STDIO_PGWIRE`, `oliphaunt_wasix_set_protocol_stdio(1)`, `ProcessStartupPacket(MyProcPort, true, true)` | Only the explicit browser-worker contract enters the blocking stdio path; Rust, browser-direct, and Node hosts keep the export-driven lifecycle. |
