...
| Optional ICU data stays optional during initdb | `0016-liboliphaunt-skip-icu-collation-version-without-icu-data.patch` | `OLIPHAUNT_INTERNAL_ICU_READY`, `strcmp`, `pg_collation_actual_version`, `pg_import_system_collations` | Standard seeds skip ICU-backed catalogue setup; only a producer-controlled readiness signal after exact ICU-data verification enables the ICU profile. |
