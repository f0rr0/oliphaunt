...
const EXTERNAL_ROOT = path.join(ROOT, 'extensions/external');
...
function parseSource(raw) {
  ...
  if (!manifest.startsWith('extensions/external/')) {
    fail(`${id} source manifest must be under extensions/external/`);
  }
  const kind = raw.kind;
...
export function externalReleaseExtensionSqlNames() {
  const sqlNames = [];
  for (const entry of readdirSync(path.join(ROOT, 'extensions/external'), {
    withFileTypes: true,
  })) {
    if (!entry.isDirectory()) continue;
    const releaseFile = path.join(ROOT, 'extensions/external', entry.name, 'release.toml');
...
function externalLicenseRow(product, members) {
  ...
  const releaseToml = path.join(ROOT, 'extensions/external', row.sqlName, 'release.toml');
  let release;
  try {
...
export function stageExtensionUpstreamLicenses(sqlName, filesRoot) {
  const externalRoot = path.join(ROOT, 'extensions/external', sqlName);
