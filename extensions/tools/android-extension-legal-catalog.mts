...
const INPUT = path.join(ROOT, 'extensions/generated/sdk/extensions.json');
  ...
  if (actual !== expected) {
    ...
    fail(
      `${path.relative(ROOT, OUTPUT)} is stale (expected ${expectedDigest}, got ${actualDigest}); ` +
        'run bash extensions/tools/check-extension-model.sh --write',
    );
  }
}
