...
oliphaunt_postgis_bootstrap_repo_root() {
  ...
  while [ "$repo_root" != "/" ]; do
    if [ -f "$repo_root/package.json" ] &&
       [ -d "$repo_root/runtimes/liboliphaunt-wasix/assets/build" ]; then
      printf '%s\n' "$repo_root"
      return 0
    fi
...
ROOT="${OLIPHAUNT_WASIX_BUILD_ROOT:-$BOOTSTRAP_REPO_ROOT/runtimes/liboliphaunt-wasix/assets/build}"
...
if [ ! -f "$POSTGIS_SOURCE_DIR/configure.ac" ]; then
  echo "missing PostGIS source checkout at $POSTGIS_SOURCE_DIR; run bash third-party/tools/fetch-sources.sh wasix-runtime --force first" >&2
  exit 1
fi
...
time_helper="$REPO_ROOT/extensions/external/postgis/tools/reproducible-time.sh"
date_shim="$REPO_ROOT/extensions/external/postgis/tools/reproducible-bin/date"
