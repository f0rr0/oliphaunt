...
function generatedExtensionRow(sqlName) {
  const metadata = path.join(ROOT, 'extensions/generated/sdk/extensions.json');
...
function wasixArchiveFor(sqlName, { product = undefined, required = false } = {}) {
  ...
  if (generatedRootValue) {
    ...
    const modelPath = path.join(ROOT, 'extensions/generated/wasix/extensions.json');
    let model;
    try {
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--all') {
      ...
      console.log(
        'usage: extensions/artifacts/packages/tools/build-extension-ci-artifacts.mts [--all] [--output-root DIR] [--family native|wasix] [--require-native] [--require-native-target TARGET] [--require-wasix] [products...]',
      );
      process.exit(0);
