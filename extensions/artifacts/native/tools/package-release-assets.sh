...
source "$root/runtimes/liboliphaunt-native/bin/mobile-static-extensions.sh"
source "$root/runtimes/liboliphaunt-native/bin/build-output.bash"
packager="extensions/artifacts/native/tools/extension-artifact-packager.mts"
observed_phase="extensions/artifacts/native/tools/run-observed-phase.sh"
native_asset_index_contract="extensions/artifacts/native/tools/native-extension-asset-index-contract.mts"

target_id="${OLIPHAUNT_EXTENSION_TARGET:-${1:-}}"
if [ -z "$target_id" ]; then
  source "$root/runtimes/liboliphaunt-native/tools/runtime-preflight.sh"
...
native_runtime_version="$(tr -d '[:space:]' < "$root/runtimes/liboliphaunt-native/VERSION")"
...
prepare_windows_binary_contract_runtime() {
  ...
  tools/dev/bun.sh \
    extensions/artifacts/native/tools/stage-windows-binary-contract.mts \
  ...
  case "$target_id" in
    ...
    windows-x64-msvc)
      "$observed_phase" \
        ...
        OLIPHAUNT_BUILD_EXTENSIONS=1 \
        OLIPHAUNT_NATIVE_EXTENSION_SQL_NAMES="$build_sql_names" \
        bash runtimes/liboliphaunt-native/bin/build-postgres18-windows.sh
      ;;
    *)
  ...
  case "$target_id" in
    ios-xcframework)
      ...
      "$observed_phase" \
        ...
        runtimes/liboliphaunt-native/bin/build-postgres18-ios-simulator.sh
      }
      device_lane() {
      "$observed_phase" \
        ...
        OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$mobile_extensions" \
        runtimes/liboliphaunt-native/bin/build-postgres18-ios-device.sh
      }
      macos_lane() {
        build_mobile_host_extension_runtime
      "$observed_phase" \
        ...
        OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$mobile_extensions" \
        runtimes/liboliphaunt-native/bin/build-macos-extension-archives.sh
      }
      ...
      "$observed_phase" \
        ...
        OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$mobile_extensions" \
        runtimes/liboliphaunt-native/bin/build-ios-extension-xcframeworks.sh
      ;;
    android-arm64-v8a)
      "$observed_phase" \
        ...
        OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$mobile_extensions" \
        runtimes/liboliphaunt-native/bin/build-postgres18-android-arm64.sh
      ;;
    android-x86_64)
      "$observed_phase" \
        ...
        OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$mobile_extensions" \
        runtimes/liboliphaunt-native/bin/build-postgres18-android-x86_64.sh
      ;;
  esac
}
...
make_extension_artifact() {
  ...
  bash extensions/artifacts/native/tools/create-artifact.sh "${artifact_args[@]}" >/tmp/liboliphaunt-release-extension-artifact-"$target_id"-"$sql_name".log
}
...
package_ios_target() {
  ...
  while IFS=$'\t' read -r sql_name pg_major creates_extension stem dependencies shared_preload desktop_prebuilt mobile_prebuilt mobile_static_required mobile_static_targets data_files artifact_policy; do
    ...
    if [ "$stem" != "-" ]; then
      ...
      bash extensions/artifacts/native/tools/ios-extension-registration.sh \
        --sql-name "$sql_name" \
