...
const CATALOG_PATH = path.join(root, 'extensions/generated/extensions.catalog.json');
const CONTRIB_RECIPE_PATH = path.join(root, 'extensions/contrib/postgres18.toml');
...
function parseMobileStaticDependencyArchive(value) {
  if (value.includes('=')) {
    ...
    if (!target || !name || !archive) {
      fail(
        '--mobile-static-dependency-archive values must use <target>:<name>:<archive> or <target>:<name>=<archive>',
      );
    }
...
function parseStaticSymbolAlias(value) {
  ...
  if (separator <= 0) {
    fail(
      '--static-symbol-alias values must use <sql-symbol>:<linked-symbol> or <sql-symbol>=<linked-symbol>',
    );
  }
...
async function artifactPhase(phase, artifactRoot, argv) {
  ...
  args.nativeRuntimeVersion ??= (await readText('runtimes/liboliphaunt-native/VERSION')).trim();
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      fail(
        'usage: extension-artifact-packager.mts <list-catalog|selected-sql-names|product-version|stage-artifact|finish-artifact> [options]',
      );
  }
}
