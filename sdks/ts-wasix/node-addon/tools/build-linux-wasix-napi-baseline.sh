...
if [ "$#" -ne 3 ]; then
  fail "usage: sdks/ts-wasix/node-addon/tools/build-linux-wasix-napi-baseline.sh TARGET_DIR TARGET_TRIPLE FEATURES"
fi
...
readonly manifest="/workspace/sdks/ts-wasix/node-addon/Cargo.toml"
