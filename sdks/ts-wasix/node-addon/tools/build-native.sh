...
if [[ "$host_system" == "Linux" ]]; then
  linux_libc="$(node sdks/ts-wasix/node-addon/tools/detect-linux-libc.mts)"
  case "$linux_libc" in
    glibc) ;;
    musl)
...
manifest="sdks/ts-wasix/node-addon/Cargo.toml"
package_manifest="sdks/ts-wasix/node-addon/package.json"
metadata_contract="$(
  bun "$workspace_root/sdks/ts-wasix/node-addon/tools/native-build-data.mts" metadata "$package_manifest"
)"
...
tools/dev/bun.sh sdks/ts-wasix/node-addon/tools/check-build-inputs.mts \
  "${build_input_args[@]}" \
...
build_addon() {
  ...
  if [[ "$target_id" == linux-*-gnu ]]; then
    sdks/ts-wasix/node-addon/tools/build-linux-wasix-napi-baseline.sh \
      "$cargo_target_dir" \
      "$cargo_target" \
      release
  else
...
if [[ "$target_id" == "$host_target" ]]; then
  node "$workspace_root/sdks/ts-wasix/node-addon/tools/native-build-data.mts" check-addon \
    "$addon" \
...
OLIPHAUNT_WASIX_NAPI_ARTIFACT_SOURCE_SHA="$artifact_source_sha" \
  bash sdks/ts-wasix/node-addon/tools/package-platform.sh \
  --target "$target_id" \
...
for runtime_and_manager in \
  ...
  bash sdks/ts-wasix/node-addon/tools/smoke-packaged-addon.sh \
    --target "$target_id" \
