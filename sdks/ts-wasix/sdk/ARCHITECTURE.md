...
`sdks/ts-wasix/sdk` is one public TypeScript API over two host adapters.
...
This boundary deliberately does not depend on `sdks/ts/sdk`,
...
`@oliphaunt/ts-query` package in `sdks/ts-query`. The native and React Native
SDKs depend on the same package.

The patched Wasmer host under `runtimes/wasix-browser-host` is the browser host
...
JavaScript/WASM. `runtimes/wasix-browser-host/source.toml` pins the Wasmer JS Git source and Cargo
...
patch order is the `series` in `runtimes/wasix-browser-host/source.toml`; this document records the
...
`runtimes/wasix-browser-host/source.toml` records the intentionally coherent 0.601 source family until
that port exists.
