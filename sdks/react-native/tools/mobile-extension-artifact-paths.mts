...
async function loadRepositoryContract(root) {
  const metadataPath = join(root, 'extensions/generated/sdk/extensions.json');
  const iosOverlayPath = join(root, 'extensions/generated/sdk/ios-static-dependencies.json');
  const iosDependenciesPath = join(root, 'extensions/generated/mobile/static-extensions.tsv');
  const nativeVersionPath = join(root, 'runtimes/liboliphaunt-native/VERSION');
  const wasixVersionPath = join(root, 'runtimes/liboliphaunt-wasix/VERSION');
  let metadata;
  let iosOverlay;
