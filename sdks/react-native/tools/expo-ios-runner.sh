...
cd "$root"
. "$root/runtimes/liboliphaunt-native/bin/build-output.bash"
. "$root/sdks/react-native/tools/expo-runner-common.sh"
. "$root/sdks/react-native/tools/expo-runner-metro.sh"
. "$root/sdks/react-native/tools/expo-runner-reporting.sh"
. "$root/sdks/react-native/tools/expo-runner-workspace.sh"
. "$root/sdks/react-native/tools/mobile-extension-runtime.sh"
. "$root/sdks/react-native/tools/expo-runner-runtime-resources.sh"
. "$root/sdks/react-native/tools/expo-runner-ios-device.sh"
. "$root/sdks/react-native/tools/expo-runner-ios-installed-app.sh"

source_example_dir="$root/examples/react-native-expo"
rn_dir="$root/sdks/react-native"
mobile_platform="ios"
...
find_ios_library_artifact() {
  ...
  if [ -z "$artifact" ] && [ "$sdk" = "iphonesimulator" ]; then
    ...
    artifact="$(oliphaunt_capture_build_artifact_path \
      ...
      env OLIPHAUNT_MOBILE_STATIC_EXTENSIONS="$static_extensions" runtimes/liboliphaunt-native/bin/build-postgres18-ios-simulator.sh)"
  fi
  [ -n "$artifact" ] ||
...
prepare_swift_sdk_artifact_git_repo_if_required() {
  ...
  source_root="$artifact_repo/sdks/swift"
  rm -rf "$artifact_repo" "$extract_root"
  mkdir -p "$source_root"
  bun "$root/sdks/swift/tools/extract-verified-zip.mts" \
    --archive "$archive" \
...
configure_ios_carrier_inputs() {
  ...
  bun "$root/sdks/react-native/tools/expo-ios-runner.mts" configure-plugin "$example_dir/app.json" "$selected_extensions" "$icu_enabled"
}
...
patch_expo_modules_jsi_for_host_toolchain() {
  ...
  "$root/tools/dev/bun.sh" "$root/sdks/react-native/tools/expo-ios-runner.mts" patch-weak-references "$package_dir"
