...
require_mobile_seed() {
  ...
  bun "$root/database-resources/contracts/native-manifest.mts" "${args[@]}" >/dev/null
  printf '%s\n' "$configured"
}
...
prepare_mobile_runtime_resource_package() {
  ...
  current_sources="$(
    ...
    shasum -a 256 "$root/extensions/generated/mobile/static-registry.json"
    ...
    shasum -a 256 \
      ...
      "$root/sdks/react-native/tools/validate-mobile-runtime-files.mts"
  )"
