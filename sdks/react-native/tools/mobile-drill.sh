...
case "$platform" in
  android|ios)
    ;;
  *)
    echo "usage: sdks/react-native/tools/mobile-drill.sh [android|ios] [crash|benchmark]" >&2
    exit 2
    ;;
esac
case "$drill" in
...
case "$platform:$drill" in
  android:crash)
    ...
    exec "$root/sdks/react-native/tools/expo-android-runner.sh"
    ;;
  android:benchmark)
  ...
  ios:crash)
    ...
    exec "$root/sdks/react-native/tools/expo-ios-runner.sh"
    ;;
  ios:benchmark)
