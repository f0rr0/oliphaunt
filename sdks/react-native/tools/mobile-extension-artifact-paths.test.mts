...
const NATIVE_RUNTIME_VERSION = readFileSync(
  path.join(REPOSITORY_ROOT, 'runtimes/liboliphaunt-native/VERSION'),
  'utf8',
).trim();
const WASIX_RUNTIME_VERSION = readFileSync(
  path.join(REPOSITORY_ROOT, 'runtimes/liboliphaunt-wasix/VERSION'),
  'utf8',
).trim();
const CONTRIB_VERSION = NATIVE_RUNTIME_VERSION;
const REACT_NATIVE_EXTENSIONS = JSON.parse(
  readFileSync(path.join(REPOSITORY_ROOT, 'extensions/generated/sdk/extensions.json'), 'utf8'),
).extensions;
...
const IOS_OVERLAY_BY_SQL_NAME = new Map(
  JSON.parse(
    readFileSync(
      path.join(REPOSITORY_ROOT, 'extensions/generated/sdk/ios-static-dependencies.json'),
      'utf8',
    ),
...
const STATIC_EXTENSION_LINES = readFileSync(
  path.join(REPOSITORY_ROOT, 'extensions/generated/mobile/static-extensions.tsv'),
  'utf8',
)
  .split(/\r?\n/u)
