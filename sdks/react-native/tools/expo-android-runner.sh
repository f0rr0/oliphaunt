...
. "$root/sdks/react-native/tools/expo-android-gradle-limits.sh"
...
. "$root/sdks/react-native/tools/expo-runner-android-device.sh"
...
android_build_script_for_abi() {
  case "$android_abi" in
    arm64-v8a) printf '%s\n' "$root/runtimes/liboliphaunt-native/bin/build-postgres18-android-arm64.sh" ;;
    x86_64) printf '%s\n' "$root/runtimes/liboliphaunt-native/bin/build-postgres18-android-x86_64.sh" ;;
    *) fail "unsupported Android ABI: $android_abi" ;;
  esac
}
...
publish_local_kotlin_sdk() {
  ...
  mkdir -p "$local_maven_repo"
  run "$root/sdks/kotlin/gradlew" -p "$root/sdks/kotlin" \
...
build_apk() {
  ...
  if [ "${OLIPHAUNT_EXPO_ANDROID_SKIP_BUILD:-0}" = "1" ] && [ -f "$apk" ]; then
    ...
    if [ -n "$module_stems" ]; then
      run bun "$root/sdks/react-native/tools/validate-android-link-evidence.mts" \
  ...
  run "$root/sdks/react-native/tools/verify-android-apk.sh" "$apk"
