...
pub(crate) fn packaged_extension_candidates() -> &'static [Extension] {
    generated::CANDIDATES
}
...
pub(crate) fn extension_setup_sql(extension: Extension) -> Vec<String> {
    extension_setup_sql_with_schema_policy(extension, true)
}

pub(crate) fn native_extension_setup_sql(extension: Extension) -> Vec<String> {
    extension_setup_sql_with_schema_policy(extension, false)
}

fn extension_setup_sql_with_schema_policy(
    extension: Extension,
    preserve_pg_catalog_schema: bool,
) -> Vec<String> {
    let setup = extension.setup();
    let mut statements = Vec::new();
    if setup.create_extension {
        let create_schema = if preserve_pg_catalog_schema {
            setup.create_schema
        } else {
            setup.create_schema.filter(|schema| *schema != "pg_catalog")
        };
