...
use crate::pglite::base::{
    PreparedRoot, RootPlan, RootSource, RootTarget, prepare_native_root, prepare_root,
};
...
impl PgliteBuilder {
    ...
    /// Select the embedded runtime engine.
    ///
    /// The default is [`EngineKind::WasixLegacy`]. The native `libpglite`
    /// path is an explicit happy-path spike and requires
    /// `PGLITE_OXIDE_NATIVE_LIBPGLITE` to point at a compatible dynamic
    /// library.
    ...
    fn open_prepared_root(
        ...
        let mut instance = Pglite::new_prepared_with_config(
            outcome,
