...
fn main() {
    println!("cargo:rerun-if-env-changed=PGLITE_OXIDE_GENERATED_ASSETS_DIR");
...
fn emit_expected_asset_inputs() {
    if let Some(path) = env::var_os("PGLITE_OXIDE_GENERATED_ASSETS_DIR") {
    ...
    if let Some(repo_root) = manifest_dir.parent().and_then(Path::parent) {
        emit_manifest_probe(&repo_root.join("target/pglite-oxide/assets"));
    }
...
fn find_asset_dir() -> Option<PathBuf> {
    ...
    if let Some(repo_root) = repo_root {
        let target_assets = repo_root.join("target/pglite-oxide/assets");
...
fn write_generated_assets(out: &Path, asset_dir: &Path) {
    ...
    let runtime = asset_dir.join("pglite.wasix.tar.zst");
    ...
    for required in [&manifest, &runtime, &pg_dump, &initdb] {
        assert!(
            required.is_file(),
