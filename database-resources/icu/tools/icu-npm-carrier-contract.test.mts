...
const ICU_PACKAGE_ROOT = path.join(ROOT, 'database-resources/icu/npm');
...
test('ICU npm pack includes one canonical bundle and preserves both native descriptors byte-for-byte', () => {
  ...
  if (!root)
    throw new Error('Run bash database-resources/icu/tools/icu-npm-carrier-contract.test.sh');
