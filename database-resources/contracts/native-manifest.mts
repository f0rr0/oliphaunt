...
export function emptyDirectoryPaths(root) {
  ...
  for (const entry of readdirSync(root, { recursive: true, withFileTypes: true })) {
    ...
    const directory = path.join(entry.parentPath, entry.name);
    if (readdirSync(directory).length === 0)
      directories.push(path.relative(root, directory).split(path.sep).join('/'));
  }
...
export function bindNativeClusterSeedManifest(bytes, target, profile) {
  ...
  const expectedSourceKeys = new Set([...expectedSource.keys(), 'icuDataTreeSha256', 'cacheKey']);
  if (
    source.size !== expectedSourceKeys.size ||
    [...source.keys()].some((key) => !expectedSourceKeys.has(key))
  ) {
    throw new Error(
      'native cluster seed producer manifest: expected the exact unbound producer field set',
    );
  }
  for (const [key, value] of expectedSource) {
    if (source.get(key) !== value) {
      throw new Error(
        `native cluster seed producer manifest: ${key} must be ${JSON.stringify(value)}`,
      );
    }
  }
  ...
  if (typeof cacheKey !== 'string' || !validNativeCacheKey(cacheKey)) {
    throw new Error(
      'native cluster seed producer manifest: cacheKey must be a portable identifier',
    );
  }
  ...
  return Buffer.from(
    `${NATIVE_CLUSTER_SEED_MANIFEST_KEYS.map((key) => `${key}=${values.get(key)}`).join('\n')}\n`,
  );
}
...
export function validateNativeClusterSeedManifest(bytes, profile, options = {}) {
  ...
  if (
    values.size !== NATIVE_CLUSTER_SEED_MANIFEST_KEYS.length ||
    NATIVE_CLUSTER_SEED_MANIFEST_KEYS.some((key) => !values.has(key))
  ) {
    throw new Error(
      `${label}: fields must be exactly ${NATIVE_CLUSTER_SEED_MANIFEST_KEYS.join(',')}`,
    );
  }
  ...
  for (const [key, value] of expected) {
    if (values.get(key) !== value) {
      throw new Error(
        `${label}: ${key} must be ${JSON.stringify(value)}, got ${JSON.stringify(values.get(key))}`,
      );
    }
  }
  return values;
...
export function validateNativeClusterSeedDirectory(seed, profile, options = {}) {
  ...
  if (pgVersion !== '18')
    throw new Error(`${seed} has PostgreSQL ${JSON.stringify(pgVersion)}, expected 18`);
...
function main(args) {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (!key?.startsWith('--') || value === undefined) {
      throw new Error(
        'usage: native-cluster-seed-contract.mts --profile standard|icu --target TARGET --seed DIR [--icu-data DIR]',
      );
    }
  ...
  const { icuDataTreeSha256 } = validateNativeClusterSeedDirectory(seed, profile, {
    icuData,
    target,
  });
...
if (import.meta.main) {
  try {
    ...
    console.error(
      `native-cluster-seed-contract.mts: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(2);
  }
}
